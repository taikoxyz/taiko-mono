...
<Modal bind:open={$mintState.isModalOpen}>
    {#if $mintState.isMinting}
        ...
        <ModalFooter>
            <div class="w-full flex flex-row items-center gap-4">
                {#if $mintState.txHash}
                    <div
                        class="bg-interactive-tertiary rounded-md w-[30px] h-[30px] flex items-center justify-center"
                    >
                        <Spinner size="sm" />
                    </div>
                    <div>
                        <div class="font-sans font-bold text-sm text-content-primary">
    ...
    {:else}
        ...
        <ModalFooter>
            ...
            >
                <div
                    class="md:w-1/2 w-full px-2"
                    use:copy={`${window.location.origin}/collection/${$mintState.address}`}
                >
                    <Button
                        on:click={(event) => copyShareUrl(event.currentTarget)}
                        wide
                        block
