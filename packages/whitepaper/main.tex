...
A proposed block can only be mapped to a Taiko block in a \textit{mapping context}:

$$(\boldsymbol{\sigma}, h[1...256], \texttt{d})$$

where is $\boldsymbol{\sigma}$ is the current Taiko worldstate;  $h[1...256]$ are the most recent 256 ancestor block hashes; and $\texttt{d}$ is Taiko's chain ID.

Now we can define the block mapping function $M$ as:

\begin{eqnarray}
B & \equiv & (H, T, U), \\
\nonumber & \equiv &  M(\boldsymbol{\sigma}, h[1...256], \texttt{d}, \bar{B}, )  \\
\nonumber & \equiv & M(\boldsymbol{\sigma}, h[1...256], \texttt{d}, C, L)
\end{eqnarray}
...
\begin{eqnarray}
& & \texttt{CHAINID} \equiv \texttt{d} \quad \wedge \\
& & U \equiv [] \quad \wedge \\
& & H_p \equiv h(1) \quad \wedge \\
