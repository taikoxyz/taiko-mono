...
\end{itemize}
% At any point in time $t$, we can refer to their current value using the function notation, e.g. $f_\textnormal{base}(t)$. 
...
\includegraphics[width=\linewidth]{fig/relative_time_graph.png}
 \vspace{-0.2in}
 \captionof{figure}{The graph of $t_\textnormal{rel}$ (red) with respect to time $t$ given $t_\textnormal{last}$, $t_\textnormal{grace}$, $t_\textnormal{max}$.}
...
\begin{Figure}
 ...
 \captionof{figure}{The graphs of $\alpha_f(t)$ (blue) and $\alpha_r(t)$ (red), which bifurcate at $t_\textnormal{last}+t_\textnormal{grace}$ and are multiplicative inverses of each other throughout all time.}
...
Given an initial discount rate constant $0 < d_\textnormal{init} \leq 1$ and a halving period constant $t_\textnormal{halving}$, we can compute a discount function $d(t)$ and a discount multiplier $\gamma_f(t)$ as functions of time:

\begin{align}
d(t) &= d_\textnormal{init}\cdot (\nicefrac{1}{2})^{\lfloor \nicefrac{t}{t_\textnormal{halving}}\rfloor}
\\
\gamma_f(t)  &= 1-d(t).
\end{align}
...
Bootstrapping support is applicable only to proposer fees in order to safeguard the normal market order conditions between proposers and provers [Improve this].
