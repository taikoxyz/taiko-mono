...
\item Achieving \textit{anchoring}, an important concept in Taiko's design to ensure blocks are valid and cross-layer data exchanges are done right.
\end{itemize}
...
The txList is a RLP-serialised list of proposed L2 transactions. In a future version, when  EIP-4844 is enabled, the content of txList will be no longer direct accessible from \textit{TaikoL1} contract.

\subsubsection{Proposed Block Validity}
The proposed block must pass a validity test before it is accepted by the \textit{TaikoL1} contract.  We are able to define the proposed block validity function $V(\bar{B})$:
...
\nonumber& &  \mathrm{NUMBER}   -  K_{\mathrm{commitdelay}}
\end{eqnarray}

where $\mathrm{KEC}(C_{\mathbf{c}}, C_{\mathbf{t}})$ is called the block's \textit{commit hash}.
\subsubsection{Commit Hash} The commit hash is the Keccak 256-bit hash of a proposed block's \textbf{beneficiary} and \textbf{txListHash}. The Taiko ZK-Rollup protocol requires that a block can be proposed only \small{$K_{\mathrm{commitdelay}}$} confirmations after its commit hash has been committed to  {\textit{TaikoL1}} \small{$K_{\mathrm{commitdelay}}$}, which is a measure against MEV attack (\cite{mevethfoundation}) as block txLists are unencrypted.
