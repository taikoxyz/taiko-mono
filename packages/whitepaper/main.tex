...
where $H_p$ is the block's parent hash, $H_h \equiv \texttt{KEC}(\texttt{RLP}(H))$ is the hash of the proposed block after being mapped in L2, and $(a_n, P^{zk}_n, P^{tx}_n)$ are the n-th prover's address and the proofs. $P^{zk}$ is a zero-knowledge proof from zkEVM that proves the state transition from the parent hash is correct , and $P^{tx}$ is a merkle proof that proves the anchor transaction is valid and is the first transaction of the block in L2.
...
If $H_h$ equals $K_{\mathrm{BlockDeadEndHash}}$, the i-th block is marked as finalized, but we don't update j; otherwise, we also apply $j \equiv i$ to indicate the i-th block is the last finalized valid block. Obviously on L1, Taiko's canonical chain is a subset of all proposed blocks linked through Fork Choices; on L2, Taiko's canonical chain is a subset of all proposed blocks filtered by the Intrinsic Validity Function.

\subsection{Block Certainty}
To enable parallel zero-knowledge proof computation, all blocks, once proposed on L1, must have deterministic state transitions on L2. Our design ensures all potential public and private inputs are verified on L1 directly or indirectly to achieve block transition certainty.
