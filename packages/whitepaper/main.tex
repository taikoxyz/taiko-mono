...
Taiko protocol maintains a number of internal state variables $(n, f_\textnormal{base}, t^\textnormal{prop}_\textnormal{ave}, t^\textnormal{proof}_\textnormal{ave}, t^\textnormal{prop}_\textnormal{last}, t^\textnormal{proof}_\textnormal{last})$ that get updated after every successful block proposal or validated proof.
\begin{itemize}
    ...
    \item $t^\textnormal{prop}_\textnormal{ave}$ is the average duration between proposal submissions conditioned on they are successful.
    % , computed as a moving average of the gap between the current successful submission and the previous successful submission.
    \item $t^\textnormal{proof}_\textnormal{ave}$ is the average duration between proof submissions conditioned on they are eventually validated.
    % , computed as a moving average of the gap between the current validated submission and the previous validated submission.
    \item $t^\textnormal{prop}_\textnormal{last}$ is the submission time of the last successful proposal. Initially $t^\textnormal{prop}_\textnormal{last}$ is set to $0$.
    \item $t^\textnormal{proof}_\textnormal{last}$ is the submission time of the last validated proof. Initially $t^\textnormal{proof}_\textnormal{last}$ is set to $0$. 
\end{itemize}
...
Given a constant $\gamma > 1$, 
$\alpha_f(t): \mathbb{R}^+\rightarrow [1, \gamma]$ and $\alpha_r(t): \mathbb{R}^+\rightarrow  [ \nicefrac{1}{\gamma},1]$ are time sensitive multipliers that can decrease fees and increase rewards. 
...
$K_\textnormal{activation} > 0$, $K_\textnormal{max}^\textnormal{prop}>0$ and $K_\textnormal{max}^\textnormal{proof}>0$ and state variables, we can compute the following intermediate time variables, $t_\textnormal{grace}$ and $t_\textnormal{activation}$:
\begin{align}
    t_\textnormal{grace} \equiv 
 \begin{dcases}
 K_\textnormal{grace} \cdot \min(t^\textnormal{prop}_\textnormal{ave}, K_\textnormal{max}^\textnormal{prop}), &\text{if a \emph{proposal}} \\
K_\textnormal{grace} \cdot \min(t^\textnormal{proof}_\textnormal{ave}, K_\textnormal{max}^\textnormal{proof}), &\text{if a \emph{proof}}
\end{dcases}
\end{align}
...
% \end{align}
$t_\textnormal{grace}$ is the grace period starting at $t_\textnormal{last}$ (omitting the superscript again) in which there are no incentives yet.
...
$t_\textnormal{activation}$ is the time period after which incentives reach their maximum effects from the time they first start to take effect at $t_\textnormal{last}+t_\textnormal{grace}$. 
\begin{Figure}
 \centering
\includegraphics[width=\linewidth]{fig/relative_time_graph.png}
 \vspace{-0.2in}
 \captionof{figure}{The graph of $t_\textnormal{rel}$ (red) with respect to time $t$ given $t_\textnormal{last}$, $t_\textnormal{grace}$, $t_\textnormal{activation}$.}
...
Given a constant $\psi \geq 1$  % does not make sense to initialize it as this is not a variable, we can just change all the constants to K_something later if desired.
% (initialized as $K_{\mathrm{RewardMultiplier}}$)
and the relative normalized time function $t_\textnormal{rel}(t)$ established using the state variables, we compute incentive multipliers $\alpha_f(t)$, $\alpha_r(t)$ as
\begin{align}
...
\alpha_f(t) &= \nicefrac{1}{1 + (\psi - 1)\cdot t_\textnormal{rel}(t)}.
\end{align}
...
\begin{Figure}
 ...
 \captionof{figure}{The graphs of $\alpha_f(t)$ (blue) and $\alpha_r(t)$ (red), which deviate at $t_\textnormal{last}+t_\textnormal{grace}$ and are multiplicative inverses of each other throughout all time.}
 \label{fig:incentive_multiplier_graphs}
 
\end{Figure}

% \textbf{MEV: UP UNTIL HERE.}
...
In the opposite case, the level of competition for proposers and provers are reversed when there are many unverified blocks. 
...
\begin{align}
n'_\textnormal{slots} &= n_\textnormal{slots} + \lambda \\
\beta_f(n) &= \frac{(n'_\textnormal{slots}- 1) \cdot n'_\textnormal{slots}}{(n'_\textnormal{slots}-n-1)\cdot (n'_\textnormal{slots}-n)}\\
\beta_r(n) &= \frac{(n'_\textnormal{slots}- 1 ) \cdot n'_\textnormal{slots}}{(n'_\textnormal{slots}-n + 1)\cdot (n'_\textnormal{slots}-n)}.
\end{align}
...
% \end{align}
\begin{align}
\beta_f(0) &= \frac{(n'_\textnormal{slots}- 1 ) \cdot n'_\textnormal{slots}
...
\begin{Figure}
 ...
 \captionof{figure}{Progression of the multiplier for fees $\beta_f(n)$ (blue) and the multiplier for rewards $\beta_r(n)$ (red), when $n_{\textnormal{slots}}=50$ and $\lambda=10$. The maximum value of the multiplier is $\frac{(n_\textnormal{slots} - 1 + \lambda ) \cdot (n_\textnormal{slots} + \lambda)}{\lambda\cdot (1+\lambda)}=32.182$.}
...
\begin{Figure}
 ...
 \captionof{figure}{Progression of the multiplier for fees $\beta_f(n)$ (blue) and the multiplier for rewards $\beta_r(n)$ (red), when $n_{\textnormal{slots}}=50$ and $\lambda=20$. The maximum value of the multiplier is $\frac{(n_\textnormal{slots} - 1 + \lambda ) \cdot (n_\textnormal{slots} + \lambda)}{\lambda\cdot (1+\lambda)}=11.5$.}
...
Given an initial discount rate constant $d_\textnormal{init} \in (0, 1]$ and a halving period constant $K_\textnormal{Halving}$, we can compute a discount function $d(t)$ and a discount multiplier $\gamma_f(t)$ as functions of time:
...
Given a momentum constant $ 0\leq \mu \leq 1$, we update state variables in the following order after each successful (validated in the case of proofs) block proposal or proof submitted at time $t$:
\begin{gather}
    n \equiv 
 \begin{dcases}
