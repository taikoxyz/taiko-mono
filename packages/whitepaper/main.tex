...
\item[latestL1Height] The enclosing L1 block's parent block number; formally $C_{\mathrm{a}}$.
\item[latestL1Hash] The enclosing L1 block's parent block hash; formally $C_{\mathrm{h}}$.
\end{description}
...
\item Inserting latestL1Height $C_a$ and latestL1Hash $C_h$ into a mapping structure on L2 as a key-value pair. These L1 block hashes can be used by bridges to validate cross-chain messages.
\end{enumerate}
