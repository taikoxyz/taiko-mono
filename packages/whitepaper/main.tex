...
Formally, $F(L)$ is defined as:


\begin{eqnarray}
F(L) & \equiv & \texttt{NOERR}(T \equiv \texttt{RLP}'(L))  \quad \wedge  \\
...
Each proposed block that $F(\bar{B}_L)$ returns $\texttt{True}$ maps to an actual Taiko L2 block. Taiko blocks have the same structure as Ethereum blocks as defined in in the Yellow Paper:
...
$$\boldsymbol{\sigma} \equiv (\boldsymbol{\delta}, h[1...256], d, a)$$

where $\boldsymbol{\delta}$ is the trie, $h[1...256]$ are the most recent 256 ancestor block hashes; and $d$ is Taiko's chain ID, and $a$ is the anchor transaction.

Now we can define the block mapping function $M$ as:

\begin{eqnarray}
B & \equiv & (H, T, U), \\
\nonumber & \equiv &  M(\boldsymbol{\delta}, h[1...256], {d}, a, \bar{B}, )  \\
\nonumber & \equiv & M(\boldsymbol{\delta}, h[1...256], {d}, a, C, L)
\end{eqnarray}
...
\begin{eqnarray}
& & \texttt{CHAINID} \equiv {d} \quad \wedge \\
\nonumber& & H_p \equiv h(1) \quad \wedge \\
\nonumber& & H_o \equiv  K_{\mathrm{ommershash}} \quad \wedge \\
\nonumber& & H_c \equiv  C_c \quad \wedge \\
\nonumber& & H_r \equiv  C_r \quad \wedge \\
\nonumber& & U \equiv [] \quad \wedge \\
\nonumber& & T \equiv a::W(\texttt{RLP}'(L))
\end{eqnarray}

Where $::$ is the list concatenation operator; $W$ is the \textit{Transaction Contextual Filter Function}.
