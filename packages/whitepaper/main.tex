...
\begin{itemize}
\item Persisting the values of $i$, $d$, $h[1]$ -- the parent block hash -- to the trie for the next block to use.
...
We can add additional duties to anchor transaction implementation to enrich Taiko's functionalities without requesting changes in the zkEVM subsystem.

\subsubsection{Construction of Anchor Transactions} All anchor transactions are signed by a \textit{goldfinger} address with a revealed private-key. 
...
Where \texttt{K1ECDSA} is the ECDSA signing function with the internal variable $k$ chosen to be constant $1$, which ensures the transaction's signature only depends on the transaction itself and therefore is deterministic. 

According to ECDSA's spec, when $k$ is $1$, $\boldsymbol{\theta}_r$ must equal $\mathrm{G_x}$, the value of the x-coordinate of the base point on the SECP-256k1 curve. The TaikoL1 contract verifies such assertion while proving this block.
