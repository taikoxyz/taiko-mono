...
The anchor transaction will be the first transaction in a Taiko L2 block such that:

\begin{eqnarray}
& & \boldsymbol{\delta}_x = 0 \quad \wedge \\
\nonumber& & \boldsymbol{\delta}_n = \boldsymbol{\delta}[K_{\mathrm{goldfingeraddr}}]_n + 1 \quad \wedge \\
\nonumber& & \boldsymbol{\delta}_p = 0 \quad \wedge \\
\nonumber& & \boldsymbol{\delta}_g = K_{\mathrm{anchorgas}} \quad \wedge \\
\nonumber& & \boldsymbol{\delta}_t = K_{\mathrm{goldfingeraddr}} \quad \wedge  \\
\nonumber& & \boldsymbol{\delta}_v = 0 \quad \wedge  \\
\nonumber& & \boldsymbol{\delta}_d = K_{\mathrm{invalidateblockselector}}::C_a::C_h \quad \wedge  \\
\nonumber& & (\boldsymbol{\delta}_r,\boldsymbol{\delta}_s) = \texttt{K1ECDSA}(\boldsymbol{\delta})
\end{eqnarray}
