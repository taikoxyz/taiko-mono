...
// TODO: need to regenerate the code by AI with the protocol changes
...
func (e *Encoder) EncodeProposeInput(input *shastaBindings.IInboxProposeInput) ([]byte, error) {
	...
	isEmpty := input.Checkpoint.BlockNumber.Cmp(big.NewInt(0)) == 0 &&
		input.Checkpoint.BlockHash == [32]byte{} &&
		input.Checkpoint.StateRoot == [32]byte{}

	if isEmpty {
		pos = PackUint8(data, pos, 0)
	} else {
		pos = PackUint8(data, pos, 1)
		pos = PackUint48(data, pos, input.Checkpoint.BlockNumber.Uint64())
		pos = PackBytes32(data, pos, input.Checkpoint.BlockHash)
		_ = PackBytes32(data, pos, input.Checkpoint.StateRoot)
	}

	// 7. Encode numForcedInclusions
	pos = PackUint8(data, pos, input.NumForcedInclusions)
...
func (e *Encoder) DecodeProposeInput(data []byte) (*shastaBindings.IInboxProposeInput, error) {
	...
	if headerFlag == 1 {
		...
		input.Checkpoint.BlockNumber = new(big.Int).SetUint64(number)
		pos = newPos

		input.Checkpoint.BlockHash, pos = UnpackBytes32(data, pos)
		input.Checkpoint.StateRoot, pos = UnpackBytes32(data, pos)
	} else {
	...
	input.NumForcedInclusions, _ = UnpackUint8(data, pos)
...
func (e *Encoder) EncodeProvedEvent(payload *ProvedEventPayload) ([]byte, error) {
	...
	// Encode Checkpoint
	pos = PackUint48(data, pos, payload.Transition.Checkpoint.BlockNumber.Uint64())
	pos = PackBytes32(data, pos, payload.Transition.Checkpoint.BlockHash)
	pos = PackBytes32(data, pos, payload.Transition.Checkpoint.StateRoot)
	...
	pos = PackBytes32(data, pos, payload.TransitionRecord.CheckpointHash)
...
func (e *Encoder) DecodeProvedEvent(data []byte) (*ProvedEventPayload, error) {
	...
	number, newPos := UnpackUint48(data, pos)
	payload.Transition.Checkpoint.BlockNumber = new(big.Int).SetUint64(number)
	pos = newPos

	payload.Transition.Checkpoint.BlockHash, pos = UnpackBytes32(data, pos)
	payload.Transition.Checkpoint.StateRoot, pos = UnpackBytes32(data, pos)
	...
	payload.TransitionRecord.CheckpointHash, pos = UnpackBytes32(data, pos)
...
func (e *Encoder) encodeTransition(data []byte, pos int, transition *shastaBindings.IInboxTransition) int {
	...
	pos = PackUint48(data, pos, transition.Checkpoint.BlockNumber.Uint64())
	pos = PackBytes32(data, pos, transition.Checkpoint.BlockHash)
	pos = PackBytes32(data, pos, transition.Checkpoint.StateRoot)
...
func (e *Encoder) decodeTransition(data []byte, pos int) (*shastaBindings.IInboxTransition, int) {
	...
	// Decode BlockMiniHeader
	number, newPos := UnpackUint48(data, pos)
	transition.Checkpoint.BlockNumber = new(big.Int).SetUint64(number)
	pos = newPos

	transition.Checkpoint.BlockHash, pos = UnpackBytes32(data, pos)
	transition.Checkpoint.StateRoot, pos = UnpackBytes32(data, pos)
...
func (e *Encoder) encodeTransitionRecord(data []byte, pos int, record *shastaBindings.IInboxTransitionRecord) int {
	...
	pos = PackBytes32(data, pos, record.CheckpointHash)
...
func (e *Encoder) decodeTransitionRecord(data []byte, pos int) (*shastaBindings.IInboxTransitionRecord, int) {
	...
	record.CheckpointHash, pos = UnpackBytes32(data, pos)
...
func (e *Encoder) calculateProposeDataSize(input *shastaBindings.IInboxProposeInput) int {
	...
	// Fixed: span(1) + transitionHash(32) + CheckpointHash(32) + array length(3) = 68
