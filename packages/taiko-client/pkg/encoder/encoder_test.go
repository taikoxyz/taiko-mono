...
func TestEncodeDecodeProvedEvent(t *testing.T) {
	...
	payload := &ProvedEventPayload{
		...
		Transition: shastaBindings.IInboxTransition{
			...
			Checkpoint: shastaBindings.ICheckpointManagerCheckpoint{
				...
				BlockHash:   [32]byte{11, 12, 13, 14, 15},
		...
		TransitionRecord: shastaBindings.IInboxTransitionRecord{
			...
			CheckpointHash: [32]byte{26, 27, 28, 29, 30},
	...
	if decoded.Transition.Checkpoint.BlockNumber.Cmp(payload.Transition.Checkpoint.BlockNumber) != 0 {
		t.Errorf("Transition.Checkpoint.BlockNumber mismatch: got %v, want %v",
			decoded.Transition.Checkpoint.BlockNumber, payload.Transition.Checkpoint.BlockNumber)
	}
	if !bytes.Equal(decoded.Transition.Checkpoint.BlockHash[:], payload.Transition.Checkpoint.BlockHash[:]) {
		t.Errorf("Transition.Checkpoint.BlockHash mismatch")
	}
	if !bytes.Equal(decoded.Transition.Checkpoint.StateRoot[:], payload.Transition.Checkpoint.StateRoot[:]) {
		t.Errorf("Transition.Checkpoint.StateRoot mismatch")
	}
	...
	if !bytes.Equal(decoded.TransitionRecord.CheckpointHash[:], payload.TransitionRecord.CheckpointHash[:]) {
		t.Errorf("TransitionRecord.CheckpointHash mismatch")
	}
}
...
func TestEmptyBlockMiniHeaderOptimization(t *testing.T) {
	...
	if decodedEmpty.Checkpoint.BlockNumber.Cmp(big.NewInt(0)) != 0 {
		t.Errorf("Empty BlockMiniHeader.BlockNumber should be 0")
	}
	...
	if decodedNonEmpty.Checkpoint.BlockNumber.Cmp(big.NewInt(999)) != 0 {
		t.Errorf("Non-empty BlockMiniHeader.BlockNumber mismatch: got %v, want 999",
			decodedNonEmpty.Checkpoint.BlockNumber)
	}
}
