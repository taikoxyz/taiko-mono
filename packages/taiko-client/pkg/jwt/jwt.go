...
// Parses a ApiKey secret from a file path. This secret is required when connecting to execution nodes
...
func ParseSecretFromFile(jwtSecretFile string) ([]byte, error) {
	...
	if len(strData) == 0 {
		return nil, fmt.Errorf("provided ApiKey secret in file %s cannot be empty", jwtSecretFile)
	}
	...
	if len(secret) < 32 {
		return nil, errors.New("provided ApiKey secret should be a hex string of at least 32 bytes")
	}
	return secret, nil
}
