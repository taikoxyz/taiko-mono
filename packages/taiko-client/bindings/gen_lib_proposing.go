...
// LibProposingBondDepositedIterator is returned from FilterBondDeposited and is used to iterate over the raw logs and unpacked data for BondDeposited events raised by the LibProposing contract.
type LibProposingBondDepositedIterator struct {
	Event *LibProposingBondDeposited // Event containing the contract specifics and raw log
...
func (it *LibProposingBondDepositedIterator) Next() bool {
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(LibProposingBondDeposited)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *LibProposingBondDepositedIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *LibProposingBondDepositedIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// LibProposingBondDeposited represents a BondDeposited event raised by the LibProposing contract.
type LibProposingBondDeposited struct {
	User   common.Address
	Amount *big.Int
	Raw    types.Log // Blockchain specific contextual infos
}
...
func (_LibProposing *LibProposingFilterer) FilterBondDeposited(opts *bind.FilterOpts, user []common.Address) (*LibProposingBondDepositedIterator, error) {
	...
	logs, sub, err := _LibProposing.contract.FilterLogs(opts, "BondDeposited", userRule)
	if err != nil {
		return nil, err
	}
	return &LibProposingBondDepositedIterator{contract: _LibProposing.contract, event: "BondDeposited", logs: logs, sub: sub}, nil
}
...
//
// Solidity: event BondDeposited(address indexed user, uint256 amount)
func (_LibProposing *LibProposingFilterer) WatchBondDeposited(opts *bind.WatchOpts, sink chan<- *LibProposingBondDeposited, user []common.Address) (event.Subscription, error) {
	...
	logs, sub, err := _LibProposing.contract.WatchLogs(opts, "BondDeposited", userRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(LibProposingBondDeposited)
				if err := _LibProposing.contract.UnpackLog(event, "BondDeposited", log); err != nil {
					return err
				}
				event.Raw = log
...
//
// Solidity: event BondDeposited(address indexed user, uint256 amount)
func (_LibProposing *LibProposingFilterer) ParseBondDeposited(log types.Log) (*LibProposingBondDeposited, error) {
	event := new(LibProposingBondDeposited)
	if err := _LibProposing.contract.UnpackLog(event, "BondDeposited", log); err != nil {
		return nil, err
	}
