...
func (_TaikoToken *TaikoTokenTransactor) SetResolver(opts *bind.TransactOpts, _resolver common.Address) (*types.Transaction, error) {
	return _TaikoToken.contract.Transact(opts, "setResolver", _resolver)
}

// SetResolver is a paid mutator transaction binding the contract method 0x4e543b26.
//
// Solidity: function setResolver(address _resolver) returns()
func (_TaikoToken *TaikoTokenSession) SetResolver(_resolver common.Address) (*types.Transaction, error) {
	return _TaikoToken.Contract.SetResolver(&_TaikoToken.TransactOpts, _resolver)
}

// SetResolver is a paid mutator transaction binding the contract method 0x4e543b26.
//
// Solidity: function setResolver(address _resolver) returns()
func (_TaikoToken *TaikoTokenTransactorSession) SetResolver(_resolver common.Address) (*types.Transaction, error) {
	return _TaikoToken.Contract.SetResolver(&_TaikoToken.TransactOpts, _resolver)
}
...
// TaikoTokenResolverUpdatedIterator is returned from FilterResolverUpdated and is used to iterate over the raw logs and unpacked data for ResolverUpdated events raised by the TaikoToken contract.
type TaikoTokenResolverUpdatedIterator struct {
	Event *TaikoTokenResolverUpdated // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *TaikoTokenResolverUpdatedIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(TaikoTokenResolverUpdated)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *TaikoTokenResolverUpdatedIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *TaikoTokenResolverUpdatedIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// TaikoTokenResolverUpdated represents a ResolverUpdated event raised by the TaikoToken contract.
type TaikoTokenResolverUpdated struct {
...
func (_TaikoToken *TaikoTokenFilterer) FilterResolverUpdated(opts *bind.FilterOpts) (*TaikoTokenResolverUpdatedIterator, error) {

	logs, sub, err := _TaikoToken.contract.FilterLogs(opts, "ResolverUpdated")
	if err != nil {
		return nil, err
	}
	return &TaikoTokenResolverUpdatedIterator{contract: _TaikoToken.contract, event: "ResolverUpdated", logs: logs, sub: sub}, nil
}
...
//
// Solidity: event ResolverUpdated(address oldResolver, address newResolver)
func (_TaikoToken *TaikoTokenFilterer) WatchResolverUpdated(opts *bind.WatchOpts, sink chan<- *TaikoTokenResolverUpdated) (event.Subscription, error) {

	logs, sub, err := _TaikoToken.contract.WatchLogs(opts, "ResolverUpdated")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(TaikoTokenResolverUpdated)
				if err := _TaikoToken.contract.UnpackLog(event, "ResolverUpdated", log); err != nil {
					return err
				}
				event.Raw = log
...
//
// Solidity: event ResolverUpdated(address oldResolver, address newResolver)
func (_TaikoToken *TaikoTokenFilterer) ParseResolverUpdated(log types.Log) (*TaikoTokenResolverUpdated, error) {
	event := new(TaikoTokenResolverUpdated)
	if err := _TaikoToken.contract.UnpackLog(event, "ResolverUpdated", log); err != nil {
		return nil, err
	}
