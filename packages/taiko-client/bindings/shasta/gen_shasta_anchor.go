...
//
// Solidity: function updateState(uint48 _proposalId, address _proposer, bytes _proverAuth, bytes32 _bondInstructionsHash, (uint48,uint8,address,address)[] _bondInstructions, uint16 _blockIndex, uint48 _anchorBlockNumber, bytes32 _anchorBlockHash, bytes32 _anchorStateRoot, uint48 _endOfSubmissionWindowTimestamp) returns(bool isLowBondProposal_, address designatedProver_)
