...
func (_ShastaAnchor *ShastaAnchorCaller) CheckpointManager(opts *bind.CallOpts) (common.Address, error) {
	var out []interface{}
	err := _ShastaAnchor.contract.Call(opts, &out, "checkpointManager")
...
//
// Solidity: function checkpointManager() view returns(address)
func (_ShastaAnchor *ShastaAnchorSession) CheckpointManager() (common.Address, error) {
	return _ShastaAnchor.Contract.CheckpointManager(&_ShastaAnchor.CallOpts)
}

// CheckpointManager is a free data retrieval call binding the contract method 0xc0857ba0.
//
// Solidity: function checkpointManager() view returns(address)
func (_ShastaAnchor *ShastaAnchorCallerSession) CheckpointManager() (common.Address, error) {
	return _ShastaAnchor.Contract.CheckpointManager(&_ShastaAnchor.CallOpts)
}
...
//
// Solidity: function getDesignatedProver(uint48 _proposalId, address _proposer, bytes _proverAuth) view returns(bool isLowBondProposal_, address designatedProver_)
...
//
// Solidity: function updateState(uint48 _proposalId, address _proposer, bytes _proverAuth, bytes32 _bondInstructionsHash, (uint48,uint8,address,address)[] _bondInstructions, uint16 _blockIndex, uint48 _anchorBlockNumber, bytes32 _anchorBlockHash, bytes32 _anchorStateRoot, uint48 _lookaheadSlotTimestamp) returns(bool isLowBondProposal_, address designatedProver_)
func (_ShastaAnchor *ShastaAnchorTransactor) UpdateState(opts *bind.TransactOpts, _proposalId *big.Int, _proposer common.Address, _proverAuth []byte, _bondInstructionsHash [32]byte, _bondInstructions []LibBondsBondInstruction, _blockIndex uint16, _anchorBlockNumber *big.Int, _anchorBlockHash [32]byte, _anchorStateRoot [32]byte, _lookaheadSlotTimestamp *big.Int) (*types.Transaction, error) {
	return _ShastaAnchor.contract.Transact(opts, "updateState", _proposalId, _proposer, _proverAuth, _bondInstructionsHash, _bondInstructions, _blockIndex, _anchorBlockNumber, _anchorBlockHash, _anchorStateRoot, _lookaheadSlotTimestamp)
}
...
func (_ShastaAnchor *ShastaAnchorSession) UpdateState(_proposalId *big.Int, _proposer common.Address, _proverAuth []byte, _bondInstructionsHash [32]byte, _bondInstructions []LibBondsBondInstruction, _blockIndex uint16, _anchorBlockNumber *big.Int, _anchorBlockHash [32]byte, _anchorStateRoot [32]byte, _lookaheadSlotTimestamp *big.Int) (*types.Transaction, error) {
	return _ShastaAnchor.Contract.UpdateState(&_ShastaAnchor.TransactOpts, _proposalId, _proposer, _proverAuth, _bondInstructionsHash, _bondInstructions, _blockIndex, _anchorBlockNumber, _anchorBlockHash, _anchorStateRoot, _lookaheadSlotTimestamp)
}
...
// ShastaAnchorProverDesignatedIterator is returned from FilterProverDesignated and is used to iterate over the raw logs and unpacked data for ProverDesignated events raised by the ShastaAnchor contract.
type ShastaAnchorProverDesignatedIterator struct {
	Event *ShastaAnchorProverDesignated // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *ShastaAnchorProverDesignatedIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(ShastaAnchorProverDesignated)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *ShastaAnchorProverDesignatedIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *ShastaAnchorProverDesignatedIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// ShastaAnchorProverDesignated represents a ProverDesignated event raised by the ShastaAnchor contract.
type ShastaAnchorProverDesignated struct {
	Prover            common.Address
	IsLowBondProposal bool
	Raw               types.Log // Blockchain specific contextual infos
}

// FilterProverDesignated is a free log retrieval operation binding the contract event 0x03df0e9aedc491d3e593f76a5a8e65be8f122ca179759af7a84ca1f54f77c708.
//
// Solidity: event ProverDesignated(address prover, bool isLowBondProposal)
func (_ShastaAnchor *ShastaAnchorFilterer) FilterProverDesignated(opts *bind.FilterOpts) (*ShastaAnchorProverDesignatedIterator, error) {

	logs, sub, err := _ShastaAnchor.contract.FilterLogs(opts, "ProverDesignated")
	if err != nil {
		return nil, err
	}
	return &ShastaAnchorProverDesignatedIterator{contract: _ShastaAnchor.contract, event: "ProverDesignated", logs: logs, sub: sub}, nil
}

// WatchProverDesignated is a free log subscription operation binding the contract event 0x03df0e9aedc491d3e593f76a5a8e65be8f122ca179759af7a84ca1f54f77c708.
//
// Solidity: event ProverDesignated(address prover, bool isLowBondProposal)
func (_ShastaAnchor *ShastaAnchorFilterer) WatchProverDesignated(opts *bind.WatchOpts, sink chan<- *ShastaAnchorProverDesignated) (event.Subscription, error) {

	logs, sub, err := _ShastaAnchor.contract.WatchLogs(opts, "ProverDesignated")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(ShastaAnchorProverDesignated)
				if err := _ShastaAnchor.contract.UnpackLog(event, "ProverDesignated", log); err != nil {
					return err
				}
				event.Raw = log
...
// ParseProverDesignated is a log parse operation binding the contract event 0x03df0e9aedc491d3e593f76a5a8e65be8f122ca179759af7a84ca1f54f77c708.
//
// Solidity: event ProverDesignated(address prover, bool isLowBondProposal)
func (_ShastaAnchor *ShastaAnchorFilterer) ParseProverDesignated(log types.Log) (*ShastaAnchorProverDesignated, error) {
	event := new(ShastaAnchorProverDesignated)
	if err := _ShastaAnchor.contract.UnpackLog(event, "ProverDesignated", log); err != nil {
		return nil, err
	}
