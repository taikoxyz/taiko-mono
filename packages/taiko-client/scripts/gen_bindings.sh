...
cat ../protocol/out/LibDepositing.sol/LibDepositing.json |
	jq .abi |
...
cat ../protocol/out/TaikoTimelockController.sol/TaikoTimelockController.json |
	jq .abi |
	${ABIGEN_BIN} --abi - --type TaikoTimelockController --pkg bindings --out $DIR/../bindings/gen_taiko_timelock_controller.go

cat ../protocol/out/MainnetTierProvider.sol/MainnetTierProvider.json |
	jq .abi |
