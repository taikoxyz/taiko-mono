...
func (s *State) eventLoop(ctx context.Context) {
	...
	for {
		...
		case e := <-provedShastaCh:
			payload, err := s.rpc.DecodeProvedPayloadShasta(&bind.CallOpts{Context: ctx}, e.Data)
			if err != nil {
		...
		case e := <-proposedShastaCh:
			payload, err := s.rpc.DecodeProposedPayloadShasta(&bind.CallOpts{Context: ctx}, e.Data)
			if err != nil {
