...
func (s *TxListDecompressorTestSuite) TestZeroBytes() {
	s.Empty(s.d.TryDecompress(chainID, common.Big1, []byte{}, false, false))
}

func (s *TxListDecompressorTestSuite) TestCalldataSize() {
	s.Empty(s.d.TryDecompress(chainID, common.Big1, randBytes(rpc.BlockMaxTxListBytes+1), false, false))
	s.Empty(s.d.TryDecompress(chainID, common.Big1, randBytes(rpc.BlockMaxTxListBytes-1), false, false))
}
...
func (s *TxListDecompressorTestSuite) TestValidTxList() {
	...
	s.Equal(s.d.TryDecompress(chainID, common.Big1, compressed, true, false), decompressed)
	s.Equal(s.d.TryDecompress(chainID, common.Big1, compressed, false, false), decompressed)
}
...
func (s *TxListDecompressorTestSuite) TestInvalidTxList() {
	...
	s.Zero(len(s.d.TryDecompress(chainID, common.Big1, compressed, true, false)))
	s.Zero(len(s.d.TryDecompress(chainID, common.Big1, compressed, false, false)))
}

func (s *TxListDecompressorTestSuite) TestInvalidZlibBytes() {
	s.Zero(len(s.d.TryDecompress(chainID, common.Big1, randBytes(1024), true, false)))
	s.Zero(len(s.d.TryDecompress(chainID, common.Big1, randBytes(1024), false, false)))
}
