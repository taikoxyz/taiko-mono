...
CREATE INDEX IF NOT EXISTS idx_blocks_canonical_slot ON blocks (canonical, slot DESC);
...
CREATE INDEX IF NOT EXISTS idx_blobs_block_root_canonical ON blobs (block_root, canonical, blob_index);

-- Speed up pruning and canonicality updates that filter by canonical + slot range
CREATE INDEX IF NOT EXISTS idx_blobs_canonical_slot ON blobs (canonical, slot);
