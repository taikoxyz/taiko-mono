	...
	for {
		...
		case <-ticker.C:
			for _, address := range b.addresses {
		        ...
		        var l1Balance float64 = 0
				var l2Balance float64 = 0
				for _, tokenAddress := range b.erc20Addresses {
					l1Balance = l1Balance + b.checkErc20Balance(context.Background(), b.l1EthClient, "L1", tokenAddress, address)
					l2Balance = l2Balance + b.checkErc20Balance(context.Background(), b.l2EthClient, "L2", tokenAddress, address)
				...
				slog.Info("L1 ERC-20 Balance", "address", address.Hex(), "balance", l1Balance)
				slog.Info("L2 ERC-20 Balance", "address", address.Hex(), "balance", l2Balance)
			}
		}
	}
}
