...
import { TrailblazersBadgesV5 } from
    "../../contracts/trailblazers-season-2/TrailblazersS1BadgesV5.sol";
...
contract TrailblazersS1BadgesV6 is Test {
    UtilsScript public utils;

    TrailblazersBadgesV6 public s1BadgesV6;
    ...
    function setUp() public {
        ...
        s1BadgesV5 = TrailblazersBadgesV5(address(s1BadgesV2));

        // upgrade to v5
        s1BadgesV5.upgradeToAndCall(
            address(new TrailblazersBadgesV5()), abi.encodeCall(TrailblazersBadgesV5.version, ())
        );

        s1BadgesV5 = TrailblazersBadgesV5(address(s1BadgesV5));

        // set cooldown recruitment
        s1BadgesV5.setRecruitmentLockDuration(7 days);
    ...
    function test_upgrade() public {
        ...
        s1BadgesV6 = TrailblazersBadgesV6(address(s1BadgesV5));
        s1BadgesV6.setSeason2EndTimestamp(SEASON_2_END);
        s1BadgesV6.setSeason3EndTimestamp(SEASON_3_END);
        assertEq(s1BadgesV6.version(), "V6");
        vm.stopPrank();
    }
    ...
    function test_migrateS2_transferAfter() public {
        ...
        // mint the s1 badge
        bytes32 _hash = s1BadgesV6.getHash(minter, BADGE_ID);
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(mintSignerPk, _hash);

        bool canMint = s1BadgesV6.canMint(abi.encodePacked(r, s, v), minter, BADGE_ID);
        assertTrue(canMint);

        s1BadgesV6.mint(abi.encodePacked(r, s, v), BADGE_ID);
        uint256 tokenId = s1BadgesV6.tokenOfOwnerByIndex(minter, 0);

        // mint and transfer to minter a secondary badge with id 0

        vm.startPrank(minters[1]);
        _hash = s1BadgesV6.getHash(minters[1], BADGE_ID);
        (v, r, s) = vm.sign(mintSignerPk, _hash);
        canMint = s1BadgesV6.canMint(abi.encodePacked(r, s, v), minters[1], BADGE_ID);
        assertTrue(canMint);

        s1BadgesV6.mint(abi.encodePacked(r, s, v), BADGE_ID);
        uint256 secondTokenId = s1BadgesV6.tokenOfOwnerByIndex(minters[1], 0);

        s1BadgesV6.transferFrom(minters[1], minter, secondTokenId);

        // ensure balances
        assertEq(s1BadgesV6.balanceOf(minter), 2);
        assertEq(s1BadgesV6.balanceOf(minters[1]), 0);
        vm.stopPrank();

        // start migration with first badge, using v1 methods
        vm.startPrank(minter);
        wait(100);
        s1BadgesV6.startRecruitment(BADGE_ID, tokenId);
        assertEq(recruitment.isRecruitmentActive(minter), true);
        assertEq(s1BadgesV6.balanceOf(minter), 2);
        assertEq(s1BadgesV6.unlockTimestamps(tokenId), SEASON_2_END);
        ...
        s1BadgesV6.startRecruitment(BADGE_ID, secondTokenId);
        assertEq(recruitment.isRecruitmentActive(minter), true);
        assertEq(s1BadgesV6.balanceOf(minter), 2);
        assertEq(s1BadgesV6.unlockTimestamps(secondTokenId), SEASON_2_END);

        // ensure badge is frozen during season 2
        vm.warp(SEASON_2_END - block.timestamp - 1);
        vm.expectRevert(TrailblazersBadgesV4.BADGE_LOCKED.selector);
        s1BadgesV6.transferFrom(minter, minters[1], tokenId);
        vm.expectRevert(TrailblazersBadgesV4.BADGE_LOCKED.selector);
        s1BadgesV6.transferFrom(minter, minters[1], secondTokenId);
        ...
        s1BadgesV6.startRecruitment(BADGE_ID, tokenId);
        assertEq(recruitment.isRecruitmentActive(minter), true);
        assertEq(s1BadgesV6.unlockTimestamps(tokenId), SEASON_3_END);
        ...
        s1BadgesV6.transferFrom(minter, minters[1], tokenId);
        assertEq(s1BadgesV6.balanceOf(minters[1]), 2);
        assertEq(s1BadgesV6.balanceOf(minter), 0);
