...
function getKey(sender: Address, msgHash: Hex) {
...
function getSignalProof(proof: EthGetProofResponse, blockHeight: bigint) {
...
export async function generateProofToClaim({
  msgHash,
  sender,
  srcChainId,
  destChainId,
  ...
  const block = await getLatestBlock(crossChainSyncContract, provider);

  if (block.hash === null || block.number === null) {
    throw new PendingBlockError('block is pending');
  }

  const key = getKey(sender, msgHash);
  ...
  return getSignalProof(proof, block.number);
}

export async function generateProofToRelease({
  msgHash,
  sender,
  srcChainId,
  destChainId,
