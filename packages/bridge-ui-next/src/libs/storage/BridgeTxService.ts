...
export class BridgeTxService {
  ...
  private static async _getBridgeMessageSent({
    ...
    try {
      ...
      return messageSentEvents.find(
        ({ args }) =>
          args.message?.srcOwner.toLowerCase() === userAddress.toLowerCase(),
      );
    } catch (error) {
  ...
  private _getTxFromStorage(address: Address) {
    ...
    const txs = jsonParseWithDefault(
  ...
  private async _enhanceTx(
    ...
    if (waitForTx) {
      ...
      try {
        receipt = await BridgeTxService._getTransactionReceipt(
          Number(srcChainId),
          srcTxHash,
        );
      } catch (error) {
  ...
  async getAllTxByAddress(address: Address) {
    ...
    const enhanceWithTimeout = async (
      ...
      const timeoutPromise = new Promise<BridgeTransaction>((resolve) => {
        setTimeout(() => {
          log(
            "Enhancement timed out for tx, returning unenhanced",
            tx.srcTxHash,
          );
      ...
      try {
        const result = await Promise.race([
          this._enhanceTx(tx, address, true),
          timeoutPromise,
        ]);
        return result ?? tx; // Return original tx if _enhanceTx returns undefined
      } catch (error) {
        log(
          "Enhancement failed for tx, returning unenhanced",
          tx.srcTxHash,
          error,
        );
    ...
    const enhancedTxs = resolvedTxs.filter((tx): tx is BridgeTransaction =>
      Boolean(tx),
    );
    ...
    enhancedTxs.sort((tx1, tx2) => {
      if (
        tx1.msgStatus === MessageStatus.NEW &&
        tx2.msgStatus !== MessageStatus.NEW
      ) {
  ...
  removeTransactions(address: Address, txs: BridgeTransaction[]) {
    ...
    const filteredTxs = txsFromStorage.filter(
      (tx) => !txsToRemove.includes(tx.srcTxHash),
    );
