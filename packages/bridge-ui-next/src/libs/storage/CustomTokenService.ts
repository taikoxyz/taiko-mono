...
export class CustomTokenService implements TokenService {
  ...
  private readonly handlerMap: WeakMap<
    (tokens: Token[]) => void,
    (event: Event) => void
  > = new WeakMap();
  ...
  private _getTokensFromStorage(address: string): Token[] {
    ...
    log("Retrieved tokens from storage", {
      address,
  ...
  storeToken(token: Token, address: string): Token[] {
    ...
    if (!doesTokenAlreadyExist) {
      log("Adding new token to storage", {
      ...
      log("Token already exists in storage, skipping", {
        tokenSymbol: token.symbol,
        address,
      });
    }
    ...
    this.storageChangeNotifier.dispatchEvent(
      new CustomEvent("storageChange", { detail: tokens }),
    );

    log("Token storage updated", {
      address,
  ...
  removeToken(token: Token, address: string): Token[] {
    ...
    this.storageChangeNotifier.dispatchEvent(
      new CustomEvent("storageChange", { detail: updatedTokenList }),
    );
  ...
  updateToken(token: Token, address: Address): Token[] {
    ...
    const filteredAddresses = Object.fromEntries(
      Object.entries(token.addresses).filter(
    ...
    if (storedToken) {
      ...
      log("No matching stored token found for update", {
        tokenSymbol: token.symbol,
        address,
      });
    }
