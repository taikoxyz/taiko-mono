...
export class RelayerAPIService {
  ...
  private static _filterDuplicateAndWrongBridge(
    ...
    for (const item of items) {
      ...
      const { bridgeAddress } =
        routingContractsMap[Number(srcChainId)][Number(destChainId)];
      ...
      const isCorrectBridgeAddress =
        address?.toLowerCase() === bridgeAddress?.toLowerCase();
      const areChainsSupported =
        isSupportedChain(Number(destChainId)) &&
        isSupportedChain(Number(srcChainId));
  ...
  async getAllBridgeTransactionByAddress(
    ...
    const txsPromises = txs.map(async (bridgeTx) => {
      ...
      const receiverMatch =
        bridgeTx.message &&
        getAddress(bridgeTx.message.destOwner) === getAddress(address);
      ...
      const receipt = await RelayerAPIService._getTransactionReceipt(
        Number(srcChainId),
        srcTxHash,
      );
    ...
    ).filter((tx): tx is BridgeTransaction => Boolean(tx)); // Removes undefined values
  ...
  async getBlockInfo(): Promise<Record<number, RelayerBlockInfo>> {
    ...
    try {
      ...
      if (data?.data.length > 0) {
        data.data.forEach(
          (blockInfo: RelayerBlockInfo) =>
            (blockInfoRecord[blockInfo.chainID] = blockInfo),
        );
      }
    } catch (error) {
      console.error(error);
