...
describe("EventIndexerAPIService", () => {
  it("should fetch NFTs by address", async () => {
    ...
    const result = await service.getNftsByAddress({
      address: zeroAddress,
      chainID: 1n,
    });
    ...
    expect(axios.get).toHaveBeenCalledWith(
      "https://api.example.com/nftsByAddress",
      expect.any(Object),
    );
  });
  ...
  it("should throw an error on API failure", async () => {
    ...
    const service = new EventIndexerAPIService("https://api.example.com");
    await expect(
      service.getNftsByAddress({ address: zeroAddress, chainID: 1n }),
    ).rejects.toThrow("could not fetch transactions from API");
    expect(axios.get).toHaveBeenCalledWith(
      "https://api.example.com/nftsByAddress",
      expect.any(Object),
    );
