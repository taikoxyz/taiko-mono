...
export class BridgeProver {
   ...
   */
  async getLatestSyncedBlockNumber(
    srcChainId: bigint,
    destChainId: bigint,
  ): Promise<bigint> {
    if (isL2Chain(Number(destChainId))) {
      ...
      const anchorAddress =
        routingContractsMap[Number(destChainId)][Number(srcChainId)]
          .anchorForkRouter;
      if (!anchorAddress)
        throw new ClientError("No anchor address configured for this route");
    ...
    const fromBlock =
      currentBlock > MAX_CHECKPOINT_SEARCH_BLOCKS
        ? currentBlock - MAX_CHECKPOINT_SEARCH_BLOCKS
        : 0n;
    ...
    if (logs.length === 0)
      throw new BlockNotSyncedError("No checkpoints found");
  ...
  async getEncodedSignalProof({ bridgeTx }: { bridgeTx: BridgeTransaction }) {
    ...
    if (!blockNumber)
      throw new ProofGenerationError("Block number is not defined");

    const { srcChainId, destChainId } = message;
    const latestSyncedBlock = await this.getLatestSyncedBlockNumber(
      srcChainId,
      destChainId,
    );
  ...
  private async generateSingleHopProof({
    ...
    const ethProof = await this.getProof({
      ...
      key,
      signalServiceAddress:
        routingContractsMap[srcChainId][destChainId].signalServiceAddress,
    });
  ...
  private async verifyCheckpoint(
    ...
    const destSignalService =
      routingContractsMap[destChainId][srcChainId].signalServiceAddress;
    ...
    try {
      ...
      const anchorAddress =
        routingContractsMap[destChainId][srcChainId].anchorForkRouter;
      if (anchorAddress) {
        try {
          ...
          if (
            checkpointStore.toLowerCase() !== destSignalService.toLowerCase()
          ) {
  ...
  async getEncodedSignalProofForRecall({
    bridgeTx,
  }: {
  ...
  async getProof(args: GetProofArgs) {
    ...
    if (ethProof.storageProof[0].value === toHex(0)) {
      throw new ProofGenerationError(
        "proof will not be valid, expected storageProof to not be 0",
      );
    }
  ...
  async verifyProofPreFlight({
    ...
    try {
      ...
      throw new ProofGenerationError(
        `Checkpoint not found for blockId ${blockId}`,
      );
    }
