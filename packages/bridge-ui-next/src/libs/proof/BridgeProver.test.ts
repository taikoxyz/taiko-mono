...
describe("BridgeProver", () => {
  ...
  describe("getSignalSlot()", () => {
    it("should return signalslot", async () => {
      ...
      // When
      const actual = await bridgeProver.getSignalSlot(
        chainId,
        contractAddress,
        msgHash,
      );
      ...
      // Then
      expect(actual).toBe(
        "0xff44c639166e92e749d33ed59058223394b1e34751584ee25bc5a09f45ad4eba",
      );
    });
  });
  ...
  describe("getLatestSyncedBlockNumber()", () => {
    it("should return block number from Anchor when dest is L2", async () => {
      ...
      vi.mocked(readContract).mockResolvedValueOnce({
        anchorBlockNumber: BLOCK_NUMBER_1,
      });

      // When
      const actual = await bridgeProver.getLatestSyncedBlockNumber(
        chainA,
        chainB,
      );
    ...
    it("should return block number from CheckpointSaved events when dest is L1", async () => {
      ...
      const mockClient = {
        ...
        getContractEvents: vi
          .fn()
          .mockResolvedValue([{ args: { blockNumber: BLOCK_NUMBER_1 } }]),
      };
      ...
      expect(mockClient.getContractEvents).toHaveBeenCalledWith({
        address:
          routingContractsMap[L1_CHAIN_ID][L2_CHAIN_ID].signalServiceAddress,
    ...
    it("should throw BlockNotSyncedError when no checkpoints found", async () => {
      ...
      // When / Then
      await expect(
        bridgeProver.getLatestSyncedBlockNumber(chainA, chainB),
      ).rejects.toThrow(BlockNotSyncedError);
    });
  });
  ...
  describe("getEncodedSignalProof()", () => {
    ...
    const validSignalSlot =
      "0x1eb55981d51be65667e21e49934d6cb2f5fcc239607297a8280d18c3f64a978b";
    const validEthProofResult = {
      ...
      balance: "0x0",
      codeHash:
        "0xdc679fd48cf611aa38e906adc93928c5f8f6fae534978ea2fa4f5935f5ed1b2c",
      nonce: "0x1",
      storageHash:
        "0xbc749bb55c01b09e1b51bf8e1c179281a749d477474af9b2295db96e10b6de64",
      storageProof: [
        {
          key: "0x1eb55981d51be65667e21e49934d6cb2f5fcc239607297a8280d18c3f64a978b",
          value:
            "0x36973cd4172846df09d48d0bf428802d674848d39229962bbaec2e2fea465f15",
          proof: [
    ...
    const realBridgeTransaction: BridgeTransaction = {
      ...
      srcTxHash:
        "0xc0a3476ac80c3468a65702864ff4ef22ca5b54afac3d0911fb14165cdada7f1c",
      destTxHash: "" as Hash,
      msgHash:
        "0x36973cd4172846df09d48d0bf428802d674848d39229962bbaec2e2fea465f15",
      srcChainId: 32382n,
    ...
    it("should throw if the bridgeTx does not contain a message", async () => {
      ...
      const getEncodedSignalProof = async () =>
        await bridgeProver.getEncodedSignalProof({
          bridgeTx: { ...realBridgeTransaction, message: undefined },
        });

      // Then
      await expect(getEncodedSignalProof()).rejects.toThrow(
        ProofGenerationError,
      );
    });
    ...
    it("should throw if the block is not yet synced", async () => {
      ...
      vi.spyOn(bridgeProver, "getSignalSlot").mockResolvedValue(
        validSignalSlot,
      );
      ...
      const mockClient = {
        ...
        getBlock: vi
          .fn()
          .mockResolvedValue({ number: validBlockNo, hash: "0x0" }),
    ...
    it("should throw if the block doesn't exist on the src chain", async () => {
      ...
      const getEncodedSignalProof = async () =>
        await bridgeProver.getEncodedSignalProof({
          bridgeTx: realBridgeTransaction,
        });
    ...
    it("should return encoded signal proof for a singleHop", async () => {
      ...
      vi.spyOn(bridgeProver, "encodeHopProofs").mockReturnValue(
        validEncodedHopProofs,
      );
      vi.spyOn(bridgeProver, "verifyProofPreFlight").mockResolvedValue(
        undefined,
      );

      // When
      const encodedSignalProof = await bridgeProver.getEncodedSignalProof({
        bridgeTx: realBridgeTransaction,
      });
  ...
  describe("getProof()", () => {
    ...
    it("should fail when client cannot be found", async () => {
      ...
      expect(getPublicClient).toHaveBeenCalledWith(config, {
        chainId: L1_CHAIN_ID,
      });
    });
  });
  ...
  describe("getEncodedSignalProof() - checkpoint verification failures", () => {
    ...
    it("should throw ProofGenerationError when checkpoint is not found on destination", async () => {
      ...
      // When / Then
      await expect(
        bridgeProver.getEncodedSignalProof({ bridgeTx: realBridgeTransaction }),
  ...
  describe("verifyProofPreFlight()", () => {
    ...
    it("should succeed when checkpoint exists and state roots match", async () => {
      ...
      await expect(
        bridgeProver.verifyProofPreFlight(testParams),
    ...
    it("should throw ProofGenerationError when state root mismatches", async () => {
      ...
      vi.mocked(readContract).mockResolvedValueOnce({
        ...
        blockHash: zeroHash,
        stateRoot:
          "0xdifferentrootdifferentrootdifferentrootdifferentrootdifferentroot",
      });
