...
export async function getCanonicalInfoForToken({
  ...
  if (token.addresses[srcChainId] && token.addresses[destChainId]) {
    ...
    for (const [currentSrcChainId, address] of Object.entries(
      token.addresses,
    )) {
      ...
      if (isCanonicalAddress(address)) {
        ...
        const tokenInfo = tokenInfoStore.getState()[address];
      ...
      if (canonicalTokenAddress && canonicalChain) {
        log(
          `Found canonical address ${canonicalTokenAddress} on chain ${canonicalChain}`,
        );
    ...
    const destinationId =
      srcChainTokenChainId === srcChainId ? destChainId : srcChainTokenChainId;

    // check store first to save some time
    if (isCanonicalAddress(srcChainTokenAddress)) {
      log("found canonical info in store");

      const tokenInfo = tokenInfoStore.getState()[srcChainTokenAddress];
      const canonicalChain = tokenInfo.canonical?.chainId;
      const canonicalAddress = tokenInfo.canonical?.address;
...
export const getCanonicalInfoForAddress = async ({
  ...
  if (canonicalTokenAddress && canonicalChain) {
    ...
    log(
      "No canonical info found for address",
      address,
      srcChainId,
      destChainId,
    );
...
const _getStatus = async ({
  ...
  const srcCanonicalTokenInfo =
    (await srcTokenVaultContract.read.bridgedToCanonical([
      srcChainTokenAddress,
    ])) as Address;
  const srcCanonicalCheck = srcCanonicalTokenInfo[1] as Address;

  const destCanonicalTokenInfo =
    (await destTokenVaultContract.read.bridgedToCanonical([
      srcChainTokenAddress,
    ])) as Address;
