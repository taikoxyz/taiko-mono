...
const zeroAddressMap = chains.reduce(
  (acc, chainId) => ({ ...acc, [chainId]: zeroAddress }),
  {},
);
...
export const testERC20Tokens: Token[] = customToken.filter(
...
export const isWrapped = (token: Token): boolean =>
  hasAttribute(token, TokenAttributeKey.Wrapped);
export const isSupported = (token: Token): boolean =>
  hasAttribute(token, TokenAttributeKey.Supported);
export const isStablecoin = (token: Token): boolean =>
  hasAttribute(token, TokenAttributeKey.Stablecoin);
export const isMintable = (token: Token): boolean =>
  hasAttribute(token, TokenAttributeKey.Mintable);
