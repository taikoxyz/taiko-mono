...
// Ported from a svelte `writable` to a module-local vanilla zustand store; the
// imperative `getState()` / `setState()` access mirrors the original `get()` / `update()`.
const hashToNFTStore = createStore<{ [hash: string]: NFT }>(() => ({}));
...
export const mapTransactionHashToNFT = async ({
  ...
  // check store
  const store = hashToNFTStore.getState();
  if (store[hash]) {
  ...
  // cache the data
  hashToNFTStore.setState(
    { ...hashToNFTStore.getState(), [hash]: token },
    true,
  );
  return token;
};
