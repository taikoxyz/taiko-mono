...
export async function getTokenAddresses({
  ...
  let bridgedChainId: number | null =
    canonicalChainId === srcChainId ? destChainId : srcChainId;

  const bridgedAddress = canonicalAddress
    ? await _getBridgedAddress({
        canonicalAddress,
        canonicalChainId,
        bridgedChainId,
        type,
  ...
  const tokenInfo = {
    ...
    bridged:
      bridgedChainId && bridgedAddress
        ? { chainId: bridgedChainId, address: bridgedAddress }
        : null,
  };
...
const _getBridgedAddress = async ({
  ...
  const tokenInfo = tokenInfoStore.getState()[canonicalAddress];
  ...
  const bridgedVaultContract = getContract({
    ...
    client,
    address:
      routingContractsMap[bridgedChainId][canonicalChainId][vaultAddressKey],
  });
