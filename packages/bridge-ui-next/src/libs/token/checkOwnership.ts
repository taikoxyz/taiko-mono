...
export const checkOwnershipOfNFTs = async (
  nfts: NFT[],
  ...
  const failedOwnershipChecks = flattenedResults.filter(
    (result) => !result.isOwner,
  );
  const successfulOwnershipChecks = flattenedResults.filter(
    (result) => result.isOwner,
  );
...
export const checkOwnershipOfNFT = async (
  nft: NFT,
  accountAddress: Address,
  chainId: number,
) => {
...
export const checkOwnership = async (
  ...
  if (Array.isArray(tokenIds)) {
    const checkPromises = tokenIds.map(async (tokenId) => {
      const isOwner = await determineOwnership(
        tokenType!,
        tokenAddress,
        tokenId,
        accountAddress,
