...
export const getTokenWithInfoFromAddress = async ({
  ...
  try {
    const tokenType: TokenType =
      type ?? (await detectContractType(contractAddress, srcChainId));
    if (tokenType === TokenType.ERC20) {
      return getERC20Info(contractAddress, srcChainId, tokenType);
    } else if (tokenType === TokenType.ERC1155) {
      return getERC1155Info(
        contractAddress,
        srcChainId,
        owner,
        tokenId,
...
const getERC721Info = async (
  ...
  log(
    `getting name, symbol and uri for ERC721 token ${contractAddress} id: ${tokenId} on chain ${srcChainId}`,
  );
  const name = await safeReadContract({
    address: contractAddress,
    abi: erc721Abi,
