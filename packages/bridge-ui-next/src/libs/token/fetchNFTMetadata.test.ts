...
describe("fetchNFTMetadata()", () => {
  ...
  it("should return null if tokenInfo or tokenInfo.canonical.address is not defined", async () => {
    // Given
    connectedSourceChain.setState({ id: L1_CHAIN_ID } as Chain);
    destNetwork.setState({ id: L2_CHAIN_ID } as Chain);
      ...
      describe("fetchCrossChainNFTMetadata", () => {
        beforeAll(() => {
          ...
          vi.mock("./fetchNFTMetadata", async (importOriginal) => {
            const actual =
              await importOriginal<typeof import("./fetchNFTMetadata")>();
            return {
              ...actual,
              crossChainFetchNFTMetadata: vi
                .fn()
        ...
        it("should return metadata if canonical token has valid metadata ", async () => {
          ...
          vi.mocked(getTokenAddresses)
            .mockResolvedValue(mockTokenInfo)
            .mockResolvedValue(mockTokenInfo);
          vi.mocked(isMetadataCached).mockReturnValue(false);

          vi.mocked(getTokenWithInfoFromAddress).mockResolvedValue(
            MOCK_CANONICAL_NFT,
          );
        ...
        it("should throw FetchMetadataError if no uri is found crosschain either", async () => {
          ...
          // Then
          await expect(
            fetchNFTMetadata(MOCK_BRIDGED_NFT),
          ).rejects.toBeInstanceOf(FetchMetadataError);
        });
      });
    });
  });
