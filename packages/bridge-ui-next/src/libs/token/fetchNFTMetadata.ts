...
export async function fetchNFTMetadata(
  ...
  if (
    ...
    log(
      "found cached metadata for",
      tokenInfo.canonical?.address,
      token.metadata,
    );
    // Update cache
    const data = getMetadataFromCache({
      address: tokenInfo.canonical?.address,
      id: token.tokenId,
    });
    if (data) return data;
  ...
  try {
    ...
    throw new FetchMetadataError(
      `Failed to fetch NFT metadata: ${(error as AxiosError).message}`,
    );
  }
}
...
const crossChainFetchNFTMetadata = async (
  ...
  try {
    ...
    if (!srcChainId || !destChainId || srcChainId === destChainId)
    ...
    if (
      ...
      tokenInfo.canonical &&
      tokenInfo.canonical.address &&
      tokenInfo.canonical.chainId
    ) {
      const canonicalChainID = tokenInfo.canonical.chainId;
      const canonicalAddress = tokenInfo.canonical.address;
      log(
        `Fetching metadata for ${token.name} from chain ${canonicalChainID} at address ${canonicalAddress}`,
      );
      ...
      canonicalToken.addresses = {
        ...token.addresses,
        [canonicalChainID]: canonicalAddress,
      };
    ...
    throw new FetchMetadataError("No crosschain metadata found", {
      cause: error,
    });
  }
};
