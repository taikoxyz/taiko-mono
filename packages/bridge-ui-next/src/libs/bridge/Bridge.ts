...
export abstract class Bridge {
  ...
  protected async beforeProcessing(
    { bridgeTx, wallet }: ClaimArgs,
    skipMessageStatusCheck = false,
  ) {
    ...
    if (!wallet || !wallet.account || !wallet.chain)
      throw new Error("Wallet is not connected");

    const userAddress = wallet.account.address;
    // Are we the owner of the message, either src or dest?
    if (
      getAddress(srcOwner) !== getAddress(userAddress) &&
      getAddress(destOwner) !== getAddress(userAddress)
    ) {
      if (bridgeTx.message?.gasLimit === 0) {
        throw new WrongOwnerError(
          "user cannot process this as it is not their message",
        );
      }
    }

    const destBridgeAddress =
      routingContractsMap[destChainId][srcChainId].bridgeAddress;
  ...
  protected async beforeClaiming({
    ...
    if (messageStatus === MessageStatus.FAILED) {
      throw new MessageStatusError(
        "user can not process this as message has failed",
      );
    }
  }
  ...
  async processMessage(
    ...
    const { messageStatus, destBridgeAddress } = await this.beforeProcessing(
      args,
    ...
    if (!message || !msgHash)
      throw new ProcessMessageError(
        `message or msgHash  is not defined, ${message}, ${msgHash}, ${blockNumber}`,
      );
    ...
    const srcBridgeContract = await getContract({
      ...
      abi: bridgeAbi,
      address:
        routingContractsMap[Number(message.srcChainId)][
          Number(message.destChainId)
        ].bridgeAddress,
    });
    ...
    try {
      ...
      if (skipMessageStatusCheck) {
        ...
        txHash = await this.processNewMessage(
          { ...args, bridgeContract, client },
          force,
        );
        ...
        txHash = await this.release({
          ...args,
          bridgeContract: srcBridgeContract,
          client,
        });
      } else {
        throw new ProcessMessageError(
          "Message status not supported for claiming.",
        );
      }
      return txHash;
    } catch (err) {
  ...
  private async processNewMessage(
    ...
    const destClient = getPublicClient(config, {
      chainId: Number(message.destChainId),
    });
    if (!destClient) throw new Error("Could not get public client");

    let estimatedGas;
    try {
      const estimation1 = bridgeContract.estimateGas.processMessage(
        [message, proof],
      ...
      if (gasEstimates.length > 0) {
        ...
        estimatedGas = gasEstimates.reduce(
          (min, gas) => (gas < min ? gas : min),
          gasEstimates[0],
        );
      } else {
    ...
    if (
      message.to &&
      (await isSmartContract(message.to, Number(message.destChainId)))
    ) {
  ...
  private async retryMessage(args: RetryMessageArgs): Promise<Hash> {
    ...
    isFinalAttempt
      ? log("Retrying message for the last time")
      : log("Retrying message");

    if (!message) throw new ProcessMessageError("Message is not defined");

    let estimatedGas = await bridgeContract.estimateGas.retryMessage(
  ...
  private async release(args: ReleaseArgs) {
    ...
    const proof = await this._prover.getEncodedSignalProofForRecall({
      bridgeTx,
    });

    log("Estimating gas for recallMessage", bridgeContract.address, [
      message,
      proof,
    ]);

    let estimatedGas = await bridgeContract.estimateGas.recallMessage(
      [message, proof],
      { account: client.account },
    );
    ...
    if (
      message.from &&
      (await isSmartContract(message.from, Number(message.srcChainId)))
    ) {
