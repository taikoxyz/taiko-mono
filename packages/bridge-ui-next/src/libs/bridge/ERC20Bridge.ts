...
import type {
  ApproveArgs,
  ERC20BridgeArgs,
  ERC20BridgeTransferOp,
  RequireAllowanceArgs,
} from "./types";
...
export class ERC20Bridge extends Bridge {
  ...
  async estimateGas(args: ERC20BridgeArgs) {
    ...
    const { tokenVaultContract, sendERC20Args } =
      await ERC20Bridge._prepareTransaction(args as ERC20BridgeArgs);
    ...
    const estimatedGas = await tokenVaultContract.estimateGas.sendToken(
      [sendERC20Args],
      { value },
    );
  ...
  async approve(args: ApproveArgs, reset = false) {
    ...
    if (!requireAllowance) {
      throw new NoAllowanceRequiredError(
        `no allowance required for the amount ${amount}`,
      );
    }

    try {
      log(
        `Calling approve for spender "${spenderAddress}" for token "${tokenAddress}" with amount`,
        amount,
      );
  ...
  async bridge(args: ERC20BridgeArgs) {
    ...
    if (requireAllowance) {
      throw new InsufficientAllowanceError(
        `Insufficient allowance for the amount ${amount}`,
      );
    }

    const { tokenVaultContract, sendERC20Args } =
      await ERC20Bridge._prepareTransaction(args);
    const { fee } = sendERC20Args;
