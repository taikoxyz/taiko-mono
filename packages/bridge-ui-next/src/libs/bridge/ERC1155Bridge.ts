...
export class ERC1155Bridge extends Bridge {
  ...
  async isApprovedForAll({
    ...
    const isApprovedForAll = await tokenContract.read.isApprovedForAll([
      owner,
      spenderAddress,
    ]);
  ...
  async estimateGas(args: ERC1155BridgeArgs): Promise<bigint> {
    const { tokenVaultContract, sendERC1155Args } =
      await ERC1155Bridge._prepareTransaction(args);
  ...
  async bridge(args: ERC1155BridgeArgs) {
    ...
    try {
      ...
      throw new SendERC1155Error("failed to bridge ERC1155 token", {
        cause: err,
      });
    }
  }
