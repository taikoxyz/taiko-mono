...
export async function checkBalanceToBridge(
  args: CheckBalanceToBridgeTokenArgs,
) {
...
async function handleEthBridge(
  args: CheckBalanceToBridgeCommonArgs,
): Promise<void> {
  const { bridgeAddress } =
    routingContractsMap[args.srcChainId][args.destChainId];
  ...
  try {
    ...
    if (`${err}`.includes("transaction exceeds the balance")) {
      throw new InsufficientBalanceError(
        "you do not have enough balance to bridge ETH",
        { cause: err },
      );
    }

    if (`${err}`.includes("reverted with the following reason: Failed")) {
      throw new RevertedWithFailedError("Bridge transaction failed", {
        cause: err,
      });
    }

    throw new RevertedWithoutMessageError("reverted without reason", {
      cause: err,
    });
  }
  ...
  if (estimatedCost > balance.value - _amount) {
    throw new InsufficientBalanceError(
      "you do not have enough balance to bridge",
    );
  }
}
...
async function handleErc1155Bridge(args: CheckBalanceToBridgeTokenArgs) {
  ...
  if (tokenInfo.bridged) {
    ...
    if (address && address !== tokenAddress) {
      ...
      log(
        "Token already deployed to destination chain",
        address,
  ...
  try {
    ...
    if (`${err}`.includes("transaction exceeds the balance")) {
      throw new InsufficientBalanceError(
        "you do not have enough balance to bridge ERC1155",
        { cause: err },
      );
    }
    ...
    throw new SendERC1155Error("failed to estimate cost for ERC1155 bridge", {
      cause: err,
    });
  }
...
async function handleErc20Bridge(
  args: CheckBalanceToBridgeTokenArgs,
): Promise<void> {
    ...
    try {
      ...
      if (`${err}`.includes("insufficient allowance")) {
        throw new InsufficientAllowanceError(
          `insufficient allowance for the amount ${_amount}`,
          { cause: err },
        );
      }
      ...
      if (`${err}`.includes("transaction exceeds the balance")) {
        throw new InsufficientBalanceError(
          "you do not have enough balance to bridge ERC20",
          { cause: err },
        );
      }
      ...
      throw new SendERC20Error("failed to estimate cost for ERC20 bridge", {
        cause: err,
      });
    }
    if (!estimatedCost) throw new Error("estimated cost is undefined");
