// Ported from packages/bridge-ui/src/libs/bridge/checkQuota.test.ts
...
describe("isClaimBlockedByQuota", () => {
  ...
  it("checks ETH quota with the zero address token", async () => {
    ...
    await expect(
      isClaimBlockedByQuota(bridgeTx({ tokenType: TokenType.ETH })),
    ).resolves.toBe(true);
    expect(readContract).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({
  ...
  it("returns false when the destination route has no quota manager configured", async () => {
    await expect(
      isClaimBlockedByQuota(bridgeTx({ srcChainId: 1n, destChainId: 21n })),
    ).resolves.toBe(false);
  ...
  it("returns false for NFTs because quota does not apply", async () => {
    await expect(
      isClaimBlockedByQuota(bridgeTx({ tokenType: TokenType.ERC721 })),
    ).resolves.toBe(false);
    expect(readContract).not.toHaveBeenCalled();
  });
});
