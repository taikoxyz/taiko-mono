...
export class ETHBridge extends Bridge {
  private static async _prepareTransaction(args: ETHBridgeArgs) {
    ...
    const [senderAmount, recipientAmount] =
      to.toLowerCase() === owner.toLowerCase()
        ? [amount, BigInt(0)]
        : [BigInt(0), amount];
    let value;
  ...
  async estimateGas(args: ETHBridgeArgs) {
    const { bridgeContract, message } =
      await ETHBridge._prepareTransaction(args);
    ...
    const estimatedGas = await bridgeContract.estimateGas.sendMessage(
      [message],
      { value },
    );
