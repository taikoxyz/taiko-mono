...
export async function estimateMessageGasLimit({
  ...
  const { size } = await calculateMessageDataSize({
    token,
    chainId: srcChainId,
    tokenIds,
    amounts,
  });
  const minGasLimit = await getDestinationMessageMinGasLimit({
    srcChainId,
    destChainId,
    dataSize: size,
  });
    ...
    case TokenType.ERC20:
      return (
        ...
        (isTokenAlreadyDeployed
          ? gasLimitConfig.erc20DeployedGasLimit
          : gasLimitConfig.erc20NotDeployedGasLimit)
      );
...
async function getDestinationMessageMinGasLimit({
  ...
  return Number(
    await destBridgeContract.read.getMessageMinGasLimit([BigInt(dataSize)]),
  );
}
