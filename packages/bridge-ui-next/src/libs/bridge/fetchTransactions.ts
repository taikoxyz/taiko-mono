...
export async function fetchTransactions(
  ...
  const localTxs: BridgeTransaction[] =
    await bridgeTxService.getAllTxByAddress(userAddress);

  // Get all transactions from all relayers
  const relayerTxPromises: Promise<BridgeTransaction[]>[] =
    relayerApiServices.map(async (relayerApiService) => {
  ...
  const relayerTxsFlattened = relayerTxsArrays.reduce(
    (acc, txs) => acc.concat(txs),
    [],
  );
  ...
  log(
    `fetched ${relayerTxs?.length ?? 0} transactions from all relayers`,
    relayerTxs,
  );

  const { mergedTransactions, outdatedLocalTransactions } =
    mergeAndCaptureOutdatedTransactions(localTxs, relayerTxs);
  ...
  mergedTransactions.sort((a: BridgeTransaction, b: BridgeTransaction) => {
    const aStatusIndex =
      a.msgStatus !== undefined ? statusOrder.indexOf(a.msgStatus) : -1;
    const bStatusIndex =
      b.msgStatus !== undefined ? statusOrder.indexOf(b.msgStatus) : -1;
