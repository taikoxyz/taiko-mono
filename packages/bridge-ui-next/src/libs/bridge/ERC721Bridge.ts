...
export class ERC721Bridge extends Bridge {
  ...
  async requiresApproval({
    ...
    log(
      `Token with ID ${tokenId} requires approval ${spenderAddress}: ${requiresApproval}`,
    );
  ...
  async estimateGas(args: ERC721BridgeArgs): Promise<bigint> {
    ...
    const { tokenVaultContract, sendERC721Args } =
      await ERC721Bridge._prepareTransaction(args);
    const { fee: value } = sendERC721Args;

    log("Estimating gas for sendERC721 call with value", value);

    const estimatedGas = tokenVaultContract.estimateGas.sendToken(
      [sendERC721Args],
      { value },
    );
    ...
    try {
      ...
      if (!info)
        throw new NoCanonicalInfoFoundError(
          "No canonical info found for token",
        );
      ...
      if (canonicalTokenAddress === token) {
        ...
        if (requireApproval) {
          throw new NotApprovedError(
            `The token with id ${tokenId} is not approved for the token vault`,
          );
        }
      } else {
      ...
      throw new SendERC721Error("failed to bridge ERC721 token", {
        cause: err,
      });
    }
  ...
  async approve(args: NFTApproveArgs) {
    ...
    if (!requireApproval) {
      ...
      throw new NoApprovalRequiredError(
        `No approval required for the token ${tokenId}`,
      );
    }

    try {
      log(
        `Calling approve for spender "${spenderAddress}" for token`,
        tokenIds,
      );
  ...
  private static async _prepareTransaction(args: ERC721BridgeArgs) {
    ...
    const sendERC721Args: NFTBridgeTransferOp = {
      ...
      to,
      destOwner: destOwnerAddress.getState() || to,
      token,
