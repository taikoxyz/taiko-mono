...
export const checkForPausedContracts = async () => {
  ...
  const pausedContracts = await Promise.all(
    bridgeContractInfo.map(async (bridgeInfo) => {
      ...
      log(
        `Checking if bridge ${bridgeAddress} is paused on chain ${srcChainId}`,
      );
      try {
  ...
  if (pausedContracts.some((isPaused) => isPaused)) {
    // svelte `bridgePausedModal.set(true)` -> vanilla zustand modal store setter.
    modalStore.getState().setBridgePausedModal(true);
    return true;
  } else {
    modalStore.getState().setBridgePausedModal(false);
    return false;
  }
};
