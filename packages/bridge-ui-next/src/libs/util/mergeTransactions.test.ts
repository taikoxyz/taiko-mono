...
describe("mergeUniqueTransactions", () => {
  ...
  it("should merge transactions when no outdated local ones", () => {
    ...
    expect(extractHashes(result.mergedTransactions)).toEqual(
      extractHashes([...localTxs, ...relayerTx]),
    );
  ...
  it("should identify and capture outdated local transactions", () => {
    ...
    // When
    const result = mergeAndCaptureOutdatedTransactions(
      localWithOutdated,
      relayerTx,
    );
  ...
  it("should merge transactions and capture outdated ones, complex", () => {
    ...
    // Then
    expect(extractHashes(result.mergedTransactions)).toEqual(
      expectedMergedHashes,
    );
    expect(extractHashes(result.outdatedLocalTransactions)).toEqual(
      expectedOutdatedHashes,
    );
  });
});
