  ...
  const pollingFn = async () => {
    ...
    try {
      const messageStatus: MessageStatus =
        await destBridgeContract.read.messageStatus([bridgeTx.msgHash]);
      emitter.emit(PollingEvent.STATUS, messageStatus);

      if (messageStatus === MessageStatus.FAILED) {
        // check if the message is recalled
        const recallStatus = await srcBridgeContract.read.messageStatus([
          bridgeTx.msgHash,
        ]);
      ...
      if (messageStatus === MessageStatus.DONE) {
        log(
          `Poller has picked up the change of status to DONE for hash ${srcTxHash}.`,
        );
        stopPolling();
      }
    } catch (err) {
      console.error(err);
      stopPolling();
      throw new BridgeTxPollingError(
        "something bad happened while polling for status",
        { cause: err },
      );
    }
  };
