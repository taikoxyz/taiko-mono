...
use super::shasta_inbox_proposal_abi_gen::RindexerShastaInboxProposalGen::{
    self, RindexerShastaInboxProposalGenEvents, RindexerShastaInboxProposalGenInstance,
};
...
pub type AdminChangedData = RindexerShastaInboxProposalGen::AdminChanged;
...
pub type BeaconUpgradedData = RindexerShastaInboxProposalGen::BeaconUpgraded;
...
pub type BondInstructedData = RindexerShastaInboxProposalGen::BondInstructed;
...
pub type BondWithdrawnData = RindexerShastaInboxProposalGen::BondWithdrawn;
...
pub type InitializedData = RindexerShastaInboxProposalGen::Initialized;
...
pub type OwnershipTransferStartedData = RindexerShastaInboxProposalGen::OwnershipTransferStarted;
...
pub type OwnershipTransferredData = RindexerShastaInboxProposalGen::OwnershipTransferred;
...
pub type PausedData = RindexerShastaInboxProposalGen::Paused;
...
pub type ProposedData = RindexerShastaInboxProposalGen::Proposed;
...
pub type ProvedData = RindexerShastaInboxProposalGen::Proved;
...
pub type UnpausedData = RindexerShastaInboxProposalGen::Unpaused;
...
pub type UpgradedData = RindexerShastaInboxProposalGen::Upgraded;
...
pub enum ShastaInboxProposalEventType<TExtensions>
where
...
pub async fn shasta_inbox_proposal_contract(
    network: &str,
) -> RindexerShastaInboxProposalGenInstance<Arc<RindexerProvider>, AnyNetwork> {
    let address: Address = "0xae78736cd615f374d3085123a210448e74fc6393"
        .parse()
        .expect("Invalid address");
    RindexerShastaInboxProposalGen::new(
        address,
...
impl<TExtensions> ShastaInboxProposalEventType<TExtensions>
where
    TExtensions: 'static + Send + Sync,
{
    pub fn topic_id(&self) -> &'static str {
        match self {
            ShastaInboxProposalEventType::BondInstructed(_) => {
                "0xf2d537d03425ffbe0a312cc4b640d80e634171d856a2e5b961d4f3c6cf20e1a7"
            }
            ShastaInboxProposalEventType::Proposed(_) => {
                "0x10b2060c55406ea48522476f67fd813d4984b12078555d3e2a377e35839d7d01"
            }
            ShastaInboxProposalEventType::Proved(_) => {
    ...
    pub fn event_name(&self) -> &'static str {
        match self {
            ShastaInboxProposalEventType::BondInstructed(_) => "BondInstructed",
            ShastaInboxProposalEventType::Proposed(_) => "Proposed",
            ShastaInboxProposalEventType::Proved(_) => "Proved",
        }
    }

    pub fn contract_name(&self) -> String {
        "ShastaInboxProposal".to_string()
    }
    ...
    pub async fn register(self, manifest_path: &PathBuf, registry: &mut EventCallbackRegistry) {
        ...
        > = match self {
            ShastaInboxProposalEventType::BondInstructed(event) => {
            ...
            ShastaInboxProposalEventType::Proposed(event) => {
                let event = Arc::new(event);
                Arc::new(move |result| {
                    let event = Arc::clone(&event);
                    async move { event.call(result).await }.boxed()
            ...
            ShastaInboxProposalEventType::Proved(event) => {
                let event = Arc::new(event);
                Arc::new(move |result| {
                    let event = Arc::clone(&event);
                    async move { event.call(result).await }.boxed()
