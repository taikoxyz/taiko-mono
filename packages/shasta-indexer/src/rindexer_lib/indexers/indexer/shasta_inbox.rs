...
use super::super::super::typings::indexer::events::shasta_inbox::{
    no_extensions, BondInstructedEvent, ProposedEvent, ProvedEvent, ShastaInboxEventType,
};
    ...
    let handler = BondInstructedEvent::handler(
        |results, context| async move {
            ...
            let rows = [
                ...
                "instructions_proposal_id".to_string(),
                "instructions_bond_type".to_string(),
                "instructions_payer".to_string(),
                "instructions_receiver".to_string(),
                "tx_hash".to_string(),
            ...
            if postgres_bulk_data.len() > 100 {
                let result = context
                    ...
                    .bulk_insert_via_copy(
                        "indexer_shasta_inbox.bond_instructed",
                        &rows,
                        &postgres_bulk_data
                            .first()
                ...
                if let Err(e) = result {
                    rindexer_error!(
                        "ShastaInboxEventType::BondInstructed inserting bulk data via COPY: {:?}",
                        e
                    );
                ...
                if let Err(e) = result {
                    rindexer_error!(
                        "ShastaInboxEventType::BondInstructed inserting bulk data via INSERT: {:?}",
                        e
                    );
                    return Err(e.to_string());
                }
    ...
    let handler = ProposedEvent::handler(
        |results, context| async move {
            ...
            if postgres_bulk_data.len() > 100 {
                ...
                if let Err(e) = result {
                    rindexer_error!(
                        "ShastaInboxEventType::Proposed inserting bulk data via COPY: {:?}",
                        e
                    );
                    return Err(e.to_string());
                }
                ...
                let result = context
                    .database
                    .bulk_insert("indexer_shasta_inbox.proposed", &rows, &postgres_bulk_data)
                    .await;

                if let Err(e) = result {
                    rindexer_error!(
                        "ShastaInboxEventType::Proposed inserting bulk data via INSERT: {:?}",
                        e
                    );
                    return Err(e.to_string());
                }
    ...
    let handler = ProvedEvent::handler(
        |results, context| async move {
            ...
            if postgres_bulk_data.len() > 100 {
                ...
                if let Err(e) = result {
                    rindexer_error!(
                        "ShastaInboxEventType::Proved inserting bulk data via COPY: {:?}",
                        e
                    );
                    return Err(e.to_string());
                }
                ...
                let result = context
                    .database
                    .bulk_insert("indexer_shasta_inbox.proved", &rows, &postgres_bulk_data)
                    .await;

                if let Err(e) = result {
                    rindexer_error!(
                        "ShastaInboxEventType::Proved inserting bulk data via INSERT: {:?}",
                        e
                    );
                    return Err(e.to_string());
                }
