...
def get_block_fee(incentive, max_multiplier, avg_block_time, t_now, last_proposed_at):
    scale = fee_scale(max_multiplier, t_now, last_proposed_at, avg_block_time)
    return incentive * 10000 / scale


def get_proof_reward(incentive, max_multiplier, avg_proof_time, proven_at, proposed_at):
    scale = fee_scale(max_multiplier, proven_at, proposed_at, avg_proof_time)
    return incentive * scale / 10000
...
class Protocol(sim.Component):
    def setup(self, config):
        ...
        self.m_incentive = sim.Monitor(
            "m_incentive", level=True, initial_tally=self.incentive
        )
    ...
    def get_block_fee(self):
        fee = get_block_fee(
            self.incentive,
            self.config.incentive_multiplier,
            self.avg_block_time,
            env.now(),
    ...
    def print_me(self, st):
        ...
        st.write("incentive = {}".format(self.incentive))
    ...
    def propose_block(self):
        if self.can_propose():
            ...
            self.incentive = moving_average(self.incentive, fee, self.config.incentive_maf)
            self.avg_block_time = moving_average(
                self.avg_block_time,
                block_time,
                self.config.time_maf,
            )
            self.last_proposed_at = env.now()
            ...
            self.m_incentive.tally(self.incentive)
    ...
    def finalize_block(self):
        for i in range(0, 5):
            if self.can_finalize():
                ...
                self.incentive = moving_average(
                    self.incentive,
                    reward,
                    self.config.incentive_maf,
                )
...
def simulate(config, days):
    ...
    if st.button("Simulate {} days".format(days), key="run"):
        ...
        plot(days, [(protocol.m_incentive, "incentive")])
