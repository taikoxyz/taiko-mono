...
4. Start taiko-client
    ...
    :::note
        TODO: David/Dave
        Does L1 node have to provide `eth_getProof` still? No longer required to be an archive node.
    :::
</Steps>
...
Syncing can take several hours, depending on the size of the chain. Here are some logs you can expect to see during that time:
