...
2. Copy the sample .env files
    ...
    To run a `Taiko Hoodi` node please copy `.env.sample.hoodi` instead of `.env.sample`.
    </Aside>
...
3. Set the L1 node endpoint
    ...
    For Taiko Alethia, the L1 Node will need to be an Ethereum node.
    For Taiko Hekla, the L1 Node will need to be an Ethereum testnet (Holesky) node.
    For Taiko Hoodi, the L1 Node will need to be an Ethereum testnet (Hoodi) node.
    </Aside>
...
5. Set the profiles you want to run
    ...
    <Aside type="tip">
    The following steps 6-8 are only necessary if you wish to have your node receive preconfirmed blocks and keep up with the tip of the chain.

    **To enable preconfs in your node**, please ensure that `ENABLE_PRECONFS_P2P` is set to `true` in your `.env` file.

    If it is set to `false`, your node **will not receive preconfed blocks** and only update it's chainhead when the events are emitted on-chain.
    </Aside>
...
9. Start the node
    ...
    To run a Taiko Hoodi node please use the following command instead: `docker compose -f docker-compose-hoodi.yml up -d`.
    </Aside>
