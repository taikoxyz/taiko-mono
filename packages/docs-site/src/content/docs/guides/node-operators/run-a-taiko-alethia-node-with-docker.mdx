...
6. Ensure that the relevant ports are open on your machine
    ...
    - `4001` OR whichever port you set `P2P_LISTEN_TCP_PORT` to.

    This port is required for p2p gossiping the preconfirmed blocks, so if you if it is not set you will only subscribe to the events that are emitted on chain.
