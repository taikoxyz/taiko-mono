...
### Ensure that your node has joined the P2P network
  ...
  To ensure that your node is actually configured correctly and has joined the P2P network for preconfirmations, please set your `taiko-client` driver's verbosity level to 4. This can be done by appending `--verbosity 4` to the command that starts `taiko-client`.

  Once you have done so, you can use `grep` to find logs regarding `peering tick`. You should find something that resembles the following:

  ```
  DEBUG[07-29|23:09:21.192] peering tick                             connected=8 advertised_udp=30303 advertised_tcp=4001 advertised_ip=x.x.x.x
  ```
