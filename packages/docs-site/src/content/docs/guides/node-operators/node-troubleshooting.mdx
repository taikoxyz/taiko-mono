...
_(simple-taiko-node only)_ You're using the wrong docker-compose file for the network. Use the correct file for your target network, e.g., `docker compose -f docker-compose-hoodi.yml up -d` for Taiko Hoodi testnet.
