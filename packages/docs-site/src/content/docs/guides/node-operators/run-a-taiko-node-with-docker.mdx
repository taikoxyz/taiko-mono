...
2. Copy the sample .env files
    ...
    We've unified the branches for maintainability, to run a `hekla` Taiko node please copy `.env.sample.hekla` instead of `.env.sample`.
    </Aside>
...
6. Start the node
    ...
    <Aside type="caution" title="Attention!">
    We've unified the branches for maintainability, to run a `hekla` Taiko node please use the following command instead: `docker compose -f docker-compose-hekla.yml up -d`.

    Other commands for viewing logs will remain the same.
    </Aside>
