...
### Ensure that the relevant ports are open on your machine
  ...
  Whether you are running the node on a personal machine or a cloud provider's machine, you should ensure your the following port on your device is open:
    - `4001` OR whichever port you set `--p2p.listen.tcp` to.

  This port **is required for p2p gossiping preconfirmed blocks**, so if you if it is not set you will only subscribe to the events that are emitted on chain.
