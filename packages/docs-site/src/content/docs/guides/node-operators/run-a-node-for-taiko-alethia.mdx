...
4. Start taiko-client
    ...
    ```bash
        ./bin/taiko-client driver \
            ...
            --p2p.bootnodes enr:-J64QJBc_xYqNihi2YGH1kVu86WbbJnhSS3j5FvqzGeUco2wNuvKFRjnac0Rgt_dhXxMP5sWvWk_gzkFQmY_hSF3z4uGAZg-39MlgmlkgnY0gmlwhCKO7_uHb3BzdGFja4TYmAoAiXNlY3AyNTZrMaEDwmN0Gxd1nzhQ0k1n1sPLwwfHPhfYDGsSpjpHkqEFKdGDdGNwgg-hg3VkcILFaQ,enr:-J64QPL3oYpZ75230YMfbLfKcUEqlIyWW8-W4tsrY7c0iBMVWs6JF2Z723S7uFxPGIwBhqv3HTOix9xE8_ETZOmy-YiGAZg-3QnqgmlkgnY0gmlwhCP3n5yHb3BzdGFja4TYmAoAiXNlY3AyNTZrMaECLzfDr_2DJ0smL6KiWdMtQVEN1aSNbpFmlu_n8VmMs_mDdGNwgg-hg3VkcILQOA,enr:-J64QI3zcU780K_CEGyfoPyxZ50ZlSAwSgL4gdnFZwyiZURDMx2CR7PilARK9Ix7T495QsSkqfnhJL6N-UqoNwAUz-6GAZg-bEJ6gmlkgnY0gmlwhCJ-Wv-Hb3BzdGFja4TYmAoAiXNlY3AyNTZrMaEC3YPe3v9iLs_KDF7fMgJmUGyBFTmlU93ZFYnN_Mm7102DdGNwgg-hg3VkcIKjsQ \
            --p2p.listen.ip 0.0.0.0 \
            --p2p.useragent taiko \
            --p2p.listen.tcp 4001
    ```

    If you wish to set an ip to advertise publicly, you can append `--p2p.advertise.ip ${YOUR_EXTERNAL_IP}` to the above. If you instead want this done automatically, you can append `--p2p.nat` instead.

    We **highly recommend** that you set your external IP manually.
</Steps>
...
### Ensure that the relevant ports are open on your machine
  ...
  Whether you are running the node on a personal machine or a cloud provider's machine, you should ensure your the following ports on your device are open:
    - TCP `4001` OR whichever port you set `--p2p.listen.tcp` to.
    - UDP `30303` OR whichever port you set `--p2p.advertise.udp` to.

  These ports **are required for p2p gossiping preconfirmed blocks**, so if it is not set you will only subscribe to the events that are emitted on chain.
