...
For more details, refer to the [Multi-Proofs](/core-concepts/multi-proofs#verified-blocks-and-parallel-proving) page.
...
A Safe block is determined by checking the Ethereum L1 Safe block status of the corresponding L1 block.

---

### Finalized (Verified) Block

The Finalized state in Taiko Alethia is referred to as Verified. A block is Verified when:
...
- It is **Finalized (Verified)** if **all state transitions from genesis** have **valid proofs**.
