...
1. **Preconfer Slashing**:

    Preconfirmers opt into additional slashing conditions to ensure honest commitments.

2. **Preconfer Forced Inclusion**:

    Preconfirmers must forcefully include preconfirmed transactions in their proposer slot, ensuring execution.
...
- **Route 1**: Through an RPC endpoint directly to the gateway
- **Route 2**: Through a Taiko Node which then connects to the gateway
...
- **Sub-Second Finality**: Preconfirmations enable transactions to achieve practical finality within ~200ms, dramatically improving UX for latency-sensitive applications like DEXs, gaming, and DeFi.
...
- **Eliminating Block Time Constraints**: With based preconfirmations, transactions are secured instantly rather than waiting for fixed block proposal times, removing perceived waiting times for users.
