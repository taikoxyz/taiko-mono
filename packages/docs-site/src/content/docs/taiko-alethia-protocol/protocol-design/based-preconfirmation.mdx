...
Taiko Alethia is implementing based preconfirmations to enhance transaction finality, such that users have some confidence their transactions will be included on Layer 2 much faster than is the case at the moment (potentially sub-one second). This significantly improves UX, particularly for applications requiring fast transaction execution. Based preconfirmations allow transactions to receive indications of early inclusion from preconfirmers, providing users with some degree of immediate execution assurance.
...
<Aside>
We term this "sequencing" - when a preconfirmer is creating and gossiping blocks but have not proposed on L1 yet. The user receives a transaction receipt, and all node states are updated when a block is sequenced. Blocks can be sequenced on top of other sequenced blocks.

With this design, a preconfirmer can sequence hundreds of blocks, before proposing them onchain in what we call a "batch". Read more about batches [here](/taiko-alethia-protocol/protocol-design/pacaya-fork-taiko-alethia/#batch-based-protocol)
</Aside>
...
7. **Slashing**
    ...
    :::note
    Although slashing penalties will motivate a preconfirmer to honour its commitments, the user does not receive a share of the penalty to compensate it for the failed inclusion.
    :::
