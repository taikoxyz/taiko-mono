...
impl SgxGethProofProducer {
    ...
    pub async fn request_proof(&self, request: &ProofRequest) -> Result<()> {
        if self.dummy {
            return Ok(());
        }
        ...
        request_validated(&self.raiko, &body, request.geth_proof_generated)
            .await
    ...
    pub async fn aggregate(&self, items: &[ProofResponse]) -> Result<Bytes> {
        if items.is_empty() {
        ...
        let response =
            request_validated(&self.raiko, &body, items[0].request.geth_aggregation_generated)
                .await
                .map_err(ProverError::from)?;
