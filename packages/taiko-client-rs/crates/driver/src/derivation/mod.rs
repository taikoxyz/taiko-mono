...
pub enum DerivationOutcome {
    ...
    /// Proposal was skipped because it was already pre-confirmed.
    Preconfirmed { proposal_id: u64 },
}
...
pub trait DerivationPipeline: Send + Sync {
    ...
    async fn process_proposal(&self, log: &Log) -> Result<DerivationOutcome, DerivationError>;
}
...
{
    async fn process_proposal(&self, log: &Log) -> Result<DerivationOutcome, DerivationError> {
        // Extract the `data` field from the Proposed event, which contains
        // the encoded ProposedEventPayload. We need to decode it using the codec contract.
        let data = log.data().data.clone();
        let payload =
            self.rpc.shasta.codec.decodeProposedEvent(data).call().await.map_err(|e| {
                DerivationError::Other(anyhow::anyhow!("failed to decode proposed event: {}", e))
            })?;
        let proposal_id = payload.proposal.id.to::<u64>();

        // Ignore the genesis proposal.
        if proposal_id == 0 {
            return Ok(DerivationOutcome::Applied { proposal_id });
        }
        ...
        //         Ok(manifest) => {
        //             debug!(index = idx, blocks = manifest.blocks.len(), "decoded shasta
        // manifest");         }
        //         Err(err) => {
