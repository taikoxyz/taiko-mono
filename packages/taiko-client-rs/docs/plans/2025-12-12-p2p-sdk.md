...
**Files:** `crates/p2p/src/sdk.rs`, `crates/p2p/src/storage/memory.rs`

**Steps:**
1. Introduce bounded mpsc for commands to driver and bounded event queue; on saturation return `P2pSdkError::Backpressure` and emit error event (DONE).
...
### Task 9: SDK core orchestration ✅ (baseline)
...
### Task 11: Gossip & req/resp handlers ✅ (inbound served via shared storage; mapping/validation done in SDK). Deferred: scoring hooks/rate limits.
...
Completed: validation edge cases, storage ordering, SDK mapping, catch-up retries/txlist requests, publish helpers, latency/backpressure, eviction caps, integration smoke, doctests.
...
Notes: Implemented a minimal single-node smoke test driving synthetic gossip/reqresp through `P2pSdk::process_event_test`; exercises publish + validation + storage.
**Files:** `crates/p2p/tests/integration_smoke.rs`

**Steps:**
1. Drive `P2pSdk` with in-memory storage and synthetic gossip/reqresp; assert publish helpers store data (DONE).
...
### Task 19: SDK dedupe & self-ignore ✅
SDK now records outbound message IDs and drops inbound duplicates early; self-publish helpers mark IDs to ignore echo. Added tests for duplicate commitment/raw txlist suppression.
**Files:** `crates/p2p/src/sdk.rs`, `crates/p2p/src/types.rs`

**Steps:**
1. Wire storage message-id cache into gossip handling and publish helpers.
2. Remove per-topic LRU fields in SDK in favor of storage-backed dedupe.
3. Add SDK unit tests ensuring duplicate gossip is filtered.
...
**Files:** `crates/p2p/src/sdk.rs`, `crates/p2p/src/config.rs`
...
**Files:** `crates/p2p/src/metrics.rs`, hook sites in `sdk.rs`
...
**Files:** `crates/p2p/src/{sdk.rs,catchup.rs,storage/mod.rs}`, `crates/p2p/tests/integration_smoke.rs`
