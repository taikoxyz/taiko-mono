...
describe('getShastaFeeOverrides', () => {
  ...
  it('should return undefined when protocol version lookup fails', async () => {
    vi.mocked(isL2Chain).mockReturnValue(true);
    vi.mocked(getProtocolVersion).mockRejectedValue(new Error('boom'));
  ...
  it('should use min priority fee floor when priority estimation fails', async () => {
    vi.mocked(isL2Chain).mockReturnValue(true);
    vi.mocked(getProtocolVersion).mockResolvedValue(ProtocolVersion.SHASTA);
    ...
    const mockClient = {
      ...
      estimateMaxPriorityFeePerGas: vi.fn().mockRejectedValue(new Error('priority unavailable')),
      getGasPrice: vi.fn().mockResolvedValue(parseGwei('0.006')),
    };
  ...
  it('should use 1 second fallback when latest and parent timestamps are equal', async () => {
    vi.mocked(isL2Chain).mockReturnValue(true);
    vi.mocked(getProtocolVersion).mockResolvedValue(ProtocolVersion.SHASTA);
    ...
    const mockClient = {
      ...
      estimateMaxPriorityFeePerGas: vi.fn().mockResolvedValue(parseGwei('0.001')),
      getGasPrice: vi.fn().mockResolvedValue(parseGwei('0.006')),
    };
    ...
    expect(result?.maxPriorityFeePerGas).toBe(parseGwei('0.001'));
  });

  it('should continue with 1 second fallback when parent block fetch fails', async () => {
    vi.mocked(isL2Chain).mockReturnValue(true);
    vi.mocked(getProtocolVersion).mockResolvedValue(ProtocolVersion.SHASTA);
