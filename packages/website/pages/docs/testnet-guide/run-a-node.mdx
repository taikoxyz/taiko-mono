...
Taiko's network is fully decentralized and relies on the community to run nodes. This guide will walk you through the process of operating a Taiko node via [simple-taiko-node](https://github.com/taikoxyz/simple-taiko-node).

This guide will cover:
- What is simple-taiko-node
- System Requirements
- Prerequisites
- Steps
...
The [simple-taiko-node](https://github.com/taikoxyz/simple-taiko-node) Docker image is the easiest way to get started operating a Taiko node. You can:
- Run a Taiko node from the command line on Windows, Mac, and Linux
    -Start your Taiko node with a one line command.
- Run a Taiko node regularly or as a proposer
...
- Will need some ETH on the Layer 1 (L1) network. To receive Ethereum A1 ETH (L1), follow the Directions in [Request from faucet](/docs/testnet-guide/request-from-faucet).
...
- Your L1 proposer account ran out of ETH (needed to propose tx's). Please refer to [Request from faucet](/docs/testnet-guide/request-from-faucet).
