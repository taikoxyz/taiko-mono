...
The transaction list intrinsic validity functions checks that:

- The txList is RLP decodable into a list of transactions.
- The number of transactions is no larger than the protocol constant $K_{BlockMaxTxs}.$
- The sum of all transactions’ gasLimit is no larger than the protocol constant $K_{BlockMaxGasLimit}.$
- Each and every transaction’s signature is valid, i.e., it does not recover to the zero address.
