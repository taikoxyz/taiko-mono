...
You can connect to multiple provers at the same time in case a selected prover hits its max capacity. Just add the addresses of any provers from [prover market](/docs/reference/prover-market-page) in your .env file. Type nano .env in your command line and add more provers addresses in the PROVER_ENDPOINTS field so it looks like PROVER_ENDPOINTS=http://taiko_client_prover_relayer:9876,"URL OF ADDITIONAL PROVER FROM MARKET PLACE 1","URL OF ADDITIONAL PROVER FROM MARKET PLACE 2"... Write out, exit and restart your node. 
