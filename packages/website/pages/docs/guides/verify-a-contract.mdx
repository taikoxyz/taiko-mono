...
<Tabs items={["Hardhat", "Blockscout", "Foundry"]} defaultIndex="0">
  <Tab>
        ...
        **2. Run the verify task, but replace `DEPLOYED_CONTRACT_ADDRESS` and `"Constructor argument 1"` with the address of your deployed contract and the constructor arguments you used when deploying the contract.**

        ```sh
        npx hardhat verify --network taiko DEPLOYED_CONTRACT_ADDRESS "Constructor argument 1"
        ```

        **3. Check the [Taiko block explorer](https://explorer.test.taiko.xyz) link from the output to see your contract was verified.**
  </Tab>
  <Tab>
