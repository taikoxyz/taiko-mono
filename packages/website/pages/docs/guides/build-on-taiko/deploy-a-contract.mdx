...
- You have testnet ETH on a Taiko testnet (to pay the transaction fee for deploying the contract).
  - You can [request Sepolia ETH](/docs/guides/build-on-taiko/receive-tokens#receive-sepolia-eth) from the faucet and then [bridge tokens](/docs/guides/build-on-taiko/bridge-tokens) to send the testnet ETH to a Taiko testnet.
- You have the private key to the account with testnet ETH on a Taiko testnet.

### Additional Remix prerequisites

- You have [setup your wallet](/docs/guides/build-on-taiko/setup-your-wallet) to connect to the relevant testnets.
...
<Tabs items={["Foundry", "Hardhat", "Remix", "thirdweb"]} defaultIndex="0">
  <Tab>
      **1. Create a project with Foundry**
