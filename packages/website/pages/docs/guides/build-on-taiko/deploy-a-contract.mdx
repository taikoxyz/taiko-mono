...
- You have testnet ETH on Taiko (to pay the transaction fee for deploying the contract).
  - You can [request Sepolia ETH](/docs/guides/build-on-taiko/receive-tokens#receive-sepolia-eth) from the faucet and then [use the bridge](/docs/guides/build-on-taiko/bridge-tokens) to send testnet ETH to Taiko.
