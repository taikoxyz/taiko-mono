...
```
which should return chainId = 0x28c5d = 167005
```bash 
{"jsonrpc":"2.0","id":0,"result":"0x28c5d"}
```
Check if the Execution Layer client is synced by requesting the latest Taiko L2 block from the Execution Layer client
```bash
curl http://localhost:8547 \
  -X POST \
  -H "Content-Type: application/json" \
...
```
If the block response value is 0 or not growing, check the Taiko L2 logs here
```bash
