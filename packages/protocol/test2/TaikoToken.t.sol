...
contract TaikoTokenTest is Test {
    ...
    function setUp() public {
        ...
        addressManager.init();
        _registerAddress("proto_broker", protoBroker);
    ...
    function testBurnAmountExceeded() public {
        uint256 amountToMint = 1 ether;
    ...
    function _registerAddress(string memory name, address addr) private {
        string memory key = _keyForName(block.chainid, name);
        addressManager.setAddress(key, addr);
        console2.log(key, unicode"→", addr);
    }

    function _keyForName(
        uint256 chainId,
        string memory name
