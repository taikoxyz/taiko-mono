...
\large\mdseries Production Design Candidate --- v2.7}
\author{}
\date{August 2026}
\maketitle
\thispagestyle{fancy}
...
admission, data, reward and migration state.  It is an implementation
candidate, not a production authorization: the proving, gas, economics and
...
\item A state root is a binding commitment, not an erasure code.  If every copy
  of the canonical state trie, executed bodies and required witness data is
...
bodies and state-trie nodes sufficient to reconstruct the current prestate.
...
must retain the canonical raw bodies, receipts and Ethereum trie nodes needed
to reconstruct \code{CanonicalCore.stateRoot}.  A reconstruction package is a
content-addressed set of those objects plus the canonical block headers.  A
consumer rejects any object whose transaction/body/header hash, trie path or
final state root disagrees with the L1-canonical commitments, so an archive can
be anonymous and malicious without gaining safety authority.
...
\code{(msgHash,srcChainId,destChainId,srcOwner,destOwner,value,calldataHash,
escrowId,byteLength,accountedGas,refundAddress,deposit)}.  Only the configured \code{BridgeInboxAdapter}
may enqueue it, it has no expiry field, and value is fully escrowed.  Its
only forced effect is an idempotent inbox/signal credit keyed by \code{msgHash};
...
state.  Anyone may call \code{finalizeEnqueue(msgHash)}.  A Router
...
\begin{verbatim}
H("slot-chain-bridge-credit-result-v1" || u64(queueIndex) || msgHash ||
  u256(srcChainId) || u256(destChainId) || address20(srcOwner) ||
...
idempotent SignalService entry point.  Unknown codes or
...
off-chain canonical bodies, receipts and state-trie nodes indexed by canonical
block and state root.  Every object is root-verifiable; no archive address,
signature or availability vote appears in Settlement or the verifier statement.
...
\item AnchorV4, InboxApply and SignalService addresses, runtime code hashes,
...
\item the exact AnchorV4 checkpoint/ancestors transition, forced disposition
  rules and InboxApply one-shot/signal-credit transition; and
\item at least five complete parent/prestate/input/transaction/receipt/header/
  poststate vectors: empty escape, valid kind 0, expired kind 0 without bytes,
  kind 1, and a cap-limited multi-block prefix, plus invalid extra-reserved-
  sender and duplicate-InboxApply vectors.
\end{itemize}
...
message hash and ownership/value/calldata fields, while destination invocation
...
profile-required AnchorV4 runtime.  This is an ordinary finalized L2 governance
...
AnchorV4, InboxApply and SignalService proxy/fork-router runtime code hashes;
...
exercise.  One archive serves a clean node from only the latest finalized
package while the other two are removed.  The exercise also removes every
...
A kind-1 leaf is:
\begin{verbatim}
"slot-chain-force-bridge-v2" || u32(i) || msgHash ||
u256(srcChainId) || u256(destChainId) || address20(srcOwner) ||
address20(destOwner) || u256(value) || calldataHash || escrowId ||
...
duplicated inside it.  Its fixed byte length is 220 for kind 0 and 312 for kind
...
\code{settlement-window-model.py} reports 99 assertions over PREACTIVE/migration,
...
\code{commitment-model.py} reports 59 vectors/properties for exact EIP-712
