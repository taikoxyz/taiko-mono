...
\large\mdseries Audited Design Candidate --- v2.13}
\author{}
\date{August 2026}
\maketitle
\thispagestyle{fancy}
...
\item A source message is not yet an admitted forced envelope.  A preparer must
  obtain one fresh bounded source-state witness before creating destination-L1
  \code{PENDING}; source emission alone is not reported as forced-queue
  acceptance.  After emission the immutable source credit record can be
  re-proved beneath any later authenticated source header.
...
identified by chain ID, genesis hash, registry address and immutable
\code{bytes32} registry namespace.  Its EIP-2935-authenticated state and
append-only Bridge-epoch/credit records are proven to the same L1.  Registry
governance is an external safety dependency; a current resolver lookup never
substitutes for the immutable emission record.
\item[Source proof archive.] Any untrusted, replaceable source of the
content-addressed fresh header, account/storage trie and runtime-code objects
needed to prepare a kind-1 credit.  Objects are root/hash verified and the
archive has no protocol authority.  An orphaned preparation uses a newly
generated witness against a later header; it never depends on keeping an
expiring emission-header receipt proof.
...
expiry field.  Source BridgeV2 locks the message value and fee under the exact
\code{escrowId}; the destination adapter separately escrows its proof/queue
deposit.  Its
...
\end{verbatim}
The registry exposes exactly two public immutable \code{bytes32} fields:
\code{sourceGenesisHash} and \code{registryNamespace}.  Its constructor ABI
takes \code{(bytes32,bytes32)} in that order; both getters return
\code{bytes32}, and either zero value rejects deployment.  They are never
dynamic bytes or integers.  The deployed runtime hash commits the
substituted immutable values, and the published \code{sourceDomain} vector in
Appendix~\ref{sec:encodings} pins the resulting packed encoding.
arbitrary destination invocation and RETRIABLE processing remain later ordinary
bridge operations.  The two variants have separate byte-exact leaves.

The adapter is an exactly-once state machine
\code{NONE -> PENDING -> QUEUED(index)}, keyed by \code{creditId}, not by
\code{msgHash}.  \code{BridgeEpochRegistry} is a profile-pinned non-proxy
contract; those two constructor immutables and their exact ABI are pinned by
the execution profile, and its code has no self-upgrade path.  Replacing the registry requires a new
address and creates a new source domain even
if chain, epoch, Bridge address and message hash repeat.  Destination protocol
versions that retain the same source registry retain the same domain ID.

Launch accepts only when source and settlement chain IDs are equal.  Let
\code{sourceAge} be the current L1 block number minus \code{srcBlockNumber}.
Preparation requires
\begin{verbatim}
F_L1 <= sourceAge <= MAX_SOURCE_WITNESS_AGE = 255
\end{verbatim}
and reads the
canonical hash for \code{srcBlockNumber} from Ethereum's EIP-2935 history
contract and rejects unless it equals both \code{srcBlockHash} and legacy
Keccak of the supplied canonical RLP header.  Here \code{srcBlockNumber} and
\code{srcBlockHash} identify this refreshable witness header, while
\code{emittedAtBlock} is the immutable source-emission height.  The adapter
extracts the state root from the authenticated header; no caller-supplied
``finality certificate'' or root is authoritative.  The header is at most
2,048 bytes; the entire source witness is at most 524,288 bytes and 256
state-trie nodes, and verification receives at most 10,000,000 gas.  Extra
nodes, duplicate paths, noncanonical RLP, an expired history index or any limit
breach reject.  Supporting another source consensus requires a new protocol
version with an equally byte-exact, bounded, audited finality verifier and is
not part of the launch profile.

Source-header authentication, complete static-envelope validation, value
escrow and one queue-capacity reservation atomically create an immutable
\code{BridgeCredit} record and \code{PENDING} state.  Under that source header,
the state witness authenticates the non-proxy \code{BridgeEpochRegistry}'s
immutable \code{CreditRecord(creditId)}.  Source BridgeV2 creates this record in
the same transaction that locks the exact message value/fee in
\code{escrow[creditId]}, after checking \code{escrowId}; either both writes and
message emission succeed or all revert.  The registry recomputes
\code{creditId}, requires the caller to be the unique live Bridge for
\code{srcEpoch}, and permanently stores \code{sourceDomainId}, epoch, Bridge,
execution hash, \code{emittedAtBlock}, message hash and every
ownership/value/calldata/destination field.  A duplicate credit ID rejects.
The destination proof authenticates the record, its referenced immutable epoch
and the exact descriptor.  The registry record itself is the sole authoritative
source signal: no external source SignalService slot is required.  An event is
an indexing aid only and a receipt proof has no consensus role.
The logical record is exactly:
\begin{verbatim}
...
\end{verbatim}
The release profile pins its Solidity mapping slot and every packed
member offset; no implementation may infer them from compiler defaults.

The destination's profile-pinned, non-proxy \code{SourceVerifierRegistry}
retains an append-only \code{SourceDomainDescriptor} keyed by
\code{sourceDomainId}.  It contains the source chain ID, genesis hash, epoch
registry address and namespace, registry runtime hash, constructor ABI,
epoch/credit-record storage layouts, EIP-2935 parameters, BridgeV2 activation
height, witness limits and historical verifier address/runtime hash.  The
registry recomputes the domain ID from those exact fields.  Duplicate
registration is idempotent only for byte-identical data; no entry can be
deleted, replaced or redirected.  Thus replacing source registry R1 by R2
creates D2 while the current adapter can still route an old D1 proof to R1 and
its immutable verifier.

Only the profile-pinned delayed \code{ProtocolVersionManager} may register a
domain or support entry, and only in the same transaction that activates an
immutable protocol-version manifest containing the exact entry.  Each release
adds at most 64 domain/support entries, but the historical registry has no
global lifetime cap.  A support entry is keyed by
\code{(sourceDomainId,bridgeExecutionHash)} and permanently stores the active
protocol version, manifest hash, verifier address/runtime hash and
\code{activatedAtBlock}.  A later protocol version may omit an entry from its
own finite manifest because the historical entry and verifier remain routable;
it may never mutate their meaning.  An unauthorized caller, inactive manifest,
fresh registry address or conflicting duplicate rejects.

A Bridge-shaped clone or an app outside its live authorization epoch cannot
create the source registry record.  The adapter records the source epoch,
domain, Bridge, emission height and witness header number and hash.  Checking
only the current resolver is forbidden.  Because the domain, support, credit
and epoch records are append-only, any later header satisfying the age bound
can replace the witness header without changing \code{creditId} or source
semantics.

The source \code{BridgeEpochRegistry} retains every monotonically increasing
\code{srcEpoch}, source Bridge, \code{bridgeExecutionHash}, activation block
and deactivation block.  Epochs are nonoverlapping.  A successor and any
shortening of its predecessor's initial \code{UINT64_MAX} end must be scheduled
more than the source finality delay in advance; activation, elapsed intervals
and past endpoints are immutable.
The execution hash is
\begin{verbatim}
H("slot-chain-source-bridge-execution-v1" ||
  u32(topologyByteLength) || canonicalTopologyBytes)
\end{verbatim}
where Appendix~\ref{sec:encodings} defines the complete bounded grammar and
dispatch reconstruction algorithm.  The source registry or its immutable epoch controller owns every upgrade/selector
authority.  At emission BridgeV2 verifies its complete topology and terminal
code hashes against its live epoch; the registry binds that epoch's immutable
execution hash into the credit record.  A later destination witness proves the
record and epoch and matches the hash to the historical support entry; it does
not reinterpret emission through the source's then-current proxy state.  A
topology change requires a future successor epoch.  Its
source activation block must be at least
\code{PENDING_FINALITY_BLOCKS} after the destination profile containing that
exact support entry became active.  Source epoch scheduling requires that
already-active entry and the full lead; predecessor shortening becomes
effective atomically only if successor activation succeeds.  Activation
rechecks the same registry entry and inequality.  BridgeV2
must reject emission under an epoch whose destination-support lead has not
elapsed.  Therefore destination support is active and final before the first
new-topology emission, while all older emissions remain provable forever.
At launch, source, destination and both registries share Ethereum, so the
source controller reads this support state directly.  Cross-chain support
would require a separately reviewed finality bridge and is outside this profile.
BridgeV2 checks that its own address, execution hash and epoch are the unique
live entry before sending a V2 message.  Thus a rotation has block-boundary semantics, same-block
transaction ordering cannot change who was authorized at emission, and a later
proof can still authenticate the old epoch.
Admission also requires \code{0<srcChainId<=UINT64_MAX},
nonzero \code{sourceDomainId}, \code{0<srcEpoch<=UINT64_MAX},
\code{0<emittedAtBlock<=srcBlockNumber<=UINT64_MAX}, nonzero \code{srcBlockHash} and
\code{msgHash}, \code{destChainId=l2ChainId} and a
nonzero \code{srcBridge}; no narrowing cast or destination-chain inference is
permitted.

\code{ForcedQueue.pendingReservations} is incremented in the same transaction
that records \code{PENDING}.  Preparation first validates every immutable
append predicate---kind, authorization, source fields, gas, byte length,
deposit and escrow---and requires
\code{queueCount+pendingReservations<UINT32_MAX}.  Every kind-0 append observes
the same inequality.  Thus \code{PENDING} owns one future leaf position and
cannot be stranded by a capacity race or an envelope that will later fail a
static check.
Anyone may call \code{finalizeEnqueue(creditId)}.  A Router
\code{SYNCED} return leaves it \code{PENDING}; an \code{ADMITTED(index)} return
sets \code{enqueuedAt=block.timestamp}, computes \code{dueAt} from that exact
append time, consumes one reservation, increments queue count and changes the
record to \code{QUEUED(index)} atomically.  After static preparation, those are
the only two outcomes.  Repeated calls for the same
\code{creditId} return the stored index; two historically authorized Bridge
epochs with the same \code{msgHash} have different IDs and records.  The upstream message is never reported queued merely because
escrow became pending.

Before \code{PENDING_FINAL}, a permissionless preparer retains the source
message fields and can regenerate a source package containing a fresh canonical
RLP header, account/storage MPT nodes for the registry credit and epoch records
plus the registry runtime-bytecode preimage keyed by code hash.  A consumer
selects the append-only domain/support descriptors, then verifies every object against \code{srcBlockHash} and
its state root.  \code{PENDING_UNFINALIZED} is an operational label, not new
contract state: if its L1 transaction is orphaned, anyone rebuilds the package
against a later depth-final header and resubmits.  The new header may change
\code{srcBlockNumber/hash}, but the permanent record preserves
\code{emittedAtBlock} and every credit field.  There is no expiry race with the
emission header.  Exactly
\begin{verbatim}
PENDING_FINALITY_BLOCKS = F_L1 + ceil(REORG_MARGIN_SECONDS / 12) = 214
\end{verbatim}
canonical destination blocks after its most recent inclusion does the
operation become \code{PENDING_FINAL}; then the destination credit, reservation
and escrow are sufficient.  Wallets and source Bridge UI show a pre-final
operation as \emph{source observed}, never forced-queue accepted or
source-independent.
...
static bounds before \code{PENDING}; failure leaves neither reservation, queue
leaf nor partial credit.

Only \code{ActiveSettlementRouter} may append.  It calls the active
Settlement's \code{sync()} under a non-reentrant guard and returns \code{SYNCED}
without appending if any transition occurred.  Otherwise it computes
\code{enqueuedAt=block.timestamp} and
\code{dueAt=max(enqueuedAt+FORCE_DELAY,lastDueAt,recoveryExpiry+1)} (the last
term is zero outside recovery), and appends atomically.  A kind-1 append also
consumes its prior reservation.  \code{ForcedQueue} stores one
...
\end{verbatim}
At \code{inboxResultHash[VERSION][slot]}, an empty value stores
\code{resultHash} and sets the received bit at that same slot.  A repeated row
succeeds without writes only when the stored result matches; any conflict,
ordering error or excess row reverts the whole batch before a write.
\code{verifyInboxCredit} recomputes \code{creditId} and the slot, requires both
result and received bit, and returns the ID.  This is a new bytes32-domain ABI;
it never overloads legacy \code{getSignalSlot(uint64,address,bytes32)}.
...
\code{address} \code{srcBridge}.  The complete V2 lifecycle ABI is:
\begin{verbatim}
...
\code{msgHash=hashMessage(Message)}.  Every mutating or proof-bearing function
recomputes \code{creditId} from that hash and the context before any status,
value or external effect; the read-only \code{messageStatusV2} view deliberately
accepts the already-derived key.
Source \code{sendMessageV2} obtains the context from its live epoch and creates
the immutable CreditRecord.  It emits
\code{MessageSentV2(creditId,msgHash,SourceContextV2,Message)}; destination
changes emit \code{MessageStatusChangedV2(creditId,status)}.  Both events'
indexed fields and expanded tuple encoding are fixed by the profile.
Destination status, retry state and value release
are keyed only by \code{creditId}.  Its empty-proof path calls
\code{verifyInboxCredit}; its nonempty V2 path uses the append-only domain and
support entries to prove the same source CreditRecord directly.  Both paths
therefore share one exactly-once status.  V2 failure sends
\begin{verbatim}
H("slot-chain-bridge-failed-v2" || creditId)
\end{verbatim}
and \code{recallMessageV2} proves that exact signal; no V2 failure, recall or
view falls back to \code{msgHash}.  The release profile pins every expanded ABI
selector, event, status/failure storage slot and positive/negative lifecycle
vector.

Post-activation source BridgeV2 sends only the CreditRecord for a new message,
never the legacy \code{msgHash} signal, so the legacy proof path cannot process
the same message a second time.  Pre-activation messages remain on unchanged
V1 functions and are never converted.  InboxApply makes one bounded batch
...
Missing/pruned kind-0 bytes & Unexpired head waits; after validity it is consumed as EXPIRED\_NO\_TX from its durable descriptor. & Adds at most seven days; kind-1 credit data is durable state. \\
...
router appends.
\item \code{SourceVerifierRegistry}: non-proxy append-only domain and support
mappings with immutable historical verifier routes.  Only bounded entries from
the manifest being atomically activated by ProtocolVersionManager may be added;
there is no delete, redirect or age-proportional iteration.
\item \code{BridgeInboxAdapter}: exactly-once NONE/PENDING/QUEUED records,
escrow and one queue reservation per durable source credit.  It selects source
proof logic only through SourceVerifierRegistry and rejects while PREACTIVE.
...
\begin{itemize}
\item settlement/L2 chain IDs, Ethereum genesis hash, fork digest, EIP-2935
  address and activation block; SourceVerifierRegistry/ProtocolVersionManager
  addresses and runtime hashes; every append-only domain/support descriptor,
  manifest/activation binding and historical verifier; plus each source's
  nonzero \code{bytes32} BridgeEpochRegistry namespace, BridgeV2 activation,
  registry/epoch-controller runtime, constructor ABI, epoch/CreditRecord layouts
  and historical proof limits;
...
\item AnchorV4, InboxV2ActivationGate, InboxApply, SignalService and Bridge
  addresses, runtime code hashes,
...
\item the exact AnchorV4 checkpoint/ancestors transition, forced disposition
  rules, InboxApply one-shot/batch transition, SignalService inbox-slot and
  idempotence storage, Bridge V2 status/failure/recall state and both proof
  paths; and
\item at least six complete parent/prestate/input/transaction/receipt/header/
  poststate vectors: empty escape, valid kind 0, expired kind 0 without bytes,
  kind 1 before and after a Bridge-epoch rotation, A--B--A address reuse with
  the same message hash, registry/domain replacement, pending-capacity race,
  delayed append, PREACTIVE kind-0/kind-1 rejection, legacy attempts to activate
  or invoke V2, first post-cutover latch activation, orphan/replayed pending under a refreshed
  witness, both source-age boundaries, durable-record absence, topology-support
  manifest authorization/activation/finality, historical domain routing,
  conflicting registration/removal, multi-credit batch ordering/conflict and a cap-limited multi-block prefix,
  plus invalid forged/expired EIP-2935 header, oversized/duplicate/unsorted
  topology and source witnesses,
  extra-reserved-sender, unauthorized clone, mismatched/conflicting source
  Bridge, conflicting result/failure hash, every V2 lifecycle method,
  cross-path replay and duplicate-InboxApply vectors.
\end{itemize}
...
preparation call rejects before creating escrow, a credit or a reservation.
...
\code{BridgeInboxAdapter}; its finalized message manifest binds the existing
message hash, source domain/epoch, source Bridge/execution identity, source
emission block number and ownership/value/calldata fields and a separately refreshable source witness
header number/hash,
while destination invocation
remains a later bridge operation.
...
profile-required AnchorV4 runtime and the Bridge logic whose empty-proof path
queries SignalService's pinned credit instead of the live resolver.  One
non-proxy \code{InboxV2ActivationGate} is initialized false.  InboxApply, the SignalService
batch and every L2 BridgeV2 send/process/retry/fail/recall path independently
...
AnchorV4, InboxApply, SignalService and Bridge proxy/fork-router runtime code hashes;
every EIP-1967 implementation/beacon slot and profile-defined fork-selector
slot; the selected implementation accounts' runtime code hashes (which commit
...
beacon.  Every terminal implementation and value must equal the immutable
execution profile.
...
golden empty root/count/cursor, \code{pendingReservations=0},
\code{dueAt(0)=UINT64_MAX}, and an empty BridgeInboxAdapter with no
\code{BridgeCredit}, \code{PENDING} record or escrow, atomically points router
...
in which kind-0 or kind-1 ingress is enabled.
...
\code{queueCount+pendingReservations<=UINT32_MAX}; canonical contiguous range proof. \\
Source witness & 64--255 blocks; 2,048 / 524,288 B; 256 nodes; 10m gas &
Launch source is settlement Ethereum; a refreshable header is inside EIP-2935
history; header / total witness, state nodes and verification gas are independently bounded. \\
Source verifier release & at most 64 new entries/version; append-only &
...
New epoch waits \code{PENDING_FINALITY_BLOCKS} after its support profile
activation; predecessor shortening is conditional on successful activation. \\
...
\code{PENDING_FINALITY_BLOCKS} & 214 L1 blocks &
\code{F_L1 + ceil(REORG_MARGIN_SECONDS/12)} after the most recent preparation inclusion. \\
...
\item exact predeploy/proxy/router topology, terminal runtime code hashes,
  ...
  InboxV2ActivationGate, InboxApply, SignalService and Bridge;
\item exact nonzero \code{bytes32} Ethereum genesis/registry namespace,
  source-domain vector, EIP-2935 history lookup, canonical RLP and MPT decoders,
  append-only SourceVerifierRegistry domains/support/manifest authorization,
  historical verifier routing, BridgeEpochRegistry/CreditRecord ABI, runtime
  and storage layout, proof limits and valid/invalid source-witness corpus;
...
\item complete positive and negative vectors for parent, prestate, input,
  ...
  source-domain changes, Bridge epoch and implementation rotation, unauthorized clones,
  pending-capacity races, PREACTIVE rejection, legacy preactivation attempts,
  first-block latch activation, orphan/replayed pending with a refreshed
  witness, durable-record absence, old-domain routing, topology support
  authorization and lifecycle, multi-credit batches and every V1/V2 replay path.
\end{itemize}
...
\code{sync}, source-credit preparation/topology proof, recovery submit, slash and garbage collection fit current L1 gas
...
match the Keccak/EIP-712 golden vectors, EIP-2935/RLP source-header and MPT proofs,
...
For every supported source domain, an analogous drill starts from a clean
source node, reconstructs a fresh witness to an old immutable credit record,
prepares a kind-1 credit, orphans its unfinalized destination transaction, then
rebuilds the proof against a later depth-final header and replays it.  It also
tests both witness-age bounds and proves that a superseding destination profile
cannot remove the old topology entry.
...
duplicated inside it.  Its fixed byte length is 220 for kind 0 and 420 for kind
...
\code{canonicalTopologyBytes} has this exact packed grammar:
\begin{verbatim}
u8(1) || bytes4(entrySelector) || u8(nodeCount) ||
(u8(kind) || address20(account) || runtimeCodeHash || u8(slotCount) ||
   (bytes32(slot) || bytes32(value))*)*
\end{verbatim}
Here \code{1<=nodeCount<=8}, \code{slotCount<=8}, and the complete encoding is
at most 4,096 bytes.  Kinds are 0=terminal, 1=EIP-1967 proxy,
2=profile beacon and 3=profile fork router.  Exactly the last node is terminal;
accounts are nonzero and unique; code hashes are nonzero; and slots within a
node are strictly increasing and unique.  The first account is
\code{srcBridge}.  The immutable profile maps every admitted
\code{(kind,runtimeCodeHash,entrySelector)} to its exact required slot set and
a total next-account extractor.  Verification proves every listed account,
code hash and slot/value under the one source state root, applies each extractor
to obtain exactly the following account, and ends at the terminal node.  It
rejects an unknown tuple, missing/extra slot, duplicate, cycle, unconsumed node,
nonterminal final node or bytes absent from the profile's finite exact allowlist.
Thus ``complete topology'' is executable matching against reviewed descriptors,
not bytecode introspection or a caller-chosen list.
...
sourceDomain  0200a9c8b107399151be3a25b7ebfd4b500b616a3e9a7eab7698284ea0bfbb52
bridgeExec    902970f59bbc440b3d36429ad2add690959871287a52e65123001a635404c84b
bridgeLeaf    25166869aba48ca26b8ccb87e0172605eee3f191be9aaae8aca0b35f8b88f316
creditId      4e5ca7e70f7832fd08235821dd0e4b54c05635ab01eb2ce24562d77871307ad5
escrowId      9602fc9d79915d2e8fa052d9be72069ccbf13839b406584daf07088b34b648c5
inboxSlot     81cea6e71e51e82a170dbdcf00536cba7ecf493af91ac26b8bbf577fbf6f6d05
failedSignal  041527ac55aff43ae75e842b2c9ea7be33eab83fccb43c6813e1187af5ce5194
bridgeResult  c23272f6afd7c21ba7bc1ce211bd0b7caf5a498ffa782c63dbe3181768e82684
...
\code{settlement-window-model.py} reports 123 assertions over PREACTIVE/migration,
...
replacement/liability generations, tranche release units, bridge reservations,
L2 activation gating, refreshable source witnesses, manifest-authorized
historical source support, replay and timing
geometry.  Cryptographic booleans in that model are not cryptographic
evidence.

\code{commitment-model.py} reports 84 vectors/properties for exact EIP-712
...
source domains/topology, emission/witness separation, credit/escrow/inbox/failure slots
...
\item A kind-1 credit is prepared only from a bounded, depth-final,
EIP-2935-authenticated source header proving an append-only registry credit,
its immutable epoch and manifest-authorized historical domain/topology support.
The credit record is the source signal; no mutable external slot is required.
The witness header is refreshable and is distinct from
\code{emittedAtBlock}.  \code{PENDING} reserves capacity and validates every
static append predicate; a reorged preparation can be re-proved under a later
header, and source topology support cannot be removed.
\end{enumerate}
}
