...
\large\mdseries Production Design Candidate --- v2.3}
\author{}
\date{August 2026}
\maketitle
\thispagestyle{fancy}
...
\begin{verbatim}
CanonicalCore = (tipHash, tipSlot, stateRoot, messageCursor,
...
addresses and re-registration of a tombstoned key are forbidden.

The active table has exactly 64 indexed cells.  A registration becomes
effective only after \code{ENTRY_DELAY_WINDOWS=8}.  If the table is full it may
...
All 64 active cells,
including empty or \code{FREE} tranche leaves, are supplied.  The 64 eligible
...
generation-movement queue, and the bond remains escrowed until every generation
...
\code{globalMinReferencedSlot} is the minimum of the current dynamic
...
exact live round values.  Consequently semantically unused bytes cannot create
...
S = (uint256 chainId, uint256 protocolVersion,
     ...
     bytes32 endStateRoot, uint64 endCursor,
...
calldata, and constructs the new core from the explicit
\code{(tipHash,tipSlot,endStateRoot,endCursor,winningDataCommitment)} fields.
...
\begin{verbatim}
"slot-chain-data-fs-v2" || u256(chainId) || u256(protocolVersion) ||
...
most 32 sibling hashes and requires the resulting \code{liveNextDueAt} to exceed
...
contains its authenticated \code{endCursor,liveNextDueAt}.  Provisional replacement
changes no canonical state and pays nobody.  At or after the deadline,
\code{sync()} commits the stored best only if its boundary deadline is still
greater than \code{block.timestamp}.  The active queue router updates that one
scalar when an append occurs exactly at the best's boundary.  Thus a late close
cancels rather than committing a block that omits an already-due head, even
beyond the assumed inclusion bound.  Because
...
\code{(sender,nonce,chainId,rawTxHash,byteLength,gasLimit,maxFee,validUntil,
...
Kind 0 requires a nonexpired \code{validUntil} and
\code{accountedGas=max(gasLimit,intrinsicGas,MIN_FORCE_ACCOUNTED_GAS)}.  Kind 1
...
zero outside recovery), appends, and calls the fixed-gas
\code{onQueueAppend(index,dueAt)} hook.  The hook updates the canonical-head,
stored-best boundary, or current round's first-post-cutoff scalar only when its
cursor equals \code{index}.  A commit chooses the authenticated in-cutoff
boundary or that recorded post-cutoff value, never overwriting the latter with
\code{UINT64_MAX}.  Thus
\code{dueAt} is nondecreasing and force-due/coverage checks are O(1), without a
loop or per-item storage mapping in Settlement.

The queue is a depth-32 append-only Merkle vector with fixed capacity $2^{32}$,
...
changes the reconstructed root.  Bytes are permanent enqueue calldata and
...
last due time and deposits; payload bytes are permanent L1 calldata.  Only the
router appends.
\item \code{Settlement}: PREACTIVE flag, canonical state, one normal best,
recovery episode/round, seat pointer, queue-boundary scalars and a 256-cell
...
Forced ingress lives behind one \code{ActiveSettlementRouter}; the queue itself
is shared across the cutover.  Existing Taiko bridge messages remain on the
...
Migration has two phases.  First, anyone calls the ordinary entry point until
\code{sync()} returns unchanged and the old contract is quiescent; this may take
more than one transaction and never bypasses the early-return rule.  Second, the
governance executor calls \code{old.freezeAndExport(new)}.  It asserts that no
objective close/activation/round-roll is pending, cancels only a still-immature
provisional best, disables old candidate ingress, exports the exact canonical
state/cursor, makes the new contract verify the shared queue pair and an indexed
boundary proof for \code{headDueAt} (or empty-at-count), atomically
switches router authority, and changes new PREACTIVE to NORMAL.  The operation
contains only bounded non-reverting internal writes after its assertions; any
revert rolls all contracts back.  Old bridge signals keep their original claim
...
Entry/exit delay; generation moves & 8 / 268 windows; 4 per window & Fixed liability capacity is $4\times268$. \\
...
Queue / range proof & depth 32 / at most 129 hashes & Capacity $2^{32}$; canonical contiguous range proof. \\
...
\begin{verbatim}
coreHash = H("slot-chain-core-v2" || tipHash || u64(tipSlot) || stateRoot ||
  u64(messageCursor) || winningDataCommitment)
...
\code{H("slot-chain-manifest-node-v1"||u8(height)||left||right)} and the root is
\code{H("slot-chain-manifest-root-v1"||u16(count)||treeRoot)}; padding uses the
domain-separated empty leaf.  The InboxApply disposition commitment is:
\begin{verbatim}
...
entryRoot     2eec2b75d264b328ec558d880b822ece2f85661516b1d85056a6f20ecd0dc7f1
forcedRoot    ab03f105ee7d619fb2c81d31b38760720dff2cb35471bc28fdc063c31f71bd67
manifestRoot  bb06a375def8deb09a550f685a65096eaf14cc70816d0592c978c51f96dcb1be
statementHash 98a0e1a892e2fd43e569f64dce3478ba19636fd062aa6ca517c49e6485fad22a
recoveryId    9c2f404af0886c6f7abb51f1bbe578747b7122dbe792ae9e5f5d4e7b74c8f550
...
function sync() returns (bool changed) {
    ...
    if (mode == NORMAL && forcedHeadDue && normal window immature) {
        ...
        if (best exists && best.nextDueAt > now) commit(normalBest);
...
\code{lookahead-model.py} reports 34 assertions over legacy Keccak, EIP-4788
...
\code{settlement-window-model.py} reports 50 assertions over PREACTIVE/migration,
...
\code{commitment-model.py} reports 35 vectors/properties for exact EIP-712
