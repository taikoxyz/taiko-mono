...
\large\mdseries Production Design Candidate --- v2.5}
\author{}
\date{August 2026}
\maketitle
\thispagestyle{fancy}
...
forever.  This revision separates proved outputs from L1 close metadata,
...
A candidate has one prior-block anchor.  Normal anchors are authenticated by an
EIP-2935 block hash, the supplied execution header and MPT proofs, including the
historical forced-queue root/count.  A recovery round instead stores
...
Every admission-affecting mutation increments \code{admissionVersion} and
updates two roots: a six-level \code{registryRoot} over the 64 active cells and
an eleven-level \code{admissionRoot} over 2,048 fixed positions containing the
64 active and 1,072 liability records plus canonical empty padding.  The latter
retains displaced scheduled keys.  Normal open and every recovery round freeze
...
increments \code{admissionVersion} and publishes both new roots atomically.
...
the base canonical state, current/frozen versions and recovery round, recomputes
...
when \code{endCursor=forceCutoff}.  Normal submission additionally verifies the
current-root boundary proof described in \S\ref{sec:machine}; recovery uses the
round's post-cutoff scalar.  These values initialize the O(1) head check.
...
\centering
\begin{tikzpicture}
\node[shaded,text width=30mm] (idle) at (0,1.8) {NORMAL\_IDLE};
\node[box,text width=34mm] (open) at (5.4,1.8) {NORMAL\_OPEN\\best + deadline};
\node[shaded,text width=34mm] (recovery) at (2.7,0) {RECOVERY\_OPEN\\episode + round};
\draw[arrow] (idle) to[bend left=22]
  node[above,font=\scriptsize]{submit} (open);
\draw[arrow] (open) edge[loop right] node[right,font=\scriptsize]{replace} (open);
\draw[arrow] (open) to[bend left=22]
  node[below,font=\scriptsize]{mature close} (idle);
\draw[arrow] (idle) -- node[left,font=\scriptsize]{trigger} (recovery);
\draw[arrow] (open) -- node[right,font=\scriptsize]{cancel + trigger} (recovery);
\draw[arrow] (recovery) to[bend left=38]
  node[left,font=\scriptsize]{recovery commit} (idle);
\end{tikzpicture}
...
\code{(admissionVersion,admissionRoot)}.  It also freezes the first candidate's
\code{(anchorNumber,anchorHash)} and recomputed normal \code{contextId}; every
replacement must use that exact admission pair, anchor and context.  If the
opening transaction is removed by an L1 reorg, all of those fields roll back,
and signatures made for a replacement fork have a different context rather
than becoming false equivocation evidence.  Signatures for different pre-open
anchors are different contexts and are explicitly not slashable against each
other; only the anchor/context that opens can participate in that normal
competition.  It also freezes
\code{requiredThrough=deadline+T_INCLUDE_MAX_SECONDS}.  Every candidate first
...
\code{(forceStart,forceEnd,(queueIndex,disposition,txIndexOrResultHash)*)}; the
...
\code{txIndex=UINT32_MAX} and a profile-defined reason hash, code 4 requires the
exact Ethereum transaction index and eventual receipt hash, and code 5 requires
\code{txIndex=UINT32_MAX} and the durable credit/result hash.  Unknown codes or
...
same base, admission pair and anchor carried by both headers; tier 2 recomputes
the recovery context.  It supplies one proof under the signed admission root
...
active/liability generation.  The window tranche is \code{RESERVED(W)} or
...
After that drain, migration has two phases.  First, anyone advances the old
Inbox until its finalized transition is quiescent; the gate requires
\code{nextProposalId=lastFinalizedProposalId+1}, the old core's
\code{lastFinalizedBlockHash}, an empty native forced queue and no unsettled
escrow.  The caller supplies the canonical RLP of that final L2 header.
The adapter requires its hash to equal \code{lastFinalizedBlockHash}, its
...
Second, the governance executor invokes the audited migration adapter.  From
...
changes new PREACTIVE to NORMAL, and reconfigures or
upgrades SignalService so the immutable SettlementCheckpointAdapter is its
authorized syncer.  It also installs the reviewed InboxApply
\code{markReceivedFromInbox} authorization before kind-1 ingress is enabled.
The operation
...
H("slot-chain-admission-leaf-v1" || u16(i) || u8(present) ||
  ...
  u64(effectiveL2Slot) || trancheRoot || u64(tombstonedAtL2Slot))
\end{verbatim}
...
the pair is stored together and never reconstructed from a newer root.
...
admissionRoot 783548941546bbc76487459185c787e4ffd7677b4487692dd6b1308dcc2e13f6
...
normalContext 92aa73a63468548708ffeff5a04e1cc59912ffc3ab9bd26e87938d206e123cc9
migrationData 916b1b24deee7fde4484c3cebe42937c32b41deb7b39529feb517986e7414399
winningData   701cd88d088ce38248844c81576142541c4980ed295bb2b55d267c3c8889434a
statementHash e767eccaf8a6088c7bbfa8202ae160aa473eada26d1b04ce7b3a34c4fbb0e294
recoveryId    454ec13c67dcfe4d215b73ee1ca32645be14605e3e86b5c42ff2df1e1e368975
...
\code{settlement-window-model.py} reports 67 assertions over PREACTIVE/migration,
...
\code{commitment-model.py} reports 49 vectors/properties for exact EIP-712
