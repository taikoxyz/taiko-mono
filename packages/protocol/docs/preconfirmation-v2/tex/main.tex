...
\large\mdseries Audited Design Candidate --- v2.11}
\author{}
\date{August 2026}
\maketitle
\thispagestyle{fancy}
...
kind-1 credits only---safety/finality of the supported source domain, its
profile-authenticated historical Bridge registry and availability of the
source proof package until the destination \code{PENDING} transaction is final
plus \code{REORG_MARGIN}, the protocol claims:
...
\item A source message is not yet an admitted forced envelope.  Before its
  destination-L1 \code{PENDING_FINAL} operational milestone, kind-1 ingress liveness depends on a
  complete source proof package; total loss of every copy can strand the source
  operation and is not reported as forced-queue acceptance.
...
identified by chain ID, genesis hash, registry address and immutable registry
namespace.  Its EIP-2935-authenticated historical header and Bridge-epoch state
are proven to the same L1.  Registry governance is an external safety
dependency; a current resolver lookup never substitutes for historical proof.
\item[Source proof archive.] Any untrusted, replaceable source of the
content-addressed header, receipt/event, account/storage trie and runtime-code
objects needed to prepare a kind-1 credit.  Objects are root/hash verified and
the archive has no protocol authority; at least one complete copy must remain
reachable until the destination \code{PENDING} inclusion is L1-final plus
\code{REORG_MARGIN}.
...
sourceDomainId = H("slot-chain-source-domain-v1" || u64(srcChainId) ||
  sourceGenesisHash || address20(epochRegistry) || registryNamespace)
...
contract; its nonzero \code{registryNamespace} is a constructor immutable and
its code has no self-upgrade path.  Replacing the registry requires a new
...
\begin{verbatim}
F_L1 <= sourceAge < EIP2935_HISTORY_ENTRIES
\end{verbatim}
and reads the
...
Keccak of the supplied canonical RLP header.  The adapter extracts the state
and receipt roots from that authenticated header; no caller-supplied
``finality certificate'' or root is authoritative.  The header is at most
2,048 bytes; the entire source witness is at most 524,288 bytes, at most 64
receipt-trie nodes and 256 state-trie nodes, and verification receives at most
10,000,000 gas.  Extra nodes, duplicate paths, noncanonical RLP, an expired
history index or any limit breach reject.  Supporting another source consensus
requires a new protocol version with an equally byte-exact, bounded, audited
finality verifier and is not part of the launch profile.
...
\code{BridgeCredit} record and \code{PENDING} state.  Under that one source header, the source
witness authenticates all of: the exact BridgeV2 message record/event whose
canonical \code{msgHash} and ownership/value/calldata/destination fields match
the descriptor; a profile-listed \code{BridgeEpochRegistry} record authorizing
the exact \code{(sourceDomainId,srcEpoch,srcBridge,bridgeExecutionHash)} at
\code{srcBlockNumber}; and the SignalService slot showing that the authorized Bridge
sent \code{creditId}, rather than the legacy \code{msgHash}.  The profile pins
the source genesis/hash domain, EIP-2935 address, registry address/namespace,
code hash, storage keys, BridgeV2 activation height and proof limits.  A Bridge-shaped clone or an
app outside its historically authenticated authorization epoch rejects even if
it emitted a genuine signal.  The adapter records \code{srcEpoch},
\code{sourceDomainId}, \code{srcBridge}, \code{srcBlockNumber} and
\code{srcBlockHash}; checking only the current
...
and deactivation block.  Epochs are
nonoverlapping.  A successor and any shortening of its predecessor's initial
\code{UINT64_MAX} end must be scheduled more than the source finality delay in
advance; activation, elapsed intervals and past endpoints are immutable.
...
dispatch reconstruction algorithm.  Each destination execution profile
contains a finite allowlist of exact topology bytes and hashes.  The registry
or its immutable epoch controller owns every upgrade/selector authority.  A
same-header witness proves the complete topology and terminal code hashes,
reconstructs the unique dispatch path and matches an allowlisted hash.  A
topology change requires a future successor epoch; a topology absent from the
destination allowlist additionally requires a new destination protocol version.
...
\code{0<srcBlockNumber<=UINT64_MAX}, nonzero \code{srcBlockHash} and
...
Before \code{PENDING_FINAL}, the permissionless preparer/archive retains a source proof package
containing the canonical RLP source header, the receipt/event
proof and full message fields, account/storage MPT nodes for the epoch registry,
every proxy/router/implementation hop, the BridgeV2 message record and the
SignalService slot, plus each runtime-bytecode preimage keyed by code hash.  A
consumer verifies every object against \code{srcBlockHash} and its state or
receipt root.  \code{PENDING_UNFINALIZED} is an operational label, not new
contract state: if its L1 transaction is orphaned, the retained package is
resubmitted.  Only after its inclusion block is canonical for \code{F_L1} plus
\code{REORG_MARGIN} is it \code{PENDING_FINAL}; then the immutable credit,
reservation and escrow are sufficient and source archives leave the
queue/recovery path.  Wallets and source Bridge UI show a pre-final operation
as \emph{source observed}, never forced-queue accepted or source-independent.
...
\begin{verbatim}
H("slot-chain-bridge-credit-result-v5" || u64(queueIndex) || creditId ||
  msgHash || u256(srcChainId) || sourceDomainId ||
  u64(srcEpoch) || address20(srcBridge) ||
  u64(srcBlockNumber) || srcBlockHash || u256(destChainId) ||
  address20(srcOwner) ||
...
\item settlement/L2 chain IDs, Ethereum genesis hash, fork digest, EIP-2935
  ...
  BridgeEpochRegistry namespace, BridgeV2 activation, registry/epoch-controller
  addresses/runtime hashes/storage layouts, the finite exact topology allowlist
...
\item at least six complete parent/prestate/input/transaction/receipt/header/
  ...
  delayed append, orphan/replayed pending and a cap-limited multi-block prefix,
...
message hash, source domain/epoch, source Bridge/execution identity, source emission block number/hash and
ownership/value/calldata fields,
...
golden empty pair and \code{dueAt(0)=UINT64_MAX}, atomically points router
...
Source history / witness & 8,191 blocks; 2,048 / 524,288 B; 64 / 256 nodes; 10m gas &
Launch source is settlement Ethereum; source depth is at least \code{F_L1} and
strictly below EIP-2935 history; header / total witness, receipt / state nodes
and verification gas are independently bounded. \\
\code{MAX_FORCE_VALIDITY_SECONDS} & 604,800 s & Bounds missing kind-0 payload delay; kind 1 is durable. \\
...
\item exact Ethereum genesis/source-domain/registry namespace, EIP-2935
  history lookup, canonical RLP and MPT decoders, topology allowlist/extractors,
  proof limits and valid/invalid source-witness corpus;
...
\item complete parent/prestate/input/transaction/receipt/header/poststate
  ...
  pending-capacity races, orphan/replayed pending, source-package loss/restore
  and legacy/V2 replay.
\end{itemize}
...
For every supported source domain, an analogous drill deletes source receipt,
state and code databases, prepares a kind-1 credit from only the retained proof
package, then proves that the destination no longer depends on that package
only after \code{PENDING_FINAL}; the drill also orphans an unfinalized pending
transaction and replays it from the retained package.
...
canonical-prestate and pre-\code{PENDING_FINAL} source-package availability assumptions.
...
A kind-1 leaf is:
\begin{verbatim}
"slot-chain-force-bridge-v6" || u32(i) || msgHash ||
...
duplicated inside it.  Its fixed byte length is 220 for kind 0 and 412 for kind
...
bridgeLeaf    a061f7347357fbb15a724ddfad8f8bf8eed696ee317eaabf2bc0bb329fc86165
creditId      4e5ca7e70f7832fd08235821dd0e4b54c05635ab01eb2ce24562d77871307ad5
bridgeResult  ae71b8a9ad489f229e6f7d23ea5bb19ea701101a23b6f555cf7e58803643b629
...
\code{settlement-window-model.py} reports 114 assertions over PREACTIVE/migration,
...
replacement/liability generations, tranche release units, replay and timing
geometry.  Cryptographic booleans in that model are not cryptographic
evidence.

\code{commitment-model.py} reports 77 vectors/properties for exact EIP-712
...
source domains/topology, dispositions, recovery ID, Fiat--Shamir input, body
...
\item A kind-1 credit is prepared only from one EIP-2935-authenticated,
depth-final source header that simultaneously authenticates its source domain,
immutable message, epoch, allowlisted execution topology and \code{creditId}
signal.  \code{PENDING} reserves capacity and validates every static append
predicate; source proof objects remain available through
\code{PENDING_FINAL}, after which they are not needed for progress.
\end{enumerate}
}
