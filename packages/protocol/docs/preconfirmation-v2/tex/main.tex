...
\large\mdseries Audited Design Candidate --- v2.8}
\author{}
\date{August 2026}
\maketitle
\thispagestyle{fancy}
...
of a canonical prestate reconstruction package as defined below, the protocol claims:
...
\begin{verbatim}
(msgHash, srcChainId, srcBridge, destChainId, srcOwner, destOwner,
...
\code{(srcChainId,srcBridge,msgHash)} tuple;
...
\code{NONE -> PENDING -> QUEUED(index)}.  Source-finality verification and value
escrow create an immutable durable \code{BridgeCredit} record and \code{PENDING}
state.  The source-finality proof authenticates the exact Bridge application
that emitted the message; the adapter records it as \code{srcBridge} and
rejects zero or resolver-only values not proved by that source state/event.
Admission also requires \code{0<srcChainId<=UINT64_MAX},
\code{destChainId=l2ChainId} and \code{srcBridge!=address(0)}; no narrowing cast
or destination-chain inference is permitted.
Anyone may call \code{finalizeEnqueue(msgHash)}.  A Router
...
both the leaf and status persist or both revert.  Repeated calls return the
stored index.  The upstream message is never reported queued merely because
...
\begin{verbatim}
H("slot-chain-bridge-credit-result-v2" || u64(queueIndex) || msgHash ||
  u256(srcChainId) || address20(srcBridge) || u256(destChainId) ||
  address20(srcOwner) ||
...
idempotent SignalService entry point with the exact
\code{(srcChainId,srcBridge,msgHash)} tuple.  Destination Bridge stores that
source application against \code{(srcChainId,msgHash)} when the credit is
applied and uses the stored value, never a fresh resolver lookup, for the later zero-proof
received-signal check.  Resolver rotation affects only credits admitted from a
...
The launch fork adds one narrowly authorized
\code{markReceivedFromInbox} entry point on SignalService.
Only the immutable \code{InboxApply} predeploy may call it; it is idempotent,
checks the durable credit hash, writes only the received-signal bit/value, and
performs no recipient call.  Legacy proof-based
\code{proveSignalReceived} remains unchanged.  AnchorV4 and InboxApply are
...
\begin{itemize}
\item settlement/L2 chain IDs, Ethereum fork digest and activation block;
...
\item the exact AnchorV4 checkpoint/ancestors transition, forced disposition
  rules, InboxApply one-shot/signal-credit transition and Bridge's pinned
  source-application lookup; and
\item at least six complete parent/prestate/input/transaction/receipt/header/
  ...
  kind 1 before and after a resolver rotation, and a cap-limited multi-block
  prefix, plus invalid extra-reserved-sender, mismatched source Bridge and
  duplicate-InboxApply vectors.
\end{itemize}
...
message hash, source chain, source Bridge and ownership/value/calldata fields,
...
profile-required AnchorV4 runtime and the Bridge logic that persists and later
queries each credit's pinned \code{srcBridge}.  This is an ordinary finalized L2 governance
...
\item complete parent/prestate/input/transaction/receipt/header/poststate
  ...
  source-Bridge resolver rotation.
\end{itemize}
...
A kind-1 leaf is:
\begin{verbatim}
"slot-chain-force-bridge-v3" || u32(i) || msgHash ||
u256(srcChainId) || address20(srcBridge) || u256(destChainId) ||
address20(srcOwner) ||
address20(destOwner) || u256(value) || calldataHash || escrowId ||
...
duplicated inside it.  Its fixed byte length is 220 for kind 0 and 332 for kind
...
bridgeResult  ab9a3282875222b07a9e91fd1858b055f73a7fca873146a0aba9f95edc38ecf0
...
\code{settlement-window-model.py} reports 100 assertions over PREACTIVE/migration,
...
\code{commitment-model.py} reports 61 vectors/properties for exact EIP-712
