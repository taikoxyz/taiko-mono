...
\large\mdseries Production Design Candidate --- v2.4}
\author{}
\date{August 2026}
\maketitle
\thispagestyle{fancy}
...
next consecutive EVM block number.  The canonical event always emits
...
\code{blockhash(block.number-1)} and its timestamp while available, directly
stores the current queue root/count, and requires the parent anchor to reach
depth \code{F_L1}.  Current queue state is never claimed to be part of the
parent's historical state root.  An L1 reorg rolls the stored canonical state
and anchor back together.  Initial values and bridge cursors are migration
inputs in
...
addresses, reuse while any prior generation remains active or liable, and
re-registration of a tombstoned key are forbidden.  With
...
reservations for that generation.
...
All 64 active cells and their tranche-ring leaves at \code{W mod 512} are
supplied.  A canonical \code{EMPTY} leaf is valid input and makes the cell
...
the tranche's absolute release time is
\code{evidenceDeadline(W)+REORG_MARGIN_SECONDS}.  Equivocation evidence must
land no later than its evidence deadline.  \code{globalMinReferencedSlot} is the minimum of the current dynamic
...
Every active cell supplies its tranche leaf at index \code{W mod 512}, including
the canonical empty/\code{FREE} leaf, and the leaf's stored window must be $W$.
...
coinbase, tier, contextId, admissionVersion,
...
SlotChainBlock(
 ...
 bytes32 contextId,uint64 admissionVersion,uint64 episode,
...
\begin{center}
\code{0x579f796202662a2edf67bed2afcb07d3}\\
\code{adff1e5e7e10e7dedcb7c6632f8d49de}
\end{center}
The EIP-712
domain is
...
\code{contextId=baseCanonicalHash},
...
RLP header only authenticates the stored parent hash/timestamp; the queue pair
...
S = (uint256 settlementChainId, uint256 l2ChainId, uint256 protocolVersion,
     ...
     bytes32 forcedMetadataCommitment,
     uint64 startCursor,
...
calldata, recomputes the bounded forced-metadata commitment from authoritative
...
Manifests are ordered by block then chunk; duplicate, extra, overlapping or
uncovered data is invalid.  \code{dataManifestRoot} is the exact tree in
Appendix~\ref{sec:encodings}, not a free-form publisher value.
...
\code{(admissionVersion,admissionRoot)}.  It also freezes
...
its timestamp, current \code{forceRoot/forceCutoff}, current
...
packed authoritative \code{QueueMeta} per index:
\begin{verbatim}
(leafHash, dueAt, kind, validUntil, effectRef)
\end{verbatim}
\code{effectRef} is the raw-transaction hash for kind 0 and the durable bridge
record hash for kind 1.  Settlement reads \code{dueAt[index]} directly from the
trusted queue for the canonical head and a stored best's live boundary; there
is no callback, cached deadline or duplicated scalar.  Missing indices return
\code{UINT64_MAX}.  Thus \code{dueAt} is nondecreasing and force-due/coverage
checks are one authoritative O(1) read.
...
\code{QueueMeta} records and computes
\begin{verbatim}
H("slot-chain-force-meta-list-v1" || u64(start) || u16(count) ||
  (u32(index)||leafHash||u64(dueAt)||u8(kind)||
   u64(validUntil)||effectRef)*)
\end{verbatim}
as \code{forcedMetadataCommitment}.  The circuit requires each metadata
\code{leafHash} to equal the corresponding revealed Merkle leaf.  This bounded
current-state commitment makes old entries consumable without assuming that
arbitrary historical calldata remains served forever.
...
\code{effectRef} and reconstruct its full leaf.  Once \code{validUntil} is
strictly before the L2 block timestamp, the circuit emits the unique
\code{EXPIRED_NO_TX} disposition from authenticated \code{QueueMeta} without
...
Its timestamp is the round escape slot, coinbase is the protocol sink and there
...
key, slot, tier, context ID, admission version, episode, revision and recovery
ID, plus a
proof under the \emph{current} \code{admissionRoot} of that key's still-retained
active/liability generation and its \code{RESERVED(W)} or \code{LIABLE(W)}
tranche leaf.  The two complete struct hashes must differ.  Signing two
...
\code{evidenceDeadline(W)}.  Address reuse is forbidden while that retained
generation exists, so the proof cannot select a newer incarnation.  This rule
keeps evidence verification fixed-depth while the liability-capacity invariant
retains every slashable generation through the deadline.
...
Missing/pruned kind-0 bytes & Unexpired head waits; after validity it is consumed as EXPIRED\_NO\_TX from state metadata. & Adds at most seven days; kind-1 credit data is durable state. \\
...
last due time, packed per-index metadata and deposits; expired kind-0 metadata
and durable kind-1 credits remain sufficient after historical calldata is
unavailable.  Only the router appends.
...
strictly greater \code{l2BlockNumber}, and writes exactly
...
value and timestamp; it never queries an expired history entry or mutates the
meaning of a past header.  Its queue pair is round storage, not an MPT claim.
...
new legacy forced admission, then drains every admitted legacy record under the
legacy validity/expiry rules and settles or refunds all associated value.
Activation is blocked until the audited legacy predicates report
\code{head==tail} and zero unsettled forced escrow.  Only then is the new empty
...
Inbox until its finalized transition is quiescent; the gate requires its exact
finalized L2 height, block hash, state root, bridge checkpoint and zero pending
proposal/forced state.  Second, the governance executor invokes the audited
migration adapter.  It freezes old proposal/forced ingress, imports those exact
canonical fields into new PREACTIVE state, verifies the new queue's golden
empty pair, atomically points router authority at the new queue/Settlement,
...
winningData   20e1edde3f8f232ad46ea5662781e691732c1cb094da109c752729a706258b75
statementHash e944338943078b59c93dde936ddd2bc8ac47fbf241e4ccdd02f52dc358121be6
recoveryId    838cab6519eafe0e2a13d2d79be09a7029fd304c9f6d97ac6b878a1f1d2e0e0f
...
\code{lookahead-model.py} reports 35 assertions over legacy Keccak, EIP-4788
...
\code{settlement-window-model.py} reports 62 assertions over PREACTIVE/migration,
early-return semantics, force-due precedence, renewable recovery, no-seat
escape, bounded Merkle-prefix predicates, immutable anchor chronology, O(1) due
...
\code{commitment-model.py} reports 43 vectors/properties for exact EIP-712
domain/struct/digest, canonical and statement hashes, registry/admission/entry/
tranche commitments, forced vector/range proofs, session MMR, manifest,
...
admission version/root pair, strict slot progress and the candidate's one authenticated
