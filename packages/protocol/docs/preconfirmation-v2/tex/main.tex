...
\large\mdseries Production Design Candidate --- v2.6}
\author{}
\date{August 2026}
\maketitle
\thispagestyle{fancy}
...
\code{activateNormalContext()}.  It again begins with \code{sync()}, requires
\code{block.number>armBlockNumber} and no more than 255 intervening blocks,
...
normal \code{contextId} and emits \code{NormalContextActivated}.  A stale arm
may only be discarded and re-armed; no candidate is accepted while armed.
...
InboxApply calldata would create a fixed point through cumulative gas.  Code 5 requires
...
AnchorV4, InboxApply and SignalService runtime code hashes, InboxApply one-shot
initial state, and SignalService authorized-caller slot.  Every value must equal
the immutable execution profile.
...
EIP history & 8,191 entries &
$\lceil(W_{settle}+CLOCK\_SKEW+384+EVIDENCE\_DELAY+REORG\_MARGIN)/12\rceil+2<8,191$;
...
\code{settlement-window-model.py} reports 95 assertions over PREACTIVE/migration,
...
\code{commitment-model.py} reports 56 vectors/properties for exact EIP-712
