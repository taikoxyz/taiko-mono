...
\large\mdseries Audited Design Candidate --- v2.10}
\author{}
\date{August 2026}
\maketitle
\thispagestyle{fancy}
...
kind-1 credits only---safety/finality of the supported source chain, its
profile-authenticated historical Bridge registry and availability of the
source proof package until L1 records \code{PENDING}, the protocol claims:
...
\item A supported source chain finality failure or unauthorized change to its
...
\item A source message is not yet an admitted forced envelope.  Before its
  destination-L1 \code{PENDING} event, kind-1 ingress liveness depends on a
...
\item[Bridge source chain.] For kind 1 only, a profile-listed chain whose
finalized header and historical Bridge-epoch registry state are proven to L1.
Its consensus and registry governance are external safety dependencies; a
current resolver lookup never substitutes for historical authentication.
...
reachable until L1 records \code{PENDING}.
...
\begin{verbatim}
(msgHash, srcChainId, srcEpoch, srcBridge, srcBlockNumber, srcBlockHash,
...
identifier
\begin{verbatim}
creditId = H("slot-chain-bridge-credit-id-v2" || u64(srcChainId) ||
             u64(srcEpoch) || address20(srcBridge) || msgHash)
\end{verbatim}
...
\code{msgHash}.  Source-finality verification and value
escrow create an immutable durable \code{BridgeCredit} record and \code{PENDING}
state.  Under one finalized source header at
\code{(srcBlockNumber,srcBlockHash)}, the source
...
the exact \code{(srcEpoch,srcBridge,bridgeExecutionHash)} at
...
the source registry address, code hash, storage key, BridgeV2 activation height
and proof format for every supported source chain.  A Bridge-shaped clone or an
...
\code{srcBridge}, \code{srcBlockNumber} and \code{srcBlockHash}; checking only the current
...
\end{verbatim}
where the profile's canonical bytes list, in execution order, the proxy/router,
implementation/beacon, admin and selector-slot accounts, runtime code hashes,
immutable values and storage-slot/value pairs.  The registry or its immutable
epoch controller owns every upgrade/selector authority.  A same-header witness
proves the complete topology and terminal code hashes against the epoch value;
an implementation or topology change requires a future successor epoch.
...
\code{0<srcEpoch<=UINT64_MAX}, \code{0<srcBlockNumber<=UINT64_MAX}, nonzero
\code{srcBlockHash} and \code{msgHash}, \code{destChainId=l2ChainId} and a
...
and the transition to \code{QUEUED(index)} occur in one transaction, so either
both the leaf and status persist or both revert.  Repeated calls for the same
...
Before \code{PENDING}, the permissionless preparer needs a source proof package
containing the canonical source header/finality certificate, the receipt/event
...
receipt root.  Once L1 records \code{PENDING}, the immutable credit and escrow
are sufficient and source archives are no longer on the queue/recovery path.
Wallets and source Bridge UI must show a pre-PENDING operation as
\emph{source observed}, not forced-queue accepted.
...
permanent calldata/state costs.  The adapter verifies kind-1 escrow before
append; failure leaves neither queue leaf nor partial credit.
...
\code{dueAt=max(now+FORCE_DELAY,lastDueAt,recoveryExpiry+1)} (the last term is
zero outside recovery), and appends atomically.  \code{ForcedQueue} stores one
...
\begin{verbatim}
H("slot-chain-bridge-credit-result-v4" || u64(queueIndex) || creditId ||
  msgHash || u256(srcChainId) || u64(srcEpoch) || address20(srcBridge) ||
...
\begin{verbatim}
markReceivedFromInbox(uint64 srcChainId, uint64 srcEpoch, address srcBridge,
                      bytes32 msgHash, bytes32 resultHash)
verifyInboxCredit(uint64 srcChainId, uint64 srcEpoch,
                  address srcBridge, bytes32 msgHash)
...
\code{resultHash} and sets the existing received bit at exactly
\code{getSignalSlot(srcChainId,srcBridge,creditId)}.  A repeated call succeeds
...
\begin{itemize}
\item settlement/L2 chain IDs, Ethereum fork digest and activation block, plus
  each supported source chain's BridgeV2 activation, BridgeEpochRegistry and
  epoch-controller addresses/runtime hashes/storage layouts, canonical topology
  encoding and exact historical authorization proof;
...
\item at least six complete parent/prestate/input/transaction/receipt/header/
  ...
  the same message hash, and a cap-limited multi-block prefix, plus invalid
...
message hash, source chain/epoch, source Bridge/execution identity, source emission block number/hash and
...
Queue / range proof & depth 32 / at most 129 hashes & Capacity \code{UINT32_MAX}; canonical contiguous range proof. \\
...
\item complete parent/prestate/input/transaction/receipt/header/poststate
  ...
  source-Bridge epoch/implementation rotation, unauthorized clones,
  source-package loss/restore and legacy/V2 replay.
\end{itemize}
...
For every supported source chain, an analogous drill deletes source receipt,
...
canonical-prestate and pre-PENDING source-package availability assumptions.
...
A kind-1 leaf is:
\begin{verbatim}
"slot-chain-force-bridge-v5" || u32(i) || msgHash ||
u256(srcChainId) || u64(srcEpoch) || address20(srcBridge) ||
u64(srcBlockNumber) ||
srcBlockHash ||
u256(destChainId) ||
address20(srcOwner) ||
...
duplicated inside it.  Its fixed byte length is 220 for kind 0 and 380 for kind
...
bridgeLeaf    f004dc1367a1a782b1d28ab02c77d8c6ff4a54aa8fe194a128b877a387d335a6
creditId      bfb7f2280ae785be98770a67e8d5c11d42995a0a961baeb479bde6bf368fcaee
bridgeResult  d2b68225ba3ea936159e0ec2157b07b1c72d268b89b31fc8d10fece7f9015c8e
...
\code{settlement-window-model.py} reports 104 assertions over PREACTIVE/migration,
...
\code{commitment-model.py} reports 71 vectors/properties for exact EIP-712
...
dispositions, recovery ID, Fiat--Shamir input, body chunks and blob codec.  The
...
\item A kind-1 credit is admitted only from one finalized source header that
simultaneously authenticates its immutable message, source epoch, execution
topology and \code{creditId} signal.  Before \code{PENDING} no forced-queue
acceptance is claimed; afterward source proof objects are not needed for progress.
\end{enumerate}
