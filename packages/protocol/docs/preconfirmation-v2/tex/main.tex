...
\large\mdseries Audited Design Candidate --- v2.12}
\author{}
\date{August 2026}
\maketitle
\thispagestyle{fancy}
...
\end{verbatim}
Only the configured \code{BridgeInboxAdapter} may enqueue it, it has no expiry
field, and value is fully escrowed.  Its
...
the same transaction as message emission.  The registry recomputes
...
ownership/value/calldata/destination field.  It also writes the corresponding
durable SignalService slot under the registry's application identity; either
write failing reverts emission.  A duplicate credit ID rejects.  The
destination proof authenticates the record, its referenced immutable epoch,
the signal and the exact descriptor.  An event is an indexing aid only and a
receipt proof has no consensus role.
...
The profile pins
the source genesis/hash domain, EIP-2935 address, registry address and namespace,
code hash, exact credit-record ABI/storage keys, BridgeV2 activation
height and proof limits.  A Bridge-shaped clone or an app outside its live
authorization epoch cannot create the registry record.  The adapter records
the source epoch, domain, Bridge, emission height and witness header
number and hash.  Checking only the current resolver is forbidden.  Because the credit and epoch records
are append-only, any later header satisfying the age bound can replace the
witness header without changing \code{creditId} or source semantics.
...
dispatch reconstruction algorithm.  Destination source support is a
profile-pinned, append-only catalog with at most 64 exact topology byte strings
and hashes per source domain.  Every successor destination profile is a strict
superset of the prior catalog; removal or reinterpretation is forbidden.  The
registry or its immutable epoch controller owns every upgrade/selector
...
record and epoch and matches the hash to the historical catalog entry; it does
...
\code{PENDING_FINALITY_BLOCKS} after the destination transaction adding that
exact \code{(sourceDomainId,bridgeExecutionHash)} became canonical.  BridgeV2
...
At launch, source and destination share Ethereum, so this is enforced by one
profile-pinned non-proxy \code{DestinationSourceSupportCatalog}.  Its
\code{add(domain,topologyBytes)} verifies the canonical hash, stores bytes/hash
and immutable \code{addedAtBlock}, and rejects after 64 entries for that domain;
it exposes no delete or update entry point.  Source epoch scheduling names that
entry, and activation rechecks the exact lead inequality.  Cross-chain support
would require a separately reviewed finality bridge and is outside this
profile.
...
and SignalService slot, plus the registry runtime-bytecode preimage keyed by
code hash.  A consumer verifies every object against \code{srcBlockHash} and
...
\end{verbatim}
InboxApply recomputes it from the authenticated descriptor before calling the
single idempotent SignalService entry point with the exact tuple and
\code{resultHash}.  SignalService, not Bridge, owns the persistent source-app
pin described below.  Resolver rotation affects only credits admitted from a
subsequent source-finality event; it cannot retarget or orphan an existing
credit.  Unknown codes or
noncanonical auxiliary fields reject.

The AnchorV4 and InboxApply sender addresses are reserved.  The circuit requires
their exact profile transactions once each at indices 0 and 1 and rejects any
forced or discretionary transaction recovered from either sender.  InboxApply
also stores the last applied L2 block number and rejects a second application
in that block.  These rules protect system nonces and the privileged signal
write even if a reserved signing algorithm or key is publicly reproducible.

The launch fork adds these exact SignalService operations:
\begin{verbatim}
markReceivedFromInbox(uint64 srcChainId, bytes32 sourceDomainId,
...
\end{verbatim}
Only the immutable \code{InboxApply} predeploy may call the first operation.
It requires nonzero app, message and result hashes.  At
\code{inboxResultHash[VERSION][creditId]}, an empty value stores
\code{resultHash} and sets the existing received bit at exactly the
\code{getSignalSlot} key of \code{(sourceDomainId,srcBridge,creditId)}.  A repeated call succeeds
without writes only when the stored result matches; a conflicting result
reverts before any write.  \code{verifyInboxCredit} recomputes \code{creditId},
requires its result and corresponding received bit to exist, and returns the ID.

Destination BridgeV2 computes the same ID and keys message status, retry state
and value release by it.  Its empty-proof path calls \code{verifyInboxCredit};
its nonempty V2 proof path authenticates the same historical source-Bridge
authorization and \code{creditId} signal.  Both paths therefore share one
exactly-once status.  Post-activation source BridgeV2 sends only \code{creditId}
for a new message, never its legacy \code{msgHash}, so the legacy msgHash path
cannot process the same message a second time.  Pre-activation messages remain
on the unchanged legacy path and are never converted.  InboxApply makes one
...
profile-defined protocol transactions: their reserved senders, sequential
system nonces, zero beneficiary tip and fee treatment are consensus fork rules,
not ordinary user accounts whose balance can run out.  Their gas still counts
...
\item settlement/L2 chain IDs, Ethereum genesis hash, fork digest, EIP-2935
  address and activation block, plus the launch source domain ID,
  nonzero \code{bytes32} BridgeEpochRegistry namespace, BridgeV2 activation,
  registry/epoch-controller addresses/runtime hashes, exact constructor ABI,
  epoch/CreditRecord/storage layouts, the append-only exact topology catalog
  and historical authorization proof limits;
...
\item AnchorV4, InboxApply, SignalService and Bridge addresses, runtime code hashes,
  selectors and ABI tuples; reserved senders, transaction types, signature
  bytes, system nonce rules, fee exception and gas ceilings;
\item the exact AnchorV4 checkpoint/ancestors transition, forced disposition
  rules, InboxApply one-shot transition, SignalService pin/idempotence storage
  and Bridge's empty-proof lookup; and
\item at least six complete parent/prestate/input/transaction/receipt/header/
  ...
  delayed append, PREACTIVE rejection, orphan/replayed pending under a refreshed
  witness, both source-age boundaries, durable-record absence, topology-support
  activation/finality and attempted removal, and a cap-limited multi-block prefix,
  ...
  Bridge, conflicting result hash, cross-path replay and duplicate-InboxApply vectors.
\end{itemize}
...
upgrades the L2 SignalService with \code{markReceivedFromInbox}, setting its
...
queries SignalService's pinned credit instead of the live resolver.  This is an ordinary finalized L2 governance
...
constructor immutables); InboxApply one-shot initial state; and SignalService
authorized-caller storage.  The proof follows the exact proxy/router topology
...
syncer.  It does not write L2 code or storage.  Kind-1 ingress is enabled only
...
Source topology catalog & at most 64 entries/domain; append-only &
New source epoch waits \code{PENDING_FINALITY_BLOCKS} after destination support;
successor profiles are strict supersets and removal is forbidden. \\
...
\item exact predeploy/proxy/router topology, terminal runtime code hashes,
  ...
  InboxApply, SignalService and Bridge;
\item exact nonzero \code{bytes32} Ethereum genesis/registry namespace,
  ...
  append-only topology-support catalog, BridgeEpochRegistry/CreditRecord ABI,
  runtime and storage layout, proof limits and valid/invalid source-witness corpus;
\item exact raw system transactions, signatures, nonces, ABI, fee handling,
  gas ceilings and all fork/header recurrence rules; and
\item complete parent/prestate/input/transaction/receipt/header/poststate
  positive and negative vectors listed in \S\ref{sec:implementation}, including
  source-domain/Bridge epoch/implementation rotation, unauthorized clones,
  pending-capacity races, PREACTIVE rejection, orphan/replayed pending with a
  refreshed witness, durable-record absence, topology-support lifecycle and
  legacy/V2 replay.
\end{itemize}
...
bridgeLeaf    603bae49422417961b222b3a7acee015c06d0a2e42e3b37bdef94b12cfed81b1
creditId      4e5ca7e70f7832fd08235821dd0e4b54c05635ab01eb2ce24562d77871307ad5
bridgeResult  1b9a1385a39ff47054e46494e629b8eeaeacdee5df9acdcf4e7b39b6aa473684
...
\code{settlement-window-model.py} reports 120 assertions over PREACTIVE/migration,
...
refreshable source witnesses, append-only topology support, replay and timing
geometry.  Cryptographic booleans in that model are not cryptographic
evidence.

\code{commitment-model.py} reports 79 vectors/properties for exact EIP-712
...
source domains/topology, emission/witness separation, dispositions, recovery ID, Fiat--Shamir input, body
...
its immutable epoch, historical topology support and \code{creditId} signal.
