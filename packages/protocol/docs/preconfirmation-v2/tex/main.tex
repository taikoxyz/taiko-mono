...
\large\mdseries Audited Design Candidate --- v2.9}
\author{}
\date{August 2026}
\maketitle
\thispagestyle{fancy}
...
kind-1 credits only---safety/finality of the supported source chain and its
profile-authenticated historical Bridge registry, the protocol claims:
...
\begin{verbatim}
(msgHash, srcChainId, srcBridge, srcBlockNumber, srcBlockHash,
...
identifier
\begin{verbatim}
creditId = H("slot-chain-bridge-credit-id-v1" || u64(srcChainId) ||
             address20(srcBridge) || msgHash)
\end{verbatim}
...
that \code{srcBridge} at \code{srcBlockNumber}; and the SignalService slot showing that the authorized Bridge
...
it emitted a genuine signal.  The adapter records \code{srcBridge},
\code{srcBlockNumber} and \code{srcBlockHash}; checking only the current
resolver is forbidden.

The source \code{BridgeEpochRegistry} retains every epoch's source Bridge,
activation block and deactivation block.  Epochs are
...
BridgeV2 checks that its own address is in the unique live epoch before sending
a V2 message.  Thus a rotation has block-boundary semantics, same-block
...
\code{destChainId=l2ChainId} and a nonzero \code{srcBridge}; no narrowing cast
or destination-chain inference is permitted.
...
\begin{verbatim}
H("slot-chain-bridge-credit-result-v3" || u64(queueIndex) || creditId ||
  msgHash || u256(srcChainId) || address20(srcBridge) ||
...
\begin{verbatim}
markReceivedFromInbox(uint64 srcChainId, address srcBridge,
                      bytes32 msgHash, bytes32 resultHash)
verifyInboxCredit(uint64 srcChainId, address srcBridge, bytes32 msgHash)
...
\item settlement/L2 chain IDs, Ethereum fork digest and activation block, plus
  each supported source chain's BridgeV2 activation, BridgeEpochRegistry
  address/runtime hash/storage layout and exact historical authorization proof;
...
\item at least six complete parent/prestate/input/transaction/receipt/header/
  ...
  kind 1 before and after a Bridge-epoch rotation, same-message distinct authorized
  epochs, and a cap-limited multi-block prefix, plus invalid
...
message hash, source chain, source Bridge, source emission block number/hash and
...
\item complete parent/prestate/input/transaction/receipt/header/poststate
  ...
  source-Bridge epoch rotation, unauthorized clones and legacy/V2 replay.
\end{itemize}
...
architecture defect is known under the stated L1, proof, funding and canonical
prestate-availability assumptions.  Nevertheless the design is \emph{not
...
A kind-1 leaf is:
\begin{verbatim}
"slot-chain-force-bridge-v4" || u32(i) || msgHash ||
u256(srcChainId) || address20(srcBridge) || u64(srcBlockNumber) ||
srcBlockHash ||
u256(destChainId) ||
address20(srcOwner) ||
address20(destOwner) || u256(value) || calldataHash || escrowId ||
...
duplicated inside it.  Its fixed byte length is 220 for kind 0 and 372 for kind
...
bridgeLeaf    f0dcc65ee5dae640105f071f23506747fa82868d142d5493e116ca76d70d9d92
creditId      daea3132762c38a40218b6233011febfcc834e19e6dbda894994ea9b7b0d0330
bridgeResult  daeb57aafde16e5a81695b768cc1a2d63a30d019103f06da57c6baa67284e467
...
\code{settlement-window-model.py} reports 103 assertions over PREACTIVE/migration,
...
\code{commitment-model.py} reports 69 vectors/properties for exact EIP-712
