...
| 文档 | 说明 |
| --- | --- |
| [`slot-chain-spec.md`](slot-chain-spec.md) | **主规范（草案 v1.25，中文）**。核心机制：1 秒一个 slot 的构建者排班表（lookahead）、逐块签名并以父块哈希串联的签名链（signature chain）、原子落地（数据 + 有效性证明一笔交易，接受即最终）、基于 L1 可观测滞后量（lag）的无许可兜底落地与失职计次（聚合者无论掉线还是恶意拖延都不是活性单点）、逐块电路强制的强制包含（forced inclusion，双侧条件的仅强制块逃生阀）、双签的 L1 直接验签罚没（窗口累积敞口定价）。相对 v15 删除了 EBC/epoch 判定、默认派生、完全无政府模式与取消级联等整套机制；§9 的活性核算表给出各角色（含 Byzantine 聚合者）故障下的恢复界。 |
...
草案 v1.25（2026-08-25）：初稿 + 二十五轮对抗评审修复 + 规范性信任模型（所有者
决定）。机制骨架完整，参数为初始建议值。
