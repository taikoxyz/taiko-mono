...
factory and real compiled adapter artifact, O(1) indexes for the remaining append-only-history hot
paths, and an exact O(1) `verifyInboxCredit` ABI carrying the credit ID and liquidity fee. Restart and
serialization coverage, the complete root-deployment transcript, historical release/reclamation
watermarks and measured EIP-150 gas certificates also remain open. The exact
