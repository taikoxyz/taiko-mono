# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范，草案 v1.18）
...
- **聚合者：不受信任（untrusted）。** 聚合者可以任意作恶——拖延、截断、择叉、
  ...
  r12-4 的精确化）**：§9 的界是**活性**界（最终性推进、席位更替）；聚合者与单个
  恶意构建者合谋对**预确认安全**的敞口——深度 ≤ `G_max − 1` 的无罚没孤立——是
  §5.4 明码标价的安全残留，不在"有界"承诺之内。
- **L1：安全且活。** 所有 L2 的标准假设。
...
- 父块规则（评审 r6-1 后收紧）：`parent.slot < s` 且 **`s − parent.slot ≤ G_max`**
  ...
  尾巴——而它只签了自己 slot 的一个块，无可罚没。缺口上限把单块能回退的深度封顶
  在 `G_max`，且约束逐块生效、不可接力叠加（每块的父块都必须在自己 `G_max` 以内，
  链头单调前进）。**恢复例外（评审 r9-1/r9-3 后重做）**：`parent` = 已落地 L1 链头
...
- **剩下的：相邻构建者的跳过（skip）——且它是可获利的（评审 r5-3 更正）。** builder
  ...
  **深跳过（父块指到 `s − G_max` 附近，评审 r11-2 补明）**一次签名可波及至多
  `G_max − 1` 个未落地块——§5.2 修订后的协调规范使诚实网络不跟随深跳过叉，它须被
  抢先落上 L1 才能生效。**竞速的要害（评审 r12-4 的补刀）**：若合谋落地者只是
  兜底者，这确是与在途诚实批次的竞速（窗口 ≈ 正常落地节奏，且兜底权在
  `lag > Δ_lag` 前根本不开）；**若合谋者就是在任聚合者，正常运行期它是唯一常态
  落地者，无竞速可言**——在任聚合者 + 单个恶意构建者可以无罚没地孤立至多
  `G_max − 1` 个未落地块。该组合是本设计**明码标价的安全残留**：深度 ≤
  `G_max − 1`（约 1 分钟的尾巴）、频率 ≤ 恶意构建者拿到的排班 slot 数、全程可
  归责（构建者的 parent 选择签名在案，聚合者的截断落地在 L1 公开），机械威慑
  只有席位价值与声誉，协议不罚没；结构性消除要求落地入口能证明"不存在更重
  分支"，即协议级 DA/链头承诺（附录 A-3 的成本恒等式），按 §1 信任模型不采纳。
  `G_max` 由此是一枚治理旋钮：调大增强缺口容忍（§6.4），调小压缩此敞口。直接
  重打包收益仍以攻击块自身容量为上限，每次尝试消耗一个排班 slot；两种跳过的
  **频率都由不诚实构建者占比约束——§1 信任模型的"诚实多数"正是在这里承重**
  （不是保守冗余）；
...
- **推论——预确认语义（写给用户文档的版本，评审 r1-1/r1-6 后的诚实版）：** 一个
  ...
  2. 某个后继构建者**跳过**它——对跳过者可获利（费用/MEV 转移）且不可罚没
     （r5-3）；公开可见、签名固定；相邻跳过波及单 slot，深跳过可波及至多
     `G_max − 1` 个未落地块且一般须赢得落地竞速（r11-2，§5.2）——但**在任聚合者
     合谋时无需竞速**（r12-4，上文的明码残留）；频率由诚实多数假设约束；纯包含
     类承诺通常经重打包兑现（§5.4 上文）；
...
- **存在挑战（liveness challenge）——附保证金、按结局结算（评审 r12-3：v1.11 的
  ...
  **结算按结局**：公告最终被取消（真有落地发生，"尾巴存活"被事实证实）⇒ 退还
  ...
  块头本身。
- **取消——同分支延伸即取消，冲突叉只重钉不重置（评审 r12-1/r12-5 合并 + r18-1
  收窄）**：v1.11 的取消条件是 `lag ≤ Δ_lag`（部分落地追不平即被斩，已由 r12-1
  改掉）；但 r12-1 的"任何批次被接受即取消"又过宽——Codex 第 10 轮指出一个恶意
  排班构建者可以签一条**冲突短叉**（deep-skip，≤ `G_max`，无双签，§5.4 已定性），
  合谋兜底者预证后抢落，它同样"推进了链头"从而作废本 episode，还让诚实证明者
  按旧链头做的证明全废、把首证优势拱手让给攻击者，如此在墙钟之下反复滴灌即可
  永久开着 lag、永久取消重锚——且这**不**需要对已有效交易做整窗口的持续审查，
  不落入 §5.4 明码接受的审查残留。修复——区分**同分支延伸**与**冲突叉重组**：
  - **同分支延伸**（被接受批次的落地后缀以钉定父块为祖先，即沿本 episode 关心的
    分支真正推进）：这是诚实恢复在进行的信号，作废公告；押金处置同下。
  - **冲突叉重组**（被接受批次把钉定父块 reorg 出正统）：**不作废、不重置**——
    L1 入口把 episode 的钉定父块**重钉**到新正统链头，**保留已流逝的窗口计时**
    （攻击者无法靠滴灌冲突叉重置时钟或刷新首证优势），并记一次 fork 事件。每条
    冲突叉须消耗攻击者一个排班 slot、且按 r16-1 至多 reorg `G_max − 1` 个未落地
    块，所以其频率被攻击者的排班占比硬性约束（同 §5.4 深跳过残留）；被浪费的
    诚实证明成本是**深跳过残留的证明层放大**（§5.4 明码，r18-1 补入"证明成本
    griefing"一项），不是新的无界活性破坏——重锚仍会在有界个 slot 内执行到最新
    正统头。
  由此只要诚实**同分支**落地还在持续，重锚就让位于它；冲突叉滴灌被"重钉不重置"
  中和；真实大缺口下无物可落，无人能取消。这与 §6.3 计次的"存在可落内容由落地
  事实自证"是同一判定手法。押金处置：被同分支延伸取消时 `B_ra` 付给取消批次的
  落地者（有挑战则与挑战者平分，见上）；恶意反复公告每次损失 `B_ra`，自限。
- **执行**：窗口期满（含延长）且期间无任何批次被接受，L1 入口核对钉定父块仍为
  当前落地头，重锚块即可落地，押金退还公告者。执行没有截止时刻——窗口是下限不是
  上限，证明未就绪可以晚落；期间一旦有批次落地则回到取消分支。
...
- **两类情形的走线核算（r12-1 后重算）**：
  - **真实大缺口**（连续缺勤 ≥ `G_max` 个 slot 且无人补签，r15-1 统一边界）：故障前尾巴会被落干净，无人能
    ...
    概率界（诚实多数下概率项 ≈ 秒级，可忽略）。
  - **r9-1 攻击情形**（尾巴存活、聚合者扣落地）：公告即被挑战（挑战者预期落地
    必然发生、稳收 `B_ra` 分成），窗口延至 `Δ_ra_ext`；兜底窗口自 `Δ_lag` 起
    已开、诚实兜底证明已在跑，**任何一个批次被接受即取消公告**（r12-1，不再
    要求把 lag 追平到 `Δ_lag` 以内）。攻击要成功，必须让整个窗口期内 L1 上没有
    任何兜底批次被接受——对可无限加价交易的持续审查，落回 §6.3 已定价的"最贵
    审查类别"，不再是偏向攻击方的裸竞速。
...
- **仅强制块同规 + 续接 episode（评审 r18-3——避免每批次重付一个重锚窗口）**：
  ...
  不变；钉定父块、同分支取消/冲突叉重钉规则同上）。但积压超过单批 blob/证明容量
...
- **非法条目的准入与弃置消费（评审 r9-2，Codex——P1）**：v1.8 的规则对一条畸形或
  ...
  1. **入队验证（L1 合约执行）**：载荷已改为 calldata 随交易提交（本节首条），合约
     ...
     签名可恢复、`chainId` 正确、**条目 gas 上限不超过其所属队列配额的单条目份额
     ——普通条目按 `C_force`、桥接条目按 `C_bridge` 计（评审 r16-2 的修正：若桥接
     条目只按 `C_force` 验，gas 落在 `(C_bridge, C_force]` 的条目能入队却永远塞
     不进每块 `C_bridge` 的桥接前缀，又因 FIFO 顺序不可越过，到达队头即死锁所有
     块）**。不过关即拒绝入队；桥接条目的上限由桥合约在构造消息时同步强制（设置
...
| 构建者跳过前块 / 缺勤 | 不构成过错（出块是机会不是义务） | 无罚没；跳过对跳过者**可获利**（费用/MEV 转移，r5-3；相邻跳过波及单 slot，深跳过至多 `G_max − 1` 个未落地块且须赢得落地竞速，r11-2），但签名在案、公开归责，频率受诚实多数假设约束；缺勤者损失自己 slot 的费收入 |
...
| 重锚流程 | re-anchor episode | 公告钉定父块 + 存在挑战窗口 + **任何批次被接受即取消**的两步恢复通道（§6.4，r12-1；r14-2 修正本表滞后的旧定义） |
