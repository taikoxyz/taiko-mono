# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范,草案 v1.35）
...
5. 块内交易执行合法（正常的状态转换验证）；锚定交易（anchor tx）满足新鲜度规则（§8）；
   **且 L1→L2 入站消息按 §8 的确定性最大前缀规则消费到 `min(C_anchor, 可消费数)`——少
   消费即块不合法（评审 r36，Codex line 1031：与规则 4 强制队列对称，杜绝"用新鲜 anchor
   却零处理入站消息"的无限期审查）**。
...
- **仅强制块（forced-only block）——全卡特尔下的审查底线，现为 §4.2 档 (ii) 的直接
  ...
  - **合法性谓词 `P_forced`（精确、电路可验，取代 §5.1 旧"三条件"引用；评审 r36,Codex
    ...
    5. **消息消费**：按 §8 最大前缀消费 `min(C_anchor, 可消费数)` 条 L1→L2 消息。
