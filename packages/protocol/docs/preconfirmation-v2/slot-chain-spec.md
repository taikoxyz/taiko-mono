# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范，草案 v1.24）
...
- **仅强制块同规 + 续接 episode（评审 r18-3——避免每批次重付一个重锚窗口）**：
  ...
  构建者回归落一个真实批次，即触发取消 (a)/(b) 或被记为一次重钉（上文取消规则），
...
- **episode 状态机（评审 r22-建议——把散落的规则收成一张转移表，降低实现歧义）**：
  ...
  | Executed（仅强制） | `Δ_cont` 内延伸本 episode 仅强制头的下一批 | 免公告免窗口直接落地；刷新 `Δ_cont`（续接，r18-3） |
