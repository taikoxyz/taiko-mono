# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范,草案 v1.39）
...
- **生效还须挂在【批次接受时点】上,堵住历史 slot 回填（评审 r39,独立审核第 2 轮发现 5）**：
  ...
  继续出合法块（还能被用于陈旧尖端 grief,§6.4/独立审核发现 5）。补第二道闸,判据用**不可
  伪造的接受时点**：**罚没在 L1 生效（记入合约）后,凡"接受它的 L1 块 ≥ 生效 L1 块 + `δ_land`
  （最坏落地滞后,§12）"的批次,一律不得包含该 signer 的任何块——无论 `header.slot` 填多少**。
  祖父化正当情形:该 signer 在被罚没前**已合法签**的块本会在 `δ_land` 内落地,落进宽限窗内的
  批次仍有效;超过 `δ_land` 才出现的该 signer 块必是生效后回填,拒绝。由此回填被封死在 `δ_land`
  内、不再无限。（`δ_land` 的精确取值 = 最坏证明+落地滞后,并入 §12。）
...
5. 块内交易执行合法（正常的状态转换验证）；锚定交易（anchor tx）满足新鲜度规则（§8）
   **且满足 §8 的因果序不变量 `anchor.L1_timestamp ≤ L2 时间戳(slot)`（评审 r37，Codex
   line 598：禁止"旧 slot 配新 anchor"破坏 L1→L2 因果序）**；**且 L1→L2 入站消息按 §8 的
   确定性最大前缀规则消费到 `min(C_anchor, 可消费数)`——少消费即块不合法（评审 r36，Codex
   line 1031：与规则 4 强制队列对称，杜绝"用新鲜 anchor 却零处理入站消息"的无限期审查）**。
...
- **最优链的全序（best-chain total order）——分叉选择、落地义务、挑战三处共用（评审 r39,
  所有者决定引入挑战罚没后重写）**。**本条取代 v1.35 的"恢复优先／冻结死尾让位"规则**——
  独立审核（第 1/2 轮）指出那条会**命令诚实落地者丢弃一条完整可见、可落地的健康长尾**（例
  1536 块仅因落后 65 slot 被判"冻结"即被舍弃),这是错的:"不可 P2P 扩展"不等于"应被丢弃"。
  给定分叉点 `F`,在延伸 `F` 的候选链中比较优劣,**依次**：
  1. **lane 优先**：含 ≥1 个自由裁量（非仅强制）块的链 **优于** 纯仅强制块链（内容 lane >
     仅强制 lane,不论块数）。理由:仅强制块只是全卡特尔审查底线,有真实内容时不应顶替内容
     尾巴;且内容块本负强制前缀义务（§7）,内容 lane 优先不牺牲审查抵抗。**这同时消解 r38 的
     "仅强制 vs 排班同 slot 同父平局"**（排班普通块含内容 → 内容 lane 直接胜,无需比哈希;
     也堵住 v1.37 同-lane 续接让任意密钥用更重强制链盖过内容尾巴——独立审核第 2 轮发现 3）。
...
- **挑战（permissionless、L1 机械判定）**：一批 `B` 被接受后开一个**问责窗口 `W_chal`**
  （初始建议 1–2 epoch,§12）。窗口内任何人可提交一条**竞争链 `C`** 主张"`B` 落地时,存在
  一条已沉淀、比 `B` 所落更优的链,`B` 却跳过了它"。**L1 只做纯验签检查（同 §4.3 双签罚没
  手法,不重跑 ZK 有效性证明）**：
  1. **`C` 是合法签名链**：`C` 每个块的签名有效且签名者 = `lookahead(slot)`（排班表 L1 可
     重算）,以 `parent_hash` 串联,缺口满足 §4.2 两档规则,仅强制块满足 `P_forced`——即 `C`
     是真实构建者/合法密钥签出的链,**举报人无法伪造**（伪造需要盗排班构建者的签名）;
  2. **`C` 自 ≤ `B` 的分叉点起、按 §5.2 全序严格优于 `B` 实际所落的链**；
  3. **`C` 已沉淀**：`C` 的 tip slot ≤ `B` 落地所在 L1 块时间对应 L2_slot − `Δ_prop`（`C`
     在 `B` 落地前即已沉淀,故落地者**够得着**、跳过它有责——这是"本该发却没发"的可机械判定
     形式,即所有者所说的"够得着"判据）。
  三条全过 → **没收落地者的落地保证金 `L_land`**（≥80% 销毁、余付举报人,沿用销毁主导原则
  防自罚套利）,并记该落地者一次落地过错。**举报人须同押保证金 `L_chal`**：若三条任一不过
  （`C` 非法/不更优/未沉淀）,没收举报人保证金——防滥告。
...
- **它如何堵住独立审核的三条严重发现**：
  ...
  - **任意密钥更重强制链盖内容尾巴（发现 3-二）**：`C` = 那条内容尾巴,按 §5.2 lane 优先严格
    优于纯仅强制链,举证即罚。
- **明示残留（诚实标注）**：(a) 挑战是**经济威慑,非安全保证**——只在 `L_land` > 跳过收益、
  且**有人愿意举证**（举证有赏金、是有偿无许可工作,但极端 gas/拥堵下可能不经济,同 §6.3
  兜底可行性,§12/§1）时生效;卡特尔可吞下罚金硬跳,属模型外。(b) **诚实但被 eclipse 的落地者
  可能被误罚**：若一条已沉淀更优链因 P2P 致盲没到落地者、却到了举报人,落地者会被判有责——
  这是 §5.2 已列的"落地者视图完整"依赖,现在带罚金后果;`Δ_prop` 取够大（沉淀 = 充分传播）
  压低误判,残留归 §12 P2P/eclipse 抵抗。(c) 挑战**不改最终性**,故被跳过预确认仍是"延迟不是
  找回"。这三条都是 §1 无 DA 模型下的明码接受项,换来的是"跳过好链可罚"这一硬约束。
...
- **堵住"自擦 lag + 落更差链"的零罚重置（评审 r39,独立审核第 2 轮发现 3-一）**：仅按
  ...
  (a) **落更差链本身可罚（主堵点,§5.6）**：它每次盖过的那条健康内容尾巴,是一条已沉淀、
     ...
     越阈值无关;"零罚可反复"不成立。
  (b) **lag 债按接受前状态结算,不容整数骑线（评审 r39）**：迟到罚金/迟到计数的判定改为
     **`lag ≥ Δ_lag`（含等于）** 且按**接受前瞬时 `lag` 相对 `Δ_lag` 的超出量**定价（骑在
     `Δ_lag` 整数点不再免罚）;`m_agg'` 计数仍按 `G_strike` 限速（防误伤,r13-1）。由此
     "掐点到 `= Δ_lag`"不再是免罚点。两条合力:自擦 lag 的循环要么落更优真尾巴（无害）、
...
- **anchor 新鲜度对恢复块的约束——"永不过期"须加限定（评审 r38,Codex P1）**：档 (ii)
  ...
  **自由可设的参数**,设置器不变量要求 **`D_anchor_max ≥ 证明时延 + 最坏可容忍落地延迟
  （含临时 L1 拥堵/审查余量）`**（§12 新增),于是恢复块在正常 L1 活性下**不会**过期——旧
  设计输在窗口结构性太小、非机制本身。若 L1 对 `R` 的落地交易持续审查超过 `D_anchor_max`,
  `R` 以新鲜 anchor 重建重证（不丢排序、建在同一落地头上,重证成本相对停摆很小）——而
  "L1 对某交易持续审查超过 `D_anchor_max`"属 L1 活性退化,§1 假设 L1 安全且活时不发生。
...
- **仅强制块（forced-only block）——全卡特尔下的审查底线，现为 §4.2 档 (ii) 的直接
  ...
  - **合法性谓词 `P_forced`（精确、电路可验，取代 §5.1 旧"三条件"引用；评审 r36,Codex
    ...
    3. **`slot` 合法**：`parent.slot < slot` 且 `slot ≤ L2_slot(落地所在 L1 块时间戳)`
       ...
       验证**形式,不依赖任何在册身份;
    4. **内容仅强制**：块体 = anchor（§8）+ 按 §7 队列顺序、至 `C_force`/`C_bridge` 上限的
       到期强制前缀,**别无自由裁量交易**;且该前缀**非空**（存在到期未消强制条目）——空
       前缀的"仅强制块"无意义、不被接受,杜绝空块刷 slot;
    5. **消息消费 + 因果序**：按 §8 最大前缀消费 `min(C_anchor, 可消费数)` 条 L1→L2
       消息;且满足 §8 因果序不变量 `anchor.L1_timestamp ≤ L2 时间戳(slot)`（r37,Codex
       line 598 明确要求仅强制块同受此约束）。
...
3. **落地参数**：`Δ_lag`、`m_agg`、`δ_slash`、`H_force`、兜底报酬的指数成本上限；
   ...
   **挑战参数（v1.39,§5.6）**：`W_chal`（问责窗口,初始建议 1–2 epoch）、`L_land`（落地保证金,
   须 ≥ 跳过更优链的最大收益）、`L_chal`（举报人押金,防滥告）、`Δ_prop`（沉淀深度,同上,
   兼作 §5.6 "够得着"判据）。**罚没回填窗口 `δ_land`**（§4.3,= 最坏证明+落地滞后,祖父化正当
   late-landing、封死历史 slot 回填）。**共享 gas 份额（§8）**：`G_anchor`、`C_force`/`C_bridge`
   gas 份额、L1→L2 消息 gas 份额,满足 `三者之和 ≤ block_gas_limit` 的设置器不变量 + 单条准入
   上限 + 水位线。
...
| 挑战罚没 | challenge / slashing | 举证落地者跳过更优沉淀链 → 没收 `L_land`、赏举报人；**不改最终性、不回滚**（§5.6，r39） |
| 传播沉淀 | propagation-settled | 一条链 tip 至少 `Δ_prop` 老、已传遍 P2P，落地者"够得着"（§5.6/§5.2） |
