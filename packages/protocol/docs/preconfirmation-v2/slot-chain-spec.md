...
> 出现时在括号内标注英文。
> **草案 v1.1，2026-08-25**：v1 初稿经一轮内部对抗评审
> （[`review-round-1.md`](review-round-1.md)）修订——修复了五个高严重度发现：
> 落地义务改为 L1 可观测的滞后量判定 + 兜底落地即罪证（§6.3）、仅强制块加双侧
> 条件防抢跑（§7）、罚没生效时点规则（§4.3）、`L_eq` 改按窗口累积敞口定价（§4.1）、
> 等高分叉的收敛规范与合谋重组的诚实定界（§5.2/§5.4）；并补上签头不发体
> （withhold-body）的定性（§5.5）与签名域唯一性不变量（§4.2）。**草案 v1.2** 应用
> 第 2 轮评审（[`review-round-2.md`](review-round-2.md)，DeepSeek 机器人）：修复其
> 1 个严重发现——兜底落地的可用性前提被显式化（构建者只能在可执行的父块上构建，
> 扣体后缀被自动绕开；Byzantine 聚合者有界性的确切条件写明——§5.5/§6.3/§9），并
> 处理仅强制块的停摆期残留竞速披露（§7）、`L_eq` 敞口视界修正（§4.1）、lag 的规范
> 定义（§6.3）等次级发现。机制骨架完整，参数为初始建议值，§12 列出待定项。
