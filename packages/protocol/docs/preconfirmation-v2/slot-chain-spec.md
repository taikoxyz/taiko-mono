# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范，草案 v1.11）
...
- **未落地部分（P2P 尾巴）的协调规范（评审 r11-2 重写——v1.10 及之前是"接最高
  ...
  跳过叉只能靠抢先落地取胜（敞口定界见 §5.4）。这仍是链下协调规范、不是电路
  规则，与 r1-1 同款定位：它剥夺的是旁观者（聚合者、落地竞争者）与深跳过者的
  免费挑选空间，磨哈希不给双签者新增能力。
...
- **剩下的：相邻构建者的跳过（skip）——且它是可获利的（评审 r5-3 更正）。** builder
  ...
  **深跳过（父块指到 `s − G_max` 附近，评审 r11-2 补明）**一次签名可波及至多
  `G_max − 1` 个未落地块——但 §5.2 修订后的协调规范使诚实网络不跟随深跳过叉，
  它必须**抢在诚实批次之前把自己落上 L1** 才能生效（竞速窗口 ≈ 正常落地节奏，
  直接重打包收益仍以自己一个块的容量为上限），且每次尝试消耗攻击者一个排班
  slot；两种跳过的**频率都由不诚实构建者占比约束——§1 信任模型的"诚实多数"正是
  在这里承重**（不是保守冗余）；
...
- **推论——预确认语义（写给用户文档的版本，评审 r1-1/r1-6 后的诚实版）：** 一个
  ...
  2. 某个后继构建者**跳过**它——对跳过者可获利（费用/MEV 转移）且不可罚没
     ...
     `G_max − 1` 个未落地块但须赢得落地竞速（r11-2，§5.2）；频率由诚实多数假设
     约束；纯包含类承诺通常经重打包兑现（§5.4 上文）；
...
- **重锚块（re-anchor block）**：`parent` = 执行时刻已落地 L1 链头、父距 > `G_max`
  的块。其余合法性规则（签名者 = `lookahead(slot)`、强制前缀、状态转换、不得落地
  未来 slot）照常适用。重锚块**只能**按本流程落地：电路豁免其缺口检查（§5.1
  规则 2），L1 入口验证流程状态。
- **公告（announce）**：`lag > Δ_lag`（即兜底窗口开启态，§6.3）时，任何地址可发布
  重锚公告，附小额押金 `B_ra`。同一时间至多一个活跃公告。
- **基础窗口 `Δ_ra`**（初始 16 个 L1 slot ≈ 3.2 分钟）：公告后的强制等待期，也给
  出块者留出预生成证明的时间。
- **存在挑战（liveness challenge）**：窗口内任何人可提交**一个**签名块头作为"尾巴
  可能还活着"的廉价证据（一笔普通交易，无需证明），L1 合约验证：（i）签名有效且
  签名者是注册表在册构建者（注册表在 L1，直查）；（ii）`落地头.slot < header.slot
  ≤ 当前 L2_slot`。首个有效挑战把窗口一次性延长至 `Δ_ra_ext`（初始 96 个 L1 slot
  ≈ 19.2 分钟，≥ 兜底者自开窗起跑完一轮证明的时间）；**每个公告期至多延长一次**——
  一个恶意在册构建者伪造挑战（在自己的 slot 上签一个假头即可，无从判伪）最多买到
  一次 `Δ_ra_ext − Δ_ra` 的拖延，有界。不检验排班匹配是有意的：排班检验挡不住
  在册的内部人，却要求把窗口种子搬上 L1 合约，成本换不来安全。
- **取消**：窗口内任一时刻 `lag ≤ Δ_lag`（有落地把链追平——尾巴用落地事实自证
  存活），公告即作废，押金付给首个挑战者（无挑战则退还公告者）。这与 §6.3 计次的
  "存在可落内容由落地事实自证"是同一判定手法；重复恶意公告每次损失 `B_ra`，
  自限。
- **执行**：窗口期满且 `lag` 仍 > `Δ_lag`，重锚块可落地，`parent` 取**执行时刻**的
  落地头（窗口内被部分落地的前缀因此不会被孤立），押金退还公告者。
...
- **两类情形的走线核算**：
  - **真实大缺口**（> `G_max` 连续缺勤且无人补签）：故障前尾巴会被落干净，无人能
    出示满足条件（ii）的块头 ⇒ 无挑战。恢复 ≈ `lag` 爬到 `Δ_lag`（自故障起
    ≈ 26 分钟，按 r10-2 校准值）+ `Δ_ra` ≈ **29 分钟**（对照同口径下走
    `Δ_stall` 的 v1.8 途径 ≈ 32 分钟；遭恶意假挑战再加一次窗口延长
    ≈ +16 分钟，仍有界）。
  - **r9-1 攻击情形**（尾巴存活、聚合者扣落地）：公告即被挑战（任何诚实构建者出示
    自己的最新块头即可），窗口延至 `Δ_ra_ext`；而兜底窗口自 `Δ_lag` 起已开、诚实
    兜底证明已在跑，落地即取消公告。攻击要成功，必须对全部兜底落地交易维持整个
    `Δ_ra_ext` 的 L1 层持续审查——落回 §6.3 已定价的"最贵审查类别"，不再是偏向
    攻击方的裸竞速。
- **仅强制块同规**：§7 仅强制块的落地同样必须经过本流程的公告与窗口（其自身
  `lag > Δ_stall` 等三条件不变，叠加本流程；`Δ_stall > Δ_lag` 保证公告条件先于
  其开关满足）。这顺带把 r2-2 的停摆期残留竞速收窄，见 §7。
...
| 连续缺口 > `G_max` 且缺勤者不回归（灾难级） | 分钟级无新块 | 重锚流程（§6.4）：公告 + 窗口期满后从落地头重锚 | ≈ `Δ_lag + Δ_ra`（自故障起 ≈ 29 分钟；遭恶意假挑战 +16 分钟，有界） |
...
3. **落地参数**：`Δ_lag`、`m_agg`、`δ_slash`、`H_force`、兜底报酬的指数成本上限；
   重锚流程参数（`Δ_ra`、`Δ_ra_ext`、`B_ra`，r9-1）；
...
| 存在挑战 | liveness challenge | 窗口内出示一个在册构建者签名块头，证明尾巴可能存活并延长窗口 |
