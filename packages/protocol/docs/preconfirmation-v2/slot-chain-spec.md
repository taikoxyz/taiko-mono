# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范,草案 v1.31）
...
- 构建者把（块头 + 签名 + 块体）在 P2P 广播。**这个签名块本身就是预确认**：给用户的
  ...
  是公开可转发的证据（§5.5 会用到）。
- **父块规则——两档（评审 r6-1 → r32 重做,方向 B 恢复重设计）**：`parent.slot < s`，
  且父块满足以下**其一**：
  - **(i) 父块是未落地（P2P）块**：`s − parent.slot ≤ G_max`（缺口上限,初始 64 slot）。
    这一档防深重组:`G_max` 封顶单块回退深度,恶意构建者要孤立更深的**未落地**尾巴
    需接力稀疏分支(见下残留)。
  - **(ii) 父块是【最终的 L1 已落地块】**（final,达 `F_l1` L1 最终性深度）：**无缺口
    上限**（`s − parent.slot` 任意）。**为什么可以无上限**:已落地块按 §5.2 是最终
    的,建在它上面**不可能 reorg 任何已落地块**;于是它天然安全,不需要 `G_max`。这一
    档是**恢复的唯一机制**（取代旧的整套重锚 episode,见 §6.4）。确定性由**落地规则**
    兜底:批次只能延伸**当前 canonical 落地尖端**（§7）,一个建在旧尖端上的恢复块若
    尖端已被推进则**良性搁浅、重建即可**（无 episode/押金可乱;真停摆时尖端不动,恢复
    块必落）。`F_l1` 使"final 落地块"这一谓词在浅层 L1 重组下稳定（挂钩 §12 L1 重组）。
...
2. `parent.slot < slot` 且**父块满足 §4.2 的两档规则之一**（r32）：(i) 父块是未落地
   块时 `slot − parent.slot ≤ G_max`；(ii) 父块是最终 L1 已落地块时无缺口上限。
   `parent_hash` 指向该合法父块（档 ii 指向一个 final 落地块,其"是当前 canonical
   尖端"由 §7 落地规则在落地时校验,非落地时刻校验）；
...
- **剩下的：相邻构建者的跳过（skip）——且它是可获利的（评审 r5-3 更正）。** builder
  ...
  **深跳过 / 稀疏私有分支（评审 r11-2 补明、r20-1 更正深度）**：一个恶意构建者
  ...
  - **档 (ii) 单块经落地头（v1.31 新,`G_max_landed = ∞`）**：一个恶意构建者以 final
    ...
    但**深度上界不变**（仍 ≤ 未落地尾巴 ≈ `Δ_lag`,超出部分已落地最终）,把满孤立分支的
...
- **仅强制块（forced-only block）——全卡特尔下的审查底线，现为 §4.2 档 (ii) 的直接
  ...
  - **审查时间界**：一条强制条目在"有人出一个含它的块"后 `backlog / C_force` 个块内被
    包含；含它的块可以是**普通块**（强制前缀义务,§7）或**仅强制块**（全卡特尔下）。仅
...
- **anchor 的逐块 L1 推进上限 `C_anchor`（评审 r32,方向 B 恢复重设计的设计者自审
  第 5 轮引入）**：单个 anchor 相对上一个 anchor 前进的 L1 区块范围（以及其间待摄入的
  L1→L2 消息量）**每块封顶 `C_anchor`**。**为什么需要**：§4.2 档 (ii) 恢复块可从数天前
  的落地头一跳接回墙钟,其 anchor 会想一次跳过数天 L1、一次摄入海量 L1→L2 消息、超单
  块 gas。加上限后:**L2 排序一跳恢复,但 L1 视图/桥接消息按每块 `C_anchor` 逐块追平**,
  完整同步用 `L1积压 / C_anchor` 个块（恢复后 1 块/秒,很快）。这与 §7 强制条目每块
  `C_force` 的积压分摊是同一手法。`C_anchor` 为 §12 校准项。
...
| 连续缺勤 ≥ `G_max` 个 slot，乃至数小时/数天（灾难级停摆） | 停摆期无新块 | **§4.2 档 (ii)**：任一回归的排班构建者（或仅强制任意密钥）以 final 落地头为父出一块,**排序一跳恢复**（§6.4,v1.31 方向 B 重设计——无 episode） | **排序 ≈ 一轮证明 + 落地（≈10–15 分钟,与停摆时长无关,一跳）**；L1-sync/强制积压按每块 `C_anchor`/`C_force` 追平；需 ≥1 排班构建者回归（r12-2 概率界）或仅强制底线 |
...
| `C_anchor` anchor 逐块推进上限 | 待定 | L1 slot/消息 | §8 恢复分摊，v1.31 |
