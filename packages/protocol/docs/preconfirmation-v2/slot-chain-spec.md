# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范,草案 v1.30）
...
- 父块规则（评审 r6-1 后收紧）：`parent.slot < s` 且 **`s − parent.slot ≤ G_max`**
  ...
  **少数派联盟**，只要其排班 slot 彼此间隔 ≤ `G_max`，就能私下把每个自己的块接在
  ...
  诚实策略：**接在 §5.2 的分叉选择规范给出的链头上**（评审 r31-1 更正——此前此处
  写"接最高 slot 合法链头",与 §5.2 r11-2 用"自分叉点起合法块数最多"取代该旧规则
  相矛盾,照旧写会让实现重新引入 r11-2 修掉的深跳过被自动跟随漏洞;以 §5.2 为准）。
...
- **包含规则（电路强制，逐块，带容量上限）**：条目在提交后 `F_delay` 到期。任何
  ...
  **块首前缀不足上限却遗漏了到期条目的块不合法**（§5.1 规则 4 的精确形式）——审查
  ...
  (2) 证明的公共输入是队列快照 =（两队列根、两尾游标）。**快照绑定改为逐块、
