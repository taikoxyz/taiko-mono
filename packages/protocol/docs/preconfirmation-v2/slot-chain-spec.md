# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范,草案 v1.28）
...
2. `parent.slot < slot` 且 `slot − parent.slot ≤ G_max`（§4.2 的缺口上限，r6-1；
   ...
   且其 `slot` 落在**接受时刻墙钟的 `G_max` 窗口内**（`≥ L2_slot(接受 L1 块) −
   G_max + 1`，新鲜度地板 r23-1/r24-1），r9-1/r9-3），`parent_hash` 指向一个合法
   块（重锚块则指向 §6.4 **公告钉定的父块**——它经重钉后在执行时刻恰等于当前已
   落地 L1 链头，二者一致，r22-2）；
...
> **⚠️ DRAFT——本节（及 §7 仅强制路径的重锚相关部分）冻结待形式化（所有者决定
> 2026-08-25）。** 经二十余轮对抗评审,恢复子系统暴露出一族同根问题:**证明时延
> （10–15 分钟）与 L1 落地延迟 >> slot / `G_max` 时序窗口**,使任何以墙钟 slot 为
> 参照的新鲜度/时序规则(重锚新鲜度地板、`Δ_cont` 续接窗口)本质脆弱;逐点补丁只
> 把张力在 §6.4/§7 各处搬移而不收敛(r23→r24→r25→r28→r29-P1 同根)。据此**停止对
> 本子系统的逐点补丁**,转人工设计复盘 + 状态机模型检查。**待复盘定夺的开放项**:
> (1) 新鲜度/时序模型选型(有界落地延迟假设 / 滚动预证候选集 / 非 slot 锚定重启);
> (2) 押金结算的确定性函数 `f(state, event, 有序挑战 id)`——含多 `H_ch` 命中的唯一
> 赢家/平分规则、`Executed` 后正常批次的终态转移(避免二次结算),消解 r29-P2 的
> 三处矛盾;(3) 续接授权在一轮证明期内的存活方式。**在复盘完成前,本节以下机制
> 描述为暂定(provisional),其数值界(如"自故障起 36–41 分钟""一个窗口清空积压")
> 不作为可依赖的保证。** 本节其余非重锚不变量(§5.2 最终性、§5.4 深跳过残留定性、
> §6.3 兜底计次)不在冻结范围。
>
...
- **取消——按"被保护分支实际落地"判定，落地推进头本身不取消（评审 r12-1/r12-5/
  ...
  - 存在挑战提交的签名块头 `H_ch`（§上，slot > `P`.slot）即"待保护尾巴上的一个
    具体区块"的 L1 承诺。**多挑战的登记与结算（评审 r24-3，DeepSeek 第 5 轮 W4——
    此前"至多延一次窗口"与"可累计多个 `H_ch`"表述冲突、多挑战结算未定义）**：**窗口
    延长只发生一次**（首个有效挑战触发，见上）；但其后仍可**登记**更多 `H_ch`
    （每个各付 `B_ch`），作用是**加宽分支承诺**（更多块头计入取消条件 (a)）。取消
    由 (a) 触发时，**其落地链包含的那个 `H_ch` 的提交者**独得 `B_ra` 分成 + `B_ch`
    退还；同一 episode 其余已登记 `H_ch`（未被该落地链包含）各自按下方"孤立/未兑现"
    规则结算。每个 `H_ch` 的押金独立结算，互不影响。
  - **取消当且仅当**：(a) 某个被接受批次的落地链**包含某个已登记的 `H_ch`**
    （被证明存活的那条尾巴真的落上了 L1 ⇒ 恢复已由正常落地达成，无需重锚），或
    (b) `lag ≤ Δ_lag` **且当前无任何未兑现的已登记 `H_ch`**（评审 r24-2，DeepSeek
    第 5 轮 W1——此前 (b) 只看 lag，但一条**排除** `H_ch` 的稀疏冲突叉可以把落地头
    推到接近墙钟、令 `lag ≤ Δ_lag` 成立却从未落地被保护尾巴，等于绕过 r21-1 的分支
    承诺；加"无未兑现 `H_ch`"这一合取项后，只要有尾巴被承诺待保护，就只能靠 (a)
    真落地它来取消，(b) 仅在无待保护尾巴的真实恢复场景兜底）。
  - **其他任何推进落地头的批次**（含孤立掉 `H_ch` 的冲突 deep-skip 叉）**不取消**：
    L1 入口把钉定父块**重钉**到新落地头、**保留已流逝的窗口计时**（攻击者无法靠
    滴灌冲突叉重置时钟或刷新首证优势），记一次 fork 事件；被孤立的 `H_ch` 作废，
    挑战者可在窗口内对新头重新挑战。**术语澄清（评审 r22-1）**：钉定父块本身**已
    落地、按 §5.2 是最终的，任何批次都不能把它 reorg 出正统**；冲突叉是**延伸**钉定
    父块（§6.1 要求批次延伸当前落地头，而钉定父块经重钉恰是该头）、把落地头推进到
    一条孤立掉**未落地**诚实尾巴的新分支——被孤立的是 P2P 尾巴，不是已落地的钉定
    父块。"重钉"即把钉定指针从旧落地头前移到这个新落地头（其后代），不涉及任何对
    已落地历史的回滚。（v1.20 及之前"把钉定父块 reorg 出正统"的措辞据此更正。）
  ...
  （明码）。押金处置（衔接 r12-3 的按结局结算；r27-2 补明 `B_ra` 在 (a)/(b) 的
  不同去向，DeepSeek 第 6 轮 W3）：
  - **因 (a) 取消**（挑战的尾巴真落地，说明公告"链死了"的判断在该叉上过早）⇒
    命中 `H_ch` 的挑战者收 `B_ch` 退还 + **`B_ra` 分成**（`B_ra` 从公告者转出）；
  - **因 (b) 取消**（`lag ≤ Δ_lag` 且无未兑现挑战——真实故障由正常兜底自愈，公告者
    此前在 `lag > Δ_lag` 下发起公告本身**无过错**）⇒ **`B_ra` 全额退还公告者**
    （r27-2 修正：此前笼统写"付落地者/挑战者"会惩罚一个尽责的公告者、抑制正当
    公告——DeepSeek W3）；
  - **走到执行**（无 (a)/(b)）⇒ `B_ra` 退还公告者，未命中的已登记挑战各罚没 `B_ch`；
  - **被冲突叉孤立的 `H_ch`**（既未兑现也非虚假）⇒ 退还 `B_ch`、不奖不罚——攻击者
    已为孤立它付出一个排班 slot，不再对被 reorg 的诚实挑战者二次追罚。
- **执行 + 新鲜度地板绑定到"接受时刻"（评审 r23-1 引入地板，r24-1 修正参照点——
  终结这一族）**：窗口期满（含延长）且期间**未触发取消 (a)/(b)**，L1 入口核对：
  (1) 钉定父块（经重钉后的当前值）仍为当前落地头；(2) **重锚块的 `slot` 落在
  `[L2_slot(接受时 L1 块) − G_max + 1, L2_slot(接受时 L1 块)]`** ——即**距接受时刻
  墙钟不超过 `G_max`**，二者皆满足方可落地，押金退还公告者。**为什么参照点必须是
  接受时刻、不是窗口期满（r24-1 修正 r23-1）**：r23-1 把地板钉在窗口期满的
  `s_base`，但证明约 10–15 分钟且执行无截止——到接受时 `s_base` 已陈旧数百 slot，
  单块重锚落地后头仍落后墙钟数百 slot、当轮构建者仍接不上（父距 > `G_max`），
  攻击者照样用预签稀疏链反复赢公告拖长恢复。绑定到接受时刻则**按构造**保证落地瞬间
  头距墙钟 ≤ `G_max`，当轮构建者立即可续，一次重锚真正追平。**可行性（证明延迟
  远大于 `G_max` 也成立）**：落地者不为窗口期满 slot 签，而为一个**近未来 slot
  `s_ra`** 签——`s_ra` 由排班表在 768 slot 前公布、其构建者已知（诚实多数下即诚实
  者，r12-2 概率界），对 `s_ra` 预签预证，恰在墙钟到达 `s_ra` 时落地；此刻
  `s_ra ≤ 接受墙钟`（满足 r5-2 不落未来 slot）且 `≥ 接受墙钟 − G_max`（满足地板）。
  仅强制路径（§7，任意密钥签）同样把首块 slot 绑到接受时刻的 `G_max` 窗口内——
  这**统一并取代** r17-1/r23-1 的窗口期满 `s_base` 说法，且顺带修掉 DeepSeek 第 5
  轮 W2 指出的"仅强制块落在陈旧 slot、`Δ_stall` 与 `Δ_lag+Δ_ra` 时序错位"问题
  （落点随接受时刻走，不再与开阀阈值时序打架）。执行仍无截止时刻——晚落只是把
  `s_ra` 取得更靠后，地板始终相对**当次**接受时刻计算；重钉只前移钉定父块指针、
  不重置窗口计时。
- **签名者的可得性，明说（评审 r12-2：公告期满不自动产生出块权）**：重锚块仍需
  其 slot 的排班构建者签名。窗口期满后，每个新 slot 的排班构建者都可为**当前钉定
  父块**签一个当轮重锚块（**在两次重钉之间**父块不变、候选块只差 slot 与 coinbase；
  **一旦发生重钉，钉定父块改变，旧父块上预生成的证明全部作废，须对新钉定父块重新
  生成证明**——评审 r22-7 明说，此前"父块不随时间变化"仅在无重钉时成立），因此
  执行时点是**信任模型内的概率界**。**`s_ra` 须留足证明前置期（评审 r27-4，DeepSeek
  第 6 轮 W1）**：落地者选的 `s_ra` 不能只看"下一个在线诚实构建者"，还须
  `s_ra ≥ 签名起算时刻 + 证明时延（10–15 分钟）`，否则预签 + 出证明赶不及在接受
  时刻的 `G_max` 新鲜度窗口内落地；排班表提前 768 slot（≈12.8 分钟）公布恰好覆盖
  一轮证明前置期。**期望等待 = 证明前置期 + 其后诚实构建者出现的 `≈ 1/p` slot 之
  和**（诚实构建者占比 `p`；诚实多数下后一项 ≈ 秒级，前一项即一轮证明），§9 的
  "自故障起 36–41 分钟"按此理解为**已含**一轮证明前置期的估计。**全员卡特尔下
  普通重锚无人可签——属模型外情形，唯一确定性出口是仅强制块**（任意密钥可签，走
  同一公告流程）。§9 表按此标注，不把普通重锚写成无条件确定性恢复。
...
- **恢复时间的四分法（评审 r12-5——"强制路径恢复"与"系统恢复"是不同的界，分列
  ...
  3. **lag 追平墙钟**：由重锚跳跃在落地瞬间一次完成——两条重锚路径的首块 `slot`
     都落在**接受时刻墙钟的 `G_max` 窗口内**（r24-1，取代 r17-1 的窗口期满基准；
     episode 之间的后续/续接块仍 `parent.slot + 1`）。全员卡特尔期间，若续接
     授权（r25-1）失效后 lag 重新积累、兜底窗口周期性重开，是**预期行为**，不是
     缺陷；
...
- **仅强制块同规 + 续接 episode（评审 r18-3——避免每批次重付一个重锚窗口）**：
  ...
  **续接批次豁免 §7 的三个开阀条件——持久化的 episode 级授权（评审 r25-1，Codex
  第 15 轮：r24-1 让首个仅强制块把头追到墙钟、`lag` 掉到 ≤ `G_max`，而 §7 要求仅
  强制块满足 `lag > Δ_stall`（≈32 分钟）；`Δ_cont` 只有几个 L1 slot，`lag` 要再
  超 `Δ_stall` 得等约 32 分钟、届时 episode 早关——于是每个后续仅强制批次都得再
  等一次完整停摆 + 重锚窗口，与"一个窗口清空积压"矛盾，且让卡特尔能对每批施加一次
  停摆）**：**只有首个仅强制批次**须满足 §7 三条件（存在逾期条目 + `lag > Δ_stall`
  + 经公告窗口）；进入续接态后，`Δ_cont` 内**延伸本 episode 仅强制头**的续接批次
  **不再各自复核 `lag > Δ_stall` 与逾期/公告条件**——它们riding 本 episode 已经
  建立的仅强制授权（该授权是被证明的链状态的一部分），只需：仍是仅强制内容、
  `slot = parent.slot + 1`、延伸本 episode 仅强制头、且队列中仍有到期条目。头已在
  墙钟附近，续接批次以 `parent.slot + 1` 逐块把队列以每块 `C_force` 排空，`lag`
  始终保持低位（这正是"不该再要求 `lag > Δ_stall`"的原因）。由此仅强制积压清空的
  总代价 = 一个完整窗口 + `积压/C_force ×（证明+落地）`，与 §7 的复合界一致
  （§6.4 四分法第 1、2 项与 §7 复合界按此，只在**首批**计一次窗口）。安全性：
  授权严格绑定"延伸本 episode 仅强制头 + `Δ_cont` 未空过 + 队列仍有到期条目"三者，
  队列排空或 `Δ_cont` 空过即失效，无法被延用到正常出块；全卡特尔下无诚实内容竞争，
  续接只承载仅强制内容；一旦有诚实构建者回归落一个真实批次，即触发取消 (a)/(b) 或
  被记为一次重钉（上文取消规则），自然结束仅强制态、回到正常出块。这也把 r2-2 的
  停摆期残留竞速收窄，见 §7。
- **episode 状态机（评审 r22-建议——把散落的规则收成一张转移表，降低实现歧义）**：

  | 当前态 | 事件 | 迁移 / 动作 |
  | --- | --- | --- |
  | （无 episode） | `lag > Δ_lag` 且有人公告（付 `B_ra`） | → **Open**：钉定父块 `P` = 当前落地头；窗口计时起 `Δ_ra` |
  | Open | 首个有效挑战（付 `B_ch`） | 窗口一次性延至 `Δ_ra_ext`；记录 `H_ch`（本 episode 至多延一次） |
  | Open | (a) 被接受批次的落地链含某已登记 `H_ch` | → **Cancelled**：命中 `H_ch` 提交者得 `B_ra` 分成 + `B_ch` 退还，余挑战独立结算（r27-2） |
  | Open | (b) `lag ≤ Δ_lag` **且无未兑现 `H_ch`**（r24-2） | → **Cancelled**：**`B_ra` 全额退还公告者**（无过错，r27-2） |
  | Open | 其他批次推进落地头（含冲突叉） | → Open（**重钉** `P` := 新落地头，窗口计时**不重置**）；被孤立 `H_ch` 退还 `B_ch`；旧证明作废须重证 |
  | Open | 窗口期满、未触发取消 | → **Expired**（等待可执行重锚：证明就绪 + 排班构建者签名；仍受取消 (a)/(b) 与重钉规则约束——晚到的普通批次照常按取消/重钉处理，不特殊对待） |
  | Expired | 排班构建者对 `P` 出重锚块（`slot` 在接受时刻墙钟 `G_max` 窗口内，r24-1）并落地 | → **Executed**：`B_ra` 退公告者；未命中挑战各罚没 `B_ch` |
  | Executed（仅强制） | `Δ_cont` 内延伸本 episode 仅强制头的下一批 | 免公告免窗口、**豁免 §7 三条件**直接落地（riding episode 级仅强制授权，r25-1）；刷新 `Δ_cont`（续接，r18-3） |
  | Executed（仅强制） | `Δ_cont` 内一个**正常（含自由裁量交易）批次**推进落地头 | 按 Open 的取消/重钉规则处理该批次（含 `H_ch` 命中 → 取消，或重钉），**仅强制授权即失效**、回正常出块（r27-3，DeepSeek 第 6 轮 W2 补此前缺失的转移） |
  | Executed（仅强制） | `Δ_cont` 空过 | → （无 episode）：下一批须重新公告 |

  不变量：任一时刻至多一个非终态 episode；`P` 始终 = 某个**已落地**块（最终，
  §5.2 不可回滚）；"重钉"只前移 `P` 指针、不回滚历史。
...
- **全体串谋的底线：仅强制块（forced-only block），双侧条件版**（评审 r1-3 后收紧；
  ...
  **仅强制块的规范格式（r4-3 制定；评审 r7-4/r7-5 修正）**：
  ...
  - **slot 唯一（r7-5：此前允许"填补缺口或追加"且 slot 不唯一——历史缺口在哈希链
    ...
    **首个**仅强制块的 `slot` 落在**接受时刻墙钟的 `G_max` 窗口内**（`≥ L2_slot(接受
    L1 块) − G_max + 1` 且 `≤ L2_slot(接受 L1 块)`，与 §6.4 r24-1 的新鲜度地板同规
    ——r25-2 更正：r17-1 曾写"= 窗口期满执行基准 slot"，但 §6.4/r24-1 已把参照点从
    窗口期满改为**接受时刻**，因证明 10–15 分钟 >> `G_max` 会使窗口期满 slot 到接受
    时通不过新鲜度检查；本格式规则据此对齐，落地者为近未来 `s_ra` 预签预证、落地瞬间
    落入该窗口），同批次后续仅强制块 `slot = parent.slot + 1`；其 anchor 引用 = 批次
    队列快照所在的同一 L1 高度（批次级确定）。落点由接受时刻墙钟确定、非落地者自由
    选择（r7-5 反对的"自由裁量时间跳跃"仍不存在）；同一 (接受高度, 队列快照) 下候选块
    仍**除 `coinbase` 外字节唯一**，gas 竞赛论断成立；首块跳到该 slot 后，到期
    slot ≤ 该 slot 的条目全部进入包含义务，自举完成。（§7 条件 3 的"填补缺口"措辞
    相应作废，以本条为准。）
...
| `G_max` 缺口上限 | 64 | slot | §4.2；深度旋钮非硬帽 r20-1 |
| `δ_slash` 罚没生效延迟 | 64 | slot | §4.3 |
| `Δ_lag` 兜底阈值 | 4 | epoch（≈25.6 min） | `≥ 正常滞后带上界`，§6.3 r10-2 |
| `Δ_stall` 停摆阈值 | 5 | epoch（≈32 min） | `> Δ_lag`，§7 r6-2/r10-2 |
| `G_strike` 计次限速 | 1 | epoch | §6.3 r13-1 |
| `m_agg` / `m_agg'` | 2 / 4 | 次 | 终止阈值，§6.3 |
| `Δ_ra` / `Δ_ra_ext` | 16 / 96 | L1 slot | 重锚基础/延长窗口，§6.4 |
| `B_ra` / `B_ch` | 待定 | ETH | 公告/挑战押金，§6.4 |
| `Δ_cont` 续接时长 | 待定（几个 L1 slot） | L1 slot | §6.4 r18-3 |
