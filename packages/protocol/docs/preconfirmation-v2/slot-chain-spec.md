# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范，草案 v1.3）
...
> 出现时在括号内标注英文。
> **草案 v1.3，2026-08-25**：初稿经三轮对抗评审修订（一轮内部评审、PR 上的
> DeepSeek 与 Codex 机器人各一轮，共 22 项发现全部修复或明码定性；正文中以
> "评审 r1-N / r2-N / r3-N"标注各修复位置，评审原文见 PR 记录）。第 3 轮的三项
> 修复：强制条目数据载体改为 calldata + 链上内容哈希（消除 blob 过期对 §7 逃生
> 路径的破坏——r3-2）、迟到落地计费堵住聚合者骑线策略（§6.3——r3-1）、§5.1 合法性
> 规则补上仅强制块豁免例外（r3-3）。机制骨架完整，参数为初始建议值，§12 列出
> 待定项。
...
- **迟到落地本身也计费——堵住"骑线"策略（评审 r3-1，Codex）**：只按"兜底批次被
  ...
  都对聚合者收取按超出量定价的迟到罚金**（从活性保证金扣），且"窗口被打开"这一
  事件本身计入一个较轻的违约计数，累计 `m_agg'` 次（初始 4 次）同样触发终止。
  于是无论谁落地、窗口开了就对聚合者不免费；被抢跑的兜底者的落地 gas 损失从迟到
  罚金中按预注册意向补偿（细则并入 §12 第 3 项）。
- **终止与换人**：累计 `m_agg` 次计次（初始 2 次）**或 `m_agg'` 次迟到/开窗违约
