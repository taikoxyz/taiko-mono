# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范，草案 v1.16）
...
- **非法条目的准入与弃置消费（评审 r9-2，Codex——P1）**：v1.8 的规则对一条畸形或
  ...
  1. **入队验证（L1 合约执行）**：载荷已改为 calldata 随交易提交（本节首条），合约
     ...
     器不变量：桥接消息的最大 gas ≤ `C_bridge` 单条目份额）。**遗留超额条目的
     确定性泄流**：若参数下调使已入队条目超出所属队列份额（唯一可能的来源），
     该条目在位于队头且 gas ≤ `C_force` 份额时，允许一次性借用当块未用的普通
     配额消费——规则是队列状态的纯函数，电路可验，死锁不可能。桥接队列条目由桥
     合约构造，格式良好由构造保证。
