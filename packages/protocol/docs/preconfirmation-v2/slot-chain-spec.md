# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范，草案 v1.9）
...
- **剩下的：相邻构建者的跳过（skip）——且它是可获利的（评审 r5-3 更正）。** builder
  ...
  流传），归责有实锤；(2) 单次收益上限 = 一个 slot 的费用/MEV 转移，**频率由不诚实
  构建者占比约束——§1 信任模型的"诚实多数"正是在这里承重**（不是保守冗余）；
...
- **推论——预确认语义（写给用户文档的版本，评审 r1-1/r1-6 后的诚实版）：** 一个
  ...
  2. 下一个构建者**跳过**它——对跳过者可获利（费用/MEV 转移）且不可罚没（r5-3）；
     公开可见、签名固定、单 slot 范围，频率由诚实多数假设约束；纯包含类承诺通常在
     下一 slot 经重打包兑现（§5.4 上文）；
...
- **过错与罪证（L1 机械判定）**：**一次"计次"（strike）= 兜底窗口开启期间有一个
  ...
  交易维持 `Δ_lag + G_strike`（≈ 4 epoch ≈ 128 个 L1 slot）的持续审查，是 v15
...
- **两类情形的走线核算**：
  - **真实大缺口**（> `G_max` 连续缺勤且无人补签）：故障前尾巴会被落干净，无人能
    ...
    ≈ 19 分钟）+ `Δ_ra` ≈ **22–23 分钟**（对照 v1.8 的 ≈ `Δ_stall` ≈ 26 分钟；
    遭恶意假挑战再加一次窗口延长 ≈ +16 分钟，仍有界）。
...
- **包含规则（电路强制，逐块，带容量上限）**：条目在提交后 `F_delay` 到期。任何
  ...
  **块首前缀不足上限却遗漏了到期条目的块不合法**（§5.1 规则 4 的精确形式）——审查
  ...
  漏判。**原子状态转换（评审 r7-3 的形式化）**：L1 合约维护 canonical 的**已消费
  游标 `f_consumed`**（与入队尾游标分开）。落地时：(1) 批次声明的起始游标必须等于
  合约当前 `f_consumed`；(2) 证明的公共输入是固定的队列快照 =（队列根、尾游标），
  **取自该批次锚定的 L1 高度**——同一 L1 块内晚于落地交易入队的条目不在快照内、归
  下一批次，消除交易排序歧义；(3) 验证通过后合约把 `f_consumed` 原子推进到批次末
  游标；(4) 并发批次天然互斥（批次必须延伸当前 L1 链头且起始游标匹配，同一链头
  只有先落者成功）。
...
- **全体串谋的底线：仅强制块（forced-only block），双侧条件版**（评审 r1-3 后收紧；
  ...
  2. **落地时刻 `lag > Δ_stall`**（停摆阈值，初始 = `Δ_lag` + 1 epoch = 4 epoch
     ≈ 25.6 分钟；评审 r6-2 的修复——v1.5 此处误用 `H_force`（1 epoch ≈ 6.4 分钟）
  ...
  **仅强制块的规范格式（r4-3 制定；评审 r7-4/r7-5 修正）**：
  ...
  链、仅在其链头之后追加仅强制块。这继承了现行部署中 `permissionlessInclusionMultiplier` 的设计直觉
...
| 连续缺口 > `G_max` 且缺勤者不回归（灾难级） | 分钟级无新块 | 重锚流程（§6.4）：公告 + 窗口期满后从落地头重锚 | ≈ `Δ_lag + Δ_ra`（自故障起 ≈ 22–23 分钟；遭恶意假挑战 +16 分钟，有界） |
...
| 构建者跳过前块 / 缺勤 | 不构成过错（出块是机会不是义务） | 无罚没；跳过对跳过者**可获利**（单 slot 费用/MEV 转移，r5-3），但签名在案、公开归责，频率受诚实多数假设约束；缺勤者损失自己 slot 的费收入 |
