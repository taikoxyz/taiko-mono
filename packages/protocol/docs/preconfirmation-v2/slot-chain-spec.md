# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范,草案 v1.38）
...
- **恢复优先——冻结死尾让位于 档-(ii) 恢复块（评审 r36,Codex line 822 P1）**：上一条
  "最多块"规则单独看会与 §6.4 一跳恢复**死锁**——真停摆（> `G_max` 连续缺勤）后,停摆前
  的长尾（可能已数百块）不可再扩展,但按"最多块"它仍是最重叉,诚实节点死守它、无视只有
  一块的恢复叉,链卡死。修正:**一条未落地分支若其链头已落后当前墙钟 slot > `G_max`（即
  > `G_max` 个连续 slot 无人扩展它）,即为【冻结死分支】**——任何 档-(i) 子块（父距 >
  `G_max`）非法、它又非 final 落地块故无 档-(ii) 子块可引用它,**结构上不可再扩展**。诚实
  分叉选择**放弃冻结死分支,采纳建在最新 final 落地头上的 档-(ii) 恢复块**,即便死分支自
  落地分叉点起块数更多。**为什么安全、且不重开 r11-2 深跳过攻击**:(a) 死分支的块从未
  final（仅第 2 档预确认,本可撤销）;(b) 让位**仅当分支链头 > `G_max` 落后墙钟**时触发,而
  **健康尾巴的链头恒在墙钟 `G_max` 内**（正被积极扩展）——故恶意构建者对**健康**尾巴出
  档-(ii) 块时该尾巴不冻结、本规则不触发,其块仍按 §5.4 只能靠合谋落地者上位,深跳过敞口
  界不变;(c) > `G_max` 连续缺勤在诚实多数下正是 `G_max` 校准要防的灾难级事件（r12-2 概率
  界）,非常态。落地者同样按此择叉:面对冻结死尾与恢复块,落恢复块（§5.2 落地者策略同步
  适用）。这条把"何时该跟随恢复块"与"何时该守住健康尾巴"用**同一个 `G_max` 墙钟落后判据**
  干净分开,不引入新参数。
- **仅强制块 vs 排班块的同 slot/同父平局（评审 r38,DeepSeek 警告 2）**：`P_forced` 允许
  任意密钥在**与某排班构建者的普通块相同 slot、相同父**上签一个仅强制块。此时两个候选
  链头**分叉点相同**（同一父）→ 上文"分叉点块头哈希较小"的 tie-break **无法区分**（分叉点
  哈希相同）。补一条确定性优先规则:**同 slot、同父下,排班构建者的普通块优先于仅强制块**
  （普通块的签名者 = `lookahead(slot)`,有在册身份与保证金;仅强制块任意密钥、无 stake）
  ——**除非该普通尾巴已冻结**（> `G_max` 落后墙钟,即真停摆,§5.2 恢复优先接管,此时无
  竞争普通块或其已冻结,仅强制/恢复块胜。**效果**:正常运行时无 stake 的任意密钥**不能**
  顶替排班块、不会劈裂诚实节点;仅强制块只在"无排班普通块或普通尾巴冻结"（全卡特尔/真
  停摆）时才成为被跟随的头。这与 §5.2 恢复优先、§7 两 lane 分离一致,tie-break 有唯一确定解。
...
- **恢复块凭 §5.2 恢复优先被跟随（评审 r36,Codex line 822 P1）**：真停摆后,停摆前的
  长尾已**冻结**（链头 > `G_max` 落后墙钟、不可再扩展）,§5.2 的"恢复优先"规则让诚实
  节点与诚实落地者**放弃冻结死尾、跟随并落地这个 档-(ii) 恢复块**——否则单看"最多块"
  规则,冻结长尾块数更多会把恢复块饿死、恢复与 fork-choice 死锁。让位判据（分支链头 >
  `G_max` 落后墙钟）与恢复触发判据是**同一个**,故只在真停摆触发、绝不孤立健康尾巴。
...
- **仅强制块（forced-only block）——全卡特尔下的审查底线，现为 §4.2 档 (ii) 的直接
  ...
  - **合法性谓词 `P_forced`（精确、电路可验，取代 §5.1 旧"三条件"引用；评审 r36,Codex
    ...
    2. **父块满足以下之一（评审 r37-2,Codex "Allow forced-only blocks to continue
       ...
       - **(同 lane 续接) 档 (i)**：父是一个 `slot − parent.slot ≤ G_max` 的**前一个
         ...
         按 §5.1 规则 1 扩展;两条 lane 由"父是否 `P_forced`"分开,恶意仅强制块对健康
         内容尾巴依旧受 §5.2 恢复优先约束（只在其冻结时才被跟随）。
  ...
  - **单方 Byzantine 聚合者可自源孤立分支——必须明说的残留收紧（评审 r36,Codex line
    ...
    **但边界不变、且被两条既有规则夹住**：(a) 它**打不动健康尾巴**——按 §5.2 恢复优先,
    仅强制短分支只在正常尾巴**冻结**（> `G_max` 落后墙钟）时才被诚实节点/诚实落地者跟随;
    对健康尾巴,只有这个 Byzantine 聚合者**自己作为恶意落地者**在 L1 上抢落它才能孤立
    尾巴——而"不受信任的聚合者能在 §5.4 深度界内排除未落地尾巴"**本就是 §1 信任模型明码
    接受的残留**,聚合者不受信任是前提,不是新洞;(b) 它换来的分支**内容仅强制**——被孤立
    尾巴里的自由裁量交易回到 mempool/强制队列,不被销毁,且深度仍 ≤ 未落地尾巴（条件性
    ≈ `Δ_lag`,§4.2/§5.4 的 Codex line 475 条款）。**要如实修正的是措辞**：仅强制路径的
    频率**不受"恶意排班 slot 数"约束**（任意密钥、无排班),受约束的是它的**危害**——只及
    第 2 档可撤销预确认、须由不受信任聚合者亲自落地、被 §5.2 恢复优先挡在健康尾巴之外。
    把这条单方聚合者攻击**显式并入 §1/§5.4 的untrusted-聚合者残留**,不另设机制去消除
    （消除仍需协议级 DA,§1 不采纳）。
...
| 连续缺勤 ≥ `G_max` 个 slot，乃至数小时/数天（灾难级停摆） | 停摆期无新块 | **§4.2 档 (ii)**：任一回归的排班构建者（或仅强制任意密钥）以 final 落地头为父出一块,**排序一跳恢复**（§6.4,v1.31 方向 B 重设计——无 episode） | **排序 ≈ 一轮证明 + 落地（≈10–15 分钟,与停摆时长无关,一跳）+ 紧接落地后起停的最坏瞬态叠加 ≤ `F_l1` 的尖端 final 等待（r35,DeepSeek 关键 2；稳态停摆无此项）**；L1-sync/强制积压按每块 `C_anchor`/`C_force` 追平；**持续推进需排班构建者以 ≥1/`G_max` 窗口密度持续回归（= 常态出块的 `G_max` 活性底线,非恢复独有；单次一块只重新生根、需后续接续，r36/Codex line 916）**或仅强制底线（同密度） |
...
3. **落地参数**：`Δ_lag`、`m_agg`、`δ_slash`、`H_force`、兜底报酬的指数成本上限；
   ...
   **`D_anchor_max` 设置器不变量（评审 r38,Codex P1）：`D_anchor_max ≥ 证明时延 + 最坏
   可容忍落地延迟（含临时 L1 拥堵/审查余量）`**——否则恢复块（及任何块）会在证明+落地
   窗口内 anchor 过期、须重签重证（§6.4 anchor 新鲜度条款）；这是恢复"不因缺口过期"的
   前提,把旧 `s_ra` 的 64 秒结构性窗口换成可设足够大的自由参数。
...
18. **形式化规范补强**（评审 r36，DeepSeek/Codex 建议）：(a) 一份块合法性 / 父块选择 /
    落地 / 强制包含 / 恢复优先 的**伪代码或状态机**（当前散文重度依赖评审注记与交叉引用,
    P1 级的父块/fork-choice 交互易被读者漏看——Codex line 822/475 都是这类交互）;(b)
