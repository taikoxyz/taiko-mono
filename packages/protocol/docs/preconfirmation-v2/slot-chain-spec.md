# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范，草案 v1.17）
...
- **存在挑战（liveness challenge）——附保证金、按结局结算（评审 r12-3：v1.11 的
  ...
  **结算按结局**：公告最终被取消（真有落地发生，"尾巴存活"被事实证实）⇒ 退还
  ...
  块头本身。
- **取消——任何链头推进即取消（评审 r12-1/r12-5 合并修复：v1.11 的取消条件是
  `lag ≤ Δ_lag`，诚实兜底者受 blob 容量/证明吞吐/L1 拥堵所限每次只落一段、追不平
  阈值时，公告仍存活，窗口一到剩余未落地尾巴照样被斩——"部分落地的前缀不被孤立"
  只护住了已落地的块）**：窗口内**任何批次被接受**（已落地链头前进，无论推进
  多少；本 episode 自身的执行除外）即作废公告——钉定父块不再是链头，episode 前提
  消失；要重锚必须对新链头重新公告、重开窗口。由此只要诚实落地还在持续（哪怕
  每次只追一小段），重锚就永远追不上它；而真实大缺口下无物可落，无人能取消。
  这与 §6.3 计次的"存在可落内容由落地事实自证"是同一判定手法。押金处置：被取消
  时 `B_ra` 付给取消批次的落地者（有挑战则与挑战者平分，见上）；恶意反复公告
  每次损失 `B_ra`，自限。
...
- **恢复时间的四分法（评审 r12-5——"强制路径恢复"与"系统恢复"是不同的界，分列
  ...
  4. **自由裁量服务恢复**：等待 ≥ 1 个诚实排班构建者出现（r12-2 的概率界；
     模型外无界）。
- **仅强制块同规**：§7 仅强制块的落地同样必须经过本流程的公告与窗口（其自身
  `lag > Δ_stall` 等三条件不变，叠加本流程；`Δ_stall > Δ_lag` 保证公告条件先于
  其开关满足；钉定父块与"任何落地即取消"规则同上，r12-1）。这顺带把 r2-2 的
  停摆期残留竞速收窄，见 §7。
...
- **包含规则（电路强制，逐块，带容量上限）**：条目在提交后 `F_delay` 到期。任何
  ...
  **块首前缀不足上限却遗漏了到期条目的块不合法**（§5.1 规则 4 的精确形式）——审查
  ...
  (2) 证明的公共输入是固定的队列快照 =（两队列根、两尾游标），**取自该批次锚定的
  L1 高度**——同一 L1 块内晚于落地交易入队的条目不在快照内、归下一批次，消除交易
  排序歧义；(3) 验证通过后合约把 `F_consumed` **整对原子**推进到批次末游标对；
  (4) 并发批次天然互斥（批次必须延伸当前 L1 链头且起始游标对匹配，同一链头只有
  先落者成功）。前文 r5-6 的单游标表述按分量意义理解（每条队列各自"消费到期
  前缀、推进游标"）。
