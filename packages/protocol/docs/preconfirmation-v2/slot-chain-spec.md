# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范，草案 v1.4）
...
> 出现时在括号内标注英文。
> **草案 v1.4，2026-08-25**：初稿经四轮对抗评审修订（一轮内部评审、PR 上的
> DeepSeek 两轮与 Codex 一轮，共 25 项发现全部修复或明码定性；正文以
> "评审 rN-M"标注各修复位置，评审原文见 PR 记录）。第 4 轮（DeepSeek）修复：
> 强制包含加每块容量上限 `C_force` 与确定性溢出（堵"塞满即停链"DoS——r4-1）、
> 聚合者违约计数改挂"自身迟到批次被接受"以恢复零误伤（r4-2）、仅强制块的规范
> 格式与防抢跑受益人绑定（r4-3）。机制骨架完整，参数为初始建议值，§12 列出
> 待定项。
