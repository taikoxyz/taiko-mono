# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范,草案 v1.40）
...
- 轮到 slot `s` 的构建者构造**块头**（block header）：
  ...
  ```text
  header = (chainId, slot, parent_hash, anchor, txs_root, state_root_claim(可选), 
...
- **父块规则——两档,判据是【缺口大小】而非父块落地状态（评审 r6-1 → r32 重做,方向 B
  ...
  - **(ii) 无界缺口档（恢复）**：`s − parent.slot` 任意——但父块**必须是【最终的 L1 已
    落地块】**（**`L1-final`**,达 `F_l1` L1 最终性深度；**术语区分,评审 r38 DeepSeek 警告
    3**：这里的"final"专指 **`L1-final`** = L1 达 `F_l1` 深度,**区别于 §6.1 的 `landed-final`
    = L2 批次"接受即最终"这一 L2 层最终性**——两档父块规则一律以 `L1-final` 为准;全文术语
  ...
  - **档 (ii) 块头须携【签名时父已 `F_l1`-final】的 L1 引用——堵 `F_l1` 边界预签循环（评审
    ...
    那时诚实内容尾巴已有时间被承诺 §5.7 并沉淀,成为 §5.6 的 `C`）。正常恢复不受影响:恢复块本
    就在父 final 后才出。
...
- **"尾巴 ≤ `Δ_lag`"是条件性的,不是无条件（评审 r36,Codex line 475 P1——必须明说）**：
  ...
  **随停摆无界增长、`> Δ_lag`**;此时档 (ii) 的 `G_max_landed = ∞` 让**一个恶意构建者一
  ...
  说清**。可执行的落地响应时限需协议级 DA/链头承诺（§1 不采纳,附录 A-3）,故此处只作
  条件性如实标注,不加不可兑现的界。
...
- **生效还须挂在【批次接受时点】上,堵住历史 slot 回填（评审 r39,独立审核第 2 轮发现 5）**：
  ...
  继续出合法块（还能被用于陈旧尖端 grief,§6.4/独立审核发现 5）。**v1.40 改用 §5.7 承诺时间戳
  作祖父化判据（独立审核第 3 轮发现 7 指出:接受时点推不出签名时点——`δ_land` 宽限窗内仍放行
  回填、窗外又误杀被拥堵延迟的正当在途块）**：判据改为**可证的存在时间**——**罚没在 L1 生效后,
  一个该 signer 的块只在"其所在链头已于生效【之前】进入 §5.7 承诺累加器（有更早的 `commit_L1_time`）"
  时才被祖父化接受;否则（生效后才出现、无更早承诺）一律拒绝,无论 `header.slot` 填多少**。这样:
  被罚没旧密钥事后回填的历史块**没有生效前的承诺** → 拒绝（关掉假阴性）;而 signer 在生效前
  已合法签、已被承诺的在途块**有生效前承诺** → 即便因拥堵晚落也接受（关掉假阳性）。承诺时间
  不可伪造（L1 盖章）,故祖父化精确到"生效前是否真的存在过",不再靠 `δ_land` 猜。未被承诺的
  在途块退化为旧规则（`δ_land` 宽限,单位以 §12 参数表为准，r40 统一为 L1 slot）——但正常运行
  下诚实块都会被承诺,故该退化仅在无人承诺的边角。
...
1. 签名有效，且签名者 = `lookahead(slot)`（排班表在电路内从 L1 锚定数据重算）——
   **唯一例外**：满足 §7 **合法性谓词 `P_forced`** 的仅强制块（forced-only block）豁免
   本条排班检查——`P_forced` 是精确、电路可验的准入测试（任意密钥 + 档 (ii) final 父 +
   `slot ≤ 墙钟` + 非空强制前缀至容量 + 最大消息消费,详见 §7）,取代 v1.31 前已删除的旧
...
- **未落地部分（P2P 尾巴）的协调规范——排序判据统一由下条"最优链全序"给出（评审 r11-2 引入,
  ...
  兜底者/私有中继/L1 提议者的择叉;对**恶意落地者**的真实约束由 §5.6 挑战罚没（锚定 §5.7 承诺）
  给出,深跳过残留敞口的定界仍见 §5.4。
- **最优链的全序（best-chain total order）——分叉选择、落地义务、挑战三处共用（评审 r39,
  所有者决定引入挑战罚没后重写；r40 修 lane 洗白）**。**本条取代 v1.35 的"恢复优先／冻结死尾
  让位"规则**——独立审核（第 1/2 轮）指出那条会**命令诚实落地者丢弃一条完整可见、可落地的健康
  长尾**,这是错的:"不可 P2P 扩展"不等于"应被丢弃"。给定分叉点 `F`,在延伸 `F` 的候选链中比较,
  **依次**：
  1. **lane 优先——lane 在【自 `F` 起第一个块】处固定、由整条分支继承,不可被后继普通块洗白
     （评审 r40,独立审核第 3 轮发现 3）**：比较两条链时,看它们**自 `F` 起第一个相异块**属哪个
     lane——**内容 lane（自由裁量块）> 仅强制 lane**。**关键:lane 由分叉后第一个块定,不是"链中
     任意处含一个内容块"**——否则攻击者在一长串仅强制脚手架后接**一个**普通块,就把整条 1472 块
     仅强制链洗成"内容链"、按块数压过健康内容尾巴（发现 3 的洗白）。固定在首块后:那条链的首块
     仍是仅强制块 → 整条仍属仅强制 lane → 输给首块即内容块的健康尾巴,不论后面接多少块。理由同
     前:仅强制只是审查底线,不应顶替内容;内容块负强制前缀义务,内容 lane 优先不牺牲审查抵抗。
  2. **块数**：同 lane 档内,自 `F` 起合法块数多者胜。
  3. **高 slot**：块数同,取 tip slot 高者。
  4. **末级 tie-break**：仍同,取**首个相异子块的块头哈希较小**者（修正 r38/独立审核一致性 7:
     旧"分叉点哈希"在同父双仅强制块下无法区分——比较点改为**首个相异子块**的块头哈希）。
...
- **落地者的尾巴选择策略（评审 r33,所有者提出；r40 改为按全序 + §5.6/§5.7 背书）**：落地者
  面对多条候选尾巴时,**对自己的视图应用上文最优链全序**落最优者。这在 v1.39 前只是"策略、非
  协议强制"（落地者不受信任）;**v1.40 起由 §5.6 挑战 + §5.7 承诺给它经济背书**——落地者跳过一条
  §5.7 已承诺满 `Δ_prop` 的更优链去落更差的,可被机械举证没收 `L_land`。故"落最优链"从纯诚实
  策略变为**可举证的义务**。**"有更好的尾巴但落地者没看到"的处置**：落地是原子且最终的,落错即
  ...
  尾巴预确认安全**还依赖落地者视图完整**（不止诚实）——见 §1 诚实条款。**这条从"仅诚实
  策略"升级为"可举证罚没的义务"（评审 r39,所有者决定）**：落地者不受信任,单靠链下策略
  无法约束恶意落地者,故 §5.6 引入**挑战罚没**——落地者若跳过一条**已传播沉淀、更优（按上
  文全序）**的链,任何人可事后向 L1 举证并没收其保证金。**注意这与 v1.30 前否决的"重锚
  episode"是两回事**：那个是"挑战成功就换正统链/改最终性",需回滚且与证明时延赛跑;§5.6
  的挑战**只罚没、不改最终性、不回滚**（落地即最终仍由证明系统定），举证的是**早已签好
  存在**的链、纯 L1 验签,**无时钟赛跑**——正是这一点让它避开老 episode 的死穴。详见 §5.6。
...
- **正统性仍只由 L1 决定（G2）——但落地"该落哪条"现在是可罚没的义务**：链下 fork-choice
  给诚实节点收敛（上文全序）,而"落地者必须落最优沉淀链"由 §5.6 挑战罚没**用经济手段**背书
  （不改 L1 正统性判定本身:先落的仍正统,挑战只事后罚落错者、不回滚）。故本设计不是"L1 强制
  最长链"（那需协议级 DA）,而是"L1 事后惩罚未落最优链者"——弱于 DA、强于纯链下策略,是所有
  者在 §1 无 DA 约束下选定的中间点。
...
- 因此本设计的诚实结论是（**r40 更新以纳入 §5.6/§5.7，独立审核一致性 4**）：**可罚没的过错是
  L1 机械可判的三类——双签（§4.3）、落地超时计次（§6.3）、以及【落地者跳过更优承诺链】（§5.6
  挑战 + §5.7 承诺）**；而**构建者层面的跳过、扣体仍不可罚没**（针对的是 builder 的出块选择,
  不是 lander 的落地选择——§5.6 罚的是落地者落更差链,不罚构建者跳过）。§10、§11 的表述按此更新。
  弱执行类的如实清单（评审 r12-6 更正——此处曾残留 r5-3 之前的"弱执行类不含任何
  能获利的攻击"旧结论，与 §5.4 直接矛盾）：**扣体**通常无直接协议收益；**跳过
  （含深跳过）**可获得费用/MEV、可撤销排序/状态类预确认，且不可罚没——处置只有
  频率约束（诚实多数）与公开归责。

### 5.6 落最优链的义务与挑战罚没（challenge）——不受信任落地者的经济背书（评审 r39，所有者决定）

> **本节是所有者 2026-08-25 的设计决定**：在"聚合者/兜底者完全不受信任 + §1 无协议级
> DA"下，独立审核（第 1/2 轮）证明纯链下 fork-choice 无法阻止恶意落地者跳过健康尾巴、
> 落一条更差的链（孤立其中预确认），也无法阻止单个聚合者反复零罚重置。所有者选定的解法
> 不是加 DA、也不是回滚式挑战，而是**一个只罚不改链的问责层**。**与 v1.30 前删掉的"重锚
> episode"的本质区别**：episode 的挑战会**改正统/改最终性**、需回滚、且要与证明时延赛跑
> （其死穴）；本节挑战**只没收落地者保证金、绝不回滚、不改最终性**（落地即最终仍由证明
> 系统定），举证对象是**早已签好存在**的签名链、L1 纯验签，**无任何时钟赛跑**。

- **落地义务（normative）**：落地者（聚合者或兜底者）**应当落"最优链"**——按 §5.2 全序
  （内容 lane > 仅强制 lane；同 lane 取块数多、再高 slot、再首个相异子块哈希小）在延伸当前
  L1 落地头的候选链中最优、**且已传播沉淀**者。"传播沉淀"= 该链 tip 的 slot ≤ `当前 L1
  时间对应 L2_slot − Δ_prop`（tip 至少 `Δ_prop` 老,已有时间传遍 P2P；`Δ_prop` << `Δ_lag`,
  §12）。落地者可以落**更深**（更老）的前缀,但**不得跳过一条已沉淀的更优链去落一条更差的**。
- **挑战（permissionless、L1 机械判定，v1.40 改为【锚定在 §5.7 链头承诺时间戳】上——独立
  审核第 3 轮证明"用块自报 slot 判存在"会被事后补签绕过）**：一批 `B` 被接受后开一个**问责
  窗口 `W_chal`**（初始 1–2 epoch,§12）。窗口内任何人可提交一条**竞争链 `C`** 主张"`B` 落地
  时已存在一条更优的链、`B` 却落了更差的"。L1 机械核验（同 §4.3 手法,不重跑 ZK）：
  1. **`C` 是合法签名链**：每块签名有效、且**签名者 = `lookahead(slot)` 或该块满足 `P_forced`**
     （对仅强制块本条读作后者——消除独立审核一致性 2 的"signer=lookahead 与任意密钥"矛盾）,
     `parent_hash` 串联,缺口满足 §4.2;举报人无法伪造（需盗排班构建者签名）;
  2. **分歧点在 `B` 已落区间之内**：`C` 必须在 `B` **实际落地的某个位置**就已与 `B` 分歧并按
     §5.2 全序更优,**不是**仅在 `B` 尖端之后再接更高块（消除独立审核发现 2/一致性 3 的"事后补
     一个更高 slot 延伸块来挑战"漏洞——落地者合法地落更老前缀不应被 `B+1` 挑战）;
  3. **`C` 在 `B` 落地【之前】即已存在——由 §5.7 链头承诺的【L1 时间戳】证明,不用 `C` 自报
     slot（这是修复的关键：自报 slot 可造假、L1 盖的承诺时间戳不可造假）**：存在一个 §5.7 的
     已承诺链头 `hc`,其**L1 承诺时间 ≤ `B` 落地 L1 时间 − `Δ_prop`**,且 `C` 是 `hc` 承诺链的
     前缀或该链本身。
  三条全过 → 落地者被推定跳过了一条**够得着的更优链**。**落地者的抗辩（validity 反证）**：可
  提交**简洁反证**证明 `hc` 承诺的链自某块起【执行非法/不可落地】（错 nonce、假状态根、漏强制
  或消息最大前缀、块体不可得 §5.5）——这类链本不可落,跳过无责;反证成立 → 没收**承诺者**的
  `L_commit`（§5.7）、举报作废。反证不成立或落地者不抗辩 → 没收**落地者**的 `L_land`（≥80%
  销毁、余付举报人）,记一次落地过错。**举报人押 `L_chal`**,举证不成立则没收——防滥告。
- **最终性与链不变（关键,所有者强调）**：挑战**成立也不回滚 `B`、不改任何最终性**。`B` 落地
  即最终由证明系统定,`C` 不会因挑战成立而"转正"——`C` 与 `B` 都延伸同一分叉点,`B` 已落且
  最终,`C` 就此永久成孤块,其交易回内存池、由后续建在 `B` 尖端上的批次重新打包。**挑战纯为
  威慑**：把"跳过好链"从"白跳、无罚"变成"可举证、赔 `L_land`"。所以它不给被跳过尾巴的用户
  "找回预确认"的保证（那需 DA）,只让落地者**理性上不敢跳**。
- **它如何堵住独立审核的三条严重发现**：
  - **诚实落地者丢健康长尾（发现 1/2）**：根因是 v1.35 的错误 fork-choice 规则,已在 §5.2
    改为"最优链含冻结长尾、须先落再恢复";§5.6 再加经济背书——落地者跳过它可被罚,双保险。
  - **单聚合者零罚反复重置（发现 3-一）**：聚合者每次落一条更差的仅强制/短链盖过健康内容
    尾巴,该内容尾巴（更优、已沉淀）即是现成的 `C`,**每次都可被举证罚 `L_land`**——"零罚
    可反复"不再成立;配合 §6.3 的 lag 债结算,自擦 lag 也不免罚。
  - **任意密钥更重强制链盖内容尾巴（发现 3-二）**：`C` = 那条内容尾巴,按 §5.2 lane 优先严格
    优于纯仅强制链,举证即罚。
- **明示残留（诚实标注，v1.40 因引入 §5.7 承诺层而收紧）**：(a) 挑战仍是**经济威慑**——需
  `L_land` > 跳过收益、且**有人愿意承诺+举证**（有赏金,极端 gas/拥堵下可能不经济,同 §6.3,
  §12/§1）;卡特尔可吞罚硬跳,属模型外。(b) **eclipse 误罚已大幅收窄**：存在性现由 §5.7 的 L1
  承诺时间戳定,不再靠"沉淀=充分传播"的软推定——只有当一条更优链**被承诺满 `Δ_prop`** 落地者
  仍未落时才判责,且落地者可用 validity/块体不可得反证;残余误罚只发生在"落地者被 eclipse 到
  连 L1 上的承诺都没同步"这一极端,归 §12 P2P。(c) 挑战**不改最终性**,被跳过预确认仍是"延迟
  非找回"。(d) **新增依赖**：`HC` 的真实性依赖"≥1 诚实承诺者"（§5.7）。这些都是 §1 模型下
  的明码接受项,换来的是"跳过更优链可**机械举证**罚没"这一相对 v1.39 的实质增强。

### 5.7 链头承诺累加器（head-commitment accumulator）——给 L1 一个"什么链何时存在过"的可信时间戳（评审 r40，所有者决定 option B）

> **本节是所有者 2026-08-25 的第二个设计决定（option B）**：独立审核第 3 轮证明,§5.6 若用块
> 自报的 slot 判"C 是否落地前已存在",会被**事后补签**（用旧 anchor 签一个高 slot 块冒充"早已
> 沉淀"）绕过——L1 从签名分不清一条链是真早存在还是刚造的。要让挑战机械成立,L1 必须有一个
> **自己盖时间戳、参与者不可造假**的"什么链何时存在过"记录。这就是本节的**轻量链头承诺**：只
> 承诺**链头哈希 + 权重**（几十字节,不含块体/交易数据）,是**部分 DA**（仅头,远轻于全 DA）;
> 所有者明码接受这点成本以换"挑战可机械执行"。它**不改最终性、不参与落地的出块/证明流程**,
> 只是一个旁挂的、permissionless 的时间戳账本;因此**不重蹈老 episode**（那是改正统 + 与证明
> 时延赛跑;本节只盖时间戳,承诺不需证明、不改链）。

- **累加器状态（L1 合约）**：`HC = (head_hash, cum_weight, commit_L1_time, committer, L_commit)`
  ——当前**最优的已承诺链头**（`cum_weight` 按 §5.2 全序度量:先 lane、再块数…）。
- **承诺（permissionless、bonded）**：任何人可提交一条**更优的合法签名链头**推进 `HC`。L1 核验
  （纯验签,不重跑 ZK）：提交链头是合法签名链（每块签名者 = `lookahead(slot)` 或满足 `P_forced`、
  `parent_hash` 串联、缺口满足 §4.2）、按 §5.2 全序严格优于当前 `HC`;通过则记新 `HC`、**盖上当前
  L1 时间戳**,提交者押 `L_commit`。承诺只需链头（+ 证权重的签名链或 succinct 累加器路径）,**不需
  块体、不需 ZK 有效性证明**,故廉价、秒级、可高频。
- **谁来承诺（活性依赖,明说）**：承诺是 permissionless + 有赏（承诺一条能逮住"落地者落更差链"
  的更优链头 → §5.6 举证成功可分 `L_land`）。在**诚实多数构建者 + ≥1 个愿承诺的诚实观察者**下,
  `HC` 跟随真实最重尾巴。这是一条**新的诚实依赖**（"≥1 诚实承诺者"）,与 §6.3 兜底可行性同级,
  并入 §1。无人承诺时 `HC` 落后、§5.6 保护相应减弱——如实标注。
- **承诺 validity 的两侧博弈**：承诺只证**结构**（签名、权重）,不证执行合法。若一个承诺链头执行
  非法,任何人（尤其被指控的落地者）可提交**简洁反证**没收承诺者 `L_commit`、把 `HC` 回滚到上一
  合法承诺。故承诺是"押金背书的、结构合法的最重链头声明";`L_commit` ≥ 迫使一次反证的期望成本,
  压制无效承诺刷屏（残留:持续刷无效承诺 = 持续烧 `L_commit`,经济型抗 DoS,§12）。
- **它同时修好独立审核第 3 轮的这些发现**：
  - **发现 2（事后补签 / 无效块冒充更优链）**：存在性由 L1 承诺时间戳定,补签的高 slot 块因"承诺
    时间晚于 `B` 落地"不合格;无效链由反证博弈剔除——§5.6 挑战因此**机械成立**。
  - **发现 1（陈旧 anchor 尾巴死锁）**：被承诺的块有了**可证产生时间**（承诺时间戳）,故 §8 的
    anchor 新鲜度对它**按承诺时间判**、不按落地时间判——被恶意聚合者长期扣落地的诚实尾巴,其早
    期块 anchor 相对"当初承诺时"仍新鲜、**依旧可落地**,死锁解除（详见 §8）。
  - **发现 5（`F_l1` 边界预签循环）**：诚实内容尾巴一旦被承诺（更优、已沉淀）,聚合者在 final 边界
    抢落的稀疏仅强制链即"落更差链",§5.6 举证即罚;配合 §4.2 的**签名时 `F_l1` 引用**（拒绝在未
    final 尖端上预签,见 §4.2）双堵。
- **明示残留**：(a) `HC` 真实性依赖"≥1 诚实承诺者";(b) 承诺是**部分 DA（仅链头）**——证**头存在**
  不证**块体可得**,故被扣体（§5.5）的链即便被承诺也未必可落,§5.6 举证时落地者可用"块体不可得"
  抗辩;(c) 这层是所有者当初为省成本删掉的"链头承诺/DA"的**最小回流**（附录 A-3 成本恒等式的一个
  中间点）——option B 明码接受它,换 §5.6 从"经济威慑"升级为"L1 可机械判定"。
...
- **堵住"自擦 lag + 落更差链"的零罚重置（评审 r39,独立审核第 2 轮发现 3-一）**：仅按
  ...
  (a) **落更差链本身可罚（主堵点,§5.6）**：它每次盖过的那条健康内容尾巴,是一条已沉淀、
     按 §5.2 全序更优的现成 `C`——**每次落更差链都可被举证没收 `L_land`**,与 `lag` 是否
     越阈值无关;"零罚可反复"不成立。
  (b) **lag 债按接受前状态结算,不容整数骑线（评审 r39;r40 修等号处仍零罚——独立审核第 3
     ...
     不再免罚、也不再无计次。两条合力:自擦 lag 的循环要么落更优真尾巴（无害）、
     要么每次赔 `L_land` + 迟到罚金,§6.3/§9 的"Byzantine 聚合者有界"恢复成立（在 §5.6
     举证经济可行的同一条件下）。
...
- **机制（就一条）**：当缺口 > `G_max`（连续缺勤超上限）或聚合者扣落地导致正常出块
  ...
  窗口、状态机**（§5.6 的挑战是对**落地者落错链**的独立事后问责,不参与恢复块的出块或落地
  流程、也不改最终性,与 v1.30 前删掉的"重锚 episode"无关）。**先落最优链的顺序见下条**:
...
- **先落最优（含冻结）链、再从其 final 尖端恢复——不丢尾巴（评审 r39 更正,取代 v1.35 的
  ...
  跳过一条已沉淀的更优尾巴去落恢复块,按 §5.6 **可被举证罚没**。
...
- **anchor 新鲜度对恢复块的约束——"永不过期"须加限定（评审 r38,Codex P1）**：档 (ii)
  ...
  **自由可设的参数**,设置器不变量（**完整公式见 §12,r40 修正为** `D_anchor_max ≥ D_anchor +
  ...
  本身。**且 v1.40 起,一经 §5.7 承诺的块其新鲜度按【承诺时间】判**（§8）,恶意扣落地拖再久也
  不改其当初新鲜度,进一步解除独立审核第 3 轮发现 1 的陈旧-anchor 死锁。若 L1 对 `R` 的落地交易
...
- **仅强制块（forced-only block）——全卡特尔下的审查底线，现为 §4.2 档 (ii) 的直接
  ...
  - **合法性谓词 `P_forced`（精确、电路可验，取代 §5.1 旧"三条件"引用；评审 r36,Codex
    ...
    4. **内容仅强制 + 逾期 `H_force` 才开放（评审 r40 补 `H_force`,独立审核第 3 轮发现 9：
       ...
       **非空**,且其**队头条目已逾期 ≥ `H_force`**（即 `slot ≥ due_slot(队头) + H_force`）
       ...
       "逾期兜底"时间界一致（同-lane 续接块继承该授权,首块满足即可）;
  ...
  - **单方 Byzantine 聚合者可自源孤立分支——必须明说的残留收紧（评审 r36,Codex line
    ...
    **v1.39 起,这条从"接受的残留"改为"可罚没过错"**：(a) 它**打不动健康尾巴**——按 §5.2
    全序**内容 lane 全局优先于仅强制 lane**,诚实节点/诚实落地者不会用仅强制链盖过健康内容
    尾巴;(b) 只有这个 Byzantine 聚合者**自己作为恶意落地者**在 L1 上抢落它才能孤立尾巴,而
    **那条被孤立的健康内容尾巴正是 §5.6 挑战里现成的更优沉淀链 `C`**——**每次这么落都可被
    任何人举证、没收 `L_land`**（配合 §6.3 lag 债结算,自擦 lag 也不免罚）。故"单方零罚可反复"
    **不再成立**（独立审核第 2 轮发现 3 的处置）。残留降为 §5.6 明示的三条:挑战需举证经济
    可行、诚实但被 eclipse 的落地者可能误罚、挑战不改最终性（被跳过预确认是延迟非找回）;
    彻底机械消除仍需协议级 DA（§1 不采纳）。被孤立尾巴的自由裁量交易照旧回 mempool、不销毁。
...
- **新鲜度对【已承诺块】按承诺时间判、不按落地时间判——解陈旧-anchor 尾巴死锁（评审 r40,
  独立审核第 3 轮发现 1）**：默认新鲜度是"落地 L1 块高 − 块 anchor ≤ `D_anchor_max`"。独立审核
  指出:恶意聚合者长期扣落地时,一条诚实尾巴的早期块 anchor 会在"落地时"超期 → 该尾巴既不能被
  落（首块超期）、又不能被跳过（`parent_hash` 穿过它）、先落恢复块又会被 §5.6 挑战,形成无合法
  动作的死锁。修复:**若一个块的链头已进入 §5.7 承诺累加器（有 L1 盖的承诺时间戳 `commit_L1_time`）,
  则它的 anchor 新鲜度改判为 `commit_L1_time − 块 anchor ≤ D_anchor_max`**——即按它**当初被承诺
  （≈产生）时**是否新鲜来判,而非按被恶意拖到很晚才落地时判。诚实尾巴一经承诺,其块的产生时间
  即被 L1 钉住,**扣落地拖多久都不改变它当初的新鲜度**,故**依旧可落地**,死锁解除。未被承诺的块
  仍按落地时间判（无可信产生时间可用）——所以"想让长尾可落"要先承诺它,这正好与 §5.6/§5.7 的
  激励一致（承诺诚实尾巴既保住其可落地性、又是逮住恶意落地者的 `C`）。因果序不变量（上条）不受
  影响:它约束 `anchor ≤ slot 时间`,与用哪个时间判新鲜度无关。
...
- **L1→L2 消息处理上限 `C_anchor`——绑在【处理游标】上,不绑在 anchor 引用上（评审
  ...
  - **L1→L2 消息处理游标 `m_consumed`**（**全局消息序号**,不是 L1 高度——评审 r39,独立
    ...
    log序)` 三元组,与 §7 强制游标同款）：**每块
    处理 `min(C_anchor, 待处理数)` 条——既有上限也有【下限义务】,不是纯上限（评审
    r36,Codex line 1031 P1 / DeepSeek 警告）**。恢复块引用新鲜 anchor(覆盖数天 L1),
    但只处理其中至多 `C_anchor` 条待处理消息,余量由后续块继续(`m_consumed` 像 §7 的
    `f_consumed` 一样单调推进)。**强制最大前缀消费（电路规则,完全照搬 §7 强制队列
    形状）**：L1→L2 消息构成一条**公开 FIFO 队列**（合约维护队列根 + 尾游标,与
    `m_consumed` 分开）;§5.1 新增合法性义务——每块必须按 FIFO **确定性最大前缀**消费
    到 `min(C_anchor, 队列中 anchor 引用高度内已到达且未消费者)`,少消费即块不合法
    （同 §7 "块首前缀不足上限却遗漏到期条目的块不合法"）。**没有这条下限,`m_consumed`
...
| 连续缺勤 ≥ `G_max` 个 slot，乃至数小时/数天（灾难级停摆） | 停摆期无新块 | **先落最优链、再从其 final 尖端恢复（§5.2/§6.4,r39 更正——不丢冻结长尾）**：停摆前若有冻结长尾,落地者先落它（兑现其预确认）,达 `F_l1` 后恢复块以其尖端为父按 §4.2 档 (ii) 出块;确无更优链时恢复块本身即最优、直接落。跳过更优沉淀链可按 §5.6 罚没 | **排序 ≈ 一轮证明 + 落地（≈10–15 分钟,与停摆时长无关）+ 紧接落地后起停最坏叠加 ≤ `F_l1` 尖端 final 等待（r35；稳态无此项）**；有冻结长尾时含"先落尾巴"的一轮证明；L1-sync/强制积压按每块 `C_anchor`/`C_force` 追平；**持续推进需排班构建者以 ≥1/`G_max` 窗口密度持续回归（常态 `G_max` 活性底线，r36）**或仅强制底线（同密度） |
...
| 聚合者取前缀/落更差链（截断后缀） | 分两种（r39）：只取前缀、未跳过更优沉淀链 = **不构成过错**（后缀仍可被下批落地）；**跳过一条已沉淀、按 §5.2 全序更优的链去落更差的 = 落地过错** | 前者无罚；后者 **§5.6 挑战**：任何人举证该更优链 → 没收落地保证金 `L_land`（≥80% 销毁，余付举报人）+ 记落地过错；不改最终性 |
| 落地者对有效挑战的滥告（举证失败） | L1 验举证的三条件不过（`C` 非法/不更优/未沉淀） | 没收举报人押金 `L_chal` |
...
3. **落地参数**：`Δ_lag`、`m_agg`、`δ_slash`、`H_force`、兜底报酬的指数成本上限；
   ...
   **`D_anchor_max` 设置器不变量（评审 r38 引入,r39 修正公式漏项——独立审核第 2 轮发现 4）：
   `D_anchor_max ≥ D_anchor + P_prove,max + T_include,max + 队列/时钟余量`**（`D_anchor`=
   锚定最小深度 32 slot；`P_prove,max`=最坏证明时延；`T_include,max`=最坏落地纳入延迟）——
   ...
   **挑战/承诺参数（v1.39 §5.6 + v1.40 §5.7 option B）**：`W_chal`（问责窗口,初始 1–2 epoch）、
   `L_land`（落地保证金,≥ 跳过更优链最大收益）、`L_chal`（举报人押金,防滥告）、`Δ_prop`（沉淀
   深度,兼 §5.6 "够得着"判据）、**`L_commit`（§5.7 链头承诺保证金,≥ 迫使一次 validity 反证的
   期望成本,压制无效承诺刷屏）**、**`final_ref` 语义**（§4.2 档-(ii) 块头字段,证签名时父已
   `F_l1`-final）。承诺累加器的存储/回滚/反证证据形态并入形式化待办（§12 第 18 项）。**罚没
   祖父化改用 §5.7 承诺时间戳（§4.3,r40）**;`δ_land` 退化路径单位统一为 **L1 slot**。**共享
   gas 份额（§8）**：`G_anchor`、`C_force`/`C_bridge` gas 份额、L1→L2 消息 gas 份额,满足 `三者
   之和 ≤ block_gas_limit` + 单条准入上限 + 水位线（水位线的"下调 vs O(1)"张力见独立审核一致性
   7,列为实现细化）。
...
18. **形式化规范补强**（评审 r36，DeepSeek/Codex 建议；r39 补挑战/gas）：(a) 一份块合法性 /
    父块选择 / **最优链全序（§5.2）与落最优链义务** / 落地 `acceptBatch` / 强制包含 / **挑战
...
| `F_delay` 强制到期延迟 | 待定 | slot | **唯一到期谓词** `due_slot = submission_slot + F_delay`（§7），r39 澄清 |
...
| `D_anchor` / `D_anchor_max` | 32 / 待定 | L1 slot | 锚定深度/新鲜度上限；`D_anchor_max ≥ D_anchor + P_prove,max + T_include,max + 余量`，§8/§12 r39 |
| `W_chal` 挑战问责窗口 | 待定（1–2 epoch） | — | §5.6 落错链举证窗口（不拖最终性），r39 |
| `L_land` / `L_chal` 落地/举报保证金 | 待定 | ETH | §5.6：`L_land ≥ 跳过更优链最大收益`；`L_chal` 防滥告，r39 |
| `L_commit` 链头承诺保证金 | 待定 | ETH | §5.7：≥ 迫使一次 validity 反证的期望成本，r40 |
| `δ_land` 罚没回填窗口 | 待定（= 最坏证明+落地滞后） | L1 slot | §4.3 祖父化 late-landing、封死历史 slot 回填，r39 |
...
| 落最优链义务 | best-chain landing obligation | 落地者应落最优、已沉淀（tip ≥ `Δ_prop` 老）链；跳过更优者可被 §5.6 罚没（r39） |
| 挑战罚没 | challenge / slashing | 举证落地者跳过更优、**已承诺**链 → 没收 `L_land`、赏举报人；**不改最终性、不回滚**（§5.6，r39/r40） |
| 链头承诺累加器 | head-commitment accumulator | permissionless、bonded（`L_commit`）；把更优链头+权重提交上 L1、L1 盖时间戳；轻量部分 DA，给挑战"何时存在过"的可信基准（§5.7，r40 option B） |
| validity 反证 | validity disproof | 落地者证一个承诺链头执行非法/不可落 → 罚承诺者、举报作废（§5.6/§5.7，r40） |
| 传播沉淀 | propagation-settled | 一条链头在 L1 被承诺满 `Δ_prop`（r40 由承诺时间戳定，非块自报 slot）（§5.6/§5.7） |
| （已删）重锚 episode | re-anchor episode | v1.8–v1.30 的恢复子协议,v1.31 方向 B 整体删除,由两档父块规则取代（与 §5.6 挑战无关——那个只罚不改链） |
