# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范，草案 v1.7）
...
- **计算方式（纯函数，评审 r7-1 重写——v1.6 的"每 slot 各自向前 `D_rand` 取
  ...
  - **唯一快照高度**：窗口 `W` 的快照高度 `H_snap(W) = W 起点对应的 L1 slot −
    D_snap`，其中 **`D_snap` = 2 个 L1 epoch（最终性距离）+ 1 个 L1 epoch 余量
    = 3 L1 epoch**。约束链一目了然：
    `randao_source(W) = H_snap(W) ≤ 最终化的 L1 头 < W 起点`——对窗口内**所有**
    slot 同时成立（种子只有一个），排班在窗口开始前已定、且不受 L1 重组影响。
...
- 父块规则（评审 r6-1 后收紧）：`parent.slot < s` 且 **`s − parent.slot ≤ G_max`**
  ...
  链头单调前进）。**恢复例外**：`parent` = 当前已落地的 L1 链头时豁免缺口上限——但
  仅当落地时刻 `lag > Δ_stall`（§7 的停摆阈值：链确实死了才允许从 L1 头重启；正常
  运行时该例外关闭，防止它本身成为孤立尾巴的口子）。诚实策略：接在自己见到的最高
...
2. `parent.slot < slot` 且 `slot − parent.slot ≤ G_max`（§4.2 的缺口上限，r6-1；
   恢复例外：`parent` = 已落地 L1 链头且落地时 `lag > Δ_stall`），`parent_hash`
   指向一个合法块（或该 L1 链头）；
...
- **包含规则（电路强制，逐块，带容量上限）**：条目在提交后 `F_delay` 到期。任何
  ...
  **块首前缀不足上限却遗漏了到期条目的块不合法**（§5.1 规则 4 的精确形式）——审查
  ...
  条目数线性增长，换不来停摆，只有按序延迟）。**安全消息保留配额**：`C_force` 内
  保留子配额 `C_bridge`（初始 25%）给桥接/退出类条目，普通条目洪泛不能挤占安全
  关键消息的通道；配合现行的逐源隔离与基础费。§9 表涉及强制路径的时间界一律按
  本公式理解。**队列游标语义（评审 r5-6 的形式化）**："此前链上未包含"用**队列游标
...
- **全体串谋的底线：仅强制块（forced-only block），双侧条件版**（评审 r1-3 后收紧；
  ...
  **仅强制块的规范格式（r4-3 制定；评审 r7-4/r7-5 修正）**：
  ...
  开关。**停摆期内的残留竞速，明说（评审 r2-2）**：在 `lag > Δ_stall` 的停摆状态里，
