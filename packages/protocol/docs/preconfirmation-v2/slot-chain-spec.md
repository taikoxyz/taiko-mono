# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范，草案 v1.6）
...
- **迟到落地本身也计费——堵住"骑线"策略（评审 r3-1，Codex）**：只按"兜底批次被
  ...
  窗口就必然落一个自己的迟到批次，每次都计数。于是外部性故障期间聚合者分文不罚，
  而每一次真实的迟到都不免费；被抢跑的兜底者的落地 gas 损失从迟到罚金中按预注册
  意向补偿（细则并入 §12 第 3 项）。
...
- **包含规则（电路强制，逐块，带容量上限）**：条目在提交后 `F_delay` 到期。任何
  ...
  **块首前缀不足上限却遗漏了到期条目的块不合法**（§5.1 规则 4 的精确形式）——审查
  ...
  停摆。**队列游标语义（评审 r5-6 的形式化）**："此前链上未包含"用**队列游标
  ...
  漏判。批次证明以 L1 队列的承诺（由 L1 合约在落地时提供）为公共输入；落地时合约
  核对批次末游标与其队列记录一致。
- **全体串谋的底线：仅强制块（forced-only block），双侧条件版**（评审 r1-3 后收紧；
  ...
  **仅强制块的规范格式（评审 r4-3，DeepSeek——此前未指定）**：块头字段与普通块
  完全相同（`slot` = 所填缺口或链头后首个空位；`parent_hash` 正常；`timestamp` =
  slot 起始秒；`coinbase` = 出块者自选的受益地址，收取所含强制条目的费用）；内容
  确定性：到期条目按队列顺序、受 `C_force` 上限，无任何自由裁量交易——因此**同一
  (parent, 队列状态) 下两个仅强制块除 `coinbase` 外字节相同**。签名：出块者以任意
  密钥对块头签名（电路验签名有效性但豁免排班匹配——签名的作用是把 `coinbase` 与
  块头绑定、防篡改）。竞争与防抢跑：多方同时出仅强制块时，L1 先落者胜——由于内容
  确定性，这是 gas 竞赛而非状态彩票（同 v15 §11.3/§6.8 的结论）；`coinbase` 作为
  批次证明的公共输入（public input），从 mempool 复制交易无法改指向受益人
  （v15 I8 预承诺受益人手法）。
