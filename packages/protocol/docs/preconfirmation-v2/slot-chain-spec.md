# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范，草案 v1.26）
...
- **签名者的可得性，明说（评审 r12-2：公告期满不自动产生出块权）**：重锚块仍需
  ...
  **一旦发生重钉，钉定父块改变，旧父块上预生成的证明全部作废，须对新钉定父块重新
  ...
  执行时点是**信任模型内的概率界**：诚实构建者占比 `p` ⇒ 期望额外等待 ≈ `1/p` 个
...
- **episode 状态机（评审 r22-建议——把散落的规则收成一张转移表，降低实现歧义）**：
  ...
  | Open | (a) 被接受批次的落地链含某已登记 `H_ch`，或 (b) `lag ≤ Δ_lag` **且无未兑现 `H_ch`**（r24-2） | → **Cancelled**：公告作废；命中 `H_ch` 提交者得 `B_ra` 分成 + `B_ch` 退还，余者独立结算 |
