# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范,草案 v1.41）
...
签名链:       ... ←─块─←──块─←────────────────块─ ...     子块签名覆盖父块哈希，缺口允许
                                    │
落地(batch):  任何人把一段连续签名链 + 有效性证明一笔交易落上 L1 成为候选；结算窗口 W_settle 内最重的已证明候选收盘即最终(r41 option C)
                                    │ 超时未落地
...
- **生效还须挂在【批次接受时点】上,堵住历史 slot 回填（评审 r39,独立审核第 2 轮发现 5）**：
  ...
  生效后落地的候选里 → 必被拒;生效前已在某候选里落地的块 → 保留。**代价（如实）**:该 signer
  生效前已签、生效时**尚未落进任何候选**的在途块被一并拒绝——被罚没者（双签者）的未落块作废
  是可接受的附带后果,不误伤任何其他构建者;不需要 `δ_land` 宽限参数（删除）。
...
- **候选（candidate）与窗口（settlement window）**：设当前**窗口最终头**（window-final head）
  为 `F`。任何人可向 L1 提交**延伸 `F` 的候选批次** =（连续签名链 + 块体 + 有效性证明,同
  §6.1 格式）——landing 本就无权威（§0）,候选提交同样 permissionless。**首个**延伸 `F` 的
  合法候选被接受时,`F` 的**结算窗口**开启,长度 `W_settle`（L1 块计,确定性收盘）。窗口内,
  任何**按 §5.2 全序严格更重**的合法候选可**取代**（supersede）当前最优候选（L1 入口机械
  比较:lane 位 + 块数 + tip slot + 哈希,权重由证明的公共输入担保）。**窗口收盘时,当前最优
  候选即最终**（window-final）,其 tip 成为新的 `F'`,下一窗口从 `F'` 重新开始。
...
3. **落地参数**：`Δ_lag`、`m_agg`、`δ_slash`、`H_force`、兜底报酬的指数成本上限；
   ...
   **结算窗口参数（r41 option C,§5.6）**：`W_settle`（结算窗口长度,L1 块计;设置器不变量
   `W_settle ≥ P_prove,max + T_include,max + 余量`,初始建议 ≈ 100 L1 slot ≈ 20 分钟）;窗口
...
18. **形式化规范补强**（评审 r36，DeepSeek/Codex 建议；r39 补挑战/gas）：(a) 一份块合法性 /
    父块选择 / **最优链全序（§5.2）与结算窗口（§5.6 候选取代/收盘）** / 落地 `acceptBatch` / 强制包含 / **窗口
