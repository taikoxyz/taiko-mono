# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范，草案 v1.23）
...
2. `parent.slot < slot` 且 `slot − parent.slot ≤ G_max`（§4.2 的缺口上限，r6-1；
   ...
   且其 `slot` **≥ episode 执行基准 slot `s_base`**（新鲜度地板，r23-1），
   r9-1/r9-3），`parent_hash` 指向一个合法块（重锚块则指向 §6.4 **公告钉定的父块**
   ——它经重钉后在执行时刻恰等于当前已落地 L1 链头，二者一致，r22-2）；
...
- **执行 + 新鲜度地板（评审 r23-1：一次重锚必须把头追到墙钟，不能落一个陈旧块）**：
  窗口期满（含延长）且期间**未触发取消 (a)/(b)**，L1 入口核对：(1) 钉定父块（经
  重钉后的当前值）仍为当前落地头；(2) **重锚块的 `slot` ≥ 本 episode 的执行基准
  slot `s_base`**（= 窗口期满时刻的 `L2_slot`，episode 在 L1 上唯一钉定），二者
  皆满足重锚块方可落地，押金退还公告者。**为什么要新鲜度地板**：通用落地规则
  （r5-2）只拒未来 slot，不管多旧；若无此地板，合谋公告者 + 排班构建者可以落一个
  slot 只比钉定父块多 `G_max + 1`、却远落后墙钟的**陈旧**重锚块——头只前移了
  `G_max`，仍远离墙钟，当轮构建者照样接不上（父距 > `G_max`），攻击者遂用预签
  稀疏链反复赢下一个个公告、把恢复无限拖长，而非规范承诺的**一次性追平**。地板
  把重锚块钉在 `s_base` 之上（≈ 墙钟），使一次重锚就把头追到墙钟，与 §7 仅强制块
  首块跳到 `s_base`（r17-1）**统一**——两条重锚路径都以 episode 钉定的 `s_base`
  为落点，非落地者自由裁量（r7-5 的字节唯一性与"无时间跳跃自由"保持）。执行没有
  截止时刻——窗口是下限不是上限，证明未就绪可以晚落（`s_base` 由窗口期满的墙钟
  时刻决定、届时钉死，晚落不改它）；期间一旦触发取消则转取消分支，一旦被冲突叉
  推进则重钉后继续——**重钉只前移钉定父块指针、不重置窗口计时，故窗口期满时刻与
  `s_base` 跨重钉不变**（这正是"追平目标不被滴灌重置"的另一面）。
...
- **episode 状态机（评审 r22-建议——把散落的规则收成一张转移表，降低实现歧义）**：
  ...
  | Open | 窗口期满且未触发取消，`P` 仍为当前落地头 | 排班构建者对 `P` 出重锚块（`slot ≥ s_base` 新鲜度地板，r23-1）并落地 → **Executed**：`B_ra` 退公告者；未命中挑战罚没 `B_ch` |
