# Slot-Chain：基于槽位签名链的预确认协议（v2 设计规范，草案 v1.12）
...
- **过错与罪证（L1 机械判定）**：**一次"计次"（strike）= 兜底窗口开启期间有一个
  ...
  观测 P2P。每次计次罚没聚合者活性保证金一档。**每个开窗期至多一次计次，且带
  宽限（评审 r5-4 的修复）**：计次以"开窗期"（`lag > Δ_lag` 的一段连续区间）为
  单位——同一开窗期内无论落了多少个兜底批次都只计一次（拆分小批次刷计次无效），
  且仅当兜底批次落地时窗口已连续开启 ≥ `G_strike`（初始 1 epoch）才计次，给诚实
  聚合者从瞬时故障（证明失败重试、L1 拥堵）中恢复的余地。剩余的武器化路径只有
  "在 L1 层持续审查聚合者的落地交易、再抢落兜底批次"——那需要对一笔可无限加价的
  交易维持 `Δ_lag + G_strike`（≈ 5 epoch ≈ 160 个 L1 slot）的持续审查，是 v15
  §11.5 定价过的最贵审查类别，且要凑满 `m_agg` 个**独立**开窗期才能逼走席位；
  该残余明码接受。
- **迟到落地本身也计费——堵住"骑线"策略（评审 r3-1，Codex）**：只按"兜底批次被
  ...
  其一：迟到计数与兜底 strike 同样**按开窗期去重**（一个开窗期至多计一次）。其二：
...
- **补签优先（秒级路径，本流程只是它的兜底）**：缺口内已排班的构建者始终可以事后
  ...
  "缺勤者再也不回来"的灾难情形。
- **两类情形的走线核算（r12-1 后重算）**：
  - **真实大缺口**（> `G_max` 连续缺勤且无人补签）：故障前尾巴会被落干净，无人能
...
- **恢复时间的四分法（评审 r12-5——"强制路径恢复"与"系统恢复"是不同的界，分列
  ...
  3. **lag 追平墙钟**：仅强制链**有意不追**（`slot = parent.slot + 1` 正是 r7-5
     防时间跳跃的设计），追平由重锚块的 slot 跳跃在其落地瞬间一次完成——所以全员
     卡特尔期间 lag 持续超限、兜底窗口保持开启是**预期行为**，不是缺陷；
...
- **非法条目的准入与弃置消费（评审 r9-2，Codex——P1）**：v1.8 的规则对一条畸形或
  ...
  1. **入队验证（L1 合约执行）**：载荷已改为 calldata 随交易提交（本节首条），合约
     ...
     签名可恢复、`chainId` 正确、条目 gas 上限不超过 `C_force` 的单条目份额。不过
     关即拒绝入队。桥接队列条目由桥合约构造，格式良好由构造保证。
...
- **全体串谋的底线：仅强制块（forced-only block），双侧条件版**（评审 r1-3 后收紧；
  ...
  **仅强制块的规范格式（r4-3 制定；评审 r7-4/r7-5 修正）**：
  ...
  - **slot 唯一（r7-5：此前允许"填补缺口或追加"且 slot 不唯一——历史缺口在哈希链
    ...
    而状态不同，"gas 竞赛"论断不成立）**：仅强制块只能**追加**，且
    `slot = parent.slot + 1`（唯一确定）；其 anchor 引用 = 批次队列快照所在的同一
    L1 高度（批次级确定）。由此同一 (parent, 队列快照) 下候选块**除 `coinbase` 外
    字节唯一**，gas 竞赛论断恢复成立；每块只推进链头 1 个 slot，无时间跳跃可言。
    （§7 条件 3 的"填补缺口"措辞相应作废，以本条为准。）
...
| 连续多个构建者（缺口 ≤ `G_max`） | 若干秒无新块 | 同上；缺勤者事后补签亦可桥接（§6.4"补签优先"） | ~掉线人数 × 1 秒 |
| 连续缺口 > `G_max` 且缺勤者不回归（灾难级） | 分钟级无新块 | 重锚流程（§6.4）：公告 + 窗口 + 对钉定父块的证明，需 ≥1 诚实排班构建者签名（r12-2 概率界） | ≈ `Δ_lag` + max(`Δ_ra`, 一轮证明) ≈ 自故障起 36–41 分钟（r12-1 重算；虚假挑战 +16 分钟但赔 `B_ch`） |
...
| 重锚流程 | re-anchor episode | 公告 + 存在挑战窗口 + lag 追平即取消的两步恢复通道（§6.4） |
