...
| Fee market | Activate the §3/§5 schedule via `TaikoChainConfig.BlobScheduleConfig` (the field already exists, currently carrying inert L1 defaults — replace with Taiko entries attached to the **Osaka** schedule key, the fork slot Unzen maps the EVM to; the geth fork enum has no Taiko IDs, so entries keyed anywhere else never activate). Blob fees burn at launch per §5, so the launch STF carries no bespoke credit path | `taiko-geth/params/taiko_config.go:81-87` |
