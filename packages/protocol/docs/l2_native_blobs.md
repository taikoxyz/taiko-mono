...
`BLOBBASEFEE` returns the real lane price from the fork onward (it currently returns 1 wei, `excessBlobGas = 0`). Reference test vectors for `calc_excess_blob_gas` — floor and non-floor branches at zero, target, and max usage, plus the fork-activation boundary (§4.2), the sub-target early-return corner (both regimes), and the sub-threshold edge where the execution base fee is below `GAS_PER_BLOB / BLOB_BASE_COST = 16 wei` — MUST accompany the implementation.
