...
Launch containment, split by path: (a) **Preconf gossip** — §6.3 validation mechanically refuses sidecar-less type-3 envelopes, so nothing sidecar-less propagates or preconf-imports. (b) **Direct-to-L1 proposal** — not mechanically prevented, but only the bonded, permissioned role can take it: the on-chain preconf whitelist is also the inbox's proposer checker, so any such proposal is attributable, never anonymous. (c) **Forced inclusion** — excludes type-3 entirely (§4.3.3), so no permissionless path exists. A sidecar-less type-3 transaction in a canonical block therefore implies a misbehaving bonded operator — the same attributable, slashable fault class as §12.1 withholding — and the attacker pays the very lane price they are pumping, which §5's exponential market makes increasingly expensive to sustain. §4.3.4's tolerance of sidecar-less envelopes is defense-in-depth for derivation, not a user-reachable path.
