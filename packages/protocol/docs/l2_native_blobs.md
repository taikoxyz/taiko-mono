...
- **Code references**: `file:line` citations refer to taiko-mono `8843a78` and the taiko-geth revision pinned by the root `go.mod` replace directive (`go.mod:308`, `github.com/taikoxyz/taiko-geth v1.18.1-0.20260715122422-ca164c06e00c`); re-verify before implementation
...
| `BLOB_BASE_FEE_UPDATE_FRACTION` | `2_225_331` | realized ×1.12502 per full block at sustained max (rounded-ln derivation, §5) — deliberately the responsiveness of L1's *original Cancun* market; current L1 (since EIP-7691's re-derivation) runs ≈×1.0817 per block |
...
3. **Type-3 transactions are not force-includable at launch.** A type-3 transaction inside a forced-inclusion source is skipped **individually** at derivation/sealing — like any other per-transaction validity failure — leaving the source's remaining transactions untouched — with the standard nonce caveat: the *same sender's* subsequent transactions in that source now have gapped nonces and skip under ordinary nonce rules (as for any skipped transaction); **other senders are unaffected**, which is the blast-radius guarantee this rule actually makes. This is not a new semantic: today's sealer already drops prohibited or invalid transactions one at a time, never source-fatally, including type-3 specifically (`taiko-geth/miner/taiko_worker.go:277-281`), so V7 carries existing behavior forward, and the skip is part of the proven state transition — provers replay the same deterministic selection rule; source-level default-manifest replacement stays reserved for framing/decode failures (Derivation.md). The narrow blast radius matters: one prohibited transaction never censors an otherwise-valid forced source. Rationale for the restriction itself: forced inclusion exists for censorship resistance, which for blob bodies would require L1-posted data (design A); guaranteeing it is deferred (§15). It MUST be documented prominently to users: if all preconfers censor blob transactions, there is no forced path for them — and RPC providers and wallets SHOULD refuse to construct forced-inclusion submissions containing type-3 transactions, so no user pays the L1 forced-inclusion fee for a transaction that can never execute. A forced source consisting only of type-3 transactions derives to an anchor-only block: forced-inclusion satisfaction is the L1-side queue consumption, independent of execution outcomes — exactly as for a forced source full of otherwise-invalid transactions today — and this case belongs in the derivation test vectors. Provenance is deterministic, not inferred: `DerivationSource.isForcedInclusion` is committed on-chain inside the proposal (`IInbox.sol:52-57`), each forced source derives its own block, and the skip rule is scoped exactly to blocks derived from sources with `isForcedInclusion = true` — the sealer and prover replay the same flag from the same committed data. Derivation.md MUST be updated to state this per-transaction rule explicitly — a phase-1 deliverable (§13), since the censorship exception is user-visible and must not surprise anyone at launch.
4. Blocks derived without preconfirmation (L1-only sync) execute type-3 transactions normally using the envelope hashes — **in non-forced sources; forced sources follow §4.3.3's per-transaction skip rule regardless of sync path** (explicit precedence: rule 3 is the special case, this rule the general one). The local sidecar store records a gap for later backfill (§7.2). This includes adversarial proposals that bypassed preconf validation entirely: sidecar-less type-3 envelopes execute normally — availability suffers, derivation never does (abuse of this tolerance is analyzed in §12.5).
...
- **Reserve floor — exactly [EIP-7918](https://eips.ethereum.org/EIPS/eip-7918#specification)'s mechanism (Final; Specification section deep-linked) as deployed on L1 in Fusaka (mainnet, Dec 3 2025)**, enforced through the excess update, never as a `max()` clamp on the fee — an **anti-decay floor on the price path, not an instantaneous lower bound**: the current price can sit below it until usage accumulates excess (§4.2). `BLOB_BASE_COST = 8192` and the mechanism itself are the deployed L1 values; `BLOB_BASE_FEE_UPDATE_FRACTION = 2_225_331` is deliberately **Taiko-specific** — derived as `⌊(MAX_BLOBS_PER_BLOCK − TARGET_BLOBS_PER_BLOCK) × GAS_PER_BLOB / 0.1178⌋` — the parenthesized difference is a blob **count** (`3 − 1 = 2`), converted to gas by the `× GAS_PER_BLOB` factor (`2 × 131_072 = 262_144`) — with `ln(1.125) ≈ 0.117783` rounded to four decimals (L1's own fractions are likewise nearby integers rather than exact roundings), giving a realized max growth of ≈ ×1.12502 per full block. This is deliberately the responsiveness of L1's **original Cancun** market: since Pectra's EIP-7691 re-derivation, L1's own fractions yield only ≈×1.0817 per full block — every post-7691 fraction is re-derived to hold `(max−target)_gas / fraction ≈ 0.0785` across parameter changes (Pectra: 393,216 / 5,007,716; post-BPO2: 917,504 / 11,684,671; mixing one era's delta with another's fraction gives wrong figures) — and Taiko chooses the faster Cancun-era rate because its ~2 s cadence and small max−target delta warrant quicker reaction. **The integer constant `2_225_331` is the normative value; the derivation is provenance only** — no client may re-derive it from the floating expression, and copying any L1 fraction here would be wrong, and vice versa. Normatively:

  ```python
  def calc_excess_blob_gas(parent):
      # EIP-7918 structure, quoted from the Final EIP: the sub-target early return
      # comes FIRST — before the reserve check — so it applies in both regimes
      if parent.excess_blob_gas + parent.blob_gas_used < TARGET_BLOB_GAS_PER_BLOCK:
          return 0
      # reserve condition; per-gas equivalent: blob_base_fee < base_fee_per_gas / 16
      if BLOB_BASE_COST * parent.base_fee_per_gas > GAS_PER_BLOB * fake_exponential(MIN_BLOB_BASE_FEE, parent.excess_blob_gas, BLOB_BASE_FEE_UPDATE_FRACTION):
          # below the floor: excess grows with usage; a full block adds exactly MAX − TARGET
  ...
  The pseudocode quotes the Final EIP's Specification structure, snake_case field names included — they correspond one-to-one to §4.2's camelCase header fields (`excess_blob_gas` ↔ `excessBlobGas`, `blob_gas_used` ↔ `blobGasUsed`). Note the leading early return is load-bearing beyond replacing a `max(0, …)`: because it also precedes the reserve branch, **sub-target residual excess flushes to zero even below the floor** — a reachable corner in the floor regime (`baseFeePerGas > 16 wei`, always true on Taiko in practice: one exactly-target block from empty excess lands at `87_381 < TARGET`; a following zero-usage block resets it; at `baseFeePerGas ≤ 16 wei` the same block takes the normal branch and returns 0 directly), and an else-branch-only `max()` formulation would diverge from the EIP there. `BLOB_BASE_COST = 2**13`; the deployed EIP contains **no** `max()` clamp on the price function — the floor acts solely through this update rule, active exactly while `blob_base_fee < base_fee_per_gas / 16` in per-gas terms. The strict `>` is also the EIP's: a parent priced exactly at the floor with zero blob usage takes the normal branch once, decaying a single step (≈5.7% at Taiko's fraction) below the floor before the reserve branch engages and holds — accepted, deployed-L1-exact behavior, and a required test-vector case. Arithmetic widths are L1-identical and MUST NOT be narrowed: `excess_blob_gas` and `blob_gas_used` are unsigned 64-bit header fields, but the reserve comparison and `fake_exponential` operate on fee values (`base_fee_per_gas`, `blob_base_fee`) that are 256-bit on L1 — evaluating `BLOB_BASE_COST × base_fee_per_gas` or `GAS_PER_BLOB × blob_base_fee` in 64 bits can overflow under fee spirals and diverge from L1. The entire computation runs in full-width (unbounded/256-bit) integers exactly as the EIP's Python does, with the result stored as a uint64 header field; overflow of the stored value is economically unreachable, and any implementation MUST reproduce L1's semantics identically rather than invent saturation. `//` is floor division. Below the floor, excess at or above one target's worth never decays: it **holds** at zero usage and rises gently with usage (scaled by `(MAX − TARGET)/MAX`); sub-target residual excess instead flushes to zero via the early return — EIP-7918's intended behavior, not an omission. Above the floor the standard update applies unchanged. With Taiko's small execution base fee this floor is intentionally low — the product is *cheap*; spam protection comes from the exponential lane and the per-block cap (§12.2), not the floor.
- `BLOB_BASE_FEE_UPDATE_FRACTION = 2_225_331` gives ≈ ×1.125 per fully-utilized block — Cancun-era L1 responsiveness (current L1 runs ≈×1.0817, see above). Taiko blocks are also ~6× more frequent than L1 slots, so in wall-clock the market reacts far faster; this is deliberate (faster spam shutdown, §12.2). `fake_exponential` is EIP-4844's integer approximation of `MIN_BLOB_BASE_FEE × e^(excess/fraction)`, defined in that EIP.
...
`BLOBBASEFEE` returns the real lane price from the fork onward (it currently returns 1 wei, `excessBlobGas = 0`). Reference test vectors for `calc_excess_blob_gas` — floor and non-floor branches at zero, target, and max usage, plus the fork-activation boundary (§4.2), the sub-target early-return corner (both regimes), and the sub-threshold edges where the execution base fee is below — and exactly at — `GAS_PER_BLOB / BLOB_BASE_COST = 16 wei` (strict `>` makes equality take the normal branch) — MUST accompany the implementation.
...
2. **Selection.** The sealer includes type-3 envelopes like any transaction (removing the current skip at `taiko-geth/miner/taiko_worker.go:277-281`), subject to §4 limits. The driver pulls sidecars for included hashes from the local pool via `engine_getBlobsV2` (already implemented upstream in taiko-geth) — at selection time per the rule below, or at the latest immediately after payload build. `getBlobsV2` is all-or-nothing — null if any requested hash is unavailable — so on a null response the builder first checks the Osaka fork gate (§below) and raises a critical alert if it is unexpectedly unsatisfied (continuous monitoring, not only the one-time phase-2 check), then MUST NOT publish the envelope: it identifies the gaps (via `engine_getBlobsV3` partial responses — present in the pinned taiko-geth, `eth/catalyst/api.go:721` — or per-hash pool queries), rebuilds the payload without the affected type-3 transactions, and retries (bounded, with backoff). To eliminate the eviction window entirely, the builder SHOULD retrieve and persist sidecars **at selection time** — atomically with choosing to include a type-3 transaction, not after payload build — so pool eviction between selection and retrieval cannot occur; `taiko_blob_builder_dropped_total` MUST count drops so a starvation loop can never look healthy. Both engine methods short-circuit to null unless the execution client reports `LatestFork(head) ≥ Osaka` — the catalyst gate knows nothing of Taiko fork names — and `OsakaTime` has equaled the **Unzen** activation since August 2026 (`core/taiko_genesis.go`), so the gate is already satisfied today; V7 introduces no new EVM fork ID and MUST simply not regress that mapping — if it ever did, `getBlobsV2` would return null forever and the rebuild loop would silently drop every type-3 transaction while looking exactly like pool eviction. Devnet checklist: assert the client reports Osaka-level fork ID at V7, and that `engine_getBlobsV2` (all-or-nothing) and `engine_getBlobsV3` (partials) behave as specified on every execution client in use — a hard phase-2 gate alongside the §4.4 parity vectors. A builder can never legitimately publish an envelope it cannot fully equip with sidecars (§6.3), and it MUST verify the one-validated-sidecar-per-hash-occurrence invariant itself before publishing — never relying on the engine API's all-or-nothing contract, which is an upstream implementation detail that may differ in taiko-reth or future forks. Sidecar retrieval MUST complete — and the sidecars MUST be persisted to the builder's own store — before the block is imported or announced; blobpool retention after import (eviction and limbo behavior) is client-specific and MUST NOT be relied upon. The same ordering extends to L1 submission: a proposer MUST NOT broadcast an L1 proposal whose txlist contains type-3 envelopes until the corresponding sidecars are durably persisted (and, on the honest path, already propagated via preconf gossip) — closing the crash window between preconf announcement and L1 confirmation; a direct-to-L1 proposal carrying a sidecar-less type-3 transaction is a required phase-2 derivation vector for the governance-grade behavior (§12.5).
3. **Preconf propagation.** The preconfirmation envelope gains a `blobSidecars` field. Receiving nodes MUST verify, for each type-3 transaction, that sidecars are present, ordered, that `versioned_hash(commitment) == tx.blobVersionedHashes[i]` — the exact check the driver already runs against L1 blob servers (`packages/taiko-client/pkg/rpc/blob_datasource.go:210-218`) — and that the blob **bytes** are bound and the supplied proofs valid, via **one mandatory predicate for every accepting node**: compute the 128 cells **from the received blob** and batch-verify the sidecar's proofs against the commitment at those cells' indices (the L1 pool-ingress check; comparable cost to a bare commitment re-derivation). A single uniform predicate means no two honest nodes can disagree on the same envelope — a sidecar with a valid blob but corrupted proofs is rejected by everyone, and since the digest signs those proof bytes, attributably so. Commitment re-derivation alone remains the correct verification only for HTTP/backfill retrieval (§7.2), where proofs are recomputable rather than trusted. Verifying supplied cells without recomputing them from the blob binds nothing and MUST NOT be accepted. Sidecars entering the store from backfill (§7.2) rather than preconf validation carry unverified proofs: a node MUST batch-verify or recompute them from the blob before serving them, and the §7.2 API MUST NOT return unverified `proofs`. Duplicate versioned hashes — within one transaction or across a block's transactions — are legal under EIP-4844: the envelope carries one sidecar entry per hash occurrence, ordered by `(txIndex, blobIndex)` in the final payload (entries for duplicates are byte-identical), stores dedupe **bodies** by hash — per-occurrence order needs no separate storage, because the block's own transaction envelopes reconstruct it at serving time (the bulk endpoint joins envelopes against the body store, so `DIG-3`-style duplicate cases survive aggressive dedup) — and `engine_getBlobsV2` returns the same sidecar at each requested position. An envelope failing this MUST NOT be propagated or imported as a preconfirmed block, and validation stops at the first failure. The cost is bounded and budgeted: commitment re-derivation or batched cell verification runs a few milliseconds per blob with c-kzg/blst-class libraries — ≤ ~15 ms for a max-full block, two orders of magnitude under the ~2 s cadence — so validation cannot become a propagation choke point even at sustained max throughput. The envelope signature MUST commit to the `blobSidecars` via a fixed digest — normatively: `sha256(concat(sha256(blob_i ‖ commitment_i ‖ proofs_i) for each occurrence i, in envelope order))`, where each field is its raw fixed-length byte form — `blob_i` 131,072 bytes, `commitment_i` 48 bytes, `proofs_i` the 128 proofs concatenated in cell-index order (6,144 bytes; each proof its canonical 48-byte compressed BLS12-381 G1 encoding, exactly as in EIP-4844) — plain concatenation with no length prefixes (all lengths are fixed), and `sha256(b"")` (= `e3b0c442…7852b855`) when no type-3 txs are present. Envelope sidecars are purely positional — ordered by `(txIndex, blobIndex)`, each record exactly `{blob, commitment, proofs}` with no index/version metadata fields in the wire encoding — so the digest covers the entire sidecar content; a byte-level test vector is mandatory (Appendix C) — the signature covers the envelope body including the digest field, while sidecar bytes travel alongside and are validated against the signed digest. To be precise about what this buys: a relayer can still physically strip sidecars in transit (peers behind it refuse the envelope), but stripping becomes **detectable, and never blamed on the operator** — the signed digest proves the operator committed to specific sidecars, peers can re-fetch them from any honest path by digest, and a withholding dispute is decided on evidence rather than he-said-she-said. The §15.4 attestation design builds on this signed artifact. This extends `ValidateExecutionPayload` (`packages/taiko-client/driver/preconf_blocks/server.go:923-962`) and the Rust ingress validation.
...
Worst-case added preconf bandwidth: `MAX_BLOBS_PER_BLOCK × (BLOB_BYTES + 128×48 B proofs + 48 B commitment) ≈ 402 KiB` per ~2s block ≈ 1.65 Mbit/s — negligible for operator-grade preconfers. The preconf transport's maximum message size MUST admit the worst-case envelope (payload + ≈402 KiB of sidecars); confirming or raising that limit is a phase-2 item.

## 7. Availability guarantee and retention

### 7.1 Guarantee tier at launch: bonded-operator custody

The availability guarantee is: **a preconfer MUST NOT include a type-3 transaction whose sidecars it does not possess, and MUST retain and serve every sidecar it includes for at least `BLOB_RETENTION_SECONDS_MIN`.** Because preconf P2P validation (§6.3) refuses envelopes without sidecars, every honest node that preconf-imported the block also holds the bodies — custody is the natural by-product of validation. Every honestly-built block's transactions enter through this path at launch; the only deviation is §12.5(b)'s direct-to-L1 route, available solely to the bonded role and always attributable. No *user* can produce a sidecar-less inclusion.
...
- **Requirement (this design's headline parameter): the network MUST retain and serve blob bodies for at least `BLOB_RETENTION_SECONDS_MIN = 6_291_456 s` (≈ 72.8 days).** The absolute seconds are the normative value; the 4×-Ethereum's-window relationship (Appendix A) is provenance as of 2026 and does not track future L1 changes. Taiko out-retains L1 by design; retention length is a product feature.
- Node roles:
  - **Serving nodes** (preconfers MUST; public RPC providers SHOULD): retain ≥ the minimum window (default config 90 days) and expose a **publicly reachable** serving API (or formally delegate serving to a public provider with equivalent retention — a private-only endpoint does not discharge the obligation), and SHOULD export retention/serving metrics so §7.1 under-retention is externally observable — at minimum `taiko_blob_store_oldest_age_seconds` (retention horizon), `taiko_blob_store_backfill_gaps` (missing bodies inside the window), and `taiko_blob_served_total{endpoint}`.
...
- **Serving API** (mirrors the interface the driver already consumes from L1 blob servers, so client code is reused):
  - `GET /blobs/{versionedHash}` → `{ versionedHash, commitment, data }` — byte-for-byte the schema the existing client parser accepts (`BlobServerResponse`, `packages/taiko-client/pkg/rpc/blob_datasource.go:41-44`), so today's fetch/backfill code works unchanged. Clients MUST re-verify `data` against `commitment` and `versionedHash` on every response (the existing parser already does exactly this) — serving nodes are untrusted by construction. Additive optional fields: `proofs` (the 128 cell proofs) and, under the IPFS profile, `blobCID` and `bundleRoots` — an array, since a versioned hash may occur in multiple ranges; servers MAY return any non-empty subset of containing range roots, with `sealLogRoots` aligned by index (§7.4) — a range root **identifies and verifies** the bundle, but DHT **location** needs the matching seal-log root, since only seal-log nodes carry provider records (§7.4.1); either root plus the `blob_bundles` endpoint reaches the data. Consumers MUST ignore unknown fields. Exact types: `versionedHash` 0x-prefixed 32-byte hex; `commitment` 0x-prefixed 48-byte hex; `data` 0x-prefixed 131,072-byte hex; `proofs` exactly 128 0x-prefixed 48-byte hex strings; `blobCID` a CIDv1 string, `bundleRoots` and `sealLogRoots` index-aligned arrays of CIDv1 strings. A binary response (`Accept: application/octet-stream`, blob bytes only) SHOULD also be offered — 0x-hex JSON doubles bandwidth for bulk consumers. Error semantics: `404` for a hash never seen, `410 Gone` for a blob pruned after its retention window, and `503` with `Retry-After` for a body missing *inside* the window (a §7.2 backfill gap in progress) — so clients can distinguish "retry here later" from "gone" and "unknown". Correct classification requires memory: serving nodes MUST retain a permanent index of every versioned hash they have seen, with prune timestamps (≈15.8M hashes/year at full target and `ASSUMED_BLOCK_CADENCE`: ≈505 MB/year at 32 B/hash, ≈630 MB with 8-byte prune timestamps — still trivial next to §12.2's body volumes, and it rides with the bundle metadata that already outlives bodies); a node without such an index MUST NOT return `404` and returns `503`/`410` on best knowledge instead.
  - `GET /taiko/v1/blob_sidecars/{l2BlockNumber}` → `{ "data": [ <objects with the same schema as above> ] }`, mirroring the beacon `blob_sidecars` envelope shape (bulk sync/backfill); returns one record per occurrence in envelope order — duplicates repeated, never deduplicated
  - **Bundle metadata outlives blobs.** Serving and archiver nodes SHOULD retain sealed-bundle metadata (`blockRange → rangeRoot/sealLogRoot`) indefinitely — ≈5 MB/day at launch target, negligible against §12.2 — independent of body retention, and `GET /taiko/v1/blob_bundles` MUST keep answering for pruned ranges from this metadata (bodies return `410`; metadata never expires). The blob-indexer service is the designated long-term root-discovery surface; without one of these, post-window IPFS retrieval has no bootstrap (§7.4.1).
...
- **Serving-endpoint discovery.** "Serving peers" are not left implicit: each whitelisted preconfer registers its public serving URL as part of the operator agreement, the canonical list ships in client releases (overridable via `--taiko.blobs.peers`), and preconf P2P peers MAY additionally be queried. Inside the window, a compliant operator's bodies are therefore always *locatable*, not merely existent.
- **Backfill.** A node that imported blocks from L1 derivation without bodies (preconf outage, fresh sync inside the window) SHOULD backfill its store from the discovered serving endpoints via the bulk endpoint. Gaps beyond the window are expected and harmless.
...
| Bundle boundary | `blockNumber / IPFS_BUNDLE_BLOCKS`; `blockRange` is `[start, end]` inclusive on both ends | fixed ranges, so independent archivers produce **identical range roots** |
...
| Empty ranges | a range containing **zero** blobs is still sealed (empty `index`) | keeps sealing total and convergent, so `skippedRanges` means only "incomplete", never "empty" |
| Range root (contents node) | dag-cbor node | `{version, chainId, blockRange:[start,end], index:[{versionedHash, commitment, blobCID(link)}...]}` — a pure function of the finalized chain **plus its blob bodies, which the chain uniquely determines** (KZG commitments are binding, so any complete verified body set yields the same bytes); since sealing requires a complete verified set, **any two sealed roots for the same range are bit-identical**. Scope: a range root authenticates the ordered blob content of a `(chainId, blockRange)` — chain-level audit (which blocks contained what) uses the chain itself, not roots alone |
...
- **Announcement.** Only seal-log roots receive DHT/IPNI provider records (Kubo `Reprovider.Strategy = "roots"`); range roots are reachable through them, and individual blob CIDs are never announced. Fetching one blob over IPFS means: obtain the range root (from a serving node's `GET /taiko/v1/blob_bundles?range=…`, an indexer, or via the DHT once a seal-log root CID is already known from the HTTP endpoint, an indexer, or a prior bundle's `prev` chain — the DHT locates providers for known CIDs, it is not a discovery bootstrap: locate the seal-log root's providers, fetch the seal-log node, follow `contents` to the range root) → read the index → Bitswap the raw leaf from those same connected providers (leaf CIDs are never individually announced). Latency is accordingly worse than the HTTP API — acceptable for archival, wrong for the hot path.

#### 7.4.2 Kubo sidecar: put-on-receipt, pin-on-seal
...
Interfaces touched: the profile is entirely additive to §9's sidecar store (one new worker + two HTTP endpoints: `GET /taiko/v1/blob_bundles?range=…` returning `{ "data": [ { "blockRange": [start, end], "rangeRoot": CIDv1, "sealLogRoot": CIDv1 } ] }` (block ranges inclusive on both ends; responses paginated, ≤ 256 entries per page) for sealed bundles intersecting the queried range, and inclusion of `blobCID`/`bundleRoots` in the existing `GET /blobs/{versionedHash}` response metadata). No consensus rule, no manifest field, and no contract references IPFS in any way.
...
| Pool (pre-V7 hygiene) | **Immediately** (independent of this design): reject type-3 at the pool while `!IsV7`, with the normative message `blob transactions are not supported on this network until the V7 fork` — the error code follows the client's standard transaction-rejection class (geth's `-32000` family); the message string is normative so wallets and tooling can distinguish "not yet supported" from "invalid transaction". Post-Unzen the blobpool is Cancun-active and `Pending()` is called without `OnlyPlainTxs` (`taiko_worker.go:74-79, 377`), so a blob tx can today enter a proposed txlist only to be dropped by every sealer — the proposer pays L1 DA for dead bytes | `taiko-geth/core/txpool/validation.go` (taiko gate) |
...
| Fee market | Activate the §3/§5 schedule via `TaikoChainConfig.BlobScheduleConfig` (the field already exists, currently carrying inert L1 defaults — replace with Taiko entries attached to the **Osaka** schedule key, the fork slot Unzen maps the EVM to; the geth fork enum has no Taiko IDs, so entries keyed anywhere else never activate). Because Osaka is already active, the Taiko schedule is technically live from the moment it ships — inert pre-V7 only because no type-3 transaction can enter a payload (sealer skip + pool gate), which keeps `blobGasUsed = 0` and the driver's `== 0` pins true; any pre-V7 path that populated blob gas would break those pins, which is exactly what they assert — and phase-2 CI MUST exercise the pins with the schedule shipped. Blob fees burn at launch per §5, so the launch STF carries no bespoke credit path | `taiko-geth/params/taiko_config.go:81-87` |
...
| Area | Change | Where |
| --- | --- | --- |
| Preconf envelope | Add `blobSidecars` behind a bumped envelope/codec version (Go + Rust), with the envelope signature committing to the sidecar digest (§6.3): V7-aware nodes accept both forms until the fork timestamp, after which sidecar-less envelopes containing type-3 txs are invalid; clients upgrade ahead of the fork via the normal preconf rollout path; validation per §6.3 | `packages/taiko-client/driver/preconf_blocks/server.go:923-962`; `packages/taiko-client-rs/crates/whitelist-preconfirmation-driver/src/importer/{ingress,validation}.rs`, `codec.rs` |
...
Launch containment, split by path: (a) **Preconf gossip** — §6.3 validation mechanically refuses sidecar-less type-3 envelopes, so nothing sidecar-less propagates or preconf-imports. (b) **Direct-to-L1 proposal** — not mechanically prevented, but only the bonded, permissioned role can take it: the preconf whitelist contract itself implements the inbox's proposer gate (`PreconfWhitelist is … IProposerChecker`, `contracts/layer1/preconf/impl/PreconfWhitelist.sol:14,127`, enforced on every proposal at `contracts/layer1/core/impl/Inbox.sol:603`), so any such proposal is attributable, never anonymous. (c) **Forced inclusion** — excludes type-3 entirely (§4.3.3), so no permissionless path exists. A sidecar-less type-3 transaction in a canonical block therefore implies a misbehaving bonded operator — the same attributable, slashable fault class as §12.1 withholding — and the attacker pays the very lane price they are pumping, which §5's exponential market makes increasingly expensive to sustain. §4.3.4's tolerance of sidecar-less envelopes is defense-in-depth for derivation, not a user-reachable path.
...
| 2 | Devnet: taiko-reth + Rust driver implementation; `--taiko.devnet-v7-time 0`; Nethermind chainspec flip; bandwidth/storage soak at max utilization plus validation-latency benchmarks against the §6.3 budget, run against pinned c-kzg-4844/blst versions so the gate is measurable | internal devnet |
| 3 | Hoodi testnet fork; explorer + blob-indexer integration; IPFS archival profile pilot (§7.4; entry-gated on the Kubo announce-set and no-GC CI checks passing against the pinned version); L3 pilot (an OP-stack or Taiko-stack L3 posting via L2 blobs) | Hoodi |
| 4 | Mainnet V7 activation; preconfer serving obligations added to operator agreements; explicit, public sign-off that committee-grade custody with governance-grade enforcement is acceptable for a user-facing DA product (§7.1, §15.5); launch precondition: the proposer set remains whitelisted until the §12.5 re-closure ships | governance |
...
Numbered items below are cross-referenced elsewhere as §15.N (e.g. §15.4 = item 4).
...
5. Post-launch blob-fee routing (§5 launches with the L1 burn): should a future fork credit the including preconfer, split across all serving nodes (serving is network-wide, inclusion is individual), or keep the burn? Any credit is a state-transition + prover-parity change and warrants its own fork. To be precise about the apparent tension with §5: the routing **decision** SHOULD be made before mainnet activation (phase 4) — deciding is a governance act — while the launch **implementation** is burn-only regardless of that decision; with the launch burn the serving obligation is compensated only implicitly.
6. RIP submission timing: standardize before or after mainnet ship.
7. IPFS profile (§7.4): accept ~2× disk from a duplicated Kubo blockstore (simple, robust) or pursue filestore/`--nocopy` dedup against the sidecar store (fragile historically)? Resolve before phase 3, since it changes §12.2's serving-node storage guidance. And should sealed bundle CARs feed an incentivized persistence tier (Filecoin deals consume CARs natively; EthStorage as an alternative)?
...
## Appendix C — mandatory test-vector checklist

Vector **values** are generated by the reference implementation and never hand-authored; this checklist defines the mandatory **case set** implementers check off. Each case cross-references its normative section.

| ID | Section | Case | Expected property |
| --- | --- | --- | --- |
| FEE-1..3 | §5 | normal branch at zero / target / max usage | standard EIP-4844 update |
| FEE-4..6 | §5 | floor branch at zero / target / max usage | hold at zero; `+ (used × (MAX−TARGET)) // MAX` otherwise |
| FEE-7 | §4.2, §5 | fork-activation boundary (pre-V7 parent, zero-pinned fields) | lane opens at `MIN_BLOB_BASE_FEE` |
| FEE-8..9 | §5 | sub-target early return, both regimes (incl. the `87_381`-flush corner, which presupposes the floor regime `baseFeePerGas > 16 wei`) | returns 0 before either branch |
| FEE-10 | §5 | exact-floor strict-`>` boundary, zero usage | one ≈5.7% decay step, then hold |
| FEE-11..12 | §5 | execution base fee `< 16 wei` and `== 16 wei` | equality takes the normal branch |
| FEE-13 | §3 | fraction constant | literal `2_225_331`, never re-derived |
| FEE-14 | §5 | `excess_blob_gas` near `2^64` (theoretical) | full-width computation, uint64 storage, L1-identical semantics |
| SEL-1..2 | §4.1 | over-cap txlists, multiple orderings; interleaved valid/invalid txs | greedy in-order skip; identical sealer/prover output |
| SEL-3 | §12.3 | zk-gas truncation interplay | blob gas and envelope sidecars from post-truncation payload only |
| FI-1 | §4.3.3 | mixed forced source containing type-3 | per-transaction skip, remainder executes |
| FI-2 | §4.3.3 | forced source of only type-3 txs | anchor-only block; L1 queue consumed |
| FI-3 | §4.3.3/§4.3.4 | precedence: forced source in L1-only sync | §4.3.3 skip wins |
| FI-4 | §12.5 | direct-to-L1 sidecar-less type-3 proposal | executes; availability gap recorded; attribution path |
| DIG-1 | §6.3 | byte-level sidecar digest | matches the normative sha256 construction |
| DIG-2 | §6.3 | no type-3 txs in envelope | digest = `sha256(b"")` |
| DIG-3 | §6.3, §12.3 | duplicate versioned hashes | one record per occurrence, `(txIndex, blobIndex)` order |
| ENV-1 | §9 | envelope codec version negotiation | both forms accepted until fork timestamp, sidecar-less type-3 envelopes invalid after |
| CBOR-1 | §7.4.1 | range-root fixture | bit-identical across implementations |
| CBOR-2 | §7.4.1 | multi-bundle seal-log chain with `prev` | raw-CIDv1 bytes ↔ CID string pair round-trips |
| CBOR-3 | §7.4.1 | malformed `prev` (wrong length, non-CID bytes) | rejected |
| CBOR-4 | §7.4.1 | non-empty `skippedRanges`; late-sealed recovered range | lineage forks tolerated; range roots unaffected |
| STF-1 | §4.4 | prover parity over all of the above | hard gate for the devnet phase |
