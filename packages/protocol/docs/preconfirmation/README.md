...
| [`comparative-analysis-report.html`](comparative-analysis-report.html) | Independent round-9 HTML review comparing v9 honestly with the deployed whitelist across accountability, liveness, decentralization, user guarantees, implementation complexity, economics, verification evidence, and migration risk. States explicitly that v9 is **not implementation-ready**, identifies seven blocking workstreams and their closure criteria, and recommends gated, reversible adoption rather than a one-step replacement. |
| [`simulation/`](simulation/) | (Deliverable 3) State-machine model checker of the v9 design + [results](simulation/RESULTS.md): exhaustive bounded exploration from a curated set of initial configurations — all safety invariants hold at every reachable state and transition, and no permanent halt exists **even under an adversarial proving outage that never lifts**, with an eleven-mutant self-test proving the checks have teeth. |
| [`slides/`](slides/) | Learning deck: a 22-slide PDF walking through the v9 design (motivation → mechanisms → economics → degradation → verification), for onboarding without reading the full proposal. HTML source + rendered PDF. |
