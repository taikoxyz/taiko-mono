...
| [`redesign-proposal.md`](redesign-proposal.md) | (Deliverable 2) Draft design proposal, now **v7**: perpetual auction with commit→publish→seal epochs — nine normative invariants, single-decision epoch state machine, deadline-only proof-carrying seals, L2-evidence slashing, split ETH/TAIKO bond in one seniority-waterfall account, degradation ladder, Total Anarchy with atomic FCFS content, and game-theory analysis with an attack catalog. Converged across six adversarial review rounds (ten reviewer passes) plus an owner-approved self-review simplification round (see the proposal header and Appendix A #24–28). |
| [`simulation/`](simulation/) | (Deliverable 3) State-machine model checker of the v7 design + [results](simulation/RESULTS.md): exhaustive bounded exploration from a curated set of initial configurations — all safety invariants hold at every reachable state and transition and no permanent halt exists across millions of states, with a mutation self-test proving the invariants have teeth. |
