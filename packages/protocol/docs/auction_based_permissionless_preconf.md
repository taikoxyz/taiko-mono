...
**Status:** Research report & design proposal — draft for review. Two design-review rounds are integrated: round 1 (adversarial: reorg-robust stall evidence, real escape-hatch enforcement, bond escrow + sizing) in §4.4/§4.5/§4.8, and round 2 (auction frequency, O(1) read model, refute-rule fix, payment model, hot-standby duty) in §4.3/§4.4/§4.9/§4.12.
...
- Perpetual list of bonded standing bids (≤ 16). Every transition has `effectiveEpoch = placedEpoch + 2`; `assign(E) ← top active entry` (lazily, once per epoch), `backup(E) ← second`; `unassigned` (`address(0)`) if none. Transitions: outbid → handover at effective epoch; quit → handover at effective epoch (must serve until then); expiry (`tenureMax`) → lapse + re-rank; `renew()` → extends expiry without re-ranking; slash/eject → immediate rung-1 takeover.
