...
The `Anchor` contract now persists the active proposal identifier and expected block index, enforcing these invariants directly on-chain. Clients and provers still need to supply correct metadata, but any deviation now results in the anchor transaction reverting.
