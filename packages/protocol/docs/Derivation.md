...
A default manifest is returned when any of the following validation criteria fail. The validation limits are defined in the `IDerivationConfigProvider.Config` struct returned by `getDerivationConfig()`:

- **Blob validation**: `blobHashes.length` is zero or exceeds `config.proposalMaxBlobs`
- **Offset validation**: `offset > BLOB_BYTES * blobHashes.length - 64`
- **Version validation**: Version number is not `0x1`
- **Size validation**: `size` exceeds `config.proposalMaxBytes`
- **Decompression failure**: ZLIB decompression fails
- **Decoding failure**: RLP decoding fails
- **Block count validation**: `manifest.blocks.length` exceeds `config.proposalMaxBlocks`
- **Transaction limit**: Any block contains more than `config.blockMaxRawTransactions` transactions
...
2. **Lower bound calculation**: `lowerBound = max(parent.metadata.timestamp + 1, proposal.timestamp - config.timestampMaxOffset)` (where `config` is from `IDerivationConfigProvider.getDerivationConfig()`)
...
- **Excessive lag**: `manifest.blocks[i].anchorBlockNumber < proposal.originBlockNumber - config.anchorMaxOffset` (where `config` is from `IDerivationConfigProvider.getDerivationConfig()`)
...
Gas limit adjustments are constrained by `config.maxBlockGasLimitChangePermyriad` permyriad (units of 1/10,000) per block to ensure economic stability. With the default value of 10 permyriad, this allows ±10 basis points (±0.1%) change per block. Additionally, block gas limit must never fall below `config.minBlockGasLimit` (where `config` is from `IDerivationConfigProvider.getDerivationConfig()`):

**Calculation process**:

1. **Define bounds**:

   - `lowerBound = max(parent.metadata.gasLimit * (10000 - config.maxBlockGasLimitChangePermyriad) / 10000, config.minBlockGasLimit)`
   - `upperBound = parent.metadata.gasLimit * (10000 + config.maxBlockGasLimitChangePermyriad) / 10000`
