...
- **The proposer** publishes blob data for their own `DerivationSourceManifest` and calls `Inbox.propose(...)` with a `blobReference`. If forced inclusions are due, the inbox dequeues up to a fixed number per proposal (currently 10) and appends the proposer's source **last**.
