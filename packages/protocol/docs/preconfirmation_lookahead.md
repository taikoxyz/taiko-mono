...
### 10.1 Liveness issue: first-slot posting requirement can brick proposals

`isLookaheadRequired()` returns `false` in the first slot of an epoch (`block.timestamp ==
epochTimestamp`). The intent (per comment) is to **not require** the next epoch lookahead to be
posted in that slot.

However, `checkProposer` always calls `_handleNextEpochLookahead`, which will attempt to post the
next epoch lookahead whenever it is missing. Posting calls `_updateLookahead`, which contains:

```solidity
require(isLookaheadRequired(), LookaheadNotRequired());
```

So in the first slot of an epoch, if the next epoch lookahead is missing (the expected/common
case), the proposer path will revert with `LookaheadNotRequired()`.

**Impact**

- Proposals in the first slot of every epoch can revert even when the protocol intends to allow
  them, reducing liveness and making timing-dependent behavior harder to reason about.

**Suggested fix (high-level)**

- In `_handleNextEpochLookahead`, if the next epoch lookahead is missing but `!isLookaheadRequired()`,
  skip posting/validation and allow the proposal to proceed.
