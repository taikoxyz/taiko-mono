...
The initial bidding price for new auctions should be set at `s=2*p`, where `p` represents a moving average *bid* for all verified blocks. Each subsequent bid should be at least 10% lower than the current bid. Bidders would need to deposit `s * max_block_gas_limit * num_blocks * 1.5` Taiko tokens as the minimam for the batch. The deposit for a block will be burnt if the winner fails to submit a valid proof within the designated timeframe. Successful completion will result in a refund of the deposit.
