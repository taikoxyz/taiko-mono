...
function encodeBlockMetadata(meta: BlockMetadata) {
    return ethers.utils.defaultAbiCoder.encode(
        [
            "tuple(uint64 id,uint32 gasLimit,uint64 timestamp,uint64 l1Height,bytes32 l1Hash,bytes32 mixHash,bytes32 txListHash,uint24 txListByteStart,uint24 txListByteEnd,address beneficiary}",
        ],
        [meta]
    );
}
