...
contract SgxVerifier is TaikoL1TestBase {
    ...
    function test_addToRegistryBySgxInstance() external {
        ...
        bytes memory signature = createAddRegistrySignature(SGX_X_1,trustedInstances, 0x4);
    ...
    function test_addToRegistryBySgxInstance_with_correct_signature_but_incorrect_instanceid() external {
        address[] memory trustedInstances = new address[](2);
        trustedInstances[0] = SGX_Y;
        trustedInstances[1] = SGX_Z;
        ...
        vm.expectRevert(SGXVerifier.SGX_NOT_VALID_SIGNER_OR_ID_MISMATCH.selector);
        vm.prank(Bob, Bob);
