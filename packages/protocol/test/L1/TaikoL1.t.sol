...
contract TaikoL1Test is TaikoL1TestBase {
    ...
    function test_L1_getCrossChainBlockHash0() external {
        bytes32 genHash = L1.getSyncedData(0).blockHash;
    ...
    /// @dev getSyncedData tests
    function test_L1_getSyncedData() external {
        uint64 count = 10;
        ...
        for (uint64 blockId = 1; blockId < count; blockId++) {
            ...
            assertEq(L1.getSyncedData(blockId).blockHash, blockHash);
        ...
        assertEq(expectedSR, L1.getSyncedData(queriedBlockId).signalRoot);
