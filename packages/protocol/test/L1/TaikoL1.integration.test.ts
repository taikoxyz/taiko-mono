...
import { proposeClaimProveAndVerify, verifyBlocks } from "../utils/verify";
...
describe("integ-----disabled-----ration:TaikoL1", function () {
    ...
    describe("getForkChoice", function () {
        it("returns no empty fork choice for un-proposed, un-proven and un-verified block", async function () {
            ...
            expect(forkChoice.snippet.blockHash).to.be.eq(
                ethers.constants.HashZero
            );
        ...
        it("returns populated data for submitted fork choice", async function () {
            ...
            expect(forkChoice.snippet.blockHash).to.be.eq(block.hash);
    ...
    describe("getXchainBlockHash(0)", function () {
        it("iterates through blockHashHistory length and asserts getXchainBlockHash(0) returns correct value", async function () {
            ...
            for await (const blockNumber of chan) {
                ...
                const { verifyEvent } = await proposeClaimProveAndVerify(
                    taikoL1,
                    l2Provider,
                    blockNumber,
                    proposer,
