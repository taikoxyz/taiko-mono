...
contract BlockProposing is BlocksFixtures {
    ...
    function test_newBlockProposal_updatesLookaheadForNextEpoch() external {
        ...
        assertEq(lookaheadBuffer[3].timestamp, nextEpochStart + (9 * PreconfConstants.SECONDS_IN_SLOT));
        assertEq(lookaheadBuffer[3].prevTimestamp, currentEpochStart + (22 * PreconfConstants.SECONDS_IN_SLOT));
        assertEq(lookaheadBuffer[3].isFallback, false);

        // Check the second entry
        assertEq(lookaheadBuffer[4].preconfer, addr_2);
        assertEq(lookaheadBuffer[4].timestamp, nextEpochStart + (19 * PreconfConstants.SECONDS_IN_SLOT));
        assertEq(lookaheadBuffer[4].prevTimestamp, nextEpochStart + (9 * PreconfConstants.SECONDS_IN_SLOT));
