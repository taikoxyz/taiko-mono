...
contract TaikoL1Stub_ReturnMainnetChainId {
    function getConfigV3() external pure returns (TaikoData.ConfigV3 memory config) {
...
contract TestSP1Verifier is Layer1Test {
    ...
    function ___test_sp1_Plonk_verifyProof() external {
        ...
        TaikoData.TypedProof memory proof =
            TaikoData.TypedProof({ tier: 100, data: abi.encodePacked(vKey, sp1Proof) });

        vm.warp(block.timestamp + 5);

        (IVerifier.Context memory ctx, TaikoData.TransitionV3 memory transition) =
    ...
    function ___test_sp1_Plonk_verifyBatchProof() public {
        ...
        TaikoData.TypedProof memory proof = TaikoData.TypedProof({ tier: 0, data: data });
    ...
    function _generateTaikoMainnetContextAndTransition()
        internal
        pure
        returns (IVerifier.Context memory ctx, TaikoData.TransitionV3 memory transition)
    {
        // Context
        ...
        // Transition
        transition = TaikoData.TransitionV3({
