...
contract TestSP1Verifier is Layer1Test {
    ...
    function test_sp1_Plonk_verifyProof_1() external {
        ...
        // TypedProof
        bytes memory proof = abi.encodePacked(vKey, sp1Proof);

        vm.warp(block.timestamp + 5);

        IVerifier.Context[] memory ctxs = new IVerifier.Context[](1);
        ctxs[0] = _generateTaikoMainnetContext();

        // bytes32 pi = LibPublicInput.hashPublicInputs(
        //     ctxs[0].transition, address(sp1Verifier), address(0), ctxs[0].metaHash, 167_000
        // );

        sp1Verifier.verifyProof(ctxs, proof);
    }
