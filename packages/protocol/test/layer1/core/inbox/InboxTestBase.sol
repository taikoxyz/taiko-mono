...
abstract contract InboxTestBase is CommonTest {
    ...
    uint32 internal constant PROVER_FEE_GWEI = 1_000_000;
    ...
    function _setupMocks() internal virtual {
        ...
        // Deploy MockProverAuction with the designated prover set to Carol (prover)
        proverAuction = new MockProverAuction(prover, PROVER_FEE_GWEI);
    }
        ...
        returns (ProposedEvent memory payload_)
    {
        ...
        // Calculate required ETH (prover fee in wei)
    ...
    function _readProposedEvent() internal returns (ProposedEvent memory payload_) {
        ...
        // New event format: Proposed(uint48 indexed id, bytes data)
        payload_.id = uint48(uint256(log.topics[1]));

        // Decode the bytes data from log.data (first decode the dynamic bytes)
        bytes memory encodedProposal = abi.decode(log.data, (bytes));

        // Decode the proposal using the codec
        IInbox.Proposal memory proposal = codec.decodeProposal(encodedProposal);
