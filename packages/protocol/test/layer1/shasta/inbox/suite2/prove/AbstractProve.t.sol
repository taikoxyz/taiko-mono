...
abstract contract AbstractProveTest is InboxTestHelper {
    ...
    function _proposeAndGetProposal() internal returns (IInbox.Proposal memory) {
        ...
        // since genesis lastProposalBlockId = 1 (lastProposalBlockId, prevents blockhash(0) issue)
        if (block.number < 2) {
            vm.roll(2);
        }
        ...
        returns (IInbox.Proposal memory)
    {
        ...
        // Each proposal sets lastProposalBlockId = block.number (lastProposalBlockId)
        // Need to roll 1 block forward from the last proposal
        uint48 expectedLastBlockId;
        if (_parent.id == 0) {
            expectedLastBlockId = 1; // Genesis value - now represents lastProposalBlockId
            ...
            // lastProposalBlockId should be previous block number (last proposal block)
