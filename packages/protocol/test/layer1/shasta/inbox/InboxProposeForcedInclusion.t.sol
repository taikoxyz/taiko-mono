...
contract InboxProposeForcedInclusion is ShastaInboxTestBase {
    ...
    function test_propose_forced_inclusion_consumed() public {
        ...
        vm.expectCall(
            ...
            abi.encodeWithSelector(IForcedInclusionStore.consumeOldestForcedInclusion.selector, Bob)
        );
        ...
        for (uint256 i = 0; i < logs.length; i++) {
            ...
            if (logs[i].topics[0] == proposedEventSig) {
                ...
                if (emittedProposal.isForcedInclusion) {
                    ...
                    assertEq(
                        emittedProposal.blobSlice.timestamp,
                        uint48(block.timestamp > 1 hours ? block.timestamp - 1 hours : 0)
                    );
                    break;
                }
            }
