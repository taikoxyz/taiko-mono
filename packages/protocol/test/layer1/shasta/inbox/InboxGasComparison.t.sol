...
contract InboxGasComparison is ShastaInboxTestBase {
    ...
    function test_gas_comparison_single_claim_per_proposal() public {
        ...
        for (uint256 i = 0; i < proposalCounts.length; i++) {
            ...
            uint256 gasOptimized =
                _measureProveGas(address(inboxOptimized), testProposals, testClaims);
    ...
    function test_gas_comparison_storage_operations() public {
        ...
        uint256 gasBaseStorage =
            _measureStorageGas(address(inboxBase), testProposals[0], testClaims[0]);
        uint256 gasOptimizedStorage =
            _measureStorageGas(address(inboxOptimized), testProposals[0], testClaims[0]);
        ...
        console2.log(
            "  Gas saved:",
            gasBaseStorage > gasOptimizedStorage ? gasBaseStorage - gasOptimizedStorage : 0
        );
        console2.log("");
        ...
        uint256 gasOptimizedMultiple =
            _measureMultipleWrites(address(inboxOptimized), testProposals[0]);
        ...
        console2.log(
            "  Gas saved:",
            gasBaseMultiple > gasOptimizedMultiple ? gasBaseMultiple - gasOptimizedMultiple : 0
        );
    }
        ...
        for (uint256 i = 0; i < ringBufferSize; i++) {
            ...
            if (inboxImpl == address(inboxBase)) {
                ...
                TestInboxBase(inboxImpl).exposed_setClaimRecordHash(
                    warmupProposal.id, parentClaimHash, claimRecordHash
                );
            } else {
                TestInboxOptimized(inboxImpl).exposed_setProposalHash(
                    warmupProposal.id, proposalHash
                );
        ...
        returns (uint256)
    {
        ...
        (bool success,) =
            inboxImpl.call(abi.encodeWithSelector(IInbox.prove.selector, proveData, proofData));
        ...
        returns (uint256)
    {
        ...
        if (inboxImpl == address(inboxBase)) {
            ...
            impl.exposed_setClaimRecordHash(
                proposal.id, claim.parentClaimHash, keccak256(abi.encode(claim))
            );
            uint256 gasAfter = gasleft();
            return gasBefore - gasAfter;
        } else {
    ...
    function _setupProposal(address inboxImpl, IInbox.Proposal memory proposal) private {
        ...
        if (inboxImpl == address(inboxBase)) {
            TestInboxBase(inboxImpl).exposed_setProposalHash(
                proposal.id, keccak256(abi.encode(proposal))
            );
        } else {
            TestInboxOptimized(inboxImpl).exposed_setProposalHash(
                proposal.id, keccak256(abi.encode(proposal))
