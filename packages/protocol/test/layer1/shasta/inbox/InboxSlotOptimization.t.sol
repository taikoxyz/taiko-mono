...
contract InboxSlotOptimization is ShastaInboxTestBase {
    ...
    function test_slot_reuse_marker_encoding() public {
        ...
        proposalIds[0] = 1;
        proposalIds[1] = 65535; // Near max uint48
        proposalIds[2] = 281474976710655; // Max uint48
        ...
        for (uint i = 0; i < proposalIds.length; i++) {
            ...
            // Retrieve and verify
            bytes32 retrieved = inbox.exposed_getClaimRecordHash(
                proposalIds[i],
                parentHashes[i]
            );
    ...
    function test_partial_parent_hash_matching() public {
        ...
        bytes32 baseParentHash = bytes32(
        ...
        bytes32 variation1 = bytes32((uint256(baseParentHash) & ~uint256(0xFFFFFFFFFFFF)) | uint256(0x111111));
        bytes32 variation2 = bytes32((uint256(baseParentHash) & ~uint256(0xFFFFFFFFFFFF)) | uint256(0x222222));
        bytes32 variation3 = bytes32((uint256(baseParentHash) & ~uint256(0xFFFFFFFFFFFF)) | uint256(0x333333));
        ...
        bytes32 differentHash = bytes32(
            0xFFFFFFFFFFFFFFFFBBBBBBBBBBBBBBBBCCCCCCCCCCCCCCCCDDDDDDDDDDDDDDDD
        );
    ...
    function test_storage_collision_handling() public {
        ...
        assertTrue(inbox.exposed_getClaimRecordHash(proposalId, parentHash1) != 
