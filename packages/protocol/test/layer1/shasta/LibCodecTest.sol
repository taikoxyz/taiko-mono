...
contract LibCodecTest is Test {
    ...
    function test_packUnpackUint8_zero() public pure {
        ...
        // Pack value
        uint8 value = 0;
        uint256 newPtr = LibCodec.packUint8(buffer, ptr, value);
        assertEq(newPtr, ptr + 1);

        // Unpack value
        (uint8 unpacked, uint256 readPtr) = LibCodec.unpackUint8(buffer, ptr);
    ...
    function test_packUnpackUint8_various() public pure {
        ...
        for (uint256 i = 0; i < testValues.length; i++) {
            ...
            uint256 newPtr = LibCodec.packUint8(buffer, writePtr, testValues[i]);
            assertEq(newPtr, writePtr + 1);

            (uint8 unpacked,) = LibCodec.unpackUint8(buffer, writePtr);
    ...
    function test_packUnpackUint16_zero() public pure {
        ...
        uint16 value = 0;
        uint256 newPtr = LibCodec.packUint16(buffer, ptr, value);
        assertEq(newPtr, ptr + 2);

        (uint16 unpacked, uint256 readPtr) = LibCodec.unpackUint16(buffer, ptr);
        assertEq(unpacked, value);
    ...
    function test_packUnpackUint32_zero() public pure {
        ...
        uint32 value = 0;
        uint256 newPtr = LibCodec.packUint32(buffer, ptr, value);
        assertEq(newPtr, ptr + 4);

        (uint32 unpacked, uint256 readPtr) = LibCodec.unpackUint32(buffer, ptr);
        assertEq(unpacked, value);
    ...
    function test_packUnpackUint32_bigEndian() public pure {
        ...
        LibCodec.packUint32(buffer, ptr, value);
        ...
        (uint32 unpacked,) = LibCodec.unpackUint32(buffer, ptr);
        assertEq(unpacked, value);
    }
    ...
    function test_packUnpackUint48_zero() public pure {
        ...
        uint48 value = 0;
        uint256 newPtr = LibCodec.packUint48(buffer, ptr, value);
        assertEq(newPtr, ptr + 6);

        (uint48 unpacked, uint256 readPtr) = LibCodec.unpackUint48(buffer, ptr);
        assertEq(unpacked, value);
    ...
    function test_packUnpackUint48_bigEndian() public pure {
        ...
        LibCodec.packUint48(buffer, ptr, value);
        ...
        (uint48 unpacked,) = LibCodec.unpackUint48(buffer, ptr);
        assertEq(unpacked, value);
    }
    ...
    function test_packUnpackUint256_zero() public pure {
        ...
        uint256 value = 0;
        uint256 newPtr = LibCodec.packUint256(buffer, ptr, value);
        assertEq(newPtr, ptr + 32);

        (uint256 unpacked, uint256 readPtr) = LibCodec.unpackUint256(buffer, ptr);
        assertEq(unpacked, value);
    ...
    function test_packUnpackUint256_various() public pure {
        ...
        for (uint256 i = 0; i < testValues.length; i++) {
            uint256 newPtr = LibCodec.packUint256(buffer, currentPtr, testValues[i]);
            assertEq(newPtr, currentPtr + 32);

            (uint256 unpacked,) = LibCodec.unpackUint256(buffer, currentPtr);
            assertEq(unpacked, testValues[i]);
    ...
    function test_packUnpackBytes32_zero() public pure {
        ...
        uint256 newPtr = LibCodec.packBytes32(buffer, ptr, value);
        assertEq(newPtr, ptr + 32);

        (bytes32 unpacked, uint256 readPtr) = LibCodec.unpackBytes32(buffer, ptr);
        assertEq(unpacked, value);
        assertEq(readPtr, ptr + 32);
    }
    ...
    function test_packUnpackAddress_zero() public pure {
        ...
        uint256 newPtr = LibCodec.packAddress(buffer, ptr, value);
        assertEq(newPtr, ptr + 20);

        (address unpacked, uint256 readPtr) = LibCodec.unpackAddress(buffer, ptr);
        assertEq(unpacked, value);
    ...
    function test_packUnpackAddress_various() public pure {
        ...
        for (uint256 i = 0; i < testAddresses.length; i++) {
            uint256 newPtr = LibCodec.packAddress(buffer, currentPtr, testAddresses[i]);
            assertEq(newPtr, currentPtr + 20);

            (address unpacked,) = LibCodec.unpackAddress(buffer, currentPtr);
    ...
    function test_packUnpackAddress_bigEndian() public pure {
        ...
        LibCodec.packAddress(buffer, ptr, value);
        ...
        (address unpacked,) = LibCodec.unpackAddress(buffer, ptr);
        assertEq(unpacked, value);
    }
    ...
    function test_sequentialPackUnpack() public pure {
        ...
        // Test uint8 + uint16
        {
            ...
            writePtr = LibCodec.packUint8(buffer, writePtr, val1);
            writePtr = LibCodec.packUint16(buffer, writePtr, val2);

            uint256 readPtr = ptr;
            (uint8 read1, uint256 newPtr1) = LibCodec.unpackUint8(buffer, readPtr);
            (uint16 read2, uint256 newPtr2) = LibCodec.unpackUint16(buffer, newPtr1);
        ...
        // Test uint32 + uint48
        {
            ...
            writePtr = LibCodec.packUint32(buffer, writePtr, val3);
            writePtr = LibCodec.packUint48(buffer, writePtr, val4);

            uint256 readPtr = ptr + 3;
            (uint32 read3, uint256 newPtr3) = LibCodec.unpackUint32(buffer, readPtr);
            (uint48 read4, uint256 newPtr4) = LibCodec.unpackUint48(buffer, newPtr3);
        ...
        // Test address + bytes32
        {
            ...
            writePtr = LibCodec.packAddress(buffer, writePtr, val5);
            writePtr = LibCodec.packBytes32(buffer, writePtr, val6);

            uint256 readPtr = ptr + 13;
            (address read5, uint256 newPtr5) = LibCodec.unpackAddress(buffer, readPtr);
            (bytes32 read6, uint256 newPtr6) = LibCodec.unpackBytes32(buffer, newPtr5);
    ...
    function testFuzz_packUnpackUint8(uint8 value) public pure {
        ...
        LibCodec.packUint8(buffer, ptr, value);
        (uint8 unpacked,) = LibCodec.unpackUint8(buffer, ptr);
        assertEq(unpacked, value);
    }
    ...
    function testFuzz_packUnpackUint256(uint256 value) public pure {
        ...
        LibCodec.packUint256(buffer, ptr, value);
        (uint256 unpacked,) = LibCodec.unpackUint256(buffer, ptr);
        assertEq(unpacked, value);
    }
    ...
    function testFuzz_packUnpackBytes32(bytes32 value) public pure {
        ...
        LibCodec.packBytes32(buffer, ptr, value);
        (bytes32 unpacked,) = LibCodec.unpackBytes32(buffer, ptr);
        assertEq(unpacked, value);
    }
