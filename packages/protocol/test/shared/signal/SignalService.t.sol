...
contract SignalServiceNoHopCheck is LegacySignalService {
    constructor(address _resolver) LegacySignalService(_resolver) { }
...
contract TestSignalService is CommonTest {
    LegacySignalService private mockSignalService;
    LegacySignalService private signalService;
    ...
    function setUpOnEthereum() internal override {
        ...
        signalService = deploySignalService(address(new LegacySignalService(address(resolver))));
    ...
    function test_SignalService_proveSignalReceived_revert_zero_size_proof() public {
        ...
        vm.expectRevert(LegacySignalService.SS_SIGNAL_NOT_RECEIVED.selector);
        mockSignalService.proveSignalReceived({
            _chainId: ethereumChainId,
            _app: randAddress(),
            _signal: randBytes32(),
        ...
        ISignalService.HopProof[] memory proofs = new ISignalService.HopProof[](0);

        vm.expectRevert(LegacySignalService.SS_EMPTY_PROOF.selector);
        mockSignalService.proveSignalReceived({
            _chainId: ethereumChainId,
            _app: randAddress(),
            _signal: randBytes32(),
    ...
    function test_SignalService_proveSignalReceived_revert_last_hop_incorrect_chainid() public {
        ...
        // proofs[0].chainId must be ethereumChainId in order not to revert
        proofs[0].chainId = ethereumChainId + 1;
        proofs[0].blockId = 1;

        vm.expectRevert(LegacySignalService.SS_INVALID_LAST_HOP_CHAINID.selector);
        mockSignalService.proveSignalReceived({
            _chainId: ethereumChainId,
            _app: randAddress(),
            _signal: randBytes32(),
    ...
    function test_SignalService_proveSignalReceived_revert_mid_hop_incorrect_chainid() public {
        ...
        // proofs[0].chainId must NOT be ethereumChainId in order not to revert
        proofs[0].chainId = ethereumChainId;
        proofs[0].blockId = 1;

        vm.expectRevert(LegacySignalService.SS_INVALID_MID_HOP_CHAINID.selector);
        mockSignalService.proveSignalReceived({
            _chainId: ethereumChainId,
            _app: randAddress(),
            _signal: randBytes32(),
    ...
    function test_SignalService_proveSignalReceived_local_chaindata_not_found() public {
        ...
        vm.expectRevert(LegacySignalService.SS_SIGNAL_NOT_FOUND.selector);
        mockSignalService.proveSignalReceived({
            _chainId: ethereumChainId,
            _app: randAddress(),
            _signal: randBytes32(),
    ...
    function test_SignalService_proveSignalReceived_one_hop_cache_signal_root() public {
        ...
        vm.expectRevert(LegacySignalService.SS_UNAUTHORIZED.selector);
        vm.prank(taiko);
        mockSignalService.syncChainData(
            ethereumChainId, LibSignals.SIGNAL_ROOT, proofs[0].blockId, proofs[0].rootHash
        );
    ...
    function test_SignalService_proveSignalReceived_multiple_hops_no_caching() public {
        ...
        ISignalService.HopProof[] memory proofs = new ISignalService.HopProof[](3);

        // first hop with full merkle proof
        proofs[0].chainId = ethereumChainId + 2;
        proofs[0].blockId = 1;
        proofs[0].rootHash = randBytes32();
        proofs[0].accountProof = new bytes[](1);
        proofs[0].storageProof = new bytes[](10);
    ...
    function test_SignalService_proveSignalReceived_revert_with_a_loop() public {
        ...
        vm.expectRevert(LegacySignalService.SS_INVALID_HOPS_WITH_LOOP.selector);
        mockSignalService.proveSignalReceived({
            _chainId: ethereumChainId,
            _app: randAddress(),
            _signal: randBytes32(),
    ...
    function test_SignalService_proveSignalReceived_multiple_hops_caching() public {
        ...
        ISignalService.HopProof[] memory proofs = new ISignalService.HopProof[](9);
        ...
        view
    {
        assertEq(
            mockSignalService.isChainDataSynced(
                chainId, LibSignals.STATE_ROOT, blockId, stateRoot
            ),
            stateRootCached
        );
