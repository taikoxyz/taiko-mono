...
contract TestSignalService is CommonTest {
    ...
    function test_real_signal() public {
        ...
        uint64 blockId = 5570;
        vm.prank(deployer);
        signalService.syncChainData(32_382, LibNames.H_STATE_ROOT, blockId, stateRoot);
    ...
    function test_SignalService_proveSignalReceived_one_hop_cache_signal_root() public {
        ...
        mockSignalService.syncChainData(
            ethereumChainId, LibNames.H_SIGNAL_ROOT, proofs[0].blockId, proofs[0].rootHash
        );
        mockSignalService.proveSignalReceived({
            _chainId: ethereumChainId,
            _app: randAddress(),
    ...
    function test_SignalService_proveSignalReceived_one_hop_state_root() public {
        ...
        vm.prank(taiko);
        mockSignalService.syncChainData(
            ethereumChainId, LibNames.H_STATE_ROOT, proofs[0].blockId, proofs[0].rootHash
        );
        ...
        assertEq(
            mockSignalService.isChainDataSynced(
                ethereumChainId, LibNames.H_SIGNAL_ROOT, proofs[0].blockId, bytes32(uint256(789))
            ),
            false
        );
    }
    ...
    function test_SignalService_proveSignalReceived_multiple_hops_no_caching() public {
        ...
        vm.prank(taiko);
        mockSignalService.syncChainData(
            proofs[1].chainId, LibNames.H_STATE_ROOT, proofs[2].blockId, proofs[2].rootHash
        );
    ...
    function test_SignalService_proveSignalReceived_multiple_hops_caching() public {
        ...
        vm.prank(taiko);
        mockSignalService.syncChainData(
            proofs[7].chainId, LibNames.H_STATE_ROOT, proofs[8].blockId, proofs[8].rootHash
        );
    ...
    {
        assertEq(
            mockSignalService.isChainDataSynced(chainId, LibNames.H_STATE_ROOT, blockId, stateRoot),
            stateRootCached
        );

        assertEq(
            mockSignalService.isChainDataSynced(
                chainId, LibNames.H_SIGNAL_ROOT, blockId, bytes32(uint256(789))
            ),
            signalRootCached
        );
    }
    ...
    function test_SignalService_getSyncedChainData_isSignalSent() public {
        ...
        bytes32 kind = LibNames.H_STATE_ROOT;
