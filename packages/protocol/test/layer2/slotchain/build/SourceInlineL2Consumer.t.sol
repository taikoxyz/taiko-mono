...
contract SourceInlineL2ConsumerTest is Test {
    /// @notice Verifies L2 source-inline compilation and shared owner-bytecode consumption.
    function test_sourceInlineAndOwnerBytecodeConsumption() external {
        bytes32 value = keccak256("l2-probe-value");
        assertEq(
            LibSourceInlineProbe.hash(value, 2),
            keccak256(
                abi.encodePacked(keccak256("slot-chain-source-inline-probe-v1"), uint64(2), value)
            )
        );
    ...
    /// @dev Deploys the shared-profile artifact without compiling its implementation in L2.
