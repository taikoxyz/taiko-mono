...
contract BondManagerTest is Test {
    ...
    function test_depositTo_SuccessfulDepositToSelf() external {
    ...
    function test_depositTo_MultipleDepositsAccumulate() external {
        vm.startPrank(Alice);
        bondManager.depositTo(address(0), 30 ether);
        bondManager.depositTo(address(0), 20 ether);
        vm.stopPrank();
    ...
    function test_depositTo_DoesNotCancelWithdrawalRequest() external {
        vm.prank(Alice);
        bondManager.depositTo(address(0), MIN_BOND);

        vm.prank(Alice);
        bondManager.requestWithdrawal();

        vm.prank(Alice);
        bondManager.depositTo(address(0), 1 ether);
    ...
    function test_depositTo_RevertWhen_ZeroAmountApproval() external {
        vm.prank(Alice);
        bondToken.approve(address(bondManager), 0);

        vm.expectRevert();
        vm.prank(Alice);
        bondManager.depositTo(address(0), 10 ether);
    }
    ...
    function test_depositTo_ZeroAddressDepositsSelf() external {
        uint256 depositAmount = 50 ether;
    ...
    function test_depositTo_ForOtherDoesNotCancelWithdrawalRequest() external {
        vm.prank(Bob);
    ...
    function test_withdraw_RevertWhen_ExceedingBalanceAfterMaturity() external {
        ...
        vm.prank(Bob);
        bondManager.depositTo(address(0),100 ether);

        vm.prank(Carol);
        bondManager.depositTo(address(0),75 ether);
    ...
    function test_hasSufficientBond_ReturnsFalseWhenBelowMinBond() external {
        vm.prank(Alice);
        bondManager.depositTo(address(0),5 ether);
    ...
    function test_edgeCase_MaxUint256Deposit() external {
        ...
        vm.prank(Alice);
        bondManager.depositTo(address(0),largeAmount);
    ...
    function test_processBondInstruction_transfersBonds() external {
        ...
        vm.prank(Alice);
        bondManager.depositTo(address(0),LIVENESS_BOND * 2);
    ...
    function test_processBondInstruction_allowsOutOfOrderConsumption() external {
        ...
        vm.prank(Alice);
        bondManager.depositTo(address(0),500 ether);
        vm.prank(Carol);
        bondManager.depositTo(address(0),500 ether);
