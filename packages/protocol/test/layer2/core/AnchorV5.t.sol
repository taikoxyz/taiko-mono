...
import { MockCheckpointStore } from "./AnchorTestHelpers.sol";
import { ERC1967Proxy } from "@openzeppelin/contracts/proxy/ERC1967/ERC1967Proxy.sol";
import "forge-std/src/Test.sol";
import { Anchor } from "src/layer2/core/Anchor.sol";
import { ICheckpointStore } from "src/shared/signal/ICheckpointStore.sol";

contract AnchorV5Test is Test {
    ...
    function test_anchorV5_savesCheckpointAndUpdatesState() external {
        ...
        anchor.anchorV5(proposalParams, blockParams);
    ...
    function test_anchorV5_allowsMultipleAnchorsAcrossBlocks() external {
        Anchor.ProposalParams memory proposalParams =
            Anchor.ProposalParams({ submissionWindowEnd: 0 });
        Anchor.BlockParams memory blockParams = _blockParams(1000, 0x1234, 0x5678);
    ...
    function test_anchorV5_rejectsInvalidSender() external {
        Anchor.ProposalParams memory proposalParams =
            Anchor.ProposalParams({ submissionWindowEnd: 0 });
        Anchor.BlockParams memory blockParams = _blockParams(1000, 0x1234, 0x5678);
    ...
    function test_anchorV5_ignoresStaleCheckpoint() external {
        Anchor.ProposalParams memory proposalParams =
            Anchor.ProposalParams({ submissionWindowEnd: 0 });
        ...
        vm.roll(SHASTA_FORK_HEIGHT);
        vm.prank(GOLDEN_TOUCH);
        anchor.anchorV5(proposalParams, freshBlockParams);

        Anchor.BlockParams memory staleBlockParams = _blockParams(999, 0xAAAA, 0xBBBB);
        vm.roll(SHASTA_FORK_HEIGHT + 1);
        vm.prank(GOLDEN_TOUCH);
        anchor.anchorV5(proposalParams, staleBlockParams);
