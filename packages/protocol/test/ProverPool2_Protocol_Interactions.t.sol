...
contract TaikoL1ProverPool is TaikoL1TestBase {
    ProverPool2 public realProverPool;
    ...
    function setUp() public override {
        ...
        realProverPool = new ProverPool2();
    ...
    // The function will print out slot distribution
    function printOutSlots() public view {
        // According to this Xenia will have the most chance to prove
        // and Ivy the least
        // 32 slots we have
        for (uint256 index; index < 32; index++) {
            console2.log(
                ...
                index,
                " slots:",
                realProverPool.getNumClaimableSlots(proverArray[index])
            );
            console2.log(
                "Index:",
                index,
                " weight:",
                realProverPool.getWeight(proverArray[index])
            );
        }
    }
    ...
    function stakeButOnly5Provers() public {
        ...
        for (uint256 index; index < 5; index++) {
            vm.prank(proverArray[index], proverArray[index]);
            realProverPool.stake((index + 1) * 1e10, 10, 128);
        }
    }
    ...
    function test_one_two_staker_v2() external {
        depositTaikoToken(Alice, 1000 * 1e8, 1000 ether);
        console2.log("Alice balance:", tko.balanceOf(Alice));

        vm.prank(proverArray[0], proverArray[0]);
        realProverPool.stake(10e8, 10, 110);
        vm.prank(proverArray[1], proverArray[1]);
        realProverPool.stake(10e8, 10, 110);
    ...
    function test_asigned_prover_distribution_if_prover_pool_is_full_v2()
        external
    {
        depositTaikoToken(Alice, 1000 * 1e8, 1000 ether);
        console2.log("Alice balance:", tko.balanceOf(Alice));
    ...
    function test_asigned_prover_distribution_if_only_have_1_prover_v2()
        external
    {
        depositTaikoToken(Alice, 1000 * 1e8, 1000 ether);
        console2.log("Alice balance:", tko.balanceOf(Alice));
        ...
        vm.prank(Ivy, Ivy);
        realProverPool.stake(2 * 1e10, 10, 128);
    ...
    function test_everyone_can_prove_if_there_are_no_stakers_v2() public {
        depositTaikoToken(Alice, 1000 * 1e8, 1000 ether);
        console2.log("Alice balance:", tko.balanceOf(Alice));
    ...
    function test_asigned_prover_distribution_if_prover_pool_is_not_full_v2()
        external
    {
        depositTaikoToken(Alice, 1000 * 1e8, 1000 ether);
        console2.log("Alice balance:", tko.balanceOf(Alice));
    ...
    function test_distribution_if_biggest_staker_exits_and_a_new_biggest_comes_in_v2(
    )
        external
    {
        depositTaikoToken(Alice, 1000 * 1e8, 1000 ether);
        ...
        for (
            ...
            if (blockId == (conf.blockMaxProposals * 9 / 2)) {
                ...
                vm.prank(Kai, Kai);
                realProverPool.unstake();

                // Now Khloe will be the new top staker
                vm.prank(Khloe, Khloe);
                realProverPool.stake(6 * 1e10, 10, 128);
            }
            printVariables("before propose");
            TaikoData.BlockMetadata memory meta =
                proposeBlock(Alice, 1_000_000, 1024);
    ...
    function test_slashing_and_that_outside_proof_window_others_can_prove_v2()
        external
    {
        depositTaikoToken(Alice, 1000 * 1e8, 1000 ether);
        console2.log("Alice balance:", tko.balanceOf(Alice));
        ...
        for (
            ...
            (uint256 beforeSlashAmount,,,,) = realProverPool.stakers(prover);
            ...
            (uint256 afterSlashAmount,,,,) = realProverPool.stakers(prover);

            assertTrue(afterSlashAmount < beforeSlashAmount);
    ...
    function test_others_cannot_prove_within_proof_window_v2() external {
        depositTaikoToken(Alice, 1000 * 1e8, 1000 ether);
        console2.log("Alice balance:", tko.balanceOf(Alice));
