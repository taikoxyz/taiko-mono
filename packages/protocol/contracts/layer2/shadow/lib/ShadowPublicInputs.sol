...
/// @notice Serializes Shadow public inputs for ZK circuit verification.
/// @dev Layout (120 elements): blockNumber[0], stateRoot[1-32], chainId[33],
/// noteIndex[34], amount[35], recipient[36-55], nullifier[56-87], powDigest[88-119].
...
library ShadowPublicInputs {
    ...
    /// @dev Serializes PublicInput to uint256 array for circuit verification.
    ...
    /// @dev Returns true if powDigest has required trailing zero bits.
    function powDigestIsValid(bytes32 _powDigest) internal pure returns (bool) {
        return (uint256(_powDigest) & _POW_MASK) == 0;
    }

    /// @dev Writes bytes32 as individual bytes to array (big-endian).
    ...
    /// @dev Writes address as individual bytes to array (big-endian).
