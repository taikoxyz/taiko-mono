...
interface ITierProvider {
    ...
    /// @return tier Tier struct containing the tier's parameters.
    function getTier(uint16 tierId) external view returns (Tier memory tier);
    ...
    function getTierIds() external view returns (uint16[] memory tierIds);
    ...
    function getMinTier(address proposer, uint256 rand) external view returns (uint16 tierId);
}
