...
/// @title LibPackCodec
/// @notice Library providing fundamental packing/unpacking functions for compact encoding using assembly
/// @dev All functions are optimized for gas efficiency and compact storage
/// @custom:security-contact security@taiko.xyz
library LibPackCodec {
    // ---------------------------------------------------------------
    ...
    function unpackUint8(bytes memory data, uint256 pos)
        internal
        pure
    ...
    /// @return value The unpacked value
    /// @return newPos Position after reading
    function unpackUint16(bytes memory data, uint256 pos)
        internal
        pure
        returns (uint16 value, uint256 newPos)
    {
        assembly {
            value := or(
                shl(8, byte(0, mload(pos))),
                byte(0, mload(add(pos, 1)))
            )
            newPos := add(pos, 2)
        }
    }
    ...
    /// @return value The unpacked value
    /// @return newPos Position after reading
    function unpackUint32(bytes memory data, uint256 pos)
        internal
        pure
        returns (uint32 value, uint256 newPos)
    {
        assembly {
            value := or(
                or(
                    shl(24, byte(0, mload(pos))),
                    shl(16, byte(0, mload(add(pos, 1))))
                ),
                or(
                    shl(8, byte(0, mload(add(pos, 2)))),
                    byte(0, mload(add(pos, 3)))
                )
            )
            newPos := add(pos, 4)
        }
    ...
    /// @return value The unpacked value
    /// @return newPos Position after reading
    function unpackUint48(bytes memory data, uint256 pos)
        internal
        pure
        returns (uint48 value, uint256 newPos)
    {
        assembly {
            value := or(
                or(
                    or(
                        shl(40, byte(0, mload(pos))),
                        shl(32, byte(0, mload(add(pos, 1))))
                    ),
                    or(
                        shl(24, byte(0, mload(add(pos, 2)))),
                        shl(16, byte(0, mload(add(pos, 3))))
                    )
                ),
                or(
                    shl(8, byte(0, mload(add(pos, 4)))),
                    byte(0, mload(add(pos, 5)))
                )
            )
            newPos := add(pos, 6)
        }
    ...
    /// @return value The unpacked value
    /// @return newPos Position after reading
    function unpackUint256(bytes memory data, uint256 pos)
        internal
        pure
    ...
    /// @return value The unpacked value
    /// @return newPos Position after reading
    function unpackBytes32(bytes memory data, uint256 pos)
        internal
        pure
    ...
    /// @return value The unpacked value
    /// @return newPos Position after reading
    function unpackAddress(bytes memory data, uint256 pos)
        internal
        pure
        ...
        assembly {
            ...
            value := or(
                or(
                    or(
                        or(
                            shl(152, byte(0, mload(pos))),
                            shl(144, byte(0, mload(add(pos, 1))))
                        ),
                        or(
                ...
                or(
                    ...
                    or(
                        ...
                        or(
                            ...
                            or(
                                shl(8, byte(0, mload(add(pos, 18)))),
                                byte(0, mload(add(pos, 19)))
                            )
                        )
                    )
                )
