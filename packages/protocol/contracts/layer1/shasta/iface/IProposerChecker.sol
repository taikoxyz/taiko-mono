...
interface IProposerChecker {
    ...
    /// @return lookaheadSlotTimestamp_ The timestamp of the last slot where the current preconfer can propose.
    /// @dev This function must revert if the address is not a valid proposer
    function checkProposer(address _proposer) external view returns (uint48 lookaheadSlotTimestamp_);
}
