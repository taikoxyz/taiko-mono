...
contract TokenUnlock is EssentialContract {
    ...
    /// @notice Emitted when token is re-vested.
    /// @param amount The re-vested amount.
    event TokenReVested(uint256 amount);
    ...
    function vest(uint128 _amount) external nonReentrant {
        ...
        tko().safeTransferFrom(msg.sender, address(this), _amount);
    }
        ...
        onlyRecipient
    {
        ...
        tko().safeTransfer(_proverSet, _amount);
    }
    ...
    function delegate(address _delegatee) external onlyRecipient nonReentrant {
        ERC20VotesUpgradeable(address(tko())).delegate(_delegatee);
    }
    ...
    function tko() public view returns (IERC20) {
