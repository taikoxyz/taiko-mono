...
contract AVSDirectory is IAVSDirectory {
    ...
    bytes32 private constant OPERATOR_AVS_REGISTRATION_TYPEHASH = keccak256(
        ...
        returns (bytes32)
    {
        ...
        bytes32 digestHash =
            keccak256(abi.encodePacked("\x19\x01", _calculateDomainSeparator(), structHash));
        return digestHash;
    }
    ...
    function _calculateDomainSeparator() internal view returns (bytes32) {
        return keccak256(
            abi.encode(
                DOMAIN_TYPEHASH, keccak256(bytes("EigenLayer")), block.chainid, address(this)
            )
        );
    }
}
