...
contract PreconfTaskManager is EssentialContract {
    ...
    function init(
       ...
       IERC20(resolve(LibStrings.B_TAIKO_TOKEN, false)).approve(resolve(LibStrings.B_TAIKO, false), type(uint256).max);
    }
    ...
    function proposeBatch(
        ...
        external
    {
        require(IPreconfWhitelist(resolve("preconf_whitelist", false)).isEligibleProposer(msg.sender), SenderIsNotThePreconfer());

        // Forward the block to Taiko's L1 contract
        ITaikoInbox(resolve(LibStrings.B_TAIKO, false)).proposeBatch(abi.encode(msg.sender, batchParams.coinbase, batchParams), txList);
    }
}
