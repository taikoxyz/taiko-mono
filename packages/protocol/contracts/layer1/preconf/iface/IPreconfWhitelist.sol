...
interface IPreconfWhitelist {
    ...
    /// @param _randomnessDelayEpochs The number of epochs to delay randomness for operator selection.
    /// @dev Configuration note: If you want to ensure operator changes don't affect whitelist lookahead,
    ///      set _operatorChangeDelay >= _randomnessDelayEpochs. If you want to allow lookahead
    ///      to be affected by operator changes (e.g., for emergency evictions), set _operatorChangeDelay = 0.
    function init(
        address _owner,
