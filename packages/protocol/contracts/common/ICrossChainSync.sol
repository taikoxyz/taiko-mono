 ...
 * @dev This interface is implemented by both the TaikoL1 and TaikoL2 contracts.
 *
 * It outlines the essential methods required for synchronizing and accessing
 * block hashes across chains. The core idea is to ensure that data between both
 * chains remain consistent and can be cross-referenced with integrity.
 */
interface ICrossChainSync {
     ...
     * @param number The target block number. Specifying 0 retrieves the hash of
     * the latest block.
     ...
     * block from the opposite chain.
     * @param number The target block number. Specifying 0 retrieves the root of
     * the latest block.
