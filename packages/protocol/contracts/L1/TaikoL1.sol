...
contract TaikoL1 is EssentialContract {
    ...
    uint256 public constant PROPOSING_DELAY_MIN = 3;
    uint256 public constant PROPOSING_DELAY_MAX = 100;
    ...
    function isCommitValid(bytes32 hash) public view returns (bool) {
        return
            hash != 0 &&
            commits[hash] != 0 &&
            block.number >= commits[hash] + PROPOSING_DELAY_MIN &&
            block.number <= commits[hash] + PROPOSING_DELAY_MAX;
    }
