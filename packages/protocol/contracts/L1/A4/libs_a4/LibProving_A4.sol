...
library LibProving {
    ...
    function proveBlock(
        ...
        if (
            evidence.prover != address(1) && evidence.prover != blk.assignedProver
                && blk.assignedProver != address(0)
                && block.timestamp <= blk.proposedAt + blk.proofWindow
        ) revert L1_NOT_PROVEABLE();

        if (evidence.prover == address(1) && msg.sender != resolver.resolve("oracle_prover", false))
        {
        ...
        if (!blk.proverReleased && blk.assignedProver == fc.prover) {
            ...
            IProverPool(resolver.resolve("prover_pool", false)).releaseProver(blk.assignedProver);
        }
        ...
        if (evidence.prover != address(1)) {
            bytes32 instance;
            {
                ...
                inputs[1] = uint256(
                    uint160(address(resolver.resolve(config.chainId, "signal_service", false)))
                );
                inputs[2] =
                    uint256(uint160(address(resolver.resolve(config.chainId, "taiko", false))));
