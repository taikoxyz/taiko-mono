...
contract ProverPool is EssentialContract, IProverPool {
    ...
    event Staked(address indexed addr, uint64 amount, uint16 rewardPerGas, uint16 currentCapacity);
    ...
    function assignProver(uint64 blockId, uint32 /*feePerGas*/ )
        external
        onlyFromProtocol
        returns (address prover, uint32 rewardPerGas)
    {
        unchecked {
            ...
            bytes32 rand = keccak256(abi.encode(blockhash(block.number - 1), blockId));
    ...
    function slashProver(address addr) external onlyFromProtocol {
        ...
        unchecked {
            uint64 amountToSlash = uint64(
                (slashableAmount * SLASH_POINTS / 10_000).max(MIN_SLASH_AMOUNT).min(slashableAmount)
            );
    ...
    function stake(uint64 amount, uint16 rewardPerGas, uint16 maxCapacity) external nonReentrant {
        // Withdraw first
    ...
    function _stake(address addr, uint64 amount, uint16 rewardPerGas, uint16 maxCapacity) private {
        // Check parameters
        if (
            maxCapacity < MAX_CAPACITY_LOWER_BOUND || amount / maxCapacity < MIN_STAKE_PER_CAPACITY
                || rewardPerGas == 0
        ) revert INVALID_PARAMS();
        ...
        staker.exitRequestedAt = staker.exitAmount == 0 ? 0 : uint64(block.timestamp);
    ...
    function _calcWeight(uint64 stakedAmount, uint16 rewardPerGas) private pure returns (uint64) {
