...
library LibProving_A3 {
    ...
    function proveBlock(
        ...
        if (blockId <= state.slot8.lastVerifiedBlockId || blockId >= state.slot7.numBlocks) {
            revert L1_BLOCK_ID();
        }

        TaikoData.Block_A3 storage blk = state.blocks_A3[blockId % config.blockRingBufferSize];
        ...
        if (fcId == 0) {
            ...
            if (fcId == 1) {
                ...
                state.forkChoiceIds[blk.blockId][evidence.parentHash][evidence.parentGasUsed] = uint24(fcId); //Explicity conversion - uint24 shall be safe
            }
        ...
        if (evidence.prover == address(1)) {
            fc.provenAt = uint64(block.timestamp.max(blk.proposedAt + state.slot8.proofTimeTarget));
        } else {
