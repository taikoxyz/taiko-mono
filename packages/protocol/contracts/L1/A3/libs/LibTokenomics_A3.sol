...
library LibTokenomics_A3 {
    ...
    function getProofReward(TaikoData.State storage state, uint64 proofTime)
        ...
        returns (uint64)
    {
        uint64 numBlocksUnverified = state.slot7.numBlocks - state.slot8.lastVerifiedBlockId - 1;

        if (numBlocksUnverified == 0) {
            return 0;
        } else {
            uint64 totalNumProvingSeconds =
                uint64(uint256(numBlocksUnverified) * block.timestamp - state.slot7.accProposedAt);
            // If block timestamp is equal to state.slot7.accProposedAt (not really,
            ...
            return uint64((uint256(state.slot7.accBlockFees) * proofTime) / totalNumProvingSeconds);
        }
    }
    ...
    {
        newProofTimeIssued = (state.slot8.proofTimeIssued > state.slot8.proofTimeTarget)
            ? state.slot8.proofTimeIssued - state.slot8.proofTimeTarget
            : uint64(0);
        newProofTimeIssued += proofTime;

        uint256 x = (newProofTimeIssued * Math.SCALING_FACTOR_1E18)
            / (state.slot8.proofTimeTarget * state.slot6.adjustmentQuotient);
