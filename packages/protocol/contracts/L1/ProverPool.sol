...
//     /// @dev These values are used to compute the prover's rank (along with
// the
//     /// protocol proverFee).
//     struct Prover {
//         uint64 stakedAmount;
...
//     event Slashed(uint64 indexed blockId, address indexed addr, uint64
// amount);
//     event Staked(
...
//         if (staker.proverId != 0 && prover.currentCapacity <
// staker.maxCapacity)
//         {
//             unchecked {
...
//         (Staker memory staker, Prover memory prover) = getStaker(addr);
//         unchecked {
//             // if the exit is mature, we do not count it in the total
// slash-able
//             // amount
...
//                 (slashableAmount * SLASH_POINTS / 10_000 /
// staker.maxCapacity)
...
//     /// @notice Requests an exit for the staker. This will withdraw the
// staked
...
//     /// @notice Calculates and returns the current total capacity of the
// pool.
...
// proverFee
//                     if (_prover.rewardPerGas > proverFee * 125 / 100) {
//                         erpg[i] = proverFee * 125 / 100;
//                     } else if (_prover.rewardPerGas < proverFee * 75 / 100) {
//                         erpg[i] = proverFee * 75 / 100;
//                     } else {
//                         erpg[i] = _prover.rewardPerGas;
//                     }
//                     // Calculates the prover's weight based on the staked
// amount
...
//                 TaikoToken(resolve("taiko_token", false)).burn(addr,
// burnAmount);
//                 staker.exitAmount = 0;
//             }
//         }
...
//         if (
//             checkExitTimestamp
//                 && block.timestamp <= staker.exitRequestedAt +
// MIN_CHANGE_DELAY
//         ) {
...
//             return 0;
//         }
//         // The weight has a direct linear relationship to the staked amount
// and
...
//     /// @dev See the documentation for the callers of this internal function.
//     /// This function will select a prover and those that have a higher
// weight
...
//             // Go through each prover and check if the random number is in
// range
