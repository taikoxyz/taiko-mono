...
/// - The txList is well-formed RLP, with no additional trailing bytes;
/// - The total number of transactions is no more than a given threshold;
/// - The sum of all transaction gas limit is no more than a given threshold and;
///
/// A transaction is valid if and only if:
/// - The transaction is well-formed RLP, with no additional trailing bytes (rule#1 in Ethereum yellow paper);
/// - The transaction's signature is valid (rule#2 in Ethereum yellow paper), and;
/// - The transaction's the gas limit is no smaller than the intrinsic gas (rule#5 in Ethereum yellow paper).
