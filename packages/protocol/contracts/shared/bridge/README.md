...
Marking a message as "FAILED" and recalling it are currently disabled: `failMessage`, `recallMessage` and a failed last-attempt `retryMessage` all revert with `B_RECALL_DISABLED` while `Bridge.RECALL_ENABLED` is false. A message whose invocation fails on the destination chain stays "RETRIABLE" and can be retried until it succeeds; if it can never succeed, the value it carries stays in the destination chain's bridge and the deposit that backs it stays locked in the source chain's bridge or vault, since there is no refund path while recalls are off. Messages that were already "FAILED" when the disabled implementation went live can be neither retried nor recalled until recalls are re-enabled. The recall path is released by a destination-chain failure proof, the same primitive that releases a delivery, and the source chain never learns that a message was delivered; disabling it removes that path as a payout route under a forged proof. The code stays in place so it can be re-enabled by flipping the constant in a new implementation, in which case a "FAILED" message could again be recalled on the source chain's Bridge contract with `recallMessage` (with `message` and `proof` input params), which sends the assets back to the user.
