...
import "./ISignalServiceLegacy.sol";
...
contract LegacySignalService is EssentialResolverContract, ISignalServiceLegacy {
    ...
    /// @inheritdoc ISignalServiceLegacy
        ...
        returns (CacheAction[] memory actions)
    {
        ...
        address signalService = resolve(chainId, bytes32("signal_service"), false);
        ...
        for (uint256 i; i < hopProofs.length; ++i) {
            ...
            if (isLastHop) {
                ...
                signalService = resolve(hop.chainId, bytes32("signal_service"), false);
                if (signalService == address(this)) revert SS_INVALID_MID_HOP_CHAINID();
            }
