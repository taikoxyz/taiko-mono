...
library SlotChainTypes {
    ...
    /// @dev Forced-message kinds use their consensus wire values. Only kind 0 exists; wire
    ///      value 1 is unassigned and reserved.
    enum ForcedKind {
        USER_TRANSACTION
    }
    ...
    /// @dev Forced-message dispositions use their consensus wire values. Code 5 is unassigned;
    ///      the placeholder member only preserves `INVALID_NO_TX = 6`.
    enum Disposition {
        EXPIRED_NO_TX,
        NONCE_NO_TX,
        FUNDS_NO_TX,
    ...
    /// @dev Canonical state written by the Settlement at every canonical commit.
    ...
    /// @dev Exact 204-byte kind-0 admission body: the durable descriptor without the
