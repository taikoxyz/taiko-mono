 ...
 * It also manages the mapping between canonical ERC20 tokens and their bridged tokens.
 * @dev Ether is held by Bridges on L1 and by the EtherVault on L2, not TokenVaults.
...
contract TokenVault is EssentialContract {
     ...
     * The value can be an Ether amount or the amount of a token being transferred.
     */
    error TOKENVAULT_INVALID_VALUE();

    /**
     * Thrown when the token address in a transaction is invalid.
     * This could happen if the token address is zero or doesn't conform to the ERC20 standard.
     */
    ...
    /**
     * Thrown when a canonical token address could not be found for a bridged token.
     * This could happen when trying to send a bridged token back to its original chain.
     */
    error TOKENVAULT_CANONICAL_TOKEN_NOT_FOUND();

    /**
     * Thrown when the owner address in a message is invalid.
     * This could happen if the owner address is zero or doesn't match the expected owner.
     */
    error TOKENVAULT_INVALID_OWNER();

    /**
     * Thrown when the source chain ID in a message is invalid.
     * This could happen if the source chain ID doesn't match the current chain's ID.
     */
    error TOKENVAULT_INVALID_SRC_CHAIN_ID();

    /**
     * Thrown when a message has not failed.
     * This could happen if trying to release a message deposit without proof of failure.
     */
    error TOKENVAULT_MESSAGE_NOT_FAILED();

    /**
     * Thrown when the sender in a message context is invalid.
     * This could happen if the sender isn't the expected token vault on the source chain.
     */
     ...
     *
     * @param message The message that corresponds to the ERC20 deposit on the source chain.
     * @param proof The proof from the destination chain to show the message has failed.
     */
    ...
    /**
     * Internal function to deploy a new BridgedERC20 contract and initializes it.
     * This must be called before the first time a bridged token is sent to this chain.
     * @param canonicalToken Canonical token information
