...
Usage:
  PRIVATE_KEY=0x... RPC_URL=... AUTOMATA_DCAP_ATTESTATION=0x... \\
    PCCS_JSON=/path/pccs.json PCCS_REPO=/path/automata-on-chain-pccs \\
    [SGX_BOOTSTRAP_JSON=/path/bootstrap.json | SGX_BOOTSTRAP_URL=http://host:port | FMSPC=00606a000000] \\
    ./setup_sgx_pccs_extras.sh

Helpers:
  ./setup_sgx_pccs_extras.sh --extract-fmspc /path/bootstrap.json
EOF
}
