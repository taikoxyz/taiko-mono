...
#   4. Optionally upload legacy TDX collateral for a given FMSPC
#      (disabled by default; SGX collateral is handled by setup_sgx_pccs_extras.sh)
#
...
FMSPC="${FMSPC:-}"
UPLOAD_LEGACY_TDX_COLLATERALS="${UPLOAD_LEGACY_TDX_COLLATERALS:-false}"
...
TMP_ROOT="${TMPDIR:-/tmp}"
WORK_DIR="${WORK_DIR:-$TMP_ROOT/automata-deploy-$$}"
KEEP_REPOS="${KEEP_REPOS:-false}"
DEPLOY_DAIMO_P256="${DEPLOY_DAIMO_P256:-auto}" # auto | true | false
...
Optional env:
  ...
  FMSPC          6-byte FMSPC. For SGX, leave unset here and use
                 setup_sgx_pccs_extras.sh after deployment.
                 For legacy TDX collateral upload, also set
                 UPLOAD_LEGACY_TDX_COLLATERALS=true.
  ...
  DEPLOY_DAIMO_P256
                 auto | true | false. Default auto probes the RIP-7212 P256
                 precompile at 0x0000000000000000000000000000000000000100
                 and skips Daimo P256 deployment when the precompile works.

Output:
  AutomataDcapAttestationFee address — use as AUTOMATA_DCAP_ATTESTATION
  in the SGX verifier deployment/configuration scripts.
...
wait_for_code() {
    ...
    die "$label has no code at $addr after deployment"
}

supports_rip7212_p256() {
    local ret

    ret=$(cast call "$P256_RIP7212_PRECOMPILE" "$P256_RIP7212_TEST_INPUT" \
...
DAIMO_P256="0xc2b78104907F722DABAc4C69f826a522B2754De4"
SHOULD_DEPLOY_DAIMO_P256=false

case "$DEPLOY_DAIMO_P256" in
    auto)
        if supports_rip7212_p256; then
            echo "RIP-7212 P256 precompile verified at $P256_RIP7212_PRECOMPILE"
            echo "Skipping Daimo P256 deployment"
        else
            echo "RIP-7212 P256 precompile not available; deploying Daimo P256Verifier"
            SHOULD_DEPLOY_DAIMO_P256=true
        fi
        ;;
    true)
        SHOULD_DEPLOY_DAIMO_P256=true
        ;;
    false)
        echo "Skipping Daimo P256 deployment (DEPLOY_DAIMO_P256=false)"
        ;;
    *)
        die "DEPLOY_DAIMO_P256 must be auto, true, or false (got: $DEPLOY_DAIMO_P256)"
        ;;
esac
...
if [[ "$SHOULD_DEPLOY_DAIMO_P256" == "true" ]]; then
    if has_code "$DAIMO_P256"; then
        ...
        if ! cast send "$NICK_FACTORY" \
            ...
            has_code "$DAIMO_P256" || die "Daimo P256Verifier deployment failed and $DAIMO_P256 has no code"
        fi
        wait_for_code "Daimo P256Verifier" "$DAIMO_P256"
        echo "Daimo P256Verifier: deployed at $DAIMO_P256"
    fi
fi
...
if [[ -n "$PCCS_JSON" ]]; then
    ...
    # Step 2: deploy helpers on-chain.  The upstream deploy-helpers target deploys
    # all helper contracts in one script run; if any deterministic CREATE2 address
    # already has code, the whole run stops and can leave later helpers missing.
    # Deploy each helper independently and verify code before any DAO is created.
    deploy_helper_if_missing() {
        local label="$1"
        local sig="$2"
        local addr

        addr=$(jq -r ".$label // empty" "deployment/${CHAIN_ID}.json")
        [[ -z "$addr" || "$addr" == "null" ]] && die "$label address missing after helper simulation"

        if has_code "$addr"; then
            echo "  $label already on-chain at $addr"
            return 0
        fi

        echo "  Deploying $label at $addr..."
        if ! OWNER="$DEPLOYER" forge script script/helper/DeployHelpers.s.sol:DeployHelpers \
            --sig "$sig" \
            --rpc-url "$RPC_URL" --private-key "$PRIVATE_KEY" \
            --broadcast --skip-simulation -vv; then
            # CREATE2 deployments can race with a previous attempt.  Treat that as
            # idempotent only if the expected address now has code.
            has_code "$addr" || die "$label deployment failed and $addr has no code"
        fi

        wait_for_code "$label" "$addr"
        echo "  $label deployed at $addr"
    }

    deploy_helper_if_missing "EnclaveIdentityHelper" "deployEnclaveIdentityHelper()"
    deploy_helper_if_missing "FmspcTcbHelper" "deployFmspcTcbHelper()"
    deploy_helper_if_missing "PCKHelper" "deployPckHelper()"
    deploy_helper_if_missing "X509CRLHelper" "deployX509CrlHelper()"
    deploy_helper_if_missing "TcbEvalHelper" "deployTcbEvalHelper()"
...
if [[ -n "$AUTOMATA_DCAP_ATTESTATION" ]]; then
    ...
    if ! PRIVATE_KEY="$PRIVATE_KEY" make deploy-all RPC_URL="$RPC_URL"; then
        ...
        for VER in $VERSIONS; do
            SIM_OUT=$(OWNER="$DEPLOYER" QUOTE_VERIFIER_VERSION="$VER" forge script DeployVerifier \
                --private-key "$PRIVATE_KEY" -vv 2>&1 || true)
            echo "$SIM_OUT"
            SIM_ADDR=$(echo "$SIM_OUT" | \
                grep -oE "new V${VER}QuoteVerifier@0x[0-9a-fA-F]{40}" | \
                grep -oE '0x[0-9a-fA-F]{40}' | head -1)
            if [[ -n "$SIM_ADDR" ]]; then
                [[ -f "$SIM_DIR/dcap.json" ]] || echo "{}" > "$SIM_DIR/dcap.json"
                jq --arg k "V${VER}QuoteVerifier" --arg a "$SIM_ADDR" \
                    '.[$k] = $a' "$SIM_DIR/dcap.json" > "$SIM_DIR/dcap.json.tmp" \
                    && mv "$SIM_DIR/dcap.json.tmp" "$SIM_DIR/dcap.json"
            fi
        ...
        ensure_quote_verifier_registered() {
            local ver="$1"
            local addr="$2"
            local current

            current=$(cast call "$ATTEST_ADDR" "quoteVerifiers(uint16)(address)" "$ver" \
                --rpc-url "$RPC_URL" 2>/dev/null || echo "0x0000000000000000000000000000000000000000")
            if [[ "${current,,}" == "${addr,,}" ]]; then
                echo "  V${ver}QuoteVerifier already registered on AutomataDcapAttestationFee"
                return 0
            fi

            echo "  Registering V${ver}QuoteVerifier on router + attestation..."
            cast send "$ROUTER_ADDR" "setAuthorized(address,bool)" "$addr" true \
                --private-key "$PRIVATE_KEY" --rpc-url "$RPC_URL" --quiet
            cast send "$ATTEST_ADDR" "setQuoteVerifier(address)" "$addr" \
                --private-key "$PRIVATE_KEY" --rpc-url "$RPC_URL" --quiet
        }
        ...
        for VER in $VERSIONS; do
            ...
            [[ -z "$V_ADDR" ]] && die "$V_KEY address missing from $DCAP_REGISTRY_DIR/dcap.json"
            if [[ -n "$V_ADDR" ]]; then
                ...
                ensure_quote_verifier_registered "$VER" "$V_ADDR"
            fi
        done
    fi
...
# ---------------------------------------------------------------
# 5. Legacy TDX collaterals (disabled by default)
# ---------------------------------------------------------------
if [[ -n "$FMSPC" && "$UPLOAD_LEGACY_TDX_COLLATERALS" == "true" ]]; then
    echo ""
    ...
    echo "$CERT_CHAIN" | awk -v out="$ASSETS" '
        /-----BEGIN CERTIFICATE-----/ { n++ }
        n==1 { print > out "/tcb_signing.pem" }
        n==2 { print > out "/tcb_root.pem" }
    '

    for i in signing root; do
        PEM="$ASSETS/tcb_${i}.pem"
    ...
    echo ""
    echo "--- Skipping legacy TDX collateral upload for FMSPC=$FMSPC ---"
    echo "Set UPLOAD_LEGACY_TDX_COLLATERALS=true to enable it."
    echo "For SGX, run setup_sgx_pccs_extras.sh after this deploy script."
fi
