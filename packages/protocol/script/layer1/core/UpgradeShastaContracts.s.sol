...
contract UpgradeShastaContracts is DeployCapability {
    ...
    function run() external broadcast {
        ...
        SignalServiceForkRouter(config.signalServiceProxy).upgradeTo(config.signalServiceForkRouterImpl);
    }
    ...
    require(config.preconfWhitelistProxy != address(0), "PRECONF_WHITELIST_PROXY not set");
        ...
        require(config.signalServiceForkRouterImpl != address(0), "SIGNAL_SERVICE_FORK_ROUTER_IMPL not set");
    }
