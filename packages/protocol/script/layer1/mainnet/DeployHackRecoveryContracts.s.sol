...
contract DeployHackRecoveryContracts is Script {
    ...
    uint24 public constant QUOTA_PERIOD = 15 minutes;
    ...
    function _deployImplementations() private returns (Deployment memory deployment_) {
        ...
        deployment_.quotaManagerImpl = address(
            new QuotaManager(LibL1Addrs.DAO_CONTROLLER, LibL1Addrs.BRIDGE, LibL1Addrs.ERC20_VAULT, QUOTA_PERIOD)
        );
        ...
        deployment_.mainnetErc20VaultImpl = address(new MainnetERC20Vault(SHARED_RESOLVER, QUOTA_MANAGER));
    ...
    function _logDeployment(Deployment memory _deployment) private pure {
        ...
        console2.log("QUOTA_MANAGER_NEW_IMPL:", _deployment.quotaManagerImpl);
