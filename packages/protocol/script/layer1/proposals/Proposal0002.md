...
Note that the first action is created manually through the DAO’s UI. The remaining three actions are generated by a script and then grouped into a single action using the DAO’s UI.
