...
It executes **7 L1 actions** from the DAO Controller and **no L2 actions**:

1. Register **3 RISC0 image IDs** on the Shasta RISC0 verifier.
...
| `RISC0_SHASTA_AGGREGATION_IMAGE_ID`              | `0xa9cc799b246826a3a1b9545e82a290227a65044612a6273b0aaf90dd51169831` |
...
3. Call `setImageIdTrusted(RISC0_SHASTA_BOUNDLESS_AGGREGATION_IMAGE_ID, true)` on `RISC0_SHASTA_VERIFIER`.
4. Call `setProgramTrusted(SP1_SHASTA_PROPOSAL_PROGRAM_VKEY_BN254, true)` on `SP1_SHASTA_VERIFIER`.
5. Call `setProgramTrusted(SP1_SHASTA_PROPOSAL_PROGRAM_VKEY_HASH_BYTES, true)` on `SP1_SHASTA_VERIFIER`.
6. Call `setProgramTrusted(SP1_SHASTA_AGGREGATION_PROGRAM_VKEY_BN254, true)` on `SP1_SHASTA_VERIFIER`.
7. Call `setProgramTrusted(SP1_SHASTA_AGGREGATION_PROGRAM_VKEY_HASH_BYTES, true)` on `SP1_SHASTA_VERIFIER`.
...
3. Confirm the JSON contains these entries:
   ...
   - `risc0_shasta_aggregation` `image_id`
...
4. Match those 7 digests to the **Guest Digests** table above.
