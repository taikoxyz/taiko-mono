...
contract SetRemoteBridges is Script {
    ...
    address[] public remoteERC20Vaults =
        vm.envAddress("REMOTE_ERC20_VAULTS", ",");
    address[] public remoteERC721Vaults =
        vm.envAddress("REMOTE_ERC721_VAULTS", ",");
    address[] public remoteERC1155Vaults =
        vm.envAddress("REMOTE_ERC1155_VAULTS", ",");

    function run() external {
        require(
            remoteChainIDs.length == remoteBridges.length,
            "invalid remote bridge addresses length"
        );
        require(
        ...
        ProxiedAddressManager proxy =
            ProxiedAddressManager(payable(bridgeSuiteAddressManager));
        for (uint256 i; i < remoteChainIDs.length; ++i) {
            proxy.setAddress(
                uint64(remoteChainIDs[i]), "bridge", remoteBridges[i]
            );
            proxy.setAddress(
                uint64(remoteChainIDs[i]), "erc20_vault", remoteERC20Vaults[i]
            );
            proxy.setAddress(
                uint64(remoteChainIDs[i]), "erc721_vault", remoteERC721Vaults[i]
            );
            proxy.setAddress(
