...
#   profile: shared, layer1, layer2, or surge
#
...
# Surge contracts (use layer1 profile, output to centralized layouts folder)
contracts_surge=(
"contracts/layer1/surge/SurgeVerifier.sol:SurgeVerifier"
"contracts/layer1/surge/ccip/CCIPStateStore.sol:CCIPStateStore"
"contracts/layer1/surge/deployments/internal-devnet/SurgeInbox.sol:SurgeInbox"
"contracts/layer1/surge/SurgeTimelockController.sol:SurgeTimelockController"
)

# Output directory for surge layouts
surge_layouts_dir="contracts/layer1/surge/layouts"

# Update storage layout for a single contract
# Args: contract, profile, [output_dir]
update_contract_layout() {
    local contract=$1
    local profile=$2
    local output_dir=${3:-""}  # Optional: custom output directory
    ...
    # Derive layout file path
    local base_name="${file_path##*/}"
    local base_name_no_ext="${base_name%.sol}"
    local layout_file_path
    
    if [ -n "$output_dir" ]; then
        # Use custom output directory
        mkdir -p "$output_dir"
        layout_file_path="${output_dir}/${base_name_no_ext}_Layout.sol"
    else
        # Default: place layout file next to source file
    ...
    # Determine security contact based on profile
    local security_contact="security@taiko.xyz"
    if [ "$profile" = "layer1" ] && [[ "$file_path" == *"/surge/"* ]]; then
        security_contact="security@nethermind.io"
    fi
...
/// @custom:security-contact ${security_contact}
...
case "$profile" in
    shared) echo "Generating shared contract layouts..."; contracts=("${contracts_shared[@]}"); forge_profile="shared"; output_dir="" ;;
    layer1) echo "Generating layer 1 contract layouts..."; contracts=("${contracts_layer1[@]}"); forge_profile="layer1"; output_dir="" ;;
    layer2) echo "Generating layer 2 contract layouts..."; contracts=("${contracts_layer2[@]}"); forge_profile="layer2"; output_dir="" ;;
    surge) echo "Generating Surge contract layouts..."; contracts=("${contracts_surge[@]}"); forge_profile="layer1"; output_dir="$surge_layouts_dir" ;;
    *) echo "❌ Error: Invalid profile '$profile'"; echo "Usage: $0 <shared|layer1|layer2|surge>"; exit 1 ;;
esac

# Process contracts
failed=0
for contract in "${contracts[@]}"; do
    update_contract_layout "$contract" "$forge_profile" "$output_dir" || ((failed++))
done
