...
import { deployBridge } from "../test/bridge/Bridge.test"
...
describe("Bridge", function () {
    async function deployBridgeFixture() {
        ...
        const addressManager: any = await (
            await ethers.getContractFactory("AddressManager")
        ).deploy()
        await addressManager.init()

        const l2AddressManager: any = await (
            await ethers.getContractFactory("AddressManager")
        )
            .connect(l2Signer)
            .deploy()
    ...
    describe("retryMessage()", function () {
        async function retriableMessageSetup() {
            ...
            const testBadReceiver: any = await (
