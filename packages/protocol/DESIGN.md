...
1. A prover-selected address $a$ only which can transact the `proveBlock` transaction for this block using _this_ to-be generated ZKP, though anyone else can verify the ZKP's validity, and;
...
$$ p_i^a = \mathbb{Z} (h_i, X_i, A_i, a, T_i) $$
...
1. The hash of ancestor hashes: $\mathbb{H}(A_i)$, and;
