...
The root build contains exactly twenty-one artifacts from one release-pinned compiler invocation and
...
| `BuilderRegistrySeatLifecycleFacetV1` | Independently ERC-2470-deployed, immutable full-authority lifecycle facet pinned by Factory and BuilderRegistry; not a component role. |
| `BuilderRegistryLeaseLifecycleFacetV1` | Independently ERC-2470-deployed, immutable full-authority lifecycle facet pinned by Factory and BuilderRegistry; not a component role. |
...
`SourceBundleFactory` occurs once in the twenty-one-artifact count even though both the role manifest
...
| Complete twenty-one-artifact root set listed above                                                                                                    | L1                  | One release-pinned compiler invocation; `artifact-owned` only by `layer1`, `out/layer1`; no shared or mixed-profile exception | Root-lifetime objects repeat address/runtime/configuration; release-scoped members use fresh instances as specified.                      |
| Settlement, kind-0 ingress and source bundle release-scoped instances                                                                                  | L1                  | `artifact-owned` by `layer1`; an artifact in the twenty-one-root set can never be sourced from `shared` or another profile | Fresh release-scoped accounts. Historical accounts serve only retained liabilities/proofs and never become current again.                |
...
commit authority. Its exact static 45-word `BuilderRegistryConstructorV1` tuple follows the three
...
verifier has no state, callback, arbitrary target, delegatecall, value, or mutation path.

BuilderRegistry's stateful lifecycle code is statically sharded into the immutable Seat and Lease
facets defined by `2026-09-06-builder-registry-lifecycle-facets-design.md`. Registry retains the only
state/custody address and explicit public ABI; its fixed wrappers check codehash and delegate only
the seven enumerated selectors. The two full-authority runtimes, exact BRF1 configurations,
selector sets, and common storage-layout digest are transitively pinned by Factory and Registry
topology. A shared operation lock covers delegated calls plus facade claim/evidence. No fallback,
mutable implementation, arbitrary selector routing, or uncommitted delegatecall is permitted.

Factory pins the proof verifier's init-code/runtime/configuration and derived address separately.
The exact ABI, opcodes,
...
`SourceBundleFactory` child and not part of the frozen twenty-one-artifact root cohort. The execution
