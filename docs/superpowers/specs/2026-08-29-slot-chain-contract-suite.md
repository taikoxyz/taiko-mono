...
- `AggregatorSeatMarket`: protocol-lifetime, non-proxy continuous reverse-ask market with one active
  ...
  release, per-stage/per-term reserve deltas, and asynchronous breach enforcement as specified in
...
- `Settlement`: `PREACTIVE`, `ACTIVE`, `MIGRATION_ARMED`, `MIGRATION_READY`, and `FROZEN` phases;
  ...
  single-duty ranks; satisfaction/failover latches; an explicit selected-successor-before-service
  record; noncanonical duty-cell reclamation; fail-open
...
| Settlement <-> `AggregatorSeatMarket`                                   | Settlement constructor fixes the market; the delayed release manager append-only authorizes the static exact `(settlementChainId, protocolVersion, settlement, settlementRuntimeHash, settlementConfigHash, market)` tuple. Exactly one authorization is installation-enabled for the global four-cell book. A separate permissionless exact-target sync initializes/monotonically advances its cached seat generation only while Settlement reports installation-open `ACTIVE`; only quotes equal to that cache may enter ranking. Generation is not manager-authorized state. | No generic setter. The first authorization is current; later ones are disabled until an atomic manager rotation. Offers/ETH remain market-owned; Settlement begins economically vacant. | Each rank installs one immutable single-duty seat term. Roster revisions retain untouched standby term IDs. Installation is delayed, noncanonical, exact-chain/commitment checked, and pre-funds every rank. A generation advance boundedly purges old pending offers before current offers can rank; an old staged offer requires its authenticated tombstone. Successful cutover rotation requires exact manager/router proof of new-routed/ACTIVE and old-FROZEN state, executes the invalid-stage bond/reserve cancellation, purges the old book, disables old insertion, and enables the new authorization atomically; abort leaves old current but requires post-abort sync and fresh consent. Canonical paths only write local duties/receipts. Old Settlements remain the sole premium-cap/breach/release source for their terms; migration arm increments the seat generation without governance, invalidates old quotes, excuses unresolved non-breached duties and selected successors, and leaves both abort and target economically vacant. |
...
| Seat duty and failover                   | Early lag opens recovery without penalty and stores one local duty against a single-use tranche. Every canonical commit scans at most the fixed seat count for satisfaction latches. At failover Settlement selects only the next pre-funded standby. A cure before the next revision starts it as normal service; an outage that remains open starts its duty only when the next usable revision opens. At the later slash threshold—whose reorg margin is measured from nominal worst-case recovery completion—Settlement records a retained immutable breach receipt. Ring exhaustion, no-duty funding expiry, unusable successor, and successor-funding failure use one bounded full-lineup non-fault vacancy transition, recording every removal rather than retaining hidden standbys. Canonical start/close changes no Market reserve bytes or ETH; later noncanonical reconciliation authenticates the local state. |
| Seat market maintenance                  | Offer insertion, atomic expiring stage reservation, handover/exit finalization, migration-tombstone stage cancellation, premium extension/accrual, duty-cell reclamation, release, slash enforcement, and claims are separate noncanonical transactions. Installed exit removes the local roster term and updates market custody in one revert domain; funds remain locked until then. A leading sync and health-headroom check precede responsibility handover. Release request/finalize are permissionless but credit only the immutable operator. Before release/slash makes a tranche terminal, Market reconciles and deletes every installed reserve after its full cap is claim-matured. Duty-cell reuse additionally authenticates the terminal exact binding and absence of a live reserve. Market/funding failure reverts only maintenance and cannot block proof submission or recovery.                          |
...
- perpetual-seat tests for standing-book ordering, ask maturity, minimum improvement/tenure,
  ...
  immutability/pre-funding/promotion, explicit selected-successor locking, late-cure successor
  activation, premium reserve-delta solvency/authenticated vesting/funding expiry, single-duty
  tranche rotation, early recovery, delayed failover, nominal-recovery reorg retention,
  force-recovery evasion, duty satisfaction/ring exhaustion, asynchronous enforcement, exact
  release-deadline boundaries, wrong-chain target reads, exact-target/generation re-quote, and
  migration isolation;
